...
test.describe('auth UI (anonymous state)', () => {
  ...
  test('Sign In button visible with readable text', async ({ page }) => {
    await page.goto('/');
    const signInBtn = page.locator('.auth-signin-btn').first();
  ...
  test('Sign In click opens our own auth modal', async ({ page }) => {
    const errors: string[] = [];
    page.on('pageerror', (err) => errors.push(err.message));

    await page.goto('/');
    await page.locator('.auth-signin-btn').first().waitFor({ timeout: 20000 });
    await jsClick(page, '.auth-signin-btn');

    // The modal is ours (src/components/AuthModal.ts), loaded through a dynamic
    // import on first click. It renders without the Supabase SDK, so it opens
    // even when the test env carries no Supabase keys.
