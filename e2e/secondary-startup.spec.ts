...
const SECONDARY_STARTUP_REQUEST =
  /(?:\/api\/send|fonts\.googleapis\.com\/css2|fonts\.gstatic\.com\/s\/(?:nunito|tajawal)|\.supabase\.co|ingest\.us\.sentry\.io|static\.cloudflareinsights\.com|www\.youtube\.com\/iframe_api)/;
...
test.describe('secondary startup work', () => {
  ...
  test('defers secondary startup requests until idle startup runs', async ({ page }) => {
    ...
    // SECONDARY_STARTUP_REQUEST matches the analytics collector write, Google
    // Fonts, Supabase auth, Sentry ingest, Cloudflare analytics, and the
    // YouTube iframe API — none may fire before idle startup runs.
    expect(secondaryRequests).toEqual([]);

    await page.evaluate(() => {
      (window as unknown as { __wmRunDeferredIdle?: () => void }).__wmRunDeferredIdle?.();
    });
    // The tracker ships in the bundle now, so what proves analytics started is
    // its first write to our own collector, not a third-party script fetch.
    await expect.poll(() => secondaryRequests.some((url) => url.includes('/api/send'))).toBe(true);
