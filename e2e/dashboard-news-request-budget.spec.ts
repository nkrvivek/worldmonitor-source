...
// ...and the wait above may only ever prove a NEGATIVE. It cannot prove that a
// request happened: an assertion that a retry was issued fails whenever the retry
// is merely slower than the wall clock, while the code under test is correct.
// That is #5890 — `variant-smoke-full` is a required PR gate and it runs three e2e
// suites sequentially in one job, so this spec starts under whatever load the two
// before it left behind, and the flake cost a base-vs-head bisect to clear.
//
// So the two shapes are split on purpose. Occurrence ("a second digest must be
// requested") polls for the condition. Absence ("no further digest may be
// requested", "the cache must not be overwritten") keeps the settle wait, because
// no amount of polling can establish that something never happens.
const REQUEST_ARRIVAL_TIMEOUT_MS = 30_000;
...
/**
 * Wait for at least `atLeast` digest requests, then assert it.
 *
 * The occurrence half of the split described at REQUEST_ARRIVAL_TIMEOUT_MS. Where
 * a test also asserts an absence afterwards, this replaces only the assertion —
 * the settle wait before it stays, because the absence still needs a window in
 * which the behaviour under guard would have had time to happen.
 */
async function expectDigestAttempts(
  log: NewsRequestLog,
  atLeast: number,
  message: string,
): Promise<void> {
  await expect
    .poll(() => log.digestUrls.length, { message, timeout: REQUEST_ARRIVAL_TIMEOUT_MS })
...
test.describe('dashboard news request budget (#5376)', () => {
  ...
  test('a deliberately customized category still loads, capped at the per-category feed limit', async ({ page }) => {
    ...
    // The floor is occurrence-shaped, so it polls: an enabled cross-variant news
    // panel must still have its feeds fetched, because a custom category is never
    // in the per-variant digest and direct fetch is its only path (#3687).
    await expect
      .poll(() => distinctProxiedFeeds(log.rssProxyUrls).length, {
        message:
          `a custom category must reach its per-category feed cap of ` +
          `${PER_FEED_FALLBACK_CATEGORY_FEED_LIMIT}; "${CUSTOM_CATEGORY_KEY}" fetched fewer`,
        timeout: REQUEST_ARRIVAL_TIMEOUT_MS,
      })
      .toBeGreaterThanOrEqual(PER_FEED_FALLBACK_CATEGORY_FEED_LIMIT);

    // The ceiling still needs the settle window. An uncapped fetch lands its extra
    // feeds AFTER the first three, so only elapsed time can rule one out.
    await page.waitForTimeout(SECOND_LOAD_SETTLE_MS);
  ...
  test('a degraded 200 digest with no categories is not mistaken for a landed load', async ({ page }) => {
    ...
    // Nothing here asserts an absence, so the settle wait has no work to do and is
    // gone: this test is #5890 itself, where the retry landed later than the fixed
    // 8 s window on a loaded CI runner and the assertion read 1.
    await expectDigestAttempts(
      log,
      2,
  ...
  test('a degraded 200 digest does not overwrite the persisted last-good digest', async ({ page }) => {
    ...
    // The settle stays: persistDigest is fire-and-forget, so the overwrite this
    // guards against would land some time after that request, and only elapsed
    // time can establish that it never did.
    await page.waitForTimeout(SECOND_LOAD_SETTLE_MS);
  ...
  test('a partial digest is used for the load but does not shrink the persisted last-good digest', async ({ page }) => {
    ...
    // The settle stays for the same reason as the overwrite test above: the shrink
    // this guards against would be written after the request, not with it.
    await page.waitForTimeout(SECOND_LOAD_SETTLE_MS);
  ...
  test('a failing language scope never falls back to another language digest', async ({ page }) => {
    ...
    await expect
      .poll(() => log.digestUrls.some((url) => new URL(url).searchParams.get('lang') === 'fr'), {
        message:
    ...
    // The settle stays: everything below is an absence — no English headline in the
    // panel, no `fr` entry written, the `en` entry untouched — and each of those
    // needs a window in which the leak being guarded would have had time to appear.
    await page.waitForTimeout(SECOND_LOAD_SETTLE_MS);
  ...
  test('a news load that landed nothing recovers on the next trigger', async ({ page }) => {
    ...
    // The settle stays, and here it also has to run BEFORE the count is captured:
    // a retry still in flight would land after `afterRecovery` is read and be
    // blamed on the trigger below.
    await page.waitForTimeout(SECOND_LOAD_SETTLE_MS);
