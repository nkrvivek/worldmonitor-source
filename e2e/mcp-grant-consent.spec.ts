 ...
 *
 * Drives the /mcp-grant consent page with stubbed Supabase auth and stubbed
 ...
 *
 * Stubbing strategy:
 *   - The auth service module is intercepted to provide a minimal stub
...
/**
 * Intercept the auth service module to provide a stub that looks like a
 * signed-in Pro user. This avoids needing real Supabase credentials.
 *
 * The glob matches `services/auth.ts` and its cache-busted query forms, but
 * must NOT match `services/auth-state.ts` — that is a different module, and
 * serving it this body breaks every page that imports it.
 *
 * `subscribeAuth` invokes its callback synchronously, matching Supabase's own
 * onAuthStateChange, which fires immediately with current state. That makes
 ...
 */
async function stubAuthModule(
  page: Page,
  opts: { trackSignIn?: boolean } = {},
): Promise<void> {
  // A route glob that matches nothing does not fail — the real module loads,
  // no session exists, and every test in this file waits out its full timeout.
  // That cost 25 red CI runs when `services/clerk.ts` became `services/auth.ts`
  // and this stub kept pointing at the old path. Fail in a second instead.
  if (!existsSync(fileURLToPath(new URL('../src/services/auth.ts', import.meta.url)))) {
    throw new Error(
      'src/services/auth.ts is gone — the **/services/auth.ts* route stub below now ' +
        'matches nothing. Point it at the module the grant page actually imports.',
    );
  }
  const openSignInBody = opts.trackSignIn ? 'window.__openSignInCalled = true;' : '';
  await page.route('**/services/auth.ts*', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/javascript',
      body: `
        export async function initAuth() {}
        export function getAuthToken() { return Promise.resolve('stub-jwt-token'); }
        export function getCurrentUser() { return { email: 'e2e@worldmonitor.app' }; }
        export function openSignIn() { ${openSignInBody} }
        export function subscribeAuth(cb) {
          window.__emitAuthSubscription = cb;
          cb();
          return () => {
            if (window.__emitAuthSubscription === cb) delete window.__emitAuthSubscription;
          };
        }
      `,
    });
...
async function emitAuthSubscription(page: Page): Promise<void> {
  await page.evaluate(() => {
    const emit = (window as unknown as { __emitAuthSubscription?: () => void })
      .__emitAuthSubscription;
    if (!emit) throw new Error('Auth subscription callback was not registered');
    emit();
  });
}

/**
 * Observe the consent/error visibility invariant across an asynchronous auth
 ...
 */
async function expectConsentPreservedAcrossAuthRefresh(page: Page): Promise<void> {
  await page.evaluate(() => {
  ...
  await emitAuthSubscription(page);
  await deniedRefresh;
  await page.evaluate(
    () =>
...
test.describe('MCP grant consent page (#5654)', () => {
  ...
  test('successful context load shows consent card with client metadata', async ({ page }) => {
    ...
    // authedFetch must attach the auth JWT; a regression to a bare fetch()
  ...
  test('auth refresh preserves consent while mint is in flight', async ({ page }) => {
    await stubAuthModule(page);
    ...
    await expectConsentPreservedAcrossAuthRefresh(page);
    await expect(page.locator('#authorizeBtn')).toBeDisabled();
    await expect(page.locator('#authorizeBtn')).toHaveText('Authorizing…');
  ...
  test('401 from context triggers sign-in (consent never renders)', async ({ page }) => {
    ...
    await stubAuthModule(page, { trackSignIn: true });
