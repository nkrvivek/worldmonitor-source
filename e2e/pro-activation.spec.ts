 ...
 * subscription behind a signed-in session (see src/services/entitlements.ts —
 ...
 * telemetry wiring — nothing is stubbed except the values a live Supabase/Convex
 ...
 * which throws before it ever hits the network when `getAuthToken()` is null
 * (no signed-in session). So a confirm cannot resolve to 'verified' through the real
...
const PRO_MONTHLY_PRODUCT_ID = 'wm_pro_monthly';
 ...
 * awaits a ~2s degraded-config read (getChannelsData throws with no signed-in
...
test.describe('Pro activation flow — telemetry + finish-setup chip', () => {
  test('skip-all through the real flow does not leak a chip after the owner session changes', async ({
    ...
    // The harness has no signed-in session. The flow is owned by the synthetic
  ...
  test('a failed confirm fires step-failed telemetry and is never reported as a skip', async ({
    ...
    // no signed-in session — see the HARNESS NOTE), which is exactly the day-0
  ...
  test('a failed confirm reaches Sentry with the step and activation-path tags', async ({
    ...
    // `assertExpectedAccount` (src/services/notification-channels.ts) — no signed-in
