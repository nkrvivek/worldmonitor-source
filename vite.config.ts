...
function brotliPrecompressPlugin(): Plugin {
  return {
    ...
    // Vercel's CDN serves the .br siblings by content negotiation. Workers
    // assets compresses on the fly and would upload them as ordinary extra
    // files reachable at their literal .br path — dead weight there.
    apply: (_config, env) =>
      env.command === 'build' && !process.env.WORKERS_CI && !process.env.CF_PAGES,
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      // Workers Builds sets WORKERS_CI_COMMIT_SHA; Pages sets
      // CF_PAGES_COMMIT_SHA. Local `vite build` falls back to 'dev' —
      // installStaleBundleCheck detects the marker and skips the comparison
      // so dev tabs don't reload on every focus.
      __BUILD_HASH__: JSON.stringify(resolveBuildHash(process.env)),
    },
    plugins: [
      ...
      // doesn't touch it, so the comparison reflects the web deployment.
      {
        ...
        generateBundle() {
          this.emitFile({
            ...
            source: resolveBuildHash(process.env),
          });
        },
      },
          ...
          manualChunks(id) {
            if (id.includes('node_modules')) {
              ...
              if (id.includes('/@supabase/')) {
                // Supabase is a runtime dynamic import (src/services/supabase-client.ts);
                // the stable chunk name lets Workbox keep the auth SDK out of precache.
                return 'supabase';
              }
            }
