...
// server (https://worldmonitor.sibt.ai/mcp) is the live, documented agent
...
const (
	// This deployment's hosts. Upstream ships api.worldmonitor.app here; on a
	// key issued by this instance that sends the key to a server we do not run
	// and returns 403. Override with Client.BaseURL/MCPURL or the WORLDMONITOR_*
	// env vars. See FORK.md, "Client defaults point at our host".
	DefaultBaseURL = "https://worldmonitor.sibt.ai"
	DefaultMCPURL  = "https://worldmonitor.sibt.ai/mcp"
	...
	authHint = "hint: this call needs a key - pass an API key to New or set WORLDMONITOR_API_KEY (get one at https://worldmonitor.sibt.ai/pro)"
...
type Client struct {
	...
	APIKey string
	// BaseURL is the REST base (default https://worldmonitor.sibt.ai).
	BaseURL string
	// MCPURL is the MCP endpoint (default https://worldmonitor.sibt.ai/mcp).
	MCPURL string
