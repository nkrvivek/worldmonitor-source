...
import { resolveSession } from './_shared/auth-session';
import {
...
export const RELAY_WARM_PING_PATHS = new Set<string>([
  ...
  // seed-insights.mjs warms the news digest before summarizing it. The digest
  // is the same free, cacheable payload every anonymous session reads; without
  // this row the seeder's warm call 401s and the insights seed fails on a cold
  // cache (found 2026-08-07, first run with an LLM key present).
...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request, ctx?: GatewayCtx): Promise<Response> {
    ...
    if (isTierGated || requiresDirectLlmQuota || needsProFreshnessResolution) {
      ...
      usage.orgId = session?.orgId ?? null;
      if (sessionUserId) {
    ...
    if (keyCheck.required && !keyCheck.valid && wmKey.startsWith('wm_')) {
      ...
      try {
        ...
        if (userKeyResult) {
          ...
          // The session role belongs to the bearer subject, not the user-key owner.
    ...
    if (keyCheck.required && !keyCheck.valid) {
      if (needsLegacyProBearerGate) {
        ...
        if (authHeader?.startsWith('Bearer ')) {
          ...
          // earlier resolveSession() block (only runs for tier-gated routes),
          ...
          // Accept EITHER a 'pro' role OR a Convex entitlement with tier >= 1.
          // The Stripe webhook pipeline writes Convex entitlements and never
          // wrote the role claim, so a paying subscriber's session.role stays
          // 'free' indefinitely — and this fork pins it to 'free' outright
          // (auth-session.ts). A role-only check would block every paying user
          // on legacy premium endpoints. This mirrors the two-signal logic in
          ...
          // Valid pro session (role OR entitlement) — fall through to route handling.
        } else {
          emitRequest(401, 'auth_401', null);
    ...
    if (!internalMcpVerified) {
      ...
      if (keyCheck.valid && (isUserApiKey || isEnterpriseAuth)) {
        ...
        if (perMinute > 0 && identity) {
          ...
          const upgradeUrl =
            planKey && planKey !== 'enterprise' ? `${SITE_ORIGIN}/` : undefined;
