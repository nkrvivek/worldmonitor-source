...
export interface CachedEntitlements {
  ...
  features: {
     ...
     * undefined as `false` — fail-closed. The Stripe webhook repopulates
...
const ENV_PREFIX = process.env.PAYMENTS_ENVIRONMENT === 'live_mode' ? 'live' : 'test';
...
// 5-min lapsed cooldown (billing.ts) already suppresses Stripe calls, so the
...
// in the seconds between checkout return and the Stripe webhook landing.
//
...
// buyer experiences:
//
//   wrongful-403 window = stripe_webhook_latency + min(this TTL, resync residual)
//
