...
// Fork-owned: where our briefs point readers. Mirrors server/_shared/site.ts,
// which this plain-JS module cannot import.
const SITE_ORIGIN = 'https://worldmonitor.sibt.ai';
const SITE_HOST = 'worldmonitor.sibt.ai';
...
function renderBackCover({ tz, pageIndex, totalPages, publicMode, refCode }) {
  const ctaHref = publicMode
    ? `${SITE_ORIGIN}/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`
    : SITE_ORIGIN;
  ...
  const metaLeft = publicMode
    ...
    : `<span class="mono">${SITE_HOST}</span>`;
...
// Analytics loader. This used to fetch a hosted Umami script from
// abacus.worldmonitor.app; that host is gone, and its tracker checked the
// page hostname against a fixed allowlist anyway, so it recorded nothing
// here. The replacement posts to this site's own collector
// (worker/routes/analytics-collect.ts) at a relative path, so it works on
// whichever origin serves the page.
//
// It defines the same `window.umami.track` that BRIEF_THREAD_OPEN_SCRIPT
// calls, and sends the path without the query string — share tokens live
// there. Nothing here throws: a magazine page that cannot report a click
// still shows the story.
const UMAMI_LOADER = `<script>
(function() {
  function id() {
    try { return crypto.randomUUID(); }
    catch (e) { return 'wm-' + Math.random().toString(36).slice(2) + Date.now().toString(36); }
  }
  var session = id();
  var visit = id();
  function send(type, payload) {
    ...
    return fetch('/api/send', {
      ...
      body: JSON.stringify({ type: type, payload: body })
    }).catch(function() {});
  }
  window.umami = {
    track: function(name, data) {
      return send('event', data === undefined ? { name: name } : { name: name, data: data });
    },
    identify: function(data) { return send('identify', { data: data }); }
  };
  send('event', { name: 'pageview' });
})();
</script>`;
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const publicStripHref = `${SITE_ORIGIN}/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`;
