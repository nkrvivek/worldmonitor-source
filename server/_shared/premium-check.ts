...
export async function resolvePremiumCallerIdentity(request: Request): Promise<PremiumCallerIdentity> {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    if (session.role === 'pro' && session.userId) {
      // A role grant is premium WITHOUT a Convex row to price it: the billing
      // pipeline never wrote the role claim (see server/gateway.ts), so this arm
      // is complimentary/tester/legacy grants, not paying subscribers. This fork
      // pins `role` to 'free' in auth-session.ts, so nothing reaches it today —
      // it stays because the gate below must not become role-only again.
    ...
    // Role isn't 'pro' — check the entitlement tier as second signal.
    // A Stripe subscriber (tier >= 1) is premium whatever the role says.
    if (session.userId) {
