...
import { dailyMeterShardName } from '../../worker/counters/daily-meter';

// Namespace this meter writes under. The Convex reader (apiPlanLimitUsage.ts)
// and worker/routes/counter-read.ts's ALLOWED_NAMESPACES both hardcode this
// same string — kept a plain literal, not a cross-package import, since it's
// one word and the three sites already agree (task-6a's own allowlist proves
// it). Passed through dailyMeterShardName() below so this file and the read
// route can never independently drift on WHICH DO instance answers for a
// given user (#cf-ratelimit-do task 6b).
const API_DAILY_METER_NAMESPACE = 'rl:apikey:day';
...
/**
 * Duck-typed Durable Object stub — the same minimal shape `callCounter`
 * (worker/counters/protocol.ts) accepts. Declared locally, not imported, so
 * this module keeps loading under plain Node (every real caller today) same
 * as api/mcp/quota.ts and server/_shared/rate-limit.ts.
 */
...
/**
 * The COUNTER Durable Object binding shape (`env.COUNTER` in worker/index.ts).
 * `reserveDailyMeter` derives the per-user shard itself via
 * `dailyMeterShardName` rather than accepting an already-selected stub, since
 * (unlike api/mcp/quota.ts's single fixed MCP_QUOTA_NAMESPACE shard) this
 * meter is one DO instance PER userId — the same instance
 * worker/routes/counter-read.ts's `userShard()` selects for the read side.
 */
export interface CounterNamespaceBinding {
 ...
 * allowance is now exceeded. INCR-first (atomic; no check-then-incr race),
 * mirroring api/mcp/quota.ts::reserveQuota.
 *
 * Two backends, one contract (#cf-ratelimit-do task 6b): pass `counter` (the
 * COUNTER Durable Object binding) to reserve via CounterDO; omit it to keep
 * the Redis pipeline path, unchanged. Follows the seam Task 5 established in
 * api/mcp/quota.ts::reserveQuota — optional parameter, no module-global. No
 * real caller passes `counter` yet (server/gateway.ts's call site is
 * unmodified by this task), so production keeps taking the Redis leg below
 * exactly as it does today; a later task wires a real binding in and deletes
 * this fallback.
 *
 * - `allowance < 0` (unlimited, e.g. enterprise) → no storage call; never metered.
 * - Storage unavailable → `metered:false`, `overLimit:false` (fail-open: the
 *   gateway serves uncounted). Both backends share this contract.
 */
export async function reserveDailyMeter(opts: {
  ...
  date?: Date;
  counter?: CounterNamespaceBinding | null;
}): Promise<MeterResult> {
  if (opts.counter) {
    return reserveDailyMeterViaCounter(opts.counter, opts.userId, opts.allowance, opts.date);
  }
  return reserveDailyMeterViaRedis(opts.userId, opts.allowance, opts.pipeline, opts.date);
}

/**
 * Durable Object path. Get the shard from `dailyMeterShardName` — imported,
 * never rebuilt by hand — so this writer and worker/routes/counter-read.ts's
 * reader can never independently pick different DO instances for the same
 * user. That mismatch has no error anywhere: the read just succeeds and
 * returns `present: false`, indistinguishable from "no usage yet" without
 * this note (worker/counters/daily-meter.ts's own doc comment on
 * dailyMeterShardName says the same).
 */
async function reserveDailyMeterViaCounter(
  counter: CounterNamespaceBinding,
  userId: string,
  allowance: number,
  date?: Date,
): Promise<MeterResult> {
  ...
  const stub = counter.get(counter.idFromName(dailyMeterShardName(API_DAILY_METER_NAMESPACE, userId)));

  let response: CounterResponse;
  try {
    response = await callCounter(stub, {
      op: 'daily',
      namespace: API_DAILY_METER_NAMESPACE,
      userId,
      allowance,
      ttlSeconds: API_DAILY_TTL_SECONDS,
      // 'allow': a storage outage must never punish a paying customer — serve
      // uncounted, mirroring the Redis leg's fail-open contract exactly.
      posture: 'allow',
    });
  } catch {
    return { count: 0, overLimit: false, metered: false, retryAfterSec, rollback: noop };
  ...
  if (response.op !== 'daily') {
    // Unreachable in practice (CounterDO.dispatch() echoes the op it was
    // given); treat it the same as a storage failure rather than throwing.
    return { count: 0, overLimit: false, metered: false, retryAfterSec, rollback: noop };
  }
  ...
  const rollback = async (): Promise<void> => {
    ...
    rolledBack = true;
    try {
      await callCounter(stub, { op: 'daily-rollback', namespace: API_DAILY_METER_NAMESPACE, userId });
    } catch {
      // Best-effort: a failed rollback overshoots the meter by 1, the
  ...
  return {
    ...
    // NOT `response.overCeiling` — that is the DO's own 10x safety ceiling
    // (`count > allowance * ceilingMultiplier`, worker/counters/daily-meter.ts),
    // which #4635 replaced with the sold allowance as the hard limit. Reading
    // it here would let a customer serve 10x their plan before the gateway
    // rejected, while the Redis leg rejected at 1x. Compare the returned count
    // to `allowance` — the identical expression the Redis leg rejects on — so
    // both backends land on the same boundary.
    overLimit: response.count > allowance,
...
/**
 * Redis pipeline path — production behavior, unchanged. Kept for as long as
 * any real caller reaches this file without a Durable Object binding
 * (server/gateway.ts, today — see the module comment above). Task 7 deletes
 * this once every caller passes `counter`.
 */
async function reserveDailyMeterViaRedis(
  userId: string,
  allowance: number,
  pipeline: RateLimitPipeline,
  date?: Date,
