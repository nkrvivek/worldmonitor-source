...
// @ts-expect-error — JS module, no declaration file
import { limitWithFallback, resetRateLimitFallbackForTest } from '../../api/_rate-limit-fallback.js';
// durationToSeconds itself moved to worker/counters/duration.ts (#cf-ratelimit-do
// task 4) — api/_rate-limit-fallback.js is scheduled for deletion once every
// Upstash call site is gone (task 7), so this file cannot depend on that
// module for a helper it still needs after that file is gone.
...
// NODE_TEST_CONTEXT in the child that executes each file and vitest sets
// VITEST; in either context the fail-open / fail-closed rate-limit tests point
// UPSTASH_REDIS_REST_URL at a fake host and would otherwise burn that full
// backoff on every limiter call. Skip retries under a test runner only —
// production (both unset) keeps the resilient default untouched. Mirrors the
// retry:false already shipped on the MCP limiter to unblock the suite
// (PR #3963).
const REDIS_TEST_RETRY_OPTS: { retry?: false } = process.env.NODE_TEST_CONTEXT || process.env.VITEST
  ? { retry: false }
  : {};
...
/**
 * Durable Object stub shape this file needs — deliberately the same minimal
 * duck-typed interface `callCounter` (worker/counters/protocol.ts) already
 * accepts, so this module never has to import `cloudflare:workers` or
 * `@cloudflare/workers-types` and can keep loading under plain Node (every
 * Vercel Edge Function that calls into this file today, plus test:data).
 */
export interface CounterStub {
  fetch(request: Request): Promise<Response>;
}

export interface RateLimitCounterEnv {
  COUNTER: {
...
let counterEnv: RateLimitCounterEnv | null = null;

/**
 * Lets a Cloudflare Worker fetch handler hand this module its COUNTER
 * Durable Object binding once, so checkRateLimit / checkEndpointRateLimit /
 * checkScopedRateLimit can move traffic onto CounterDO instead of Upstash.
 *
 * Nothing outside the tests calls THIS module's setter today. That is a
 * statement about the setter, not about reachability — this file already runs
 * inside the Worker. worker/index.ts routes every domain RPC through
 * worker/routes/domains.ts, which imports server/gateway.ts (line 17), which
 * imports the three limiters below (server/gateway.ts line 27). So
 * @upstash/ratelimit and @upstash/redis are in the Worker bundle already, by
 * this path and independently by way of api/_rate-limit.js. Bundle size is not
 * a reason to leave the setter unwired, and any comment here that says
 * otherwise is stale — an earlier version of this block claimed the market
 * route does not import this file, which was never true.
 *
 * The market RPCs simply do not need the counter. They call checkRateLimit
 * with no options, so the missing-Upstash branch below returns null and they
 * fail open. Configure Upstash and they use it; leave it unset and they run
 * unlimited. Neither outcome touches getCounterStub.
 *
 * One caller does need it, and it is served elsewhere: api/_rate-limit.js, the
 * plain-JS sibling with the same counterEnv/setRateLimitEnv/getCounterStub
 * shape, got its own copy of this seam in Plan 4d Task 4's fix round.
 * worker/index.ts calls THAT setter on every request so the wms_ session
 * minter (api/wm-session.js) rate-limits through CounterDO instead of failing
 * closed with a 503 when Upstash is absent.
 *
 * Wiring this module's own setter is deferred to whichever future task moves a
 * caller of checkEndpointRateLimit/checkScopedRateLimit onto the Worker. The
 * open question then is not whether to wire it but which binding instance that
 * caller should share — this module's or api/_rate-limit.js's. Two modules
 * holding separate counterEnv values will happily count the same traffic
 * twice.
 *
 * Until a caller sets THIS module's env, getCounterStub() below always
 * returns null and every limiter here takes the exact Upstash path it takes
 * today — this seam turns "move onto CounterDO" into a config change for
 * that future caller, not a rewrite of this file a second time.
 */
export function setRateLimitEnv(env: RateLimitCounterEnv | null): void {
  counterEnv = env;
}

function getCounterStub(key: string): CounterStub | null {
...
async function checkRateLimitViaCounter(stub: CounterStub, identifier: string, corsHeaders: Record<string, string>, opts: RateLimitOptions): Promise<Response | null> {
  try {
    const result = await callCounter(stub, { op: 'sliding', key: `rl:${identifier}`, limit: GLOBAL_RATE_LIMIT, windowMs: GLOBAL_RATE_WINDOW_SECONDS * 1000 });
    if (result.op !== 'sliding') throw new Error(`counter DO returned unexpected op ${result.op}`);
    if (!result.success) {
      return tooManyRequestsResponse(result.limit, result.reset, corsHeaders, GLOBAL_RATE_WINDOW_SECONDS);
    }
    return null;
  } catch (err) {
    logRateLimitDegraded('checkRateLimit:counter-do', err);
...
export async function checkRateLimit(request: Request, corsHeaders: Record<string, string>, opts: RateLimitOptions = {}): Promise<Response | null> {
  ...
  // principals use a separate namespace. Resolved ONCE, above the backend
  // branch, so the Durable Object shard and the Redis key address the same
  // principal — deriving it per-leg is how the two backends silently drift
  // into metering different buckets for one caller.
  const identifier =
    getPrincipalRateLimitIdentifier(opts.principalUserId) ??
    getClientIp(request);

  const stub = getCounterStub(`rl:${identifier}`);
  if (stub) return checkRateLimitViaCounter(stub, identifier, corsHeaders, opts);
...
async function checkEndpointRateLimitViaCounter(
  stub: CounterStub,
  pathname: string,
  policy: EndpointRatePolicy,
  identifier: string,
  ...
  try {
    ...
    logRateLimitDegraded(`checkEndpointRateLimit:${pathname}:counter-do`, err);
...
export async function checkEndpointRateLimit(request: Request, pathname: string, corsHeaders: Record<string, string>, opts: EndpointRateLimitOptions = {}): Promise<Response | null> {
  ...
  const stub = getCounterStub(`rl:ep:${pathname}:${identifier}`);
  if (stub) return checkEndpointRateLimitViaCounter(stub, pathname, policy, identifier, corsHeaders, opts);
 ...
 */
async function checkScopedRateLimitViaCounter(
  stub: CounterStub,
  scope: string,
  limit: number,
  window: Duration,
  ...
  try {
    const result = await callCounter(stub, {
      op: 'sliding',
      key: `rl:scope:${scope}:${identifier}`,
      limit,
      windowMs: durationToSeconds(window) * 1000,
    });
    if (result.op !== 'sliding') throw new Error(`counter DO returned unexpected op ${result.op}`);
    return { allowed: result.success, limit: result.limit, reset: result.reset, degraded: false };
  } catch (err) {
    // checkScopedRateLimit never fails closed itself (see ScopedRateLimitResult
    // doc comment) — the CounterDO path keeps that same contract: an error
    // here degrades to fail-open with `degraded: true`, exactly like the
    // Upstash path below, and lets the caller decide.
    logRateLimitDegraded(`checkScopedRateLimit:${scope}:counter-do`, err);
...
export async function checkScopedRateLimit(scope: string, limit: number, window: Duration, identifier: string): Promise<ScopedRateLimitResult> {
  const stub = getCounterStub(`rl:scope:${scope}:${identifier}`);
  if (stub) return checkScopedRateLimitViaCounter(stub, scope, limit, window, identifier);
