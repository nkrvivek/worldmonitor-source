...
export type AuthKind =
  // Dashboard session bearer. The tokens have been Supabase-issued since the
  // fork; the name stays because it is a wire value, not a code identifier.
  // Axiom dashboards and saved queries filter on this exact string, and
  // renaming it would silently empty every one of them.
  | 'clerk_jwt'
  | 'user_api_key'
  | 'enterprise_api_key'
  | 'widget_key'
  ...
  // JWT traffic in Axiom; customer_id carries the same userId.
  | 'mcp_oauth'
  | 'anon';
...
export function buildUsageIdentity(input: UsageIdentityInput): UsageIdentity {
  ...
  if (input.sessionUserId) {
    return {
      auth_kind: 'clerk_jwt',
      principal_id: input.sessionUserId,
      customer_id: input.orgId ?? input.sessionUserId,
      tier,
      plan_key: input.planKey,
    };
  }
