...
export async function getCableHealth(
  ...
  try {
    const result = await cachedFetchJson<GetCableHealthResponse>(CACHE_KEY, CACHE_TTL, async () => {
      ...
      // Computed response cached CACHE_TTL (2h) — long enough that the key outlives health.js's
      // 90-minute freshness bound; see the constant for why that ordering is load-bearing.
