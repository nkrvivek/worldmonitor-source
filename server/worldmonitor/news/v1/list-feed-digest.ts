...
import { getRelayBaseUrl, getRelayHeaders, relayFetch } from '../../../_shared/relay';
  ...
  try {
    ...
    if (!text) {
      ...
      if (relayBase) {
        ...
        const { controller, cleanup } = createTimeoutLinkedController(signal);
        try {
          const resp = await relayFetch(relayUrl, {
...
/**
 * Run `worker` over every item with at most `limit` in flight, refilling a slot
 * the moment one frees rather than at a batch boundary.
 *
 * This used to be fixed slices awaited with Promise.allSettled. That is a
 * barrier: a slice costs as long as its slowest feed, and every slot freed by a
 * fast feed sits idle until the straggler returns. Against a wall-clock
 * deadline the idle time is not merely slow, it truncates the list, and always
 * at the same place, because the feed order is fixed. Measured in production
 * 2026-08-13: 118 of 222 feeds marked 'timeout' on every build of the full
 * digest, and the corpus that reached seed-insights was the first four
 * categories at their per-category cap and nothing after them. Nothing threw
 * and nothing looked broken; the tail of the feed list simply never ran.
 *
 * Results keep input order. A slot that stops early leaves a hole, so callers
 * must treat an absent entry as "never ran", which is what marks it 'timeout'.
 */
export async function mapWithConcurrency<T, R>(
  ...
  // One shared iterator hands each index to exactly one runner. next() is
  // synchronous, so the runners cannot interleave inside it. Indexing with a
  // cursor did the same thing but typed the element as T | undefined under
  // noUncheckedIndexedAccess, and the only ways to spend that were a cast or a
  // skip that would silently drop work.
