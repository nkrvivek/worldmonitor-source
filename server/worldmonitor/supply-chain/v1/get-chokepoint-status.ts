...
interface PreBuiltTransitSummary {
  ...
  // Optional for the same reason, and defaulted the other way. Absence means
  // the writer predates the flag, so nothing states whether the AIS relay
  // answered — and an unmarked zero count is the fault this field exists to
  // close. Unknown reads as unavailable until a current writer says otherwise.
  transitCountsAvailable?: boolean;
}
  ...
  const chokepoints = CHOKEPOINTS.map((cp): ChokepointInfo => {
    ...
    return {
      ...
      transitSummary: ts ? {
        ...
        // Default false, unlike dataAvailable above: an absent flag means we
        // cannot say the relay was up, and a count nobody vouches for is the
        // thing this reports.
        transitCountsAvailable: ts.transitCountsAvailable ?? false,
      } : { todayTotal: 0, todayTanker: 0, todayCargo: 0, todayOther: 0, wowChangePct: 0, history: [], riskLevel: '', incidentCount7d: 0, disruptionPct: 0, riskSummary: '', riskReportAction: '', dataAvailable: false, transitCountsAvailable: false },
  ...
  try {
    const result = await cachedFetchJson<GetChokepointStatusResponse>(
      ...
      async () => {
        ...
        // Awaited: a floating write is dropped when the isolate returns, which
        // freezes freshness while every serve keeps running this line.
        await setCachedJson('seed-meta:supply_chain:chokepoints', { fetchedAt: Date.now(), recordCount: coveredCount }, 604800).catch(() => {});
        return response;
      },
    );
