...
async function fetchVesselSnapshotFromRelay(
  ...
  try {
    ...
    if (!relayBaseUrl) {
      console.warn('[maritime] WS_RELAY_URL is not set; serving no vessel data');
      return undefined;
    }
    ...
    // Logged, not swallowed. Every failure below ends as an empty snapshot at
    // the caller, which is indistinguishable from a quiet ocean unless the
    // reason is written down somewhere. 401 here means the relay secret did
    // not travel; 404 means the path is not served.
    if (!response.ok) {
      console.warn(`[maritime] relay ${relayBaseUrl}/ais/snapshot returned HTTP ${response.status}`);
      return undefined;
    }

    // @cloudflare/workers-types types Response.json() as Promise<unknown>
    // (lib.dom.d.ts typed it Promise<any>); cast at this boundary rather than
    // add shape validation this handler never had.
    const data = (await response.json()) as any;
    if (!data || !Array.isArray(data.disruptions) || !Array.isArray(data.density)) {
      console.warn('[maritime] relay answered 200 without disruptions/density arrays');
      return undefined;
    }
    ...
    console.warn(`[maritime] relay fetch threw: ${err instanceof Error ? err.message : String(err)}`);
    return undefined;
  }
}
