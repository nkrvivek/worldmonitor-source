...
// The seeder owns `market:stock-index:v1:<country>` for audit-backed snapshots.
...
const SEEDED_COUNTRY_INDEX_KEY = 'market:stock-index:v1:CN';
...
export async function getCountryStockIndex(
  ...
  // The seeder owns the CN snapshot and writes it unprefixed, so read it raw.
  // Prefer that last-good payload before serving the RPC cache. The fallback
  // cache below intentionally uses a separate key so a failed request cannot
  // replace this seed-owned record with a negative sentinel.
  if (code === 'CN') {
    const seededSnapshot = await getCachedJson(SEEDED_COUNTRY_INDEX_KEY, true);
    if (isAvailableCountryStockIndex(seededSnapshot)) {
      stockIndexCache[code] = { data: seededSnapshot, ts: Date.now() };
