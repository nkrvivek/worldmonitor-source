...
import { getRelayBaseUrl, getRelayHeaders, relayFetch } from './_shared';
            ...
            cacheKey, positiveTtl, async () => {
                ...
                if (isCallsignOnly && relayBase) {
                    try {
                        ...
                        const wbResp = await relayFetch(wbUrl, {
                ...
                if (!isCallsignOnly && relayBase && req.swLat != null && req.neLat != null) {
                    ...
                    const [osResult, wbResult] = await Promise.allSettled([
                        relayFetch(osUrl, { headers: getRelayHeaders({}), signal: AbortSignal.timeout(10_000) })
                        ...
                        relayFetch(wbUrl, { headers: getRelayHeaders({}), signal: AbortSignal.timeout(10_000) })
                ...
                if (!isCallsignOnly && relayBase && req.icao24) {
                    try {
                        ...
                        const resp = await relayFetch(osUrl, { headers: getRelayHeaders({}), signal: AbortSignal.timeout(8_000) });
                        if (resp.ok) {
                            const data = await resp.json() as OpenSkyResponse;
