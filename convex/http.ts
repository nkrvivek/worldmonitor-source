...
import { allowedReturnOrigins, resolveSiteUrl } from "./payments/returnUrl";
...
/**
 * Origins allowed to call these HTTP actions from a browser.
 *
 * Upstream listed its own apex plus a `*.worldmonitor.app` wildcard. This fork
 * controls no worldmonitor.app name, so both entries handed CORS access to
 * somebody else's site. The list now comes from the same place checkout's
 * return-URL guard reads — SITE_URL, widened by ALLOWED_RETURN_ORIGINS — so a
 * new host is one env change, not two.
 */
const DEV_ORIGIN = "http://localhost:3000";

function trustedOrigins(): string[] {
  return [
    ...allowedReturnOrigins(resolveSiteUrl(), process.env.ALLOWED_RETURN_ORIGINS),
    DEV_ORIGIN,
  ];
}
...
function corsHeaders(origin: string | null): Headers {
  ...
  const allowed = allowedOrigin(origin, trustedOrigins());
  if (allowed) {
