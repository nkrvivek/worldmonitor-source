...
// Subscription status enum — maps Stripe statuses to our internal set
...
export default defineSchema({
  ...
  // --- Payment tables (Stripe integration) ---
  // Billing moved from Dodo Payments to Stripe. Function names that still
  // carry the old provider (reconcileMissedDodoRenewals, markDodoReconcileAttempt)
  // keep it on purpose: renaming them is a separate change, and the comments
  // below name the provider that actually answers today.
  ...
  subscriptions: defineTable({
    ...
    // (the Stripe customer this sub was paid as). Optional because
    // `SubscriptionEventData.customer` is itself optional and lifecycle
    ...
    // ?? existing.providerCustomerId` (see `mergeProviderCustomerId` in
    // `subscriptionHelpers.ts`) so it survives lifecycle patches.
    //
    // Manage Billing prefers this column when populated — see
    // `payments/billing:getProviderCustomerIdForUserPortal`, which is a
    // 3-tier resolver (this column → `rawPayload.customer.customer_id`
    // → `customers.providerCustomerId` for the same userId). Pre-PR rows
    // may still rely on tiers 2-3 until
    // `backfillSubscriptionProviderCustomerId` lands their values here.
    ...
    // against the live Stripe client) so they stop starving the batch's scan
    ...
    // Count of CONSECUTIVE definitive provider 404s (reset by any non-404 reconcile
    // outcome). Distinct from `reconcileFailureCount` (which counts all failure
    // kinds for backoff) so the terminal "subscription deleted at the provider"
    ...
    // concurrent premium requests cannot fan out into duplicate Stripe lookups.
  ...
  dunningEmails: defineTable({
    ...
    email: v.string(),
    sentAt: v.number(),
  }).index("by_sub_step_episode", ["providerSubscriptionId", "step", "episodeAt"]),
  ...
  entitlements: defineTable({
    ...
    features: v.object({
      ...
      // plan 2026-05-10-001 (Pro MCP). Stripe webhooks repopulate this on
    ...
    // goodwill credits outlive subscription cancellations at the provider.
  ...
  // `customers` (which is paid-only, populated by the subscription webhook):
  ...
  // Durable dead-letter records for billing events that fail processing. Keep
  // this projection intentionally payload-free: operators need stable provider
  ...
  productPlans: defineTable({
    ...
    .index("by_providerPriceId", ["providerPriceId"])
  ...
  // Business `providerSubscriptionId` (KTD1) — NOT a fake subscription — so
