...
async function seedSub(
  ...
  await t.run(async (ctx) => {
    await ctx.db.insert("subscriptions", {
      ...
      providerSubscriptionId: overrides.providerSubscriptionId ?? SUB_ID,
      providerPriceId: "pdt_test",
      ...
      rawPayload:
        overrides.email === null
          ? { subscription_id: overrides.providerSubscriptionId ?? SUB_ID }
          : { subscription_id: overrides.providerSubscriptionId ?? SUB_ID, customer: { email: overrides.email ?? EMAIL } },
...
describe("on_hold webhook → day-0 email", () => {
  test("transition into on_hold sets the anchor and sends day-0 once", async () => {
    ...
    const sub = await t.run(async (ctx) =>
      ctx.db
        .query("subscriptions")
        .withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", SUB_ID))
        .unique(),
    );
...
describe("sendDunningEmail guards", () => {
  test("second invocation for the same step+episode is a no-op", async () => {
    ...
    const args = { providerSubscriptionId: SUB_ID, step: "dunning_day3" as const, episodeAt: anchor };
...
describe("runDunningScan windows", () => {
  ...
  test("batch scan staggers send START times (first pacing layer, WORLDMONITOR-VH)", async () => {
    ...
    for (let i = 0; i < N; i++) {
      await seedSub(t, {
        providerSubscriptionId: `sub_burst_${i}`,
...
describe("winback", () => {
  ...
  test("cancelled-but-paid-through sibling still counts as covered (round-2 F2)", async () => {
    ...
    await seedSub(t, {
      providerSubscriptionId: "sub_annual_paid_through",
      status: "cancelled",
  ...
  test("resubscribed user (live sibling sub) is not winback-emailed", async () => {
    ...
    await seedSub(t, {
      providerSubscriptionId: "sub_new_life",
      status: "active",
