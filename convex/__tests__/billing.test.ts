...
// Mock the Stripe SDK so the reconciliation action's provider reads are
// controllable per-test (no real network). billing.ts only news up Stripe
// inside getStripeClient(), so a class stub is sufficient. No other test in
// this file exercises the real SDK.
const { paymentIntentsRetrieveMock, checkoutSessionsListMock } = vi.hoisted(
  () => ({
...
const CHECKOUT_SESSION_URL = "https://checkout.stripe.com/c/pay/cs_stuck";

/**
 * Script the two reads billing.ts makes per stuck payment. The PaymentIntent
 * carries the status; the Checkout Session that created it carries the buyer's
 * email and the resume link. Neither one alone answers both questions, which
 * is why the production path makes two calls.
 *
 * The intent is scripted by id so a batch of candidates each reads back its
 * own payment id.
 */
function stubStripeStuckPayment(
  opts: { status?: string | null; email?: string | null; url?: string | null } = {},
): void {
  const {
  ...
  checkoutSessionsListMock.mockResolvedValue({
    data: [
      {
        id: "cs_stuck",
        url,
        customer_details: email ? { email, name: null } : null,
        subscription: null,
      },
    ],
  });
...
afterEach(() => {
  ...
  delete process.env.ANON_CLAIM_TOKEN_TTL_MS;
...
async function seedSubscription(
  ...
  return await t.run(async (ctx) => {
    return await ctx.db.insert("subscriptions", {
      ...
      providerSubscriptionId: `sub_billing_${opts.suffix}`,
      providerPriceId: opts.providerPriceId,
...
async function seedAnonClaimState(
  ...
  const providerPriceId = PRODUCT_CATALOG[planKey].providerPriceId!;
  await t.run(async (ctx) => {
    await ctx.db.insert("subscriptions", {
      userId: anonId,
      providerSubscriptionId: "sub_anon_claim_001",
      providerPriceId,
      planKey,
      status: "active",
    ...
    await ctx.db.insert("customers", {
      userId: anonId,
      providerCustomerId: "cus_anon_claim_001",
    ...
    await ctx.db.insert("paymentEvents", {
      userId: anonId,
      providerPaymentId: "pay_anon_claim_001",
      type: "charge",
      amount: 3999,
      currency: "USD",
      status: "succeeded",
...
describe("claimSubscription anonymous ownership proof", () => {
  test("rejects a bare anon UUID when protected payment rows exist", async () => {
    process.env.IDENTITY_SIGNING_SECRET = SIGNING_SECRET;
  ...
  test("keeps existing higher-tier entitlement precedence when proof is valid", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: CLAIMANT_A.subject,
        providerSubscriptionId: "sub_real_api_business",
        providerPriceId: PRODUCT_CATALOG.api_business.providerPriceId!,
        planKey: "api_business",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
...
describe("payments billing Business Pro grants entitlement resolution", () => {
  ...
  test("invitee with no own subscription resolves to Pro via accepted grant", async () => {
    ...
    await seedBusinessSub(t, {
      providerSubscriptionId: businessSubId,
      status: "active",
      currentPeriodEnd: NOW + 30 * DAY_MS,
    });
  ...
  test("invitee who also has own Pro subscription keeps own Pro", async () => {
    ...
    await seedSubscription(t, {
      planKey: "pro_annual",
      providerPriceId: PRODUCT_CATALOG.pro_annual.providerPriceId!,
      status: "active",
  ...
  test("invitee with higher-tier own subscription keeps own plan over grant", async () => {
    ...
    await seedSubscription(t, {
      planKey: "api_starter",
      providerPriceId: PRODUCT_CATALOG.api_starter.providerPriceId!,
      status: "active",
      currentPeriodEnd: NOW + 30 * DAY_MS,
...
describe("payments billing duplicate-checkout guard", () => {
  test("does not block checkout when the user has no subscriptions", async () => {
    ...
    const result = await t.query(
      ...
      {
        userId: TEST_USER_ID,
        productId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
      },
    );
  ...
  test("blocks checkout when an on_hold subscription exists in the same tier group", async () => {
    ...
    await seedSubscription(t, {
      planKey: "pro_monthly",
      providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
      status: "on_hold",
    ...
    const result = await t.query(
      ...
      {
        userId: TEST_USER_ID,
        productId: PRODUCT_CATALOG.pro_annual.providerPriceId!,
      },
    );
  ...
  test("blocks checkout when a cancelled subscription still has time remaining", async () => {
    ...
    const result = await t.query(
      ...
      {
        userId: TEST_USER_ID,
        productId: PRODUCT_CATALOG.api_starter_annual.providerPriceId!,
      },
    );
  ...
  test("blocks a Business checkout while an API Starter subscription is active", async () => {
    ...
    const result = await t.query(
      ...
      {
        userId: TEST_USER_ID,
        productId: PRODUCT_CATALOG.api_business.providerPriceId!,
      },
    );
  ...
  test("blocks a Starter checkout while an API Business subscription is active", async () => {
    ...
    const result = await t.query(
      ...
      {
        userId: TEST_USER_ID,
        productId: PRODUCT_CATALOG.api_starter.providerPriceId!,
      },
    );
  ...
  // The two Pro prices are one billing family: buying the annual price while
  // the monthly subscription is live must hit the duplicate dialog instead of
  // stacking two concurrent Pro subscriptions. Pro Business used to sit in
  // this family too; it merged into Pro on 2026-08-05.
  test("blocks an annual Pro checkout while a monthly Pro subscription is active", async () => {
    const t = convexTest(schema, modules);
  ...
  test("blocks an annual Pro checkout while an on_hold monthly Pro subscription exists", async () => {
    const t = convexTest(schema, modules);
    ...
    await seedSubscription(t, {
      ...
      status: "on_hold",
      currentPeriodEnd: NOW + 7 * DAY_MS,
      suffix: "on_hold_pro_blocks_pro_annual",
    });
  ...
  test("an active Pro subscription does not block an API checkout", async () => {
    const t = convexTest(schema, modules);
...
async function seedPaymentEvent(
  ...
  await t.run(async (ctx) => {
    await ctx.db.insert("paymentEvents", {
      userId: opts.userId ?? TEST_USER_ID,
      providerPaymentId: opts.providerPaymentId ?? `pay_billing_${opts.suffix}`,
...
describe("payments pending-payment dedup guard", () => {
  ...
  // pending row once the SAME providerPaymentId reached a terminal state — otherwise
  ...
  test("does not block when the same payment later FAILED (append-only terminal row)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_appendonly_001",
    });
    await seedPaymentEvent(t, {
      status: "failed",
      planKey: "pro_monthly",
  ...
  test("does not block when the same payment later SUCCEEDED (append-only terminal row)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_appendonly_002",
    });
    await seedPaymentEvent(t, {
      status: "succeeded",
      planKey: "pro_monthly",
  ...
  // A genuinely-pending payment (no terminal row for its providerPaymentId) must
  ...
  test("still blocks a genuinely-pending payment alongside an unrelated terminal payment", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_unrelated_terminal",
    });
    await seedPaymentEvent(t, {
      status: "requires_customer_action",
      planKey: "pro_monthly",
      ...
      providerPaymentId: "pay_genuinely_pending",
    });
...
describe("payments stuck-pending reconciliation", () => {
  test("finds stale unresolved pending payments but skips recent, terminal, and already-marked rows", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_reconcile_candidate",
    });
    await seedPaymentEvent(t, {
      status: "processing",
      planKey: "pro_monthly",
      ...
      providerPaymentId: "pay_reconcile_recent",
    });
    await seedPaymentEvent(t, {
      status: "requires_customer_action",
      planKey: "pro_monthly",
      ...
      providerPaymentId: "pay_reconcile_terminal",
    });
    await seedPaymentEvent(t, {
      status: "failed",
      planKey: "pro_monthly",
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_reconcile_marked",
    });
    await t.run(async (ctx) => {
  ...
  test("candidate selection is bounded by batch size", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await seedPaymentEvent(t, {
        ...
        suffix: `batch_${i}`,
        providerPaymentId: `pay_reconcile_batch_${i}`,
      });
    }
  ...
  test("scans newest-first so freshly-stuck rows win a limited batch (F5)", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await seedPaymentEvent(t, {
        ...
        providerPaymentId: `pay_scan_order_${i}`,
      });
    }
    ...
    expect(candidates.map((c) => c.providerPaymentId)).toEqual([
      "pay_scan_order_0",
      "pay_scan_order_1",
    ]);
  });
  ...
  test("claims a dropped-webhook terminal payment once, backfilling the terminal row", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_reconcile_terminal_record",
    });

    const payload = {
      userId: TEST_USER_ID,
      providerPaymentId: "pay_reconcile_terminal_record",
      providerSubscriptionId: "sub_reconcile_terminal_record",
      planKey: "pro_monthly",
      amount: 3999,
      currency: "USD",
    ...
    const rows = await t.run(async (ctx) =>
      ctx.db
        .query("paymentEvents")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_terminal_record"))
        .collect(),
    );
  ...
  test("terminal SUCCEEDED with a matching subscription row does NOT page ops", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_succeeded_withsub",
    });
    await seedSubscription(t, {
      planKey: "pro_monthly",
      providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
    ...
    const result = await t.mutation(internal.payments.billing.claimStuckPaymentReconciliation, {
      userId: TEST_USER_ID,
      providerPaymentId: "pay_succeeded_withsub",
      providerSubscriptionId: "sub_billing_reconcile_covered",
      planKey: "pro_monthly",
      amount: 3999,
      currency: "USD",
      pendingOccurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
  ...
  test("claim returns already_terminal (no marker) when a terminal row already exists (race)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      occurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
      suffix: "race_pending",
      providerPaymentId: "pay_reconcile_race",
    });
    await seedPaymentEvent(t, {
      status: "succeeded",
      planKey: "pro_monthly",
    ...
    const markers = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_race"))
        .collect(),
    );
  ...
  test("claim writes a provisional ops marker for a recognised pending status", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      occurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
      suffix: "ops_marker",
      providerPaymentId: "pay_reconcile_ops_marker",
    });
    ...
    const rows = await t.run(async (ctx) =>
      ctx.db
        .query("paymentEvents")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_ops_marker"))
        .collect(),
    );
    const markers = await t.run(async (ctx) =>
      ctx.db
  ...
  test("claim writes a marker recording the RAW status for an UNRECOGNISED non-terminal status (F1)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_reconcile_unknown",
    });
    ...
    const markers = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_unknown"))
        .collect(),
    );
    expect(markers).toHaveLength(1);
    expect(markers[0]).toMatchObject({
  ...
  test("finalize(notified) upgrades a claimed marker to customer_notified, and never downgrades", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_reconcile_finalize_customer",
    });
    ...
    const markers = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_finalize_customer"))
        .collect(),
    );
    expect(markers).toHaveLength(1);
  ...
  test("finalize(ops) keeps the marker ops_notified", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      occurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
      suffix: "finalize_ops",
      providerPaymentId: "pay_reconcile_finalize_ops",
    });
    await t.mutation(internal.payments.billing.claimStuckPaymentReconciliation, {
      userId: TEST_USER_ID,
      providerPaymentId: "pay_reconcile_finalize_ops",
    ...
    const markers = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_reconcile_finalize_ops"))
        .collect(),
    );
  ...
  test("action claims the marker BEFORE sending the customer email (F3 idempotency)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      occurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
      suffix: "action_order",
      providerPaymentId: "pay_action_order",
    });

    stubStripeStuckPayment();
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input: any) => {
      if (String(input).includes("api.resend.com")) {
        const marker = await t.run(async (ctx) =>
          ctx.db
            .query("paymentReconciliationAttempts")
            .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_action_order"))
            .first(),
        );
  ...
  test("action marks a payment whose provider status is NULL and never emails it (F1)", async () => {
    vi.setSystemTime(NOW);
    process.env.STRIPE_SECRET_KEY = "sk_test_billing";
    process.env.RESEND_API_KEY = "test_resend_key";
    const t = convexTest(schema, modules);
    ...
    await seedPaymentEvent(t, {
      ...
      occurredAt: NOW - STUCK_PAYMENT_RECONCILIATION_THRESHOLD_MS - MIN_MS,
      suffix: "null_status",
      providerPaymentId: "pay_null_status",
    });

    // The provider returns a payment object carrying no usable status.
    stubStripeStuckPayment({ status: null });
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
    ...
    const marker = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_null_status"))
        .first(),
    );
  ...
  test("action marks a non-null UNRECOGNISED status without emailing (F1)", async () => {
    ...
    await seedPaymentEvent(t, {
      ...
      providerPaymentId: "pay_unrecognised",
    });

    stubStripeStuckPayment({ status: "requires_payment_method" });
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response("{}", { status: 200 }),
    );
    ...
    const marker = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_unrecognised"))
        .first(),
    );
  ...
  test("action isolates a Resend failure to one candidate and still processes the rest (F6/F7)", async () => {
    ...
    const markerA = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_iso_a"))
        .first(),
    );
    const markerB = await t.run(async (ctx) =>
      ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_iso_b"))
        .first(),
    );
...
// at portal-open time and recovers the providerCustomerId from the
...
describe("payments billing repairCustomerFromSubscriptionPayload", () => {
  test("inserts a customers row from rawPayload.customer.customer_id and returns it", async () => {
    ...
    expect(result).toMatchObject({
      userId: TEST_USER_ID,
      providerCustomerId: "cus_recovered_001",
    ...
    expect(second?.providerCustomerId).toBe("cus_recovered_001");
  ...
  test("prefers active subscription's payload over cancelled when both exist", async () => {
    ...
    expect(result?.providerCustomerId).toBe("cus_active_winner");
  });

  test("refuses to remap when the providerCustomerId already belongs to a different userId", async () => {
    const t = convexTest(schema, modules);
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("customers", {
        ...
        providerCustomerId: "cus_collision_001",
    ...
    // TEST_USER_ID's subscription rawPayload happens to carry the same providerCustomerId
    ...
    const stillOriginal = await t.run(async (ctx) =>
      ctx.db
        .query("customers")
        .withIndex("by_providerCustomerId", (q) => q.eq("providerCustomerId", "cus_collision_001"))
        .first(),
    );
    expect(stillOriginal?.userId).toBe("user_other_owner");
  });

  test("patches existing customers row that lacks providerCustomerId instead of inserting a duplicate", async () => {
    // Greptile P1 — a customers row can exist for this userId without a
    // providerCustomerId (the field is v.optional). Repair must update the
    ...
    const existingId = await t.run(async (ctx) =>
      ctx.db.insert("customers", {
        userId: TEST_USER_ID,
        // providerCustomerId intentionally omitted (v.optional schema state)
    ...
    expect(result?.providerCustomerId).toBe("cus_patched_001");
  ...
  test("does NOT blank out a pre-existing email when payload email is missing", async () => {
    ...
    expect(result?.providerCustomerId).toBe("cus_emailless");
...
describe("payments billing backfillMissingCustomers", () => {
  test("repairs users with subscriptions but no customers row, leaves healthy users alone", async () => {
    ...
    // Confirm A now has a customers row with the right providerCustomerId.
    ...
    expect(aCustomer?.providerCustomerId).toBe("cus_a");
  ...
  test("patches an existing customers row that lacks providerCustomerId instead of inserting a duplicate", async () => {
    ...
    // row without providerCustomerId, patch it rather than inserting.
    const t = convexTest(schema, modules);

    const existingId = await t.run(async (ctx) =>
      ctx.db.insert("customers", {
        userId: "user_backfill_patch",
        // providerCustomerId intentionally omitted
    ...
    expect(rows[0]?.providerCustomerId).toBe("cus_backfill_patch");
...
// ---------------------------------------------------------------------------
// getProviderCustomerIdForUserPortal — read straight from the user's preferred
...
describe("payments billing getProviderCustomerIdForUserPortal", () => {
  test("returns null when the user has no subscriptions at all", async () => {
    const t = convexTest(schema, modules);
    const result = await t.query(
  ...
  test("returns the providerCustomerId from the active subscription's rawPayload", async () => {
    const t = convexTest(schema, modules);
    await seedSubscription(t, {
      planKey: "pro_annual",
      providerPriceId: PRODUCT_CATALOG.pro_annual.providerPriceId!,
  ...
  test("prefers active over on_hold over cancelled, ignoring the customers table entirely", async () => {
    ...
    // A customers row exists for this user but with a STALE/WRONG providerCustomerId
    // — this lookup must ignore it and read from the active subscription.
    await t.run(async (ctx) => {
      await ctx.db.insert("customers", {
        userId: TEST_USER_ID,
        providerCustomerId: "cus_stale_from_customers_table",
        email: "stale@example.com",
        normalizedEmail: "stale@example.com",
  ...
  test("returns the right providerCustomerId for each Clerk user when SAME Dodo customer is shared across multiple Clerk accounts (the WORLDMONITOR-R5 scenario)", async () => {
  ...
  test("resolves via the stable providerCustomerId column even when a later lifecycle payload wiped the rawPayload customer field (P1 regression)", async () => {
    ...
    // `customer` field. The stable top-level `providerCustomerId` column
    // written by the webhook handler (via `mergeProviderCustomerId`)
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_lifecycle_wiped",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
  ...
  test("falls back to the same-user customers row when neither stable column nor rawPayload has the customer_id (P1 reviewer regression)", async () => {
    ...
    // userId still has a usable providerCustomerId — that's the right
    // answer, better than NO_CUSTOMER for a paying user.
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.db.insert("customers", {
        userId: TEST_USER_ID,
        providerCustomerId: "cus_from_customers_tier3",
      ...
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_tier3_rescue",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
        currentPeriodEnd: NOW + 30 * DAY_MS,
        // providerCustomerId column intentionally absent
        rawPayload: {}, // wiped
        updatedAt: NOW,
      });
    });
  ...
  test("does NOT use the customers row when it belongs to a different userId (no silent re-attribution)", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("customers", {
        ...
        providerCustomerId: "cus_belongs_to_someone_else",
        email: "other@example.com",
        normalizedEmail: "other@example.com",
        createdAt: NOW - 10 * DAY_MS,
        updatedAt: NOW - 10 * DAY_MS,
      ...
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_tier3_no_match",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
  ...
  test("falls back to rawPayload.customer.customer_id when the stable column is absent (pre-schema-change rows)", async () => {
    ...
    // top-level `providerCustomerId`. The query falls back to the rawPayload
    // value so they keep working until the backfill mutation catches up.
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_pre_schema",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
        currentPeriodEnd: NOW + 30 * DAY_MS,
        // providerCustomerId intentionally omitted (pre-schema-change state)
        rawPayload: {
...
// ---------------------------------------------------------------------------
// backfillSubscriptionProviderCustomerId — one-shot populate the new column
// from rawPayload for rows that pre-date the schema change.
// ---------------------------------------------------------------------------

describe("payments billing backfillSubscriptionProviderCustomerId", () => {
  test("populates from rawPayload, falls back to customers row, skips already-populated, reports unrecoverable count", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "user_backfill_A",
        providerSubscriptionId: "sub_backfill_A",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "user_backfill_B",
        providerSubscriptionId: "sub_backfill_B",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
        currentPeriodEnd: NOW + 30 * DAY_MS,
        providerCustomerId: "cus_B_already",
    ...
    // still has providerCustomerId (P1 reviewer's scenario). Recoverable
    // via Source 2.
    await t.run(async (ctx) => {
      await ctx.db.insert("customers", {
        userId: "user_backfill_C",
        providerCustomerId: "cus_C_from_customers",
        email: "c@example.com",
      ...
      await ctx.db.insert("subscriptions", {
        userId: "user_backfill_C",
        providerSubscriptionId: "sub_backfill_C",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        ...
        providerSubscriptionId: "sub_backfill_D",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - DAY_MS,
    ...
    const summary = await t.mutation(
      internal.payments.billing.backfillSubscriptionProviderCustomerId,
      {},
    );
    expect(summary).toMatchObject({
      inspected: 4,
    ...
    expect(aRow?.providerCustomerId).toBe("cus_A");
    ...
    expect(cRow?.providerCustomerId).toBe("cus_C_from_customers");
    ...
    expect(dRow?.providerCustomerId).toBeUndefined();
...
describe("payments billing missed renewal reconciliation", () => {
  test("extends a stale local active subscription from active Dodo truth and recomputes entitlement", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_missed_renewal",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - 31 * DAY_MS,
        currentPeriodEnd: stalePeriodEnd,
        providerCustomerId: "cus_missed_renewal",
      ...
      const [sub, entitlement] = await Promise.all([
        ctx.db
          ...
          .withIndex("by_providerSubscriptionId", (q) =>
            q.eq("providerSubscriptionId", "sub_missed_renewal"),
          )
          .unique(),
        ctx.db
          .query("entitlements")
  ...
  test("continues reconciling other stale subscriptions when one Dodo lookup fails", async () => {
    ...
    await t.run(async (ctx) => {
      for (const suffix of ["ok", "missing"]) {
        ...
        await ctx.db.insert("subscriptions", {
          userId,
          providerSubscriptionId: `sub_reconcile_${suffix}`,
          providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
          planKey: "pro_monthly",
          status: "active",
          currentPeriodStart: NOW - 31 * DAY_MS,
    ...
    expect(summary.failures).toEqual([
      {
        providerSubscriptionId: "sub_reconcile_missing",
  ...
  async function seedStaleActiveForReconcile(
    ...
    const id = await t.run(async (ctx) => {
      const subId = await ctx.db.insert("subscriptions", {
        userId,
        providerSubscriptionId: `sub_${opts.suffix}`,
        providerPriceId: opts.providerPriceId ?? PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey,
        status: "active",
        currentPeriodStart: NOW - 31 * DAY_MS,
        currentPeriodEnd,
  ...
  const readSub = (t: ReturnType<typeof convexTest>, suffix: string) =>
    t.run(async (ctx) =>
      ctx.db
        .query("subscriptions")
        .withIndex("by_providerSubscriptionId", (q) =>
          q.eq("providerSubscriptionId", `sub_${suffix}`),
        )
        .unique(),
    );
  ...
  test("falls back to an enterprise entitlement for an unknown Dodo product id", async () => {
    ...
    const entitlement = await readEntitlement(t, TEST_USER_ID);
    expect(sub?.providerPriceId).toBe("pdt_unknown_reconcile_fallback");
  ...
  test("resolves a catalog-known but UNSEEDED product id to its own plan key, not the enterprise fallback", async () => {
    ...
    // window) must resolve to its own plan key — handing a $290 Pro Annual
    // buyer an Enterprise entitlement is a real over-grant, and the catalog
    // already knows the right answer. `apiAccess` is the tell: Pro does not
    // carry it and Enterprise does.
    const t = convexTest(schema, modules);
    const remotePeriodEnd = NOW + 30 * DAY_MS;
        ...
        remoteSubscriptionsForTest: [
          {
            ...
            product_id: PRODUCT_CATALOG.pro_annual.providerPriceId!,
            status: "active",
            previous_billing_date: new Date(NOW).toISOString(),
            next_billing_date: new Date(remotePeriodEnd).toISOString(),
          },
  ...
  test("mutation skips when the local row is no longer stale", async () => {
    ...
    const result = await t.mutation(
      internal.payments.billing.applyDodoSubscriptionReconciliation,
      {
        ...
        providerSubscriptionId: "sub_no_longer_stale",
        observedAt: NOW,
        remote: {
          providerSubscriptionId: "sub_no_longer_stale",
          productId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
  ...
  test("mutation skips when remote is not newer than the stale local row", async () => {
    ...
    const subId = await seedStaleActiveForReconcile(t, {
      ...
      providerCustomerId: "cus_not_newer",
      seedEntitlement: false,
    });
    ...
    const result = await t.mutation(
      ...
      {
        subscriptionId: subId,
        providerSubscriptionId: "sub_not_newer",
        observedAt: NOW,
        remote: {
          providerSubscriptionId: "sub_not_newer",
          productId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
  ...
  test("mutation refuses to clobber a concurrently-updated row (ordering guard)", async () => {
    ...
    const subId = await seedStaleActiveForReconcile(t, {
      suffix: "concurrent",
      planKey: "enterprise",
      providerPriceId: PRODUCT_CATALOG.enterprise.providerPriceId!,
      currentPeriodEnd: NOW - DAY_MS,
    ...
    const result = await t.mutation(
      ...
      {
        subscriptionId: subId,
        providerSubscriptionId: "sub_concurrent",
        observedAt: NOW,
        remote: {
          providerSubscriptionId: "sub_concurrent",
          productId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
    ...
    expect(sub?.providerPriceId).toBe(PRODUCT_CATALOG.enterprise.providerPriceId);
  ...
  test("does not let a permanently-failing row starve a healthy row sorted behind it", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "user_poison",
        providerSubscriptionId: "sub_poison",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
  ...
  test("advances the scan cursor past a backoff-saturated window to reach healthy rows behind it", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("subscriptions", {
        ...
        providerSubscriptionId: "sub_saturate_poison",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - 33 * DAY_MS,
      ...
      await ctx.db.insert("subscriptions", {
        ...
        providerSubscriptionId: "sub_saturate_healthy",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - 31 * DAY_MS,
  ...
  test("a single 404 after an unrelated prior failure does NOT downgrade (needs consecutive 404s)", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: TEST_USER_ID,
        providerSubscriptionId: "sub_mixed",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: NOW - 31 * DAY_MS,
  ...
  test("mass-404 circuit breaker caps downgrades per run and halts the rest", async () => {
    ...
    await t.run(async (ctx) => {
      for (let i = 0; i < N; i++) {
        await ctx.db.insert("subscriptions", {
          userId: `user_mass_${i}`,
          providerSubscriptionId: `sub_mass_${i}`,
          providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
          planKey: "pro_monthly",
          status: "active",
          currentPeriodStart: NOW - 31 * DAY_MS,
      ...
      const rows = await Promise.all(
        Array.from({ length: N }, (_, i) =>
          ctx.db
            .query("subscriptions")
            .withIndex("by_providerSubscriptionId", (q) =>
              q.eq("providerSubscriptionId", `sub_mass_${i}`),
            )
            .unique(),
        ),
      );
    ...
    await t.run(async (ctx) => {
      for (let i = 0; i < N; i++) {
        await ctx.db.insert("subscriptions", {
          ...
          providerSubscriptionId: `sub_cycle_${i}`,
          providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
          planKey: "pro_monthly",
          status: "active",
          currentPeriodStart: NOW - 31 * DAY_MS,
      ...
      const rows = await Promise.all(
        Array.from({ length: N }, (_, i) =>
          ctx.db
            .query("subscriptions")
            .withIndex("by_providerSubscriptionId", (q) =>
              q.eq("providerSubscriptionId", `sub_cycle_${i}`),
            )
            .unique(),
        ),
      );
          ...
          remoteSubscriptionsForTest: [
            {
              ...
              product_id: PRODUCT_CATALOG.enterprise.providerPriceId!,
              status: "expired",
              previous_billing_date: new Date(NOW - 31 * DAY_MS).toISOString(),
              next_billing_date: new Date(staleEnd).toISOString(),
            },
...
describe("getSubscriptionForUser activation onboarding eligibility", () => {
  ...
  test("admin subscription cleanup removes its activation presentation", async () => {
    ...
    await t.mutation(internal.payments.billing.deleteSubscriptionByDodoId, {
      providerSubscriptionId: "sub_billing_activation_cleanup",
      reason: "test cleanup",
    });
