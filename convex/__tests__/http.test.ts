...
const SITE_ORIGIN = new URL(DEFAULT_SITE_URL).origin;
...
describe("/api/user-prefs Convex HTTP action", () => {
  ...
  // Upstream trusted its own apex and every *.worldmonitor.app subdomain. This
  // fork owns none of them, so a browser on one of those hosts must not get a
  // CORS grant to call these actions with the visitor's session.
  test.each([
    "https://worldmonitor.app",
    "https://tech.worldmonitor.app",
    "https://evil.example.com",
  ])("refuses a CORS grant to %s", async (origin) => {
    const t = convexTest(schema, modules);
