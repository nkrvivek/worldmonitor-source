...
describe("api plan-limit usage scanner", () => {
  ...
  test("api daily detection reads the counter-route daily meter, keyed by userId", async () => {
    const t = convexTest(schema, modules);
    await seedEntitlement(t, "user-api", "api_starter");
    vi.stubEnv("AXIOM_QUERY_TOKEN", "test-token");
    vi.stubEnv("COUNTER_INTERNAL_HMAC_SECRET", "test-counter-secret");
    vi.stubEnv("WORKER_INTERNAL_BASE_URL", "https://worker.test");

    // Route the production path's outbound calls: Axiom returns nothing (so the
    // OLD count() source would yield no daily notice), while the counter route
    // (task-6a) reports present:true, count:1200 (> the 1000/day Starter cap).
    // A daily over_limit notice can therefore only come from reading the meter.
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL, init?: RequestInit) => {
      const u = String(url);
      if (u.includes("api.axiom.co")) {
        return new Response(JSON.stringify({ matches: [] }), { status: 200 });
      }
      if (u.includes("/internal/counter/daily-read")) {
        // The userId travels in the signed header, never the URL/body -- assert
        // that directly so a regression back to a hand-built key can't hide.
        const headers = new Headers(init?.headers);
        expect(headers.get("X-WM-MCP-User-Id")).toBe("user-api");
        return new Response(JSON.stringify({ present: true, count: 1200 }), { status: 200 });
      }
  ...
  test("a malformed meter body blocks the read instead of false-clearing a live notice", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL) => {
      ...
      if (u.includes("/internal/counter/daily-read")) return new Response(JSON.stringify({ present: true, count: 1200 }), { status: 200 });
      return new Response(JSON.stringify({ result: "0" }), { status: 200 });
    }));
    await t.action(usageFns.scanApiPlanLimitUsageInternal, { now: NOW });
    ...
    // Scan 2: the counter route returns a MALFORMED (non-JSON) body. That must
    // be a BLOCKED read (null), not usage:0 -- otherwise the ratio-0 recovery
    // path false-clears a paying customer's live over_limit notice on a route
    // hiccup.
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL) => {
      const u = String(url);
      if (u.includes("api.axiom.co")) return new Response(JSON.stringify({ matches: [] }), { status: 200 });
      if (u.includes("/internal/counter/daily-read")) return new Response("<html>bad gateway</html>", { status: 200 });
      return new Response(JSON.stringify({ result: "0" }), { status: 200 });
    }));
    ...
    expect(summary.blocked.some((b: { reason?: string }) => b.reason === "counter_read_failed")).toBe(true);
  });

  test("a present:false counter read reports api_daily_requests as blocked, not a zero-usage row", async () => {
    const t = convexTest(schema, modules);
    await seedEntitlement(t, "user-api", "api_starter");
    vi.stubEnv("COUNTER_INTERNAL_HMAC_SECRET", "test-counter-secret");
    vi.stubEnv("WORKER_INTERNAL_BASE_URL", "https://worker.test");

    // Correction 3 (task-6b brief): the writer's DO leg is dormant today, so a
    // real deploy sees present:false for EVERY user. That absence must surface
    // as a blocked dimension -- never as a confident usage:0 row, which would
    // silently clear a live notice and stop warning a paying customer.
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL) => {
      const u = String(url);
      if (u.includes("/internal/counter/daily-read")) {
        return new Response(JSON.stringify({ present: false, count: 0 }), { status: 200 });
      }
      return new Response(JSON.stringify({ matches: [] }), { status: 200 });
    }));
    ...
    expect(summary.blocked).toContainEqual({
      ...
      reason: "counter_meter_absent",
    });
    const rollups = await t.run((ctx) => ctx.db.query("apiUsageRollups").collect());
    expect(rollups.some((r) => r.dimension === "api_daily_requests")).toBe(false);
    const notices = await t.run((ctx) => ctx.db.query("apiPlanLimitNotices").collect());
    expect(notices.filter((n) => n.dimension === "api_daily_requests")).toHaveLength(0);
  });
  ...
  test("bounded-concurrency reads attribute each user's meter to their own notice", async () => {
    ...
    // via the signed X-WM-MCP-User-Id header (never the URL/body). A
    // misattribution bug (result paired to the wrong `read`) would blame the
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL, init?: RequestInit) => {
      ...
      if (u.includes("api.axiom.co")) return new Response(JSON.stringify({ matches: [] }), { status: 200 });
      if (u.includes("/internal/counter/daily-read")) {
        const userId = new Headers(init?.headers).get("X-WM-MCP-User-Id");
        if (userId === "user-a") return new Response(JSON.stringify({ present: true, count: 400 }), { status: 200 });
        if (userId === "user-b") return new Response(JSON.stringify({ present: true, count: 1500 }), { status: 200 });
      }
      return new Response(JSON.stringify({ result: "0" }), { status: 200 });
    }));
  ...
  test("mcp_daily_calls for a Pro account uses the Redis counter, dropping the dual Axiom row", async () => {
    ...
    // The Axiom mcp.toolcall count reads 400 (it also tallies quota-exempt
    // calls); the authoritative enforcement Redis counter reads 300. Both exceed
    // the 250/day Pro cap, but only ONE mcp_daily row must be evaluated and it
    // must carry the Redis 300, not the Axiom 400 -- the dual row otherwise
    // flaps the notice.
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL, init?: RequestInit) => {
      const u = String(url);
      if (u.includes("api.axiom.co")) {
        ...
        if (apl.includes("mcp.toolcall")) {
          return new Response(JSON.stringify({ matches: [{ data: { user_id: "user-pro", usage: 400 } }] }), { status: 200 });
        }
        return new Response(JSON.stringify({ matches: [] }), { status: 200 });
      }
  ...
  test("a capped Pro account is offered the API plan, not a dead-end notice", async () => {
    // AE6 tail: without a dodoUpgradeNotice branch, pro falls through to
    // `{ctaKind: 'none'}` — the user is told they hit the cap with no way out.
    const t = convexTest(schema, modules);
    await seedEntitlement(t, "user-pro", "pro_monthly");
  ...
  test("the annual Pro plan gets the same API CTA as monthly", async () => {
    const t = convexTest(schema, modules);
    await seedEntitlement(t, "user-pro-annual", "pro_annual");
