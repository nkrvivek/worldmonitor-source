...
async function seedCustomer(t: ReturnType<typeof convexTest>) {
  await t.run(async (ctx) => {
    await ctx.db.insert("customers", {
      ...
      providerCustomerId: "cust_test_001",
      email: "test@example.com",
...
// ---------------------------------------------------------------------------
// #5380 Medium-10: provider error contract, against REAL SDK error instances
// ---------------------------------------------------------------------------
...
// `Object.assign(new Error(), { status: 404 })` — so those assertions only ever
// proved our own stub matches our own reader.
//
// This table feeds the classifier Stripe's actual exported errors. Stripe
// spells the field `statusCode`; the retired Dodo SDK spelled it `status`, and
// the reader still accepts both because the injection seam throws the second.
// The table fails if Stripe renames the field, moves to a different error
// hierarchy, or ever starts stamping a status onto a connection error — each
// of which would silently convert a transient blip into a customer downgrade.
const { errors } = Stripe;

describe("provider error classification (real SDK instances)", () => {
  test("a real 404 StripeInvalidRequestError is a definitive not-found", () => {
    expect(
      isDefinitiveDodoNotFound(
        new errors.StripeInvalidRequestError({
          statusCode: 404,
  ...
  test.each([
    [
      ...
      new errors.StripeAuthenticationError({ statusCode: 401, message: "Invalid API Key" }),
    ],
    [
      "StripeRateLimitError (429)",
      new errors.StripeRateLimitError({ statusCode: 429, message: "Too many requests" }),
    ],
    [
      "StripeAPIError (500)",
      new errors.StripeAPIError({ statusCode: 500, message: "Server error" }),
    ],
    [
      "StripeConnectionError (no status)",
      new errors.StripeConnectionError({ message: "socket hang up" }),
    ],
    [
      "StripeInvalidRequestError 400 (bad params, resource exists)",
      new errors.StripeInvalidRequestError({ statusCode: 400, message: "Invalid parameter" }),
    ],
  ])("%s must NOT downgrade — it is transient or not a not-found", (_label, err) => {
    expect(isDefinitiveDodoNotFound(err)).toBe(false);
  });

  test("non-Error throwables never downgrade", () => {
    for (const value of [null, undefined, 404, "404", { statusCode: "404" }, {}]) {
...
describe("equal-timestamp lifecycle ordering", () => {
  ...
  test("a cancellation carrying the SAME timestamp as the activation is ignored", async () => {
    ...
    const sub = await t.run(async (ctx) =>
      ctx.db
        .query("subscriptions")
        .withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", "sub_test_001"))
        .unique(),
    );
  ...
  async function seedPendingCharge(t: ReturnType<typeof convexTest>, occurredAt: number) {
    await t.run(async (ctx) => {
      await ctx.db.insert("paymentEvents", {
        userId: "test-user-001",
        providerPaymentId: "pay_pending_001",
        type: "charge",
        amount: 999,
        currency: "USD",
  ...
  async function seedCancelledSub(t: ReturnType<typeof convexTest>, currentPeriodEnd: number) {
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "test-user-001",
        providerSubscriptionId: "sub_cancelled_boundary",
