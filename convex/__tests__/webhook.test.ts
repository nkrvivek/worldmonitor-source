...
async function processEvent(
  ...
  const providerCustomerId = payloadData.customer?.customer_id ?? "cust_test_001";
...
describe("webhook processWebhookEvent", () => {
  test("subscription.active creates new subscription", async () => {
    ...
    expect(subs[0].providerSubscriptionId).toBe("sub_test_001");
  ...
  test("new-checkout reactivation uses prior lapsed history instead of a new-subscriber alert", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "test-user-001",
        providerSubscriptionId: "sub_old_expired",
        providerPriceId: "pdt_test_pro",
        planKey: "pro_monthly",
        status: "expired",
        currentPeriodStart: BASE_TIMESTAMP - 31 * 86400000,
  ...
  test("a covering sibling prevents old lapsed history from forcing welcome-back mail", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: "test-user-001",
        providerSubscriptionId: "sub_old_expired_with_cover",
        providerPriceId: "pdt_test_pro",
        planKey: "pro_monthly",
        status: "expired",
        currentPeriodStart: BASE_TIMESTAMP - 31 * 86400000,
      ...
      await ctx.db.insert("subscriptions", {
        userId: "test-user-001",
        providerSubscriptionId: "sub_covering_sibling",
        providerPriceId: "pdt_test_pro",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: BASE_TIMESTAMP - 86400000,
  ...
  // KTD9: both Pro prices are Pro plans for lifecycle purposes — each must get
  // the Pro welcome shell (value-prop headline, brief CTA, Pro feature grid),
  // not the neutral fallback shell that api_* and unknown plan keys fall
  // through to. Pro Business used to be the second plan in that set; it merged
  // into Pro on 2026-08-05, leaving the annual price as the case that proves
  // the allowlist holds more than one key.
  test("subscription.active for annual Pro sends the Pro welcome variant", async () => {
    vi.useFakeTimers();
    process.env.RESEND_API_KEY = "re_test";
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
    ...
    await seedProductPlan(t, "wm_pro_annual", "pro_annual", "Pro Annual");

    await processEvent(
      t,
      "wh_pro_annual_welcome",
      "subscription.active",
      makeSubscriptionPayload({
        subscription_id: "sub_pro_annual_001",
        product_id: "wm_pro_annual",
      }),
      BASE_TIMESTAMP,
    );
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    ...
    expect(welcome?.subject).toBe("Welcome to World Monitor Pro (Annual)");
  ...
  test("subscription.plan_changed updates product and entitlements", async () => {
    ...
    expect(subs).toHaveLength(1);
    expect(subs[0].providerPriceId).toBe("pdt_test_api");
  ...
  test("subscription.active keeps claimed real owner when stale signed anon metadata arrives", async () => {
    ...
    const rows = await t.run(async (ctx) => {
      const [sub, customer, realEntitlement, anonEntitlement] = await Promise.all([
        ctx.db.query("subscriptions").withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", subscriptionId)).unique(),
        ctx.db.query("customers").withIndex("by_providerCustomerId", (q) => q.eq("providerCustomerId", customerId)).first(),
  ...
  test("dispute.lost expires only the disputed subscription when another active subscription covers the user", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("customers", {
        userId,
        providerCustomerId: "cust_dispute_multi",
      ...
      await ctx.db.insert("subscriptions", {
        userId,
        providerSubscriptionId: "sub_disputed_multi",
        providerPriceId: "pdt_test_pro",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: BASE_TIMESTAMP - 86400000,
      ...
      await ctx.db.insert("subscriptions", {
        userId,
        providerSubscriptionId: "sub_cover_multi",
        providerPriceId: "pdt_test_api",
        planKey: "api_starter",
        status: "active",
        currentPeriodStart: BASE_TIMESTAMP - 86400000,
    ...
    const rows = await t.run(async (ctx) => {
      const [disputed, cover, entitlement, paymentEvent] = await Promise.all([
        ctx.db.query("subscriptions").withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", "sub_disputed_multi")).unique(),
        ctx.db.query("subscriptions").withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", "sub_cover_multi")).unique(),
        ctx.db.query("entitlements").withIndex("by_userId", (q) => q.eq("userId", userId)).first(),
        ctx.db.query("paymentEvents").withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_dispute_multi")).first(),
      ]);
  ...
  test("dispute.lost preserves a future complimentary entitlement floor", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId,
        providerSubscriptionId: "sub_dispute_comp",
        providerPriceId: "pdt_test_pro",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: BASE_TIMESTAMP - 86400000,
    ...
    const rows = await t.run(async (ctx) => {
      const [sub, entitlement] = await Promise.all([
        ctx.db.query("subscriptions").withIndex("by_providerSubscriptionId", (q) => q.eq("providerSubscriptionId", "sub_dispute_comp")).unique(),
        ctx.db.query("entitlements").withIndex("by_userId", (q) => q.eq("userId", userId)).first(),
      ]);
      return { sub, entitlement };
    });
  ...
  test.each([
    ...
    async (payloadStatus, expectedStatus) => {
      ...
      expect(paymentEvents[0].type).toBe("charge");
      expect(paymentEvents[0].providerPaymentId).toBe("pay_test_001");
    },
  );
  ...
  test("3DS-pending (payment.processing) then a distinct succeeded webhook both persist", async () => {
    ...
    const paymentEvents = await t.run(async (ctx) =>
      ctx.db
        .query("paymentEvents")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", "pay_test_001"))
        .collect(),
    );
