...
describe("/api/internal-entitlements HTTP action", () => {
  ...
  test("a user with billing history never gets the not-applicable marker", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_A,
        providerSubscriptionId: "sub_http_churned",
  ...
  test("recently stale verification lease surfaces renewal_verification_pending", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_A,
        providerSubscriptionId: "sub_http_pending",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "active",
  ...
  test.each([
    ...
    async (verificationState, billingStatus, retryAfterSeconds) => {
      ...
      await t.run(async (ctx) => {
        await ctx.db.insert("subscriptions", {
          userId: USER_A,
          providerSubscriptionId: `sub_http_enterprise_${verificationState}`,
        ...
        await ctx.db.insert("subscriptions", {
          userId: USER_A,
          providerSubscriptionId: `sub_http_pro_${verificationState}`,
          providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
          planKey: "pro_monthly",
          status: "active",
  ...
  test("billing history without a verification candidate surfaces subscription_lapsed", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_A,
        providerSubscriptionId: "sub_http_lapsed",
        providerPriceId: PRODUCT_CATALOG.pro_monthly.providerPriceId!,
        planKey: "pro_monthly",
        status: "expired",
        currentPeriodStart: NOW - 31 * DAY_MS,
