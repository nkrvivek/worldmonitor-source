...
describe("handleSubscriptionExpired comp guard", () => {
  ...
  test("grantComplimentaryEntitlement + subscription.expired end-to-end: entitlement survives", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_e2e",
        providerPriceId: "wm_pro_monthly",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
...
describe("multi-active-sub guard", () => {
  async function seedTwoActiveSubs(
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: opts.lower.subscriptionId,
        providerPriceId: opts.lower.productId,
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: opts.higher.subscriptionId,
        providerPriceId: opts.higher.productId,
  ...
  test("subscription.expired on cancelled-but-still-covering other sub is treated as covering", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_a_active_lower",
        providerPriceId: "wm_pro_monthly",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_b_cancelled_higher",
        providerPriceId: "pdt_0NbttVmG1SERrxhygbbUq",
        planKey: "api_starter",
        status: "cancelled",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
  ...
  test("subscription.renewed on lower-tier sub does NOT clobber higher-tier active sub", async () => {
    ...
    await fireSubscriptionRenewed(t, "sub_pro_renewed", "wm_pro_monthly");
  ...
  test("subscription.active for a NEW lower-tier sub does NOT clobber existing higher-tier sub", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_api_starter_existing",
        providerPriceId: "pdt_0NbttVmG1SERrxhygbbUq",
        planKey: "api_starter",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
  ...
  test("same-tier precedence: api_business outranks api_starter when both cover", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_api_starter",
        providerPriceId: "pdt_0NbttVmG1SERrxhygbbUq",
        planKey: "api_starter",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_api_business",
        providerPriceId: "pdt_0Nbttg7NuOJrhbyBGCius",
  ...
  test("comparator tie-break by currentPeriodEnd: same plan, longer-lived sub wins", async () => {
    ...
    await t.run(async (ctx) => {
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_pro_short",
        providerPriceId: "wm_pro_monthly",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
      ...
      await ctx.db.insert("subscriptions", {
        userId: USER_ID,
        providerSubscriptionId: "sub_pro_long",
        providerPriceId: "wm_pro_monthly",
        planKey: "pro_monthly",
        status: "active",
        currentPeriodStart: Date.now() - 30 * DAY_MS,
    ...
    await fireSubscriptionRenewed(t, "sub_pro_short", "wm_pro_monthly");
  ...
  async function seedBothProVariants(t: ReturnType<typeof convexTest>) {
    await t.run(async (ctx) => {
      // The monthly sub deliberately outlives the annual one — this verifies
      // the capability tie-break (PLAN_PRECEDENCE) wins over the duration
      // tie-break (currentPeriodEnd). Pro Business used to be the stronger
      // row here; it merged into Pro on 2026-08-05, and pro_annual is now the
      // highest-precedence plan in tier 1.
      for (const pro of [
        {
          id: "sub_pro_monthly",
          planKey: "pro_monthly",
          productId: "wm_pro_monthly",
          periodEnd: Date.now() + 90 * DAY_MS,
        },
        {
          id: "sub_pro_annual",
          planKey: "pro_annual",
          productId: "wm_pro_annual",
          periodEnd: Date.now() + 30 * DAY_MS,
        },
      ]) {
        await ctx.db.insert("subscriptions", {
          userId: USER_ID,
          providerSubscriptionId: pro.id,
          providerPriceId: pro.productId,
          planKey: pro.planKey,
          status: "active",
          currentPeriodStart: Date.now() - 30 * DAY_MS,
          currentPeriodEnd: pro.periodEnd,
          rawPayload: {},
          updatedAt: Date.now() - 1000,
        });
      }
  ...
  test("same-tier precedence: pro_annual outranks pro_monthly when both cover", async () => {
    const t = convexTest(schema, modules);
    await seedBothProVariants(t);

    // Trigger a recompute by firing renewed on the longest-lived sub.
    await fireSubscriptionRenewed(t, "sub_pro_monthly", "wm_pro_monthly");

    const row = await readEntitlement(t);
    expect(row!.planKey).toBe("pro_annual");
    expect(row!.features.tier).toBe(1);
  ...
  test("the entitlement cache action accepts features with AND without dataExport", async () => {
    ...
    // dataExport branch would reject every Pro cache sync.
    const t = convexTest(schema, modules);
    const baseFeatures = {
      tier: 1,
      maxDashboards: 25,
