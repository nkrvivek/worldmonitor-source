...
function dodoUpgradeNotice(planKey: string, dimension: PlanLimitDimension): Omit<NoticeInput, "state"> {
  // Pro tops out below the API plan's MCP allowance, and both are self-serve
  // checkout products — so a capped Pro caller gets the "buy API" CTA. Without
  // this arm the notice falls through to `{ctaKind: 'none'}`: the customer is
  // told they hit the cap with no way out of it.
...
// api_daily_requests now reads the SAME per-account daily meter #3199
// enforcement increments (server/_shared/api-key-rate-limit.ts
// `reserveDailyMeter`, task-6b), but through the Worker's counter-read route
// (worker/routes/counter-read.ts, task-6a) instead of a direct Redis GET — the
// meter moved off Upstash onto a Durable Object, which Convex (off-platform)
// can only reach over plain HTTP. Namespace matches the writer's own
// `API_DAILY_METER_NAMESPACE` and the route's `ALLOWED_NAMESPACES` entry — all
// three sites hardcode this literal independently rather than share an import,
// since a cross-package constant isn't worth it for one string used at three
// call sites that already have to agree by protocol (wire format) anyway.
const API_DAILY_METER_NAMESPACE = "rl:apikey:day";
const COUNTER_DAILY_READ_PATH = "/internal/counter/daily-read";

// Signed POST to the Worker's counter-read route. Mirrors readRedisInteger's
// contract: `null` means BLOCKED (network/timeout/non-OK/malformed body) and
// must never be read as a confident zero. Unlike readRedisInteger, a
// present:false response is NOT collapsed to 0 here — the caller must branch
// on `present` itself (correction 3, task-6b brief): the writer's DO leg is
// dormant today, so present:false is the CORRECT reading for every user, not
// a corruption signal, and must surface as a blocked dimension rather than a
// zero-usage row that would silently clear a live over_limit notice.
async function readCounterDailyMeter(userId: string): Promise<{ count: number; present: boolean } | null> {
  let baseUrl: string;
  let secret: string;
  try {
    baseUrl = requireEnv("WORKER_INTERNAL_BASE_URL");
    secret = requireEnv("COUNTER_INTERNAL_HMAC_SECRET");
  } catch {
    return null;
  }

  const url = new URL(COUNTER_DAILY_READ_PATH, baseUrl).toString();
  const body = JSON.stringify({ namespace: API_DAILY_METER_NAMESPACE });

  // Same rejection risk as queryAxiom/readRedisInteger: a timeout, network
  // failure, or malformed JSON must degrade to a blocked read, never escape
  // and abort the whole hourly scan.
  try {
    const signed = await signInternalMcpRequest({ method: "POST", url, body, userId, secret });
    const resp = await fetch(url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...buildInternalMcpHeaders(signed),
      },
      body,
      signal: AbortSignal.timeout(5_000),
    });
    if (!resp.ok) return null;
    const data = await resp.json() as { present?: unknown; count?: unknown } | null;
    if (typeof data?.present !== "boolean" || typeof data?.count !== "number" || !Number.isFinite(data.count)) {
      // A 200 body that doesn't match the route's own contract (requirement 8:
      // `{ present, count }` only) is corruption, not a genuine reading.
      return null;
    }
    return { present: data.present, count: data.count };
  } catch {
    return null;
  }
}
...
| extend usage = coalesce(observed_limit, 60) + hits`;
  ...
  type DailyRead = { userId: string; planKey: string };
  const apiDailyReads: DailyRead[] = [];
  const mcpDailyReads: DailyRead[] = [];
  for (const ent of active) {
    ...
    if (ent.apiAccess && getPlanLimit(ent.planKey, "api_daily_requests") != null) {
      apiDailyReads.push({ userId: ent.userId, planKey: ent.planKey });
    }
    ...
    if (isRedisMeteredMcpPlan(ent.planKey) && ent.mcpAccess) {
      mcpDailyReads.push({ userId: ent.userId, planKey: ent.planKey });
    }
  }

  // api_daily_requests: gated on the counter-route's own credentials, wholly
  // independent of Upstash -- this dimension no longer touches Redis at all
  // (task-6b). Read per-user in bounded-concurrency batches instead of one
  // sequential round trip per user, so the hourly scan's wall-clock doesn't
  // grow linearly with the paid-customer count.
  let hasCounterCreds = true;
  try {
    requireEnv("COUNTER_INTERNAL_HMAC_SECRET");
    requireEnv("WORKER_INTERNAL_BASE_URL");
  } catch {
    hasCounterCreds = false;
  }
  if (!hasCounterCreds) {
    blocked.push({ dimension: "api_daily_requests", reason: "missing_counter_credentials_for_daily_meter" });
  } else {
    const apiResults = await mapWithConcurrency(apiDailyReads, REDIS_READ_CONCURRENCY, async (read) => ({
      read,
      result: await readCounterDailyMeter(read.userId),
    }));
    for (const { read, result } of apiResults) {
      if (result === null) {
        blocked.push({ userId: read.userId, dimension: "api_daily_requests", reason: "counter_read_failed" });
        continue;
      }
      if (!result.present) {
        blocked.push({ userId: read.userId, dimension: "api_daily_requests", reason: "counter_meter_absent" });
        continue;
      }
      rows.push({
        userId: read.userId,
        ...
        usage: result.count,
        source: "counter:apikey_day",
        sourceFreshAt: now,
      });
    }
  }

  // mcp_daily_calls: unchanged Upstash-gated Redis read.
  if (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN) {
    blocked.push({ dimension: "mcp_daily_calls", reason: "missing_upstash_credentials_for_pro_daily_fallback" });
  } else {
    const mcpResults = await mapWithConcurrency(mcpDailyReads, REDIS_READ_CONCURRENCY, async (read) => ({
      read,
      usage: await readRedisInteger(dailyCounterKey(read.userId, meterDate)),
    }));
    for (const { read, usage } of mcpResults) {
      if (usage == null) {
        blocked.push({ userId: read.userId, dimension: "mcp_daily_calls", reason: "redis_read_failed" });
        continue;
      }
      rows.push({
        userId: read.userId,
