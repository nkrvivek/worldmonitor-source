 ...
 *
 * Uses IDENTITY_SIGNING_SECRET as the HMAC key — a dedicated secret
 * that is SEPARATE from STRIPE_WEBHOOK_SECRET. This ensures rotating
...
function getSigningKey(): string {
  const key = process.env.IDENTITY_SIGNING_SECRET;
  if (!key) {
    throw new Error(
      "[identity-signing] IDENTITY_SIGNING_SECRET not set. " +
      "Set it in the Convex dashboard environment variables. " +
      "This is SEPARATE from STRIPE_WEBHOOK_SECRET — do not reuse."
    );
  }
  return key;
}
...
function getAnonClaimTokenTtlMs(): number {
  const raw = process.env.ANON_CLAIM_TOKEN_TTL_MS;
