 ...
 * - getCustomerPortalUrl: authenticated action to create a Stripe billing-portal session
...
import { resolveSiteUrl } from "./returnUrl";
import { EMAIL_FROM, SUPPORT_EMAIL } from "../emailBrand";
import {
  isCoveringAt,
  isNewerEvent,
...
/**
 * Returns the Stripe SDK client owned by this billing module.
 *
 * This client handles the billing portal and subscription lookups.
 * lib/stripe.ts independently owns checkout-session creation; webhook
 * signature verification lives in payments/webhookHandlers.ts.
 *
 * Test vs live is not a flag here the way it was under Dodo: the key itself
 * decides. An sk_test_ key talks to test mode, an sk_live_ key to live mode.
 *
 * httpClient: Convex's V8 runtime has fetch and no node:http, so the SDK must
 * be told to use fetch. Without it every call here fails at construction.
 *
 * Canonical env var: STRIPE_SECRET_KEY.
 */
function getStripeClient(
  ...
  if (!apiKey) {
    ...
    // The kind still reads DODO_*: it is a wire contract with
    // src/services/_billing-error.ts and the browser's Sentry tags. Renaming it
    // is its own change, made on both sides at once.
    throw new ConvexError({ kind: "PROVIDER_API_KEY_MISSING" });
  }
...
// A definitive "this subscription does not exist at the provider" — Stripe
// throws `StripeInvalidRequestError` carrying `statusCode: 404`. Transient
// failures (network, timeout, 5xx, 429) carry a different/absent status and
// must stay on the backoff-and-retry path, never downgrade.
//
// Both `statusCode` and `status` are read: Stripe spells it the first way, and
// every reconciliation test here stubs the second. Reading one alone would
// either miss every real 404 or every test's.
//
...
export function isDefinitiveDodoNotFound(err: unknown): boolean {
  ...
  const typed = err as { status?: unknown; statusCode?: unknown };
  return typed.status === 404 || typed.statusCode === 404;
}
...
type ReconciliationSummary = {
  ...
  failures: Array<{ providerSubscriptionId: string; error: string }>;
};
...
function normalizeDodoStatus(status: string): SubscriptionStatus | null {
  switch (status) {
    ...
    case "expired":
      return status;
    case "failed":
      // Legacy Dodo value, kept because rows written before the Stripe swap
      // still hold it: the subscription's mandate/payment permanently failed.
      // Locally that means the same as `expired`: no longer covering. Map to
...
/**
 * Stripe subscription → the wire shape the reconciler and its mutation already
 * speak. Kept as a translation rather than a rewrite because that shape is
 * persisted: `dodoReconciliationRemoteSubscriptionValidator` gates the mutation
 * argument, and every stored `rawPayload` on an existing row uses it.
 *
 * Four field-level traps, each a silent wrong answer if missed:
 *  - Periods moved onto the subscription ITEM in recent API versions. There is
 *    no `subscription.current_period_end` any more; reading it yields undefined,
 *    which `toReconciliationEpochMs` would turn into NaN.
 *  - Stripe timestamps are unix SECONDS. The reconciler's epoch helper treats a
 *    number as milliseconds, so seconds would land in 1970 and every row would
 *    read as long expired.
 *  - `product_id` is the price's LOOKUP KEY, not its id — that is what the
 *    catalog stores in providerPriceId and what `resolveProductToPlan` matches.
 *  - Stripe's status vocabulary is its own; `normalizeDodoStatus` speaks the
 *    local one. The map below is the only place the two meet.
 */
function stripeSubscriptionToRemote(
  sub: Stripe.Subscription,
): DodoReconciliationRemoteSubscription {
  const item = sub.items.data[0];
  if (!item) {
    throw new Error(
      `[billing/reconcile] Stripe subscription ${sub.id} has no items`,
    );
  }
  const lookupKey =
    typeof item.price.lookup_key === "string" ? item.price.lookup_key : null;
  if (!lookupKey) {
    throw new Error(
      `[billing/reconcile] Stripe price ${item.price.id} on subscription ${sub.id} has no lookup_key`,
    );
  }
  const customerId =
  ...
  return {
    ...
    previous_billing_date: item.current_period_start * 1000,
    next_billing_date: item.current_period_end * 1000,
    customer: { customer_id: customerId },
    ...(sub.metadata ? { metadata: sub.metadata } : {}),
    currency: sub.currency,
    cancelled_at: sub.canceled_at == null ? null : sub.canceled_at * 1000,
  };
}

/**
 * Stripe status → the local vocabulary the database and entitlement recompute
 * already store (`active` / `on_hold` / `cancelled` / `expired`, plus the
 * `pending` the reconciler skips on).
 *
 * `unpaid` maps to `expired`, not `on_hold`: Stripe reaches it only after the
 * whole dunning sequence has failed, so the mandate is dead and the row must
 * stop covering. `past_due` and `paused` are still recoverable, so they hold.
 * Anything unrecognised passes through untouched and lands in
 * `normalizeDodoStatus`'s default, which makes the reconciler skip and escalate
 * rather than guess.
 */
...
function normalizeRemoteSubscription(
  remote: DodoReconciliationRemoteSubscription,
) {
  ...
  if (!status) {
    return {
      ...
      providerSubscriptionId: remote.subscription_id,
    };
  }
  ...
  return {
    ...
    value: {
      ...
      providerCustomerId: remote.customer?.customer_id,
      cancelledAt,
      rawPayload: remote,
    },
  };
 ...
 * `getProviderCustomerIdForUserPortal`, which is a 3-tier resolver biased
 * toward per-Clerk-userId evidence:
 *
 *   1. `subscriptions.providerCustomerId` — the stable top-level column,
 *      preserved across lifecycle webhook patches by
 *      `mergeProviderCustomerId` in `subscriptionHelpers.ts`. Per-Clerk-
 ...
 *   3. `customers.providerCustomerId` for the SAME userId — last-resort
 ...
 * arrives with a matching providerCustomerId — same Dodo customer (one per
...
export async function createCustomerPortalUrlForUser(
  ...
  const providerCustomerId = await ctx.runQuery(
    internal.payments.billing.getProviderCustomerIdForUserPortal,
    { userId },
  );
  ...
  try {
    session = await client.billingPortal.sessions.create({
      ...
      // Where Stripe sends the buyer when they close the portal. Stripe
      // requires either this or a configured default; without it the call
      // fails outright rather than picking something sensible.
      return_url: resolveSiteUrl(),
    });
  } catch (err) {
    // The Stripe SDK throws a typed StripeError on a non-2xx response, or a
    // StripeConnectionError on a transport failure. Convex's action runtime
    // then masks any NON-ConvexError throw
    ...
    // `err.data.kind === 'PORTAL_ERROR'` for proper Sentry
    // classification (browser → `extractBillingErrorKind` → tag
    // `billing_error_kind`). The kind still reads DODO_*: it is a wire contract
    // with src/services/_billing-error.ts, renamed on both sides at once or not
    // at all. Log the underlying cause here so it survives in the Convex
    // function logs for server-side triage. WORLDMONITOR-ST.
    const cause = err instanceof Error ? err.message : String(err);
    console.error(
      `[billing] Stripe billing-portal create failed for customer ${providerCustomerId}:`,
      cause,
    );
    throw new ConvexError({ kind: "PORTAL_ERROR" });
  }
...
// Pro buyers get a day-0 activation interstitial (KTD9). Mirrored by the
// client allowlist in src/services/pro-activation-state.ts.
 ...
 * Manage Billing flow — see `getProviderCustomerIdForUserPortal` below for
 ...
 *      patches via `mergeProviderCustomerId` in `subscriptionHelpers.ts`.
 ...
 */
export const getProviderCustomerIdForUserPortal = internalQuery({
  args: { userId: v.string() },
  handler: async (ctx, args) => {
    ...
    if (subs.length > 0) {
      ...
      for (const sub of sorted) {
        ...
        if (typeof sub.providerCustomerId === "string" && sub.providerCustomerId.length > 0) {
    ...
    // customers row may still hold a usable `providerCustomerId` for this
    ...
    if (
      customer &&
      typeof customer.providerCustomerId === "string" &&
      customer.providerCustomerId.length > 0
    ) {
...
export const applyDodoSubscriptionReconciliation = internalMutation({
  ...
  handler: async (ctx, args): Promise<ReconciliationMutationResult> => {
    ...
    if (existing.providerSubscriptionId !== args.providerSubscriptionId) {
      throw new Error(
        `[billing/reconcile] subscription id mismatch: expected ${existing.providerSubscriptionId}, got ${args.providerSubscriptionId}`,
      );
    }
    ...
    if (
      ...
      args.remote.productId === existing.providerPriceId &&
      args.remote.currentPeriodEnd <= existing.currentPeriodEnd &&
      (args.remote.providerCustomerId ?? existing.providerCustomerId) === existing.providerCustomerId
    ) {
      await recordAttempt();
    ...
    if (args.remote.productId !== existing.providerPriceId) {
    ...
    await ctx.db.patch(existing._id, {
      ...
      providerPriceId: args.remote.productId,
      planKey,
      currentPeriodStart: args.remote.currentPeriodStart,
      currentPeriodEnd: args.remote.currentPeriodEnd,
      providerCustomerId: args.remote.providerCustomerId ?? existing.providerCustomerId,
...
async function reconcileOneStaleRow(
  ...
  try {
    let remote: DodoReconciliationRemoteSubscription | undefined;
    if (useTestRemotes) {
      const injected = errorInjection.get(sub.providerSubscriptionId);
      if (injected === "not_found") {
        throw Object.assign(new Error("simulated provider not found"), { status: 404 });
      }
      if (injected === "server_error") {
        throw Object.assign(new Error("simulated provider server error"), { status: 500 });
      }
      remote = remoteById.get(sub.providerSubscriptionId);
    } else {
      remote = stripeSubscriptionToRemote(
        await client!.subscriptions.retrieve(sub.providerSubscriptionId),
      );
    }
    if (!remote) {
    ...
    if (normalized.kind === "skip") {
      ...
      // daily.
      console.error(
        `[billing/reconcile] Skipping subscription ${normalized.providerSubscriptionId}: ${normalized.reason} Dodo status "${normalized.status}"`,
      );
    ...
    console.error(
      `[billing/reconcile] Failed to reconcile providerSubscriptionId=${sub.providerSubscriptionId} userId=${sub.userId}: ${message}`,
    );
  ...
  handler: async (ctx, args): Promise<OnDemandRenewalResult> => {
    ...
    const safeFinalize = async (
      ...
      try {
        ...
        console.error(
          `[billing/on-demand-renewal] Failed to finalize ${status} verification providerSubscriptionId=${claim.subscription.providerSubscriptionId} userId=${claim.subscription.userId}: ${message}`,
        );
        return false;
      }
    };
    ...
    try {
      const client = usesTestInjection
        ? null
        : getStripeClient({ timeout: ON_DEMAND_RENEWAL_TIMEOUT_MS, maxNetworkRetries: 0 });
      ...
      console.error(
        `[billing/on-demand-renewal] Failed before Dodo verification providerSubscriptionId=${claim.subscription.providerSubscriptionId} userId=${claim.subscription.userId}: ${message}`,
      );
    ...
    try {
      ...
      console.error(
        `[billing/on-demand-renewal] Failed to resolve post-reconcile user state providerSubscriptionId=${claim.subscription.providerSubscriptionId} userId=${claim.subscription.userId}: ${message}`,
      );
      await bestEffortFinalizeFailed();
      return onDemandRenewalFailureResult();
    }
      ...
      switch (outcome.kind) {
        ...
        case "terminal_not_found": {
          ...
          try {
            ...
            // sentry-coverage-ok: Convex auto-Sentry captures console.error.
            console.error(
              `[billing/reconcile] expireMissingDodoSubscription failed for providerSubscriptionId=${sub.providerSubscriptionId} userId=${sub.userId}: ${expireMsg}`,
            );
            summary.failed++;
            summary.failures.push({
              providerSubscriptionId: sub.providerSubscriptionId,
          ...
          if (expireResult.kind === "expired") {
            ...
            console.error(
              `[billing/reconcile] providerSubscriptionId=${sub.providerSubscriptionId} userId=${sub.userId} confirmed not-found in Dodo after ${(sub.reconcileFailureCount ?? 0) + 1} attempts; downgraded local row to expired.`,
            );
          } else {
...
/**
 * One-shot backfill: populate the new `subscriptions.providerCustomerId`
 ...
 *   2. `customers.providerCustomerId` matched by the sub's `userId` —
 ...
 *
 * Run:
 *   npx convex run payments/billing:backfillSubscriptionProviderCustomerId
 *
 * Returns
 ...
 * had data).
 */
export const backfillSubscriptionProviderCustomerId = internalMutation({
  args: {},
  handler: async (ctx) => {
    ...
    for (const sub of all) {
      ...
      if (typeof fromPayload === "string" && fromPayload.length > 0) {
        await ctx.db.patch(sub._id, { providerCustomerId: fromPayload });
      ...
      const customer = await ctx.db
        ...
        .first();
      const fromCustomer = customer?.providerCustomerId;
      if (typeof fromCustomer === "string" && fromCustomer.length > 0) {
        await ctx.db.patch(sub._id, { providerCustomerId: fromCustomer });
 ...
 * the providerCustomerId is recoverable from there. Walk the user's
 ...
 * a usable providerCustomerId (or the providerCustomerId already maps to a
...
export const repairCustomerFromSubscriptionPayload = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const sub of subs) {
      ...
      const providerCustomerId = typeof rawId === "string" && rawId.length > 0 ? rawId : null;
      ...
      // providerCustomerId already exists for a DIFFERENT userId, don't
      ...
      const collidingByDodo = await ctx.db
        .query("customers")
        .withIndex("by_providerCustomerId", (q) => q.eq("providerCustomerId", providerCustomerId))
        .first();
      if (collidingByDodo && collidingByDodo.userId !== args.userId) {
        console.warn(
          `[billing/repair] customers.providerCustomerId=${providerCustomerId} already mapped to userId=${collidingByDodo.userId}; refusing to remap to userId=${args.userId}.`,
        );
        return null;
      }

      // by_userId precedence: a row may already exist for this user
      // WITHOUT a providerCustomerId (the field is `v.optional(v.string())`
      ...
      if (existingByUser) {
        console.warn(
          `[billing/repair] Patching providerCustomerId=${providerCustomerId} into existing customers row for userId=${args.userId} (providerSubscriptionId=${sub.providerSubscriptionId}). Webhook gap — investigate subscriptionHelpers.ts:520-549.`,
        );
      ...
      console.warn(
        `[billing/repair] Inserting customers row for userId=${args.userId} from subscription rawPayload (providerSubscriptionId=${sub.providerSubscriptionId}). Webhook gap — investigate subscriptionHelpers.ts:520-549.`,
      );
      ...
      for (const sub of subs) {
        ...
        if (collision) {
          if (collision.userId !== userId) {
            ...
            console.warn(
              `[billing/backfill] cross-user collision: providerCustomerId=${providerCustomerId} already maps to userId=${collision.userId}; skipping userId=${userId}.`,
            );
            break;
          }
          // by_providerCustomerId match for the SAME user already covers
          // the by_userId case for the dominant path. Count as repaired.
          repairedThisUser = true;
          break;
        }
        // by_userId precedence: when `existing` row lacks `providerCustomerId`
        ...
        if (existing) {
          console.warn(
            `[billing/backfill] Patching providerCustomerId=${providerCustomerId} into existing customers row for userId=${userId} (providerSubscriptionId=${sub.providerSubscriptionId}).`,
          );
          await ctx.db.patch(existing._id, {
            providerCustomerId,
            ...(email ? { email, normalizedEmail } : {}),
          ...
          console.warn(
            `[billing/backfill] Inserted customers row for userId=${userId} from subscription providerSubscriptionId=${sub.providerSubscriptionId}.`,
          );
        }
        repairedThisUser = true;
        break;
 ...
 * concurrent API subscription (double-billing; PR #4946 review). Pro ↔ API
 * cross-line purchases remain deliberately allowed — they are complementary
 * products.
 */
...
export const getCheckoutBlockingSubscription = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      providerSubscriptionId: blocking.providerSubscriptionId,
    };
  },
});
...
const STUCK_PAYMENT_EMAIL_FROM = EMAIL_FROM;
const STUCK_PAYMENT_SUPPORT_EMAIL = SUPPORT_EMAIL;
...
/**
 * The shape the stuck-payment reconciler reads. It is not a vendor type: the
 * claim mutation persists these exact field names in `rawPayload`, and rows
 * written before the Stripe swap still carry them. `retrieveStuckPayment`
 * translates a Stripe PaymentIntent into it.
 */
type StuckPaymentLookup = {
  status?: unknown;
  payment_id?: unknown;
...
function extractDodoPaymentStatus(payment: unknown): string | null {
  ...
  const rawStatus = (payment as StuckPaymentLookup).status;
  return typeof rawStatus === "string" && rawStatus.length > 0 ? rawStatus : null;
}

/**
 * Stripe PaymentIntent status → the vocabulary `paymentEvents.status` stores
 * and `isPendingPaymentStatus` / `isTerminalPaymentStatus` test.
 *
 * `requires_payment_method` deliberately passes through unmapped: it is the
 * end-state of an abandoned 3DS challenge, matches neither predicate, and so
 * routes the row to the ops path — which is what the caller already documents
 * and wants. Mapping it to a pending value would email customers who walked
 * away weeks ago.
 */
function mapStripePaymentIntentStatus(status: Stripe.PaymentIntent.Status): string {
  switch (status) {
    ...
    case "canceled":
      return "cancelled";
    default:
      // processing / succeeded already read the same on both sides.
      return status;
  }
}

/**
 * Read one payment from Stripe and flatten it into the reconciler's shape.
 *
 * Two calls, not one, because a PaymentIntent alone cannot answer the two
 * questions the customer email needs. It has no buyer address (`receipt_email`
 * is only set when one was collected, which Checkout does not do) and no
 * resume link. The Checkout Session that created it has both, and its `url`
 * carries exactly the right semantics: a string while the session is still
 * open, null once it expired or completed. So a dead link cannot be emailed —
 * `sendStuckPaymentEmail` sees no link and returns "ops_notified".
 *
 * A missing session is not an error. Payments made outside Checkout have none,
 * and the caller treats an absent link the same way.
 */
async function retrieveStuckPayment(
  providerPaymentId: string,
): Promise<StuckPaymentLookup> {
  const client = getStripeClient({
  ...
  return {
    ...
    customer: {
      email:
        session?.customer_details?.email ?? intent.receipt_email ?? undefined,
...
export const getBlockingPendingPayment = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    // "in progress" if its providerPaymentId has NO terminal (non-pending) charge
    ...
    for (const ev of recent) {
      if (ev.type === "charge" && !isPending(ev.status)) {
        resolvedPaymentIds.add(ev.providerPaymentId);
      }
    }
    ...
    const blocking = recent
      .filter((ev) => {
        ...
        if (resolvedPaymentIds.has(ev.providerPaymentId)) return false;
...
export const listStuckPendingPaymentCandidates = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    for (const ev of paymentEvents) {
      ...
      if (seenPaymentIds.has(ev.providerPaymentId)) continue;
      seenPaymentIds.add(ev.providerPaymentId);

      const marker = await ctx.db
        .query("paymentReconciliationAttempts")
        .withIndex("by_providerPaymentId", (q) => q.eq("providerPaymentId", ev.providerPaymentId))
        .first();
      if (marker) continue;
...
export const claimStuckPaymentReconciliation = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (isTerminalPaymentStatus(args.observedStatus)) {
      ...
      if (args.observedStatus === "succeeded" && args.providerSubscriptionId) {
        ...
        if (!sub) {
          console.error(
            ...
            `paymentId=${args.providerPaymentId} userId=${args.userId} subscriptionId=${args.providerSubscriptionId}. ` +
...
export const finalizeStuckPaymentReconciliation = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!marker) {
      ...
      console.warn(
        `[billing/reconciliation] finalize found no marker for paymentId=${args.providerPaymentId}.`,
      );
    ...
    console.error(
      ...
      `paymentId=${marker.providerPaymentId} userId=${marker.userId} status=${marker.observedStatus} ` +
...
export const reportPaymentReconciliationFailure = internalMutation({
  ...
  handler: async (_ctx, args) => {
    throw new Error(
      `[billing/reconciliation] ${args.phase} phase failed paymentId=${args.providerPaymentId}: ${args.errorMessage}`,
    );
  },
});
...
export const reconcileStuckPendingPayments = internalAction({
  ...
  handler: async (ctx, args): Promise<ReconcileStuckPendingPaymentsSummary> => {
    ...
    const reportFailure = async (
      ...
      try {
        await ctx.scheduler.runAfter(
          ...
          { phase, providerPaymentId, errorMessage: message },
        );
      } catch (scheduleErr) {
        ...
        console.warn(
          `[billing/reconciliation] failed to schedule ${phase} failure report for ${providerPaymentId}: ` +
    ...
    const finalizeOps = async (providerPaymentId: string): Promise<void> => {
      try {
        await ctx.runMutation(
          (internal as any).payments.billing.finalizeStuckPaymentReconciliation,
          { providerPaymentId, notified: false },
        );
      } catch (err) {
        // sentry-coverage-ok: reportFailure schedules a throwing mutation.
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[billing/reconciliation] finalize (ops) failed for ${providerPaymentId}: ${message}`);
        await reportFailure("record", providerPaymentId, message);
      }
    };
    ...
    for (const candidate of candidates) {
      // Phase — poll the provider for the current payment status.
      let payment: StuckPaymentLookup;
      try {
        payment = await retrieveStuckPayment(candidate.providerPaymentId);
      } catch (err) {
        // sentry-coverage-ok: reportFailure schedules a throwing mutation.
        summary.pollFailed++;
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[billing/reconciliation] poll failed for ${candidate.providerPaymentId}: ${message}`);
        await reportFailure("poll", candidate.providerPaymentId, message);
        continue;
      }

      const observedStatus = extractDodoPaymentStatus(payment); // string | null
      const stuckPayment = payment;
      ...
      try {
        claim = await ctx.runMutation(
          (internal as any).payments.billing.claimStuckPaymentReconciliation,
          {
            ...
            providerSubscriptionId: readString(stuckPayment.subscription_id) ?? candidate.providerSubscriptionId,
            planKey: candidate.planKey,
            amount: readNumber(stuckPayment.total_amount) ?? readNumber(stuckPayment.amount) ?? candidate.amount,
            currency: readString(stuckPayment.currency) ?? candidate.currency,
        ...
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[billing/reconciliation] claim failed for ${candidate.providerPaymentId}: ${message}`);
        await reportFailure("record", candidate.providerPaymentId, message);
        continue;
      }
      ...
      if (!recognizedPending) {
        ...
        await finalizeOps(candidate.providerPaymentId);
        continue;
      }
      ...
      try {
        emailResult = await sendStuckPaymentEmail(stuckPayment, candidate.planKey);
      } catch (err) {
        ...
        summary.emailFailed++;
        const message = err instanceof Error ? err.message : String(err);
        console.warn(`[billing/reconciliation] email failed for ${candidate.providerPaymentId}: ${message}`);
        await reportFailure("email", candidate.providerPaymentId, message);
        continue;
      }
      ...
      if (emailResult === "customer_notified") {
        try {
          await ctx.runMutation(
            (internal as any).payments.billing.finalizeStuckPaymentReconciliation,
            { providerPaymentId: candidate.providerPaymentId, notified: true },
          );
          ...
          summary.opsNotified++;
          const message = err instanceof Error ? err.message : String(err);
          console.warn(`[billing/reconciliation] finalize failed for ${candidate.providerPaymentId}: ${message}`);
          await reportFailure("record", candidate.providerPaymentId, message);
        }
      } else {
 ...
 *     '{"providerSubscriptionId":"sub_XXX","reason":"refunded by admin, user has higher-tier active sub"}'
 */
export const deleteSubscriptionByDodoId = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!sub) {
      throw new Error(
        `[billing] deleteSubscriptionByDodoId: no subscription found with providerSubscriptionId="${args.providerSubscriptionId}"`,
      );
    }
    ...
    console.log(
      `[billing] deleteSubscriptionByDodoId userId=${userId} providerSubscriptionId=${args.providerSubscriptionId} planKey=${sub.planKey} reason="${args.reason}"`,
    );
    ...
    return {
      deleted: { _id: sub._id, providerSubscriptionId: args.providerSubscriptionId, planKey: sub.planKey },
