...
import { EMAIL_FROM, sitePath } from "../emailBrand";
import {
  extractDomain,
  isCorporateDomain,
  sameDomain,
...
export const inviteSeats = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await touchBusinessSeatLock(ctx, businessSub.providerSubscriptionId, now);
    ...
    const existingGrants = await ctx.db
      .query("businessProGrants")
      .withIndex("by_businessSubscriptionId", (q) =>
        q.eq("businessSubscriptionId", businessSub.providerSubscriptionId),
      )
      .collect();
    ...
    const currentCount = await countActiveOrPendingGrants(
      ctx,
      businessSub.providerSubscriptionId,
      now,
    );
    ...
    for (const email of newEmails) {
      const grantId = await ctx.db.insert("businessProGrants", {
        businessSubscriptionId: businessSub.providerSubscriptionId,
        ownerUserId: userId,
        inviteeEmail: email,
        domain: ownerDomain,
        status: "pending",
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
  ...
  <div style="padding: 40px 32px;">
    ...
    <div style="text-align: center;">
      <a href="${sitePath("/pro")}" style="display: inline-block; background: #ef4444; color: #fff; padding: 14px 36px; text-decoration: none; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 2px;">View Plans</a>
    </div>
    <p style="font-size: 11px; color: #666; text-align: center; margin: 24px 0 0;">
      Questions? Reply to this email or contact hello@sibt.ai.
    </p>
  </div>
</div>`;
...
export const sendBusinessInviteEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    const acceptUrl = `${sitePath("/settings")}?accept-business-invite=${encodeURIComponent(args.grantId)}&token=${encodeURIComponent(args.token)}`;
    const html = `
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
