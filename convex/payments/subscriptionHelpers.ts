...
interface PaymentEventData {
  ...
  subscription_id?: string;
  metadata?: Record<string, string>;
  // Payment intent status (succeeded | failed | cancelled | processing |
  ...
  for (const grant of acceptedGrants) {
    const businessSub = await ctx.db
      ...
      .withIndex("by_providerSubscriptionId", (q) =>
        q.eq("providerSubscriptionId", grant.businessSubscriptionId),
      )
      .unique();
 ...
 * before the customer notices their entitlement is wrong, so the fallback
...
/**
 * Resolves a Stripe price lookup key to a plan key. Lookup order:
 ...
 * behaviour (throw → webhook 500 → Stripe retries for three days) blocked
 * entitlement updates for any customer whose subscription was migrated to a
 * new price.
 *
 * Re-running `seedProductPlans` after every catalog change is what keeps this
 * branch cold. A price Stripe knows about and the productPlans table does not
 * lands here.
 */
export async function resolvePlanKey(
  ...
  const mapping = await ctx.db
    .query("productPlans")
    .withIndex("by_providerPriceId", (q) => q.eq("providerPriceId", providerPriceId))
    .unique();
  ...
  const aliasedPlan = LEGACY_PRODUCT_ALIASES[providerPriceId];
  if (aliasedPlan) {
    console.warn(
      `[subscriptionHelpers] Resolved "${providerPriceId}" via legacy alias → "${aliasedPlan}". ` +
  ...
  const catalogPlan = resolveProductToPlan(providerPriceId);
  if (catalogPlan) {
    ...
    console.error(
      `[subscriptionHelpers] Stripe price lookup key "${providerPriceId}" is in PRODUCT_CATALOG ` +
  ...
  // NOT throw — that would 500 the webhook and trigger Stripe's retries,
  ...
  console.error(
    `[subscriptionHelpers] Unknown Stripe price lookup key "${providerPriceId}" — ` +
 ...
 *   2. Customer table lookup by providerCustomerId
...
async function resolveUserId(
  ...
  if (providerCustomerId) {
    const customer = await ctx.db
      .query("customers")
      .withIndex("by_providerCustomerId", (q) =>
        q.eq("providerCustomerId", providerCustomerId),
      )
      .first();
    if (customer?.userId) {
  ...
  if (isDev) {
    console.warn(
      `[subscriptionHelpers] No user identity found for customer="${providerCustomerId}" — using dev fallback "${DEV_USER_ID}"`,
    );
    return DEV_USER_ID;
  }

  throw new Error(
    `[subscriptionHelpers] Cannot resolve userId: no verified metadata, no customer record, no providerCustomerId.`,
  );
}
 ...
 * subscriptions row.
 *
 * `SubscriptionEventData.customer` is optional and lifecycle events
 ...
 */
function mergeProviderCustomerId(
  data: SubscriptionEventData,
  existing: { providerCustomerId?: string },
): string | undefined {
...
export async function handleSubscriptionActive(
  ...
  const existing = await ctx.db
    .query("subscriptions")
    .withIndex("by_providerSubscriptionId", (q) =>
      q.eq("providerSubscriptionId", data.subscription_id),
    )
    .unique();
  ...
  // `rawPayload` blindly.
  const incomingProviderCustomerId =
  ...
  const existingCustomer = incomingProviderCustomerId
    ? await ctx.db
        .query("customers")
        .withIndex("by_providerCustomerId", (q) =>
          q.eq("providerCustomerId", incomingProviderCustomerId),
        )
        .first()
    : null;
  const resolvedUserId = existing
    ? existing.userId
    : await resolveUserId(ctx, incomingProviderCustomerId ?? "", data.metadata);
  ...
  if (existing) {
    await ctx.db.patch(existing._id, {
      userId,
      status: "active",
      providerPriceId: data.product_id,
      planKey,
      currentPeriodStart,
      currentPeriodEnd,
      providerCustomerId: incomingProviderCustomerId ?? existing.providerCustomerId,
      rawPayload: data,
    ...
    await ctx.db.insert("subscriptions", {
      ...
      currentPeriodStart,
      currentPeriodEnd,
      providerCustomerId: incomingProviderCustomerId,
      rawPayload: data,
      updatedAt: eventTimestamp,
    });
  ...
  // Upsert customer record so portal session creation can find providerCustomerId
  const email = data.customer?.email ?? "";
  const normalizedEmail = email.trim().toLowerCase();

  if (incomingProviderCustomerId) {
    if (existingCustomer) {
...
export async function handleSubscriptionRenewed(
  ...
  await ctx.db.patch(existing._id, {
    ...
    currentPeriodStart,
    currentPeriodEnd,
    providerCustomerId: mergeProviderCustomerId(data, existing),
    rawPayload: data,
    updatedAt: eventTimestamp,
...
async function revokeBusinessProGrantsForSubscription(
  ...
  const grants = await ctx.db
    ...
    .withIndex("by_businessSubscriptionId", (q) =>
      q.eq("businessSubscriptionId", providerSubscriptionId),
    )
    .collect();
...
export async function handleSubscriptionCancelled(
  ...
  if (existing.planKey === "api_business") {
    if (!isCoveringAt(existing, eventTimestamp)) {
      await revokeBusinessProGrantsForSubscription(ctx, existing.providerSubscriptionId, eventTimestamp);
    } else {
      await ctx.scheduler.runAfter(
...
export async function handleSubscriptionPlanChanged(
  ...
  // Business Pro grants are tied to the owner's providerSubscriptionId staying on
 ...
 */
/**
 * Fill in a refund's or dispute's subscription id from the payment it reverses.
 *
 * Stripe's refund, charge, payment-intent and dispute objects carry no
 * subscription reference at all in API version 2026-07-29.dahlia — only
 * invoices do. Without this, every refund would arrive with
 * `subscription_id` undefined, and the refund-without-cancellation alert
 * below (money safety, not bookkeeping) would never fire again.
 *
 * The link is the PaymentIntent id: payments/stripeWebhookEvents.ts stores it
 * as `providerPaymentId` on the invoice's charge row, and reports the same id
 * on the later refund or dispute, so the earlier row is one index read away.
 *
 * Returns a new object — `data` is also written verbatim to `rawPayload`, and
 * mutating it would rewrite the stored record of what the provider sent.
 */
export async function withInheritedSubscriptionId(
  ctx: MutationCtx,
  data: PaymentEventData,
): Promise<PaymentEventData> {
  if (data.subscription_id || !data.payment_id) return data;

  const priorPayment = await ctx.db
    .query("paymentEvents")
    .withIndex("by_providerPaymentId", (q) =>
      q.eq("providerPaymentId", data.payment_id),
    )
    .first();

  if (!priorPayment?.providerSubscriptionId) {
    console.warn(
      `[webhook] no earlier payment row for ${data.payment_id} — refund/dispute ` +
        `recorded without a subscription link, so the refund alert cannot run`,
    );
    return data;
  }
  return { ...data, subscription_id: priorPayment.providerSubscriptionId };
}
...
export async function handlePaymentOrRefundEvent(
  ...
  // Refund-without-prior-cancellation alert. Every payment provider we have
  // used treats refund and subscription cancellation as separate operations —
  // refunding a subscription payment does NOT cancel the subscription, on
  // Stripe or on the Dodo rail before it. When operators forget the cancel
  // step, the user keeps Pro access until manual cleanup (we hit this
  // 2026-04-29 with nokzbtl@gmail.com — the entitlement only downgraded after
  // the operator cancelled by hand).
  //
