...
function extractFailureContext(rawPayload: unknown) {
  ...
  return {
    providerSubscriptionId: boundedString(data?.subscription_id, 200),
    providerPaymentId: boundedString(data?.payment_id, 200),
    providerCustomerId: boundedString(customer?.customer_id, 200),
    dataKeys: data
...
async function readFailureSummaryLock(ctx: MutationCtx) {
  ...
  if (!summary) {
    throw new Error(
      "Payment webhook failure summary is not seeded; run payments/webhookMutations:_seedFailureSummary",
    );
  }
  return summary;
}
 ...
 */
export const reportWebhookFailure = internalMutation({
  args: {
    webhookId: v.string(),
    eventType: v.string(),
    errorKind: v.string(),
    ...
    switch (args.eventType) {
      ...
      case "refund.failed":
        await handlePaymentOrRefundEvent(
          ctx,
          await withInheritedSubscriptionId(ctx, data),
          args.eventType,
          args.timestamp,
        );
        break;
