...
import { EMAIL_FROM, SUPPORT_EMAIL, logoUrl, siteHost, siteOrigin, sitePath } from "../emailBrand";
...
const PLAN_DISPLAY: Record<string, string> = {
  ...
  api_business: "API Business (Monthly)",
      ...
      <tr>
        ...
        <td style="width: 50%; padding: 12px; vertical-align: top;">
          <div style="background: #111; border: 1px solid #1a1a1a; padding: 16px; height: 100%;">
            ...
            <div style="font-size: 12px; color: #888; line-height: 1.4;">Slack, Telegram, Email, Browser Push, Webhooks</div>
          </div>
        </td>
      </tr>`;
  }
...
function userWelcomeHtml(planName: string, planKey: string): string {
  ...
  const ctaHref = isPro ? sitePath("/brief") : siteOrigin();
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin: 0 auto 32px;">
      <tr>
        <td style="width: 40px; height: 40px; vertical-align: middle;">
          <img src="${logoUrl()}" width="40" height="40" alt="WorldMonitor" style="border-radius: 50%; display: block;" />
        </td>
  ...
  <div style="border-top: 1px solid #1a1a1a; padding: 24px 32px; text-align: center;">
    ...
    <p style="font-size: 11px; color: #444; margin: 0; line-height: 1.6;">
      ...
      <a href="${siteOrigin()}" style="color: #4ade80; text-decoration: none;">${siteHost()}</a>
    </p>
  </div>
</div>`;
}
      ...
      `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
        ...
        <div style="padding: 40px 32px;">
          ...
          <div style="text-align: center;">
            <a href="${sitePath("/dashboard")}" style="display: inline-block; background: #4ade80; color: #0a0a0a; padding: 14px 36px; text-decoration: none; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 2px;">Open World Monitor</a>
          </div>
...
export const getDunningContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const hasLiveSub = siblingSubs.some(
      (s) =>
        s.providerSubscriptionId !== sub.providerSubscriptionId &&
        (s.status === "active" ||
          s.status === "on_hold" ||
  ...
  handler: async (ctx, args) => {
    const row = await ctx.db
      ...
      .withIndex("by_sub_step_episode", (q) =>
        q
          .eq("providerSubscriptionId", args.providerSubscriptionId)
          .eq("step", args.step)
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
  ...
  <div style="border-top: 1px solid #1a1a1a; padding: 24px 32px; text-align: center;">
    <p style="font-size: 11px; color: #444; margin: 0; line-height: 1.6;">
      ${footerNote}<br />
      <a href="${siteOrigin()}" style="color: #f59e0b; text-decoration: none;">${siteHost()}</a>
    </p>
  </div>
</div>`;
}
...
export const sendDunningEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!sub.email) {
      console.warn(`[dunning] no resolvable email for ${args.providerSubscriptionId} — skipping ${args.step}`);
    ...
    let ctaUrl = args.step === "winback_day30" ? sitePath("/pro#pricing") : sitePath("/dashboard");
    if (args.step !== "winback_day30") {
      try {
        ...
        console.log(
          `[dunning] portal mint failed for ${args.providerSubscriptionId} (${err instanceof Error ? err.message : String(err)}) — falling back to dashboard CTA`,
        );
      }
    }
    ...
    console.log(`[dunning] sent ${args.step} for ${args.providerSubscriptionId}`);
...
export const runDunningScan = internalMutation({
  ...
  handler: async (ctx) => {
    const now = Date.now();
    const due: Array<{ providerSubscriptionId: string; step: DunningStep; episodeAt: number }> = [];
    ...
    for (const sub of onHold) {
      ...
      const step: DunningStep | null =
        ...
        : null;
      if (step) due.push({ providerSubscriptionId: sub.providerSubscriptionId, step, episodeAt });
    }
    ...
    for (const sub of cancelled) {
      ...
      due.push({ providerSubscriptionId: sub.providerSubscriptionId, step: "winback_day30", episodeAt: sub.cancelledAt });
    }
    ...
    for (const item of due) {
      ...
      const existing = await ctx.db
        ...
        .withIndex("by_sub_step_episode", (q) =>
          q
            .eq("providerSubscriptionId", item.providerSubscriptionId)
            .eq("step", item.step)
