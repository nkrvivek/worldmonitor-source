...
// The sign-up link is derived from the request rather than written in.
// Upstream hardcoded its own apex here, so on this fork every error page
// pointed the reader at a site we do not run.
function renderErrorPage(req: Request, heading: string, body: string): string {
  return (
    ...
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + `<p><a href="${resolvePublicBaseUrl(req)}/pro">Start your own WorldMonitor Brief</a></p>`
    + '</div></body></html>'
  );
}

const notFoundPage = (req: Request) => renderErrorPage(
  req,
...
export default async function handler(
  ...
  if (
    ...
    return htmlResponse(req, 404, notFoundPage(req));
  }
