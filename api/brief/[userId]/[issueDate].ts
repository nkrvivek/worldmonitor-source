...
// The "return to the site" link is derived from the request rather than
// written in. Upstream hardcoded its own apex here, so on this fork every
// error page sent the reader to a site we do not run.
function renderErrorPage(req: Request, heading: string, body: string): string {
  return (
    ...
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + `<p><a href="${resolvePublicBaseUrl(req)}">Return to WorldMonitor</a></p>`
    + '</div></body></html>'
  );
}

const expiredPage = (req: Request) => renderErrorPage(
  req,
  'This brief has expired.',
  'Briefs are kept for seven days after they are issued. Your next brief will be delivered on schedule.',
);

const forbiddenPage = (req: Request) => renderErrorPage(
  req,
...
const unavailablePage = (req: Request) => renderErrorPage(
  req,
...
export default async function handler(
  ...
  if (!secret) {
    ...
    return htmlResponse(req, 503, unavailablePage(req));
  }
  ...
  if (parts.length !== 4 || root !== 'api' || route !== 'brief' || !rawUserId || !rawIssueDate) {
    return htmlResponse(req, 404, expiredPage(req));
  }
  ...
  if (!verified) {
    return htmlResponse(req, 403, forbiddenPage(req));
  }
