 ...
 *
 * `code` is a deterministic 8-char hash of the userId (stable for the
 * life of the account).
 *
...
/**
 * Bind the share code to the userId in Convex so that
...
export default async function handler(
  ...
  // No invite/conversion count is returned on the response. Both
  // attribution paths already write the same userReferralCredits row,
  // with the same (referrer, email) dedupe: the waitlist form in
  // convex/registerInterest.ts, and checkout through the
  // affonso_referral metadata key that
  // convex/payments/subscriptionHelpers.ts reads off the Stripe
  // subscription. What is missing is a reader — nothing queries that
  // table. Write the query before adding the field.
  return jsonResponse(
    {
      code,
      shareUrl: buildShareUrl(resolvePublicBaseUrl(req), code),
    },
    200,
    cors,
  );
