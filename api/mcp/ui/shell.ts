...
// This fork's host comes first. Widgets fetch back to the server that served
// them, and upstream's two hosts were the whole list — so on this fork every
// widget's connect-src blocked its own data. Upstream's hosts stay so the
// resource is unchanged when it is served from there.
