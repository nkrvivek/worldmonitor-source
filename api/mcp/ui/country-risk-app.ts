...
<!-- MCP Apps view CSP (orank mcp-view-csp). Scopes all 4 required directive
     ...
     connect-src comes from UI_CONNECT_DOMAINS in shell.ts, so this widget and the
     generated ones allow the same hosts. It used to list upstream's two hosts and
     nothing else, which blocked this fork from fetching its own data. -->
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src 'self' data:; connect-src 'self' ${UI_CONNECT_DOMAINS.join(' ')}; frame-ancestors https://chatgpt.com https://claude.ai https://claude.com; form-action 'none'; base-uri 'none'">
