import { FORK_HOST } from '../_first-party-origin';
...
// The three docs pages these sentences point at ship from this repo
// (docs-site/src/content/docs/), so the link belongs on whichever host served
// the session. Upstream wrote its own host in, which on this fork sent every
// agent to somebody else's copy. `initialize` passes the request's base URL;
// the exported SERVER_INSTRUCTIONS is the default for callers without one.
export function serverInstructions(baseUrl: string = `https://${FORK_HOST}`): string {
  return [
  `Every tool accepts an optional \`jmespath\` string. Server-side projection applied AFTER per-tool filter/summary; typical 80-95% token reduction. Grammar: https://jmespath.org/specification.html. Guide + 12 worked examples: ${baseUrl}/docs/mcp-jmespath.`,
  '',
  `Limits: expr ≤ ${JMESPATH_MAX_EXPR_BYTES}B, output ≤ ${JMESPATH_MAX_OUTPUT_BYTES}B. Bad expressions soft-fail via {_jmespath_error, original_keys} envelope (consumes one Pro/OAuth daily quota unit on retry when that quota path applies — self-correct from original_keys). Full envelope reference: ${baseUrl}/docs/mcp-error-catalog.`,
  '',
  `tools/list ships compressed tool descriptions (≤${TOOL_DESCRIPTION_MAX_BYTES}B). Call describe_tool({tool_name}) for the full uncompressed definition — quota-exempt (still counts toward the 60/min rate limit), so use freely while exploring. describe_tool({tool_name: 'nonexistent'}) returns {error: 'unknown_tool', available: [...]} so you can self-correct. Full reference: ${baseUrl}/docs/mcp-tools-reference.`,
  '',
...
export const MARKET_FRESHNESS_CHECKS = [
  ...
  // 60 = 2x the '11,41 * * * *' container cron; was 2x the retired relay's 15m.
  { key: 'seed-meta:market:sectors', maxStaleMin: 60 },
] as const;
