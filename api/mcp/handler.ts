...
import { resolvePublicBaseUrl } from '../_first-party-origin';
import {
...
/**
 * How the discovery documents get read.
 *
 * The default below fetches them off our own deployment, which is what a
 * serverless function can do and a Worker cannot: a Worker fetching the host
 * it is serving is a subrequest loop, and Cloudflare declines it rather than
 * routing back through. Measured 2026-08-06, that turned every crawler GET of
 * /mcp and /.well-known/mcp into the handler's own 302 fallback while both
 * static files answered 200 at their canonical URLs.
 *
 * So the reader is swappable, the same way relayFetch is (server/_shared/relay.ts):
 * worker/index.ts points it at the ASSETS binding. Passing null restores the
 * self-fetch.
 */
type StaticAssetFetch = (path: string) => Promise<string | null>;
let staticAssetFetchOverride: StaticAssetFetch | null = null;

export function setStaticAssetFetch(reader: StaticAssetFetch | null): void {
  staticAssetFetchOverride = reader;
}

/** Drops the memoized documents. For tests — a deploy gets a fresh isolate. */
export function resetStaticAssetCaches(): void {
  serverCardCache = null;
  mcpGuideCache = null;
}
...
async function fetchStaticAsset(req: Request, path: string): Promise<string | null> {
  if (staticAssetFetchOverride) {
    try {
      return await staticAssetFetchOverride(path);
    } catch {
      return null;
    }
  }
...
async function serveMcpGuide(req: Request, corsHeaders: Record<string, string>, headOnly = false): Promise<Response> {
  ...
  return new Response(headOnly ? null : mcpGuideCache, {
    ...
    headers: {
      ...
      ...corsHeaders,
      Vary: DISCOVERY_VARY,
      Link: `<${resolvePublicBaseUrl(req)}/mcp>; rel="canonical"`,
    },
  });
}
  ...
  switch (method) {
    case 'initialize': {
      ...
      return maybeStreamJsonRpcResponse(req, rpcOk(id, {
        ...
        instructions: serverInstructions(resolvePublicBaseUrl(req)),
