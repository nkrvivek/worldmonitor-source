...
export function getCorsHeaders(req, methods = 'GET, OPTIONS') {
  ...
  // Denied origins get our own host echoed back, not upstream's. The literal
  // repeats SITE_ORIGIN because this file has plain-JS twins that cannot import
  // src/config/site.ts (same reason scripts/_site.mjs exists).
  const allowOrigin = isAllowedOrigin(origin) ? origin : 'https://worldmonitor.sibt.ai';
  return {
