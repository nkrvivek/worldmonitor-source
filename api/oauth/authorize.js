...
// @ts-expect-error — TS module imported from JS, same as ../_rate-limit.js does
import { isFirstPartyOrigin, resolveOrigins, UPSTREAM_ORIGINS } from '../_first-party-origin.ts';
...
<body><a href="/" class="wm-logo">${GLOBE_SVG}<span class="wm-logo-text">WorldMonitor MCP</span></a>
<div class="card"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(detail)}</p><a href="javascript:history.back()" class="back">&#8592; go back</a></div>
<p class="footer"><a href="/">WorldMonitor</a></p>
...
export function consentPage(params, nonce, errorMsg = '', origins = UPSTREAM_ORIGINS) {
  const { client_name, redirect_uri } = params;
  const redirectHost = new URL(redirect_uri).hostname;
  // U3 contract: the bridge page lives with the rest of the site, and the form
  // posts back to whichever host runs the functions. Upstream splits those two
  // across worldmonitor.app and api.worldmonitor.app; this fork runs both on one
  // host. Deriving them from the request keeps both layouts working.
  const proCtaHref = `${origins.app}/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
  const appHost = new URL(origins.app).host;
...
<body>
<a href="${escapeHtml(origins.app)}" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">WorldMonitor MCP</span></a>
<div class="card">
<div class="client-hd">
...
<form id="cf" method="POST" action="${escapeHtml(origins.api)}/oauth/authorize" style="display:none;margin-top:1.25rem">
...
<p class="hint">No key? <a href="${escapeHtml(origins.app)}/pro" target="_blank" rel="noopener">Get one at ${escapeHtml(appHost)}/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="${escapeHtml(origins.app)}" target="_blank" rel="noopener">${escapeHtml(appHost)}</a> &middot; <a href="${escapeHtml(origins.app)}/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'GET') {
    ...
    }, nonce, '', resolveOrigins(req));
  }

  if (method === 'POST') {
    // Origin validation: allow any first-party host (the consent page is served
    // host-derived — apex/www/api/variant upstream, one host on this fork — so a
    // same-origin JS fetch or the native form POST arrives with any of those
    // Origins), plus absent origin (server/CLI) and 'null' (WebView with
    // opaque/sandboxed origin). CSRF nonce provides the actual protection.
    const origin = req.headers.get('origin');
    if (origin && origin !== 'null' && !isFirstPartyOrigin(origin)) {
    ...
    if (!await timingSafeIncludes(api_key, validKeys)) {
      ...
      return consentPage({
        client_name: client.client_name ?? 'Unknown Client',
        redirect_uri, client_id, response_type: 'code', code_challenge, code_challenge_method: 'S256', state,
      }, retryNonce, 'Invalid API key. Please check and try again.', resolveOrigins(req));
    }
