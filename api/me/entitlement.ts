 ...
 * check used by every premium gate in the codebase (a pro role on the
 * session OR a Convex entitlement tier >= 1).
 *
 ...
 *     invalid or expired session. Distinguishing this from the free-
 ...
 * A user's entitlement changes when Stripe webhooks fire, and /pro reads
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // actual entitlement verdict (covers the session role AND the Convex tier).
