...
export default async function handler(req: Request, ctx: { waitUntil: (p: Promise<unknown>) => void }): Promise<Response> {
  ...
  // Asserted for the same reason as api/notify.ts: under
  // @cloudflare/workers-types, clone() hands back the Request interface's own
  // type parameters rather than instantiating them against the receiver, so the
  // inferred type will not go into anything that takes a plain Request.
  const idempotencyRequest = req.method === 'POST' ? (req.clone() as Request) : null;
  ...
  if (req.method === 'POST') {
    ...
    if (!ent || ent.features.tier < 1) {
      ...
      return json({
        ...
        upgradeUrl: `${resolvePublicBaseUrl(req)}/pro`,
      }, 403, corsHeaders);
    }
