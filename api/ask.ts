...
// Every result names the site it came from and the endpoint to call next.
// Upstream hard-coded its own two hosts here, which on this fork sent an
// asking agent to a site we do not run. api/_first-party-origin.ts derives
// both origins from the request Host, so a worldmonitor.app request still
// gets upstream's URLs and a request here gets ours.
interface AskLinks {
  site: string;
  toolsDoc: string;
  mcpEndpoint: string;
...
function askLinks(origins: FirstPartyOrigins): AskLinks {
  return {
    site: new URL(origins.app).host,
    toolsDoc: `${origins.app}/docs/mcp-tools-reference`,
    mcpEndpoint: `${origins.app}/mcp`,
...
export function buildResults(
  ...
  const results: NlwebResult[] = suggestions.map((s) => ({
    ...
    schema_object: {
      ...
      name: s.name,
      description: s.description,
      documentation: links.toolsDoc,
      // The actionable endpoint: call the tool via MCP tools/call.
      url: links.mcpEndpoint,
      provider: { '@type': 'Organization', name: 'World Monitor', url: links.siteUrl },
    },
  }));
  if (results.length === 0) {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const results = buildResults(params.query, resolveOrigins(req));
  if (params.streaming) {
