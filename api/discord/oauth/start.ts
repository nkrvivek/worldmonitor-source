...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!proAccess.allowed) {
    ...
    return new Response(JSON.stringify({ error: 'pro_required', message: 'Discord notifications are available on the Pro plan.', upgradeUrl: `${resolvePublicBaseUrl(req)}/pro` }), { status: 403, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  }
