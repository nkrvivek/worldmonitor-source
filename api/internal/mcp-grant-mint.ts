 ...
 *
 * The redirect URL host is never user-controllable — that is what defeats the
 * consent-phishing class (see plan Risks: "Cross-subdomain CSRF / consent
 * phishing"). Upstream wrote `https://api.worldmonitor.app` in; it now comes
 * from `resolveApiOrigin`, which answers from a fixed allowlist, so a request
 * to this fork returns this fork's host and a spoofed Host returns neither.
 *
...
import { resolveApiOrigin } from '../_first-party-origin';

/**
 * Fixed return path — NOT user-controllable (anti-phishing). Only the host is
 * derived, and only from the allowlist in _first-party-origin.ts: upstream keeps
 * sending the browser to api.worldmonitor.app, this fork sends it to its own
 * single host instead of to a site we do not run.
 */
const AUTHORIZE_PRO_PATH = '/oauth/authorize-pro';
...
export async function mintGrantHandler(req: Request, deps: MintDeps): Promise<Response> {
  ...
  if (!userId) {
    return jsonError('UNAUTHENTICATED', 'A valid signed-in session is required.', 401);
  }
  ...
  const redirect = `${resolveApiOrigin(req)}${AUTHORIZE_PRO_PATH}?nonce=${encodeURIComponent(nonce)}&grant=${encodeURIComponent(grantToken)}`;
