...
import { getCorsHeaders, getPublicCorsHeaders, isAllowedOrigin, isDisallowedOrigin } from './_cors.js';
...
test('allows the worldmonitor.sibt.ai host the app is now served from', () => {
  const origins = ['https://worldmonitor.sibt.ai', 'https://tech.worldmonitor.sibt.ai'];

  for (const origin of origins) {
    assert.equal(isAllowedOrigin(origin), true, `origin should be allowed: ${origin}`);
    const req = makeRequest(origin);
    assert.equal(isDisallowedOrigin(req), false, `origin should be allowed: ${origin}`);
    const cors = getCorsHeaders(req);
    assert.equal(cors['Access-Control-Allow-Origin'], origin);
  ...
  // Never widen to a bare *.sibt.ai wildcard (security allowlist).
  assert.equal(isAllowedOrigin('https://other-app.sibt.ai'), false);
...
test('rejects unrelated external origins', () => {
  ...
  const cors = getCorsHeaders(req);
  assert.equal(cors['Access-Control-Allow-Origin'], 'https://worldmonitor.sibt.ai');
  assert.equal(cors['Access-Control-Allow-Credentials'], 'true');
});
