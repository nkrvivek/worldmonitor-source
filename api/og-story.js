// Non-sebuf: returns an SVG document, not JSON.
/**
 * Dynamic OG Image Generator for Story Sharing
 * Returns an SVG image (1200x630) — rich intelligence card for social previews.
 *
 * A web-standard `(Request) => Response` handler, so the Worker route can call
 * it directly. It used to take Node's (req, res) pair and parse its relative
 * `req.url` against upstream's hardcoded origin, which also signed the footer
 * of every card this fork rendered with upstream's hostname.
 */
...
export default function handler(request) {
  ...
  // The host that answered, so a card rendered by this fork is signed by this
  // fork. Never the raw Host header — resolvePublicBaseUrl refuses one it does
  // not recognise.
  const siteLabel = new URL(resolvePublicBaseUrl(request)).host;
  ...
  <text x="60" y="610" font-family="system-ui, sans-serif" font-size="14" fill="#555"
    >${escapeXml(siteLabel)} · ${dateStr} · Free &amp; open source</text>
</svg>`;
