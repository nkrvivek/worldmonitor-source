...
let relayFetchImpl = null;

/**
 * Same swap point as server/_shared/relay.ts, for the handlers that reach the
 * relay through this file instead: /api/opensky, /api/telegram-feed,
 * /api/oref-alerts, /api/polymarket, /api/rss-proxy.
 *
 * On Workers, WS_RELAY_URL names the Worker's own hostname, and a Worker that
 * fetches its own hostname times out instead of re-entering. worker/index.ts
 * installs a fetcher here that answers /ais/snapshot from the Durable Object
 * and 404s the paths the old Node relay served but this Worker does not.
 *
 * Pass null to restore the plain network fetch. Tests rely on that.
 */
export function setRelayFetch(impl) {
  relayFetchImpl = impl;
}

export function relayFetch(url, init) {
...
export async function fetchWithTimeout(url, options, timeoutMs = 15000) {
  ...
  try {
    return await relayFetch(url, { ...options, signal: controller.signal });
  } finally {
