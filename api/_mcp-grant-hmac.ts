 ...
 *
 * The parameter is the widest shape rather than `NodeJS.ProcessEnv`, because
 * this file compiles under two different sets of ambient types. Under
 * @cloudflare/workers-types (tsconfig.worker.json) `process.env` is a plain
 * `Record<string, string | undefined>`, while the repo's own augmentation of
 * `NodeJS.ProcessEnv` marks NODE_ENV, RELAY_AUTH_HEADER and UPSTREAM_API_ORIGIN
 * required — so the default argument did not satisfy its own parameter type.
 * A real `ProcessEnv` still passes: it carries a string index signature.
 */
export function readGrantSecret(
