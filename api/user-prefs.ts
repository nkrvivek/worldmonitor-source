 ...
 *
 * Authentication: Supabase Bearer token in Authorization header.
 * Requires CONVEX_URL + SUPABASE_JWT_ISSUER env vars.
 */
...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // drift between our Supabase JWKS validation and Convex's auth config (a
      // Supabase JWKS rotation lag, an audience mismatch, a stale
      // SUPABASE_JWT_ISSUER env var); other user-bad-token cases are
