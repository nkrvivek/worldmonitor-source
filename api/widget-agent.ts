...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (await hasValidWorldMonitorKey(worldMonitorKey)) {
    ...
    if (authHeader?.startsWith('Bearer ')) {
      // Session JWT path (web users with active subscription).
      //
      // Accept EITHER a 'pro' role OR a Convex entitlement with tier >= 1. The
      // Stripe webhook pipeline writes Convex entitlements and never wrote the
      // role claim, so a paying subscriber's session.role stays 'free'
      // indefinitely (panel-gating.ts:11-27 documents the same split at the
      // frontend layer), and this fork pins it to 'free' outright
      // (server/auth-session.ts). A role-only check here would 403 every
      // paying user despite a valid subscription, with
