/**
 * Checkout session creation gateway.
 *
 * Thin auth proxy: validates the Supabase bearer token, then relays to the
 * Convex /relay/create-checkout HTTP action, which creates the Stripe
 * Checkout session and does the real validation (returnUrl allowlist, HMAC
 * signing, customer prefill).
 *
 * Used by both the /pro marketing page and the main dashboard. Reached
 * through worker/routes/payments.ts.
 */
...
export default async function handler(
  ...
  // Cast because workers-types prints clone()'s return with its Cf generic
  // still free, which no longer matches the default instantiation the
  // idempotency helper's .d.ts asks for. Same object either way.
  const idempotencyRequest = req.clone() as Request;
  ...
  // Relay to Convex
  try {
    const resp = await createCheckoutDeps.fetch(`${siteUrl}/relay/create-checkout`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const data = await resp.json() as RelayErrorBody;
    if (!resp.ok) {
      if (resp.status === 429) {
      ...
      if (resp.status === 409) {
        ...
        const blockedBody = checkoutBlockedBody(data);
