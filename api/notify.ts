...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Asserted, not inferred. Under @cloudflare/workers-types, clone() is
  // declared as `clone(): Request<CfHostMetadata, Cf>` -- it hands back the
  // Request interface's own type parameters instead of instantiating them
  // against the receiver, so the inferred type will not go into anything that
  // takes a plain Request (api/_idempotency.d.ts does). An annotation is still
  // an assignment and gets the same error, so this asserts. Same object either
  // way: types only.
  ...
  if (!proAccess.allowed) {
    ...
    return jsonResponse({ error: 'pro_required', message: 'Event publishing is available on the Pro plan.', upgradeUrl: `${resolvePublicBaseUrl(req)}/pro` }, 403, cors);
  }
