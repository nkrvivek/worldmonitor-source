...
import { resolveOrigins, UPSTREAM_ORIGINS, type FirstPartyOrigins } from './_first-party-origin';
...
// Every link an agent is told to call next. Upstream wrote its own two hosts
// in here as constants, so this fork answered agents with the address of a
// site we do not run. api/_first-party-origin.ts derives both origins from the
// request Host instead: on a worldmonitor.app request these are the same URLs
// they always were, and on this fork they point at this fork.
function howToCall(origins: FirstPartyOrigins) {
  return {
    mcp: {
      endpoint: `${origins.app}/mcp`,
      transport: 'streamable-http',
      note: `Issue tools/list for the live inventory (anonymous). Data calls need OAuth2 (scope=mcp) or an API key header 'X-WorldMonitor-Key: wm_<40-hex>' — issue one at ${origins.app}/pro.`,
    },
    rest: {
      base: origins.api,
      openapi: `${origins.app}/openapi.json`,
    },
    docs: `${origins.app}/docs/documentation`,
    agentGuidance: `${origins.app}/llms.txt`,
  } as const;
}
...
async function handleMessageSend(
  id: JsonRpcId,
  params: unknown,
  origins: FirstPartyOrigins = UPSTREAM_ORIGINS,
): Promise<Response> {
  const howTo = howToCall(origins);
  ...
  if (suggestions.length > 0) {
    lines.push(
      ...
      `Call them on the MCP server at ${howTo.mcp.endpoint} (${howTo.mcp.note})`,
      `REST equivalents are documented in the OpenAPI spec at ${howTo.rest.openapi}.`,
    );
  } else {
    lines.push(
      ...
      `Issue tools/list on ${howTo.mcp.endpoint} for the full catalog, or start from ${howTo.agentGuidance}.`,
    );
  }
...
export default async function handler(req: Request): Promise<Response> {
  ...
  switch (rpc.method) {
    case 'message/send':
      return handleMessageSend(id, rpc.params, resolveOrigins(req));
    case 'message/stream':
