...
- **MCP サーバー** — `https://worldmonitor.sibt.ai/mcp` (Streamable HTTP)。`tools/list` は公開。`tools/call` は `X-WorldMonitor-Key` ヘッダーまたは OAuth で認証します。
- **REST API** — ベース URL は `https://worldmonitor.sibt.ai`。仕様は [OpenAPI spec](https://worldmonitor.sibt.ai/openapi.yaml) を参照。
