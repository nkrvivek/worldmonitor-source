...
| Convex | Convex Cloud | Billing/entitlements (Stripe), user state and API keys, broadcast/email, contact + waitlist forms, historical intelligence memory (vector search) |
...
| `seed-fallback.yml` | Hourly cron, manual | Runs due seed scripts from GitHub when the Cloudflare Worker's scheduled invocations have gone quiet for 30+ minutes; stands down without running anything while the Worker crons are alive |
| `seed-military-flights.yml` | Hourly cron (`:07`), manual | Runs `seed-military-flights.mjs` from a GitHub runner because OpenSky drops connections from Cloudflare egress IPs, so the container's own scheduled runs can never reach the API; the `:07` offset stays clear of the container's `:00/:10/:20` ticks, whose failing runs still hold the seed lock while timing out |
| `seed-fuel-prices.yml` | Weekly cron (`10:23 UTC Sunday`), manual | Runs `seed-fuel-prices.mjs` from a GitHub runner because Mexico, Brazil and New Zealand block Cloudflare egress IPs and `MX` is not in the seed's `TOLERATED_FAILURES`, so `validateFuel` rejects every container publish and the key expires to EMPTY ten days later; a GitHub runner reaches 7/7 sources. The container keeps its own `0 10 * * SUN` tick, which is harmless, so this is the run that actually publishes |
| `warm-ping.yml` | Half-hourly cron, manual | Curls the six `RELAY_WARM_PING_PATHS` on the fork's own gateway with `WORLDMONITOR_RELAY_KEY`, because the CF-scheduled seed containers never see that key (`worker/seeds/env.ts` strips it) and their warm-pings fail closed; keeps `news:digest:v1:full:en` filled for recall-benchmark |
| `analytics-collector-monitor.yml` | 15-minute cron, manual | Probes this site's own analytics collector directly (heartbeat, tracker script, ingest route) and fails when events are being dropped — the upstream host reported a green deployment through the 4-day #5565 blackout, so deployment status is not trusted here |
| `upstream-sync.yml` | Monthly cron (2nd), manual | Merges `koala73/worldmonitor` onto a `sync/upstream-<date>` branch and opens a PR when clean, an issue listing the conflicting files when not; never touches `main`, and the PR carries no CI because `GITHUB_TOKEN` cannot start workflow runs |
...
| `deploy-web.yml` | Push to main, after `Build seed container image` completes, manual | Builds the frontend and deploys the `worldmonitor-web` Cloudflare Worker. A push that will start an image build defers to the `workflow_run` trigger, because `wrangler.jsonc` pins a container image by tag and deploying before that tag exists fails. Until 2026-08-12 nothing deployed this Worker at all, and two merged green-CI fixes sat unshipped for 17 hours |
| `seed-container-image.yml` | Push to main (seed image paths), manual | Builds `Dockerfile.seeds` and pushes it to the Cloudflare registry under the content-derived tag from `scripts/seed-image-tag.sh`, so `wrangler deploy` reads a pre-built image and no laptop needs a Docker daemon. Does not deploy. Fails if `wrangler.jsonc` names a different tag |
| `publish-source-mirror.yml` | Push to main, manual | Publishes the tracked tree to the public `nkrvivek/worldmonitor-source` as one orphan commit, which is the AGPL-3.0-only section 13 source offer for worldmonitor.sibt.ai. Scans for secrets first and stops on any finding |
