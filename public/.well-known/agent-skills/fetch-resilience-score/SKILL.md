...
`/api/resilience/v1/get-resilience-score` is Pro-tier. Agents and other server-to-server callers MUST present an API key in the `X-WorldMonitor-Key` header. `Authorization: Bearer …` is for MCP/OAuth or a Supabase session JWT — **not** raw API keys.

```
X-WorldMonitor-Key: wm_0123456789abcdef0123456789abcdef01234567
```

The key must be attached to a Pro subscription. Unauthenticated or free-tier requests return `401` / `403`. Issue a key at https://worldmonitor.sibt.ai/pro.

## Endpoint

```
GET https://worldmonitor.sibt.ai/api/resilience/v1/get-resilience-score
```
...
curl -s -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://worldmonitor.sibt.ai/api/resilience/v1/get-resilience-score?countryCode=DE' \
...
- OpenAPI: [ResilienceService.openapi.yaml](https://worldmonitor.sibt.ai/openapi.yaml) — operation `GetResilienceScore`.
- Auth matrix: https://worldmonitor.sibt.ai/docs/usage-auth
- Methodology: https://worldmonitor.sibt.ai/docs/documentation
