...
```
GET https://worldmonitor.sibt.ai/api/supply-chain/v1/get-chokepoint-status
```
...
curl -s -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://worldmonitor.sibt.ai/api/supply-chain/v1/get-chokepoint-status' \
...
- Via MCP, the equivalent tool is `get_chokepoint_status` on `https://worldmonitor.sibt.ai/mcp`.

## References

- OpenAPI: https://worldmonitor.sibt.ai/openapi.json — operation `GetChokepointStatus`.
