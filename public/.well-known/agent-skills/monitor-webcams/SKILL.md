...
```
GET https://worldmonitor.sibt.ai/api/webcam/v1/list-webcams
GET https://worldmonitor.sibt.ai/api/webcam/v1/get-webcam-image
```
...
WEBCAM_ID=$(curl -s --get \
  ...
  'https://worldmonitor.sibt.ai/api/webcam/v1/list-webcams' \
...
curl -s --get \
  -H "X-WorldMonitor-Key: $WM_API_KEY" \
  -H "User-Agent: worldmonitor-agent-skill/1.0" \
  'https://worldmonitor.sibt.ai/api/webcam/v1/get-webcam-image' \
...
- Via MCP, use the visual/infrastructure context tools on `https://worldmonitor.sibt.ai/mcp`.

## References

- OpenAPI: https://worldmonitor.sibt.ai/openapi.json - operations `ListWebcams` and `GetWebcamImage`.
