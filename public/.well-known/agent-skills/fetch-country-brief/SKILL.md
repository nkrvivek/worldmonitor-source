...
Server-to-server callers (agents, scripts, SDKs) MUST present an API key in the `X-WorldMonitor-Key` header. `Authorization: Bearer …` is for MCP/OAuth or a Supabase session JWT — **not** raw API keys.

```
X-WorldMonitor-Key: wm_0123456789abcdef0123456789abcdef01234567
```

Browser requests from `worldmonitor.sibt.ai` get a free pass via CORS Origin trust, but agents will never hit that path. Issue a key at https://worldmonitor.sibt.ai/pro.

## Endpoint

```
GET https://worldmonitor.sibt.ai/api/intelligence/v1/get-country-intel-brief
```
...
curl -s -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://worldmonitor.sibt.ai/api/intelligence/v1/get-country-intel-brief?country_code=IR' \
  | jq -r '.brief'
```
...
curl -s --get -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://worldmonitor.sibt.ai/api/intelligence/v1/get-country-intel-brief' \
...
- OpenAPI: [IntelligenceService.openapi.yaml](https://worldmonitor.sibt.ai/openapi.yaml) — operation `GetCountryIntelBrief`.
- Auth matrix: https://worldmonitor.sibt.ai/docs/usage-auth
- Documentation: https://worldmonitor.sibt.ai/docs/documentation
