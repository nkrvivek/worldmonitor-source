...
// The seeder's own hard cache expiry, restated here so the queue can ask
// whether a country still publishes without importing the runner.
export const PORTWATCH_MAX_CACHE_AGE_MS = 7 * 86_400_000;
// How many consecutive failures a coverage hole may hold its priority slot for.
// Past this it rejoins ordinary rotation: a country upstream genuinely cannot
// serve must not occupy a scarce slot on every run forever.
export const PORTWATCH_COVERAGE_RETRY_MAX_FAILURES = 3;

// True when this payload would still be published if the country were deferred
// again. `classifyDeferredPayload` in the seeder makes the same cut: only a
// cache with a finite write time inside the hard expiry serves as staleAsof.
export function hasPublishableCache(
  payload,
  now = Date.now(),
  maxCacheAgeMs = PORTWATCH_MAX_CACHE_AGE_MS,
) {
  if (!payload || typeof payload !== 'object') return false;
  if (!Number.isFinite(payload.cacheWrittenAt)) return false;
  return (now - payload.cacheWrittenAt) < maxCacheAgeMs;
}
...
// Slots are scarce: 30 per run against 174 countries. Deferring a country whose
// cache still publishes costs one cycle of freshness; deferring one whose cache
// does not costs a record out of the published count. So the countries that
// currently publish nothing are refreshed first, ahead of the oldest-attempt
// rotation.
//
// Without this, a failure was self-punishing. buildRefreshFailureState stamps
// refreshAttemptedAt, which under oldest-attempt-first sends the country that
// just failed to the back of a three-day rotation. On 2026-08-11 eleven
// countries failed together, sorted behind everything with an older attempt,
// and their caches crossed the seven-day expiry before their next slot came
// round -- recordCount 163 against a target of 174 with the data sitting
// upstream the whole time.
export function orderColdFetchQueue(needsFetch, options = {}) {
  const {
    criticalCountries = PORTWATCH_DECISION_CRITICAL_COUNTRIES,
    now = Date.now(),
    maxCacheAgeMs = PORTWATCH_MAX_CACHE_AGE_MS,
    maxConsecutiveFailures = PORTWATCH_COVERAGE_RETRY_MAX_FAILURES,
  } = options;
  ...
  const consecutiveFailures = (item) => {
    const count = item?.prevPayload?.refreshFailure?.consecutiveFailures;
    return Number.isFinite(count) ? count : 0;
  };
  const stableId = (item) => String(item?.iso2 || item?.iso3 || '');
  const priority = (item) => {
    if (critical.has(item?.iso2)) return 0;
    if (!hasPublishableCache(item?.prevPayload, now, maxCacheAgeMs)
      && consecutiveFailures(item) < maxConsecutiveFailures) return 1;
    return 2;
  };
