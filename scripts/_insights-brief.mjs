...
/** The tokens a validator objected to, for the rejection log. */
function describeValidation(validation) {
  const flagged = Array.isArray(validation?.hallucinated) ? validation.hallucinated : [];
  return flagged.length > 0 ? `ungrounded: ${flagged.join(' ')}` : 'ungrounded';
}

/**
 * A validator rejection plus the evidence it was judged against.
 *
 * Naming the rule was not enough on its own. "ungrounded: patriot" reads the
 * same whether the model invented a weapons system or the source named it in a
 * form the extractor could not see, and those need opposite fixes. The verdict
 * goes first because the caller truncates, so a long headline can never push
 * the flagged token out of the log.
 *
 * @param {{ hallucinated?: string[] }} validation
 * @param {string} sentence what the model wrote
 * @param {string} groundText the cited stories it was checked against
 */
function describeUngrounded(validation, sentence, groundText) {
  return `${describeValidation(validation)} :: said "${sentence}" :: source "${groundText}"`;
}
...
/**
 * The complete rule vocabulary of composeSynthesizedBrief's editorial gate.
 *
 * Authored here, beside the reject() calls that raise these names, because the
 * rule now travels further than a log line: seed-insights records the deciding
 * rule in seed-meta and health republishes it. Meta and health carry fixed
 * vocabularies only, never free text, so a rule missing from this list is
 * dropped rather than written.
 *
 * health cannot import this file. seed-insights runs on Railway nixpacks with
 * only scripts/ in the container, so an import of api/ from here would crash
 * the seeder on startup (tests/scripts-railway-nixpacks-no-escape-import).
 * api/_insights-reject-rules.js mirrors the list for that side, and
 * tests/brief-reject-rule-vocabulary asserts the two never drift and that both
 * still match every reject() literal below.
 */
...
export function composeSynthesizedBrief(rawText, topStories, opts = {}) {
  ...
  // Every return-null below is a distinct editorial rule, and until 2026-08-13
  // they were indistinguishable: the caller logged "output rejected by caller
  // gates" and the seed recorded INSIGHTS_SYNTHESIS_GATE for all of them. When
  // both providers were rejected on a healthy 284-item corpus there was no way
  // to tell a model that invented a proper noun from a gate that had become
  // impossible to satisfy. onReject names the rule; it never changes what is
  // accepted, and a throwing callback must not turn a rejection into a crash.
  const reject = (rule, detail) => {
    if (typeof opts.onReject === 'function') {
      try { opts.onReject(rule, detail); } catch { /* diagnostics never break the gate */ }
    }
    return null;
  };

  if (!Array.isArray(topStories) || topStories.length === 0) return reject('no-top-stories');
  ...
  if (!hasBriefCluster) return reject('no-brief-cluster');
  ...
  if (!parsed) return reject('unparseable-synthesis');

  const groundingStories = topStories.map((story) => ({ headline: story.primaryTitle }));
  // memberTitles includes the primary title, so a naive join printed every
  // cited story twice. The validators are set-based and did not care; the
  // reject log does, because it truncates and a doubled headline can push the
  // flagged token out of view. Deduped, order preserved.
  ...
  if (leadSentences.length === 0) return reject('empty-lead');
  for (const sentence of leadSentences) {
    ...
    if (cited.length === 0) return reject('uncited-lead-sentence', sentence);
    ...
    if (validatorMode === 'enforce') {
      if (!sentenceValidation.ok) {
        return reject('lead-proper-noun', describeUngrounded(sentenceValidation, sentence, scopedGround));
      }
      if (!factValidation.ok) {
        return reject('lead-fact', describeUngrounded(factValidation, sentence, scopedGround));
      }
    }
  }
  if (!checkLeadGrounding({ lead: leadCheck.text }, groundingStories, topStories.length)) {
    return reject('lead-anchor-grounding', leadCheck.text);
  }
