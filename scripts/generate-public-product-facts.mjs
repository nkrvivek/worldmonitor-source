 ...
 *
 * Outputs are committed so the Worker, static Markdown/JSON, structured data,
 * the Pro bundle, and agent-discovery clients all publish the same facts.
 *
 * Usage:
...
import { SUPPORT_EMAIL } from './_site.mjs';
...
const productsById = Object.fromEntries(
  publicCatalogEntries
    .filter(([, entry]) => entry.providerPriceId)
    .map(([planKey, entry]) => [
...
const fallbackPrices = Object.fromEntries(
  publicCatalogEntries
    .filter(([, entry]) => entry.providerPriceId && entry.priceCents != null && entry.priceCents > 0)
    .map(([, entry]) => [entry.providerPriceId, entry.priceCents]),
);
  ...
  return {
    ...
    plans: [
      ...
      },
      {
        ...
        features: ['WM Analyst', 'Scenario Engine', 'Route Explorer', 'AI digest', 'custom widget builder', dashboardAi('pro_monthly'), 'MCP — 250 calls/day', '25 custom dashboards', 'data export — CSV, JSON & PDF reports', 'priority support', 'commercial license'],
      },
      {
        name: 'API',
      ...
      },
      {
        ...
        features: ['Everything in API', '300 requests/minute', '10,000 requests/day', dashboardAi('api_business'), '5 Pro licenses included', 'same company email required', 'commercial license — for your customers', 'priority support'],
      },
      {
        name: 'Enterprise',
        price: 'Custom',
