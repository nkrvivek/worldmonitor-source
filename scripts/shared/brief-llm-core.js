...
/**
 * Words the ground text itself writes in lowercase.
 *
 * A word a source uses as a common noun is not a name, whatever the summary's
 * grammar does to its first letter. This is the evidence that lets the
 * sentence-start check stop depending on SENTENCE_START_AMBIGUOUS being
 * complete, which it can never be — see the note on that list.
 *
 * @param {string} text
 * @returns {Set<string>} lowercase words, 2+ chars.
 */
export function lowercaseWordEvidence(text) {
  const evidence = new Set();
  if (typeof text !== 'string' || text.length === 0) return evidence;
  for (const token of text.split(/[^\p{L}\p{N}'’-]+/u)) {
    const stripped = token.replace(/[.,;:'’]+$/g, '').replace(/['’]s$/i, '');
    // Only a token whose own first letter is lowercase counts. A capitalized
    // occurrence says nothing — that is the ambiguity being resolved.
    if (stripped.length >= 2 && /^\p{Ll}/u.test(stripped)) evidence.add(stripped.toLowerCase());
  }
  return evidence;
}
...
/**
 * @param {string} text
 * @param {{ commonWordEvidence?: Set<string> }} [opts] `commonWordEvidence` is
 *   the set from lowercaseWordEvidence(groundText). A sentence's FIRST token is
 *   skipped when its lowercase form is in that set, because the source proves
 *   the word is a common noun. Mid-sentence capitals are untouched: there the
 *   capital is the writer's choice and still a claim.
 * @returns {string[][]}
 */
export function extractProperNounSequences(text, opts = {}) {
  if (typeof text !== 'string' || text.length === 0) return [];
  const commonWordEvidence = opts.commonWordEvidence instanceof Set ? opts.commonWordEvidence : null;
    ...
    for (const token of tokens) {
      ...
      const isAmbiguousSentenceStart = firstToken
        ...
        && (SENTENCE_START_AMBIGUOUS.has(token.toLowerCase())
          || commonWordEvidence?.has(tokenForLookup.toLowerCase()) === true);
...
const PLURAL_FOLD_MIN_LENGTH = 5;
const PLURAL_FOLD_VOWELS = new Set(['a', 'e', 'i', 'o', 'u']);

/**
 * Fold a plural noun onto its singular, or return the word unchanged.
 *
 * A synthesis brief puts source nouns in front of other nouns, which English
 * does in the singular: a source saying "It may send Kyiv more Patriots" is
 * summarized as "more Patriot systems". Exact token matching read that as an
 * invented weapons system and rejected a correct brief (measured 2026-08-13).
 *
 * The fold is narrow on purpose, because a wrong fold lets a real invention
 * through. A vowel before the final s is the signature of a name rather than a
 * plural, so Hamas, Texas, Paris, Wales and Jones are left alone; without that
 * guard "Hamas" would ground against a headline about Hama. Doubled s is out
 * for the same reason (Ross must not reach Ros), and short tokens are acronyms
 * and country codes far more often than plurals.
 */
function foldPlural(lower) {
  if (lower.length < PLURAL_FOLD_MIN_LENGTH) return lower;
  if (!lower.endsWith('s')) return lower;
  const preceding = lower[lower.length - 2];
  if (preceding === 's' || PLURAL_FOLD_VOWELS.has(preceding)) return lower;
  return lower.slice(0, -1);
}

/**
 * Normalize a token: if it's a known acronym variant, return the
 * canonical key; otherwise return its singular form (lower-cased).
 *
 * Both sides of the comparison run through this, so the fold cannot make one
 * text agree with another by accident — it only removes a difference English
 * grammar introduced.
 */
function normalizeToken(token) {
  ...
  const singular = foldPlural(lower);
  if (singular === lower) return lower;
  if (ACRONYM_NORMALIZE.has(singular)) return ACRONYM_NORMALIZE.get(singular);
  if (DEMONYM_NORMALIZE.has(singular)) return DEMONYM_NORMALIZE.get(singular);
  return singular;
}
...
export function validateNoHallucinatedProperNouns(summary, headline) {
  ...
  try {
    // The summary is read against what the ground text already says. A word
    // the source writes lowercase ("… grain hub attacks raise fears …") is a
    // common noun, so the summary opening with "Attacks" is grammar, not an
    // invented name. Only the summary side gets this: the ground text is the
    // evidence and cannot vouch for itself.
    summarySequences = extractProperNounSequences(summary, {
      commonWordEvidence: lowercaseWordEvidence(headline),
  ...
  // Constituent grounding set for the compound fallback below — computed
  // lazily so the common all-contiguous path pays nothing.
  let headlineConstituents = null;
  const headlineConstituentSet = () => {
    if (!headlineConstituents) {
      headlineConstituents = new Set(
        headlineSequences
          .flat()
  ...
  for (const summarySeq of summarySequences) {
    ...
    if (!found) {
      // Compound fallback (2026-08-09): a synthesis brief MERGES stories, and
      // merged phrasing compounds nouns the sources carry separately —
      // "Russia-Ukraine strikes" against "Russia and Ukraine exchange
      // strikes" (the hyphen keeps it one token), "Iran's Hormuz deal"
      // against "Iran says ... open Hormuz" (the possessive chains two nouns
      // into one sequence). Neither matches contiguously, and in enforce
      // mode that rejected every L1 synthesis for two days (seed-meta
      // consecutiveFailures=90, both providers validate_reject). A sequence
      // that fails contiguous matching still passes when EVERY constituent
      // token (hyphen-exploded, re-normalized) is grounded somewhere in the
      // headline. Token-level invention stays flagged — "Michel Aoun" or an
      // added "US" fails b/c the token itself is absent; only novel
      // compositions of nouns the ground text already contains get through.
      const constituents = summarySeq
        .flatMap((token) => token.split('-'))
        .map(normalizeToken)
        .filter((part) => part.length >= 2 && !PROPER_NOUN_JOINER.has(part));
      const grounded = constituents.length > 0
        && constituents.every((part) => headlineConstituentSet().has(part));
      if (!grounded) return { ok: false, hallucinated: summarySeq };
    }
  }
