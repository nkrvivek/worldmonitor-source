...
function pickMonthValue(monthRows, iso2, product, flow, isAnomalyCapped) {
  ...
  if (!r) return null;
  if (!isReportedAssessment(r.ASSESSMENT_CODE)) return null;
...
/**
 * The least-confident JODI assessment level attached to any usable row of the
 * published month. Null when the month carried no readable level at all.
 */
function monthAssessment(monthRows) {
  return summarizeAssessment(
    monthRows
      .filter(r => parseObsValue(r.OBS_VALUE) !== null)
      .map(r => r.ASSESSMENT_CODE),
  );
}
...
export function extractCountryData(allRows, iso2) {
  ...
  for (const month of sortedMonths) {
    ...
    const hasValidCode = monthRows.some(r => isReportedAssessment(r.ASSESSMENT_CODE));
    ...
    const hasSecondaryData = monthRows.some(
      r => isReportedAssessment(r.ASSESSMENT_CODE) && r.ENERGY_PRODUCT in SECONDARY_PRODUCTS,
    );
    if (hasSecondaryData) {
      dataMonth = month;
      break;
  ...
  return {
    iso2,
    dataMonth,
    assessment: monthAssessment(monthRows),
    ...secondaryProducts,
    crude: {
...
/**
 * Both names JODI may serve one calendar year under.
 *
 * The year in progress is published only as `secondary/secondaryyear2026.csv`;
 * it is renamed to `secondary/2026.csv` once the year closes, and the old name
 * then 404s. Asking for `${year}.csv` alone therefore missed the whole current
 * year, every run fell back to the last closed file, and China's newest month
 * read eight months old against a six-month gate. Year-to-date is tried first
 * because it is the fresher of the two whenever both would resolve.
 */
export function jodiYearUrls(kind, year) {
  return [`${JODI_BASE}${kind}/${kind}year${year}.csv`, `${JODI_BASE}${kind}/${year}.csv`];
}

/** First URL that answers. Returns '' when none do, and names each miss. */
async function fetchFirstAvailable(urls) {
  for (const url of urls) {
    try {
      return await withRetry(() => fetchCsv(url), 2, 2000);
    } catch (e) {
      console.warn(`  ${url.slice(JODI_BASE.length)} failed: ${e.message}`);
    }
  }
  return '';
}
...
async function fetchAllRows() {
  ...
  const [primaryCurrent, primaryPrior, secondaryCurrent, secondaryPrior, secondaryLookback] =
    await Promise.all([
      fetchFirstAvailable(jodiYearUrls('primary', currentYear)),
      fetchFirstAvailable(jodiYearUrls('primary', priorYear)),
      fetchFirstAvailable(jodiYearUrls('secondary', currentYear)),
      fetchFirstAvailable(jodiYearUrls('secondary', priorYear)),
      // Optional: its absence only withholds the demand change, never the seed.
      fetchFirstAvailable(jodiYearUrls('secondary', lookbackYear)),
    ]);
