...
import { loadEnvFile, CHROME_UA, runSeed, writeExtraKeyWithMeta, writeUnavailableSeedMeta, sleep, verifySeedKey, resolveProxyForConnect, fredFetchJson } from './_seed-utils.mjs';
...
const KEYS = {
  ...
  // fetchTariffTrends writes one key per reporter; this is the US shard, and the
  // only one of them with a health row (tariffTrendsUs in api/health.js).
  tariffTrendsUs: 'trade:tariffs:v1:840:all:10',
};

// The four fetchers below go through wtoFetch, which returns null before it
// issues a request when WTO_API_KEY is unset. Measured 2026-08-06: the Worker
// holds 16 secrets and WTO_API_KEY is not one of them, so none of these has ever
// run here. They resolve to null rather than rejecting, every write of theirs is
// guarded, and the seeder still reports OK because FRED shipping and Treasury
// customs succeed beside them — so the absence was silent for seven days.
// These are the keys that absence leaves behind, and they are the ones a health
// row can read. Trade flows and the other tariff shards are per-reporter and
// registered nowhere, so nothing would read a meta written for them.
const WTO_GATED_KEYS = [KEYS.barriers, KEYS.restrictions, KEYS.tariffTrendsUs];
...
async function fetchAll() {
  ...
  // No WTO credential means the four WTO branches wrote nothing above. Say so on
  // every run rather than leaving it to be inferred from a missing key.
  if (!process.env.WTO_API_KEY) {
    console.warn(`  WTO_API_KEY not set: ${WTO_GATED_KEYS.length} keys reported NOT_CONFIGURED`);
    for (const key of WTO_GATED_KEYS) await writeUnavailableSeedMeta(key);
  }
