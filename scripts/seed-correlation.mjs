...
/**
 * Read the nine correlation inputs in one Redis round trip.
 *
 * Every value goes through unwrapEnvelope. Seeds write `{_seed, data}` in
 * contract mode, and this pipeline bypasses the shared redisGet that already
 * unwraps, so without it the extraction below reads `.events` and `.quotes`
 * off the envelope and finds nothing. Legacy bare values pass through.
 */
export async function fetchInputData({
  fetchImpl = (...args) => globalThis.fetch(...args),
  credentials = null,
} = {}) {
  const { url, token } = credentials ?? getRedisCredentials();
  ...
  for (let i = 0; i < INPUT_KEYS.length; i++) {
    ...
    if (raw) {
      try {
        const payload = unwrapEnvelope(JSON.parse(raw)).data;
        if (payload != null) data[INPUT_KEYS[i]] = payload;
      } catch { /* skip */ }
    }
  }
  return data;
