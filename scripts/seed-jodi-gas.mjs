...
import {
  ...
  summarizeAssessment,
} from './shared/jodi-assessment.mjs';
import {
...
/**
 * JODI republishes the gas world database monthly under an incrementing edition
 * number: `.../jodi-publisher/gas/24/GAS_world_NewFormat.zip`. Old editions keep
 * answering 200 forever, so the seeder that pinned edition 17 kept downloading
 * March 2026 data every month without a single error, and China's newest gas
 * month sat five months behind the gate that was meant to catch exactly this.
 *
 * The floor is the newest edition seen when this was wired (2026-08-19, edition
 * 24, published 2026-07-19). Editions are contiguous, so the walk stops at the
 * first miss. Nothing at or above the floor answering is reported, never
 * quietly satisfied by an older edition.
 */
const ZIP_BASE = 'https://www.jodidata.org/jodi-publisher/gas/';
const ZIP_FILENAME = 'GAS_world_NewFormat.zip';
export const ZIP_EDITION_FLOOR = 24;
const ZIP_EDITION_PROBE_LIMIT = 36;

export function gasZipUrl(edition) {
  return `${ZIP_BASE}${edition}/${ZIP_FILENAME}`;
}
...
export function parseCsvRows(csvText) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!isReportedAssessment(assessRaw)) continue;
...
export function buildCountryRecords(rows) {
  ...
  for (const [iso2, areaRows] of byArea) {
    ...
    records.push({
      iso2,
      dataMonth:         chosen,
      assessment:        summarizeAssessment(
        chosenRows.filter(r => r.obs !== null).map(r => r.assessmentCode),
      ),
...
/**
 * Highest published edition at or above the floor.
 *
 * @param {(url: string) => Promise<boolean>} [exists] injected for tests
 */
export async function resolveLatestGasEdition(exists = editionExists) {
  let latest = null;
  for (let i = 0; i < ZIP_EDITION_PROBE_LIMIT; i++) {
    const edition = ZIP_EDITION_FLOOR + i;
    if (!(await exists(gasZipUrl(edition)))) break;
    latest = edition;
  }
  if (latest === null) {
    throw new Error(
      `No JODI gas edition answered at or above ${ZIP_EDITION_FLOOR}; the publisher numbering changed`,
    );
  }
  return latest;
}

async function editionExists(url) {
...
async function fetchAndParseCsv() {
  const edition = await resolveLatestGasEdition();
  const zipUrl = gasZipUrl(edition);
  console.log(`  Fetching JODI Gas ZIP from ${zipUrl} (edition ${edition})`);
