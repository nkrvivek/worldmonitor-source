...
// 10800 = 3h, 6x the 30-minute cron in worker/seeds/registry.ts ('11,41 * * * *').
// Was 1800 = exactly one cron interval: the key expired at the moment the next
// run was due, so a single late tick reported EMPTY (crit) on a seeder that had
// succeeded 31 minutes earlier. Measured 2026-08-19: `commodityQuotes EMPTY
// records=0 age=31m max=30m`. The fleet test froze this pair as debt back when
// the Railway cron was */5 and the same 1800 really was 6x; the Worker port
// moved the cron and nothing rechecked the margin, which is the seed-economy
// failure described in tests/seed-ttl-outlives-staleness-fleet.test.mjs.
...
runSeed('market', 'stocks', CANONICAL_KEY, fetchMarketQuotes, {
  ...
  declareRecords,
  schemaVersion: 1,
  maxStaleMin: 90,  // 3x the 30-minute cron; 30 was 1x, so one late tick alarmed
