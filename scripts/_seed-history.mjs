...
/**
 * Host of the relay URL, for the error message. A 401 with no host reads as
 * "the secret is wrong", and it is just as often the right secret sent to the
 * wrong Convex deployment — opposite fixes. Host only: a URL can carry a
 * credential in its query string and this string lands in a health record that
 * `/api/health` serves. An unparseable URL reports as `unparseable-url` rather
 * than falling back to the raw string, which would defeat the point.
 */
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const error = new SeedHistoryError(
          `intel-history relay ${relayHost(url)} returned HTTP ${response.status}: ${snippet}`,
...
/**
 * How many recent attempts the flap window keeps.
 *
 * `consecutiveFailures` answers "is it broken right now" and resets on any
 * success. That is the right question when one caller owns the record. Two of
 * these seeds run in two runtimes at once — on Railway and again in the
 * Cloudflare seeds container — and both fold into the SAME health key. The
 * runtime that works resets the streak the failing one is building, so a
 * caller that has never once succeeded reads `consecutiveFailures: 0` forever.
 * Measured 2026-08-19: conflict:acled-intel and military:cross-strait-activity
 * both showed 0 failures and a recent success while every container run
 * answered HTTP 401. energy:intelligence has no second runtime, so it was the
 * only one of the three telling the truth, and it took 11.8 days to be read.
 *
 * The window remembers failures across successes instead of resetting on them.
 */
export const HISTORY_INGEST_FLAP_WINDOW = 8;

/** Separate failure runs inside the window before `sourceState` escalates. */
export const HISTORY_INGEST_FLAP_SEGMENTS = 2;
...
export function projectHistoryIngestHealth(previous, { domain, resource, runId, at, outcome }) {
  ...
  // Newest last, capped at the window. A bad value from a hand-edited record
  // is dropped rather than trusted, the same way finiteOr drops a bad number.
  const priorAttempts = typeof prev?.recentAttempts === 'string'
    ? prev.recentAttempts.replace(/[^hf]/g, '')
    : '';
  const recentAttempts = (priorAttempts + (healthy && !lossy ? 'h' : 'f'))
    .slice(-HISTORY_INGEST_FLAP_WINDOW);
  // Count SEGMENTS of failure, not failures. One outage that recovers is one
  // segment however long it ran, and recovery is exactly what the streak
  // counter is built to forgive. Failure returning AFTER a success is the
  // shape a second, permanently broken caller makes, and no amount of the
  // healthy caller's successes erases an earlier segment from the window.
  const failureSegments = (recentAttempts.match(/f+/g) ?? []).length;
  const flapping =
    consecutiveFailures < HISTORY_INGEST_ALARM_AFTER_FAILURES &&
    recentAttempts.includes('h') &&
    failureSegments >= HISTORY_INGEST_FLAP_SEGMENTS;
      ...
      : outcome.state === 'unconfigured'
        ...
        : consecutiveFailures >= HISTORY_INGEST_ALARM_AFTER_FAILURES
          ...
          : flapping
            ...
            : 'ok',
    consecutiveFailures,
    // Named so a reader can tell a contended record from a broken one: this
    // resource is being appended by a caller that keeps failing while another
    // keeps succeeding. Absent when the window is clean.
    flapping: flapping || undefined,
    lastSuccessAt,
