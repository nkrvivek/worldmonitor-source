...
const COUNTRY_EXTRA_KEYS = Object.values(COUNTRY_MAP).map(iso2 => ({
  ...
  // A country key IS the detail record, and every member carries its own
  // seededAt from parseRecord. The leak guard compares top-level field names,
  // so it reads that record field as the pipeline's top-level seededAt escaping
  // past buildIndex — which renames it updatedAt on the index. Two different
  // fields with one name. Without this the seed fails on every run
  // (measured 2026-08-05) and nothing writes.
