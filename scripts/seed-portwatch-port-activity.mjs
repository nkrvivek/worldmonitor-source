...
export {
  ...
  PORTWATCH_COVERAGE_RETRY_MAX_FAILURES,
...
// if the maxDate check ever silently short-circuits. Shared with the cold-fetch
// queue, which uses the same wall to decide whether a deferred country would
// still publish -- two copies of this number would drift into disagreement
// about which countries are holes.
const MAX_CACHE_AGE_MS = PORTWATCH_MAX_CACHE_AGE_MS;
...
// which fits inside the 7-day MAX_CACHE_AGE_MS only while the cron keeps
// firing. It stopped for ~5.7 days in August 2026; the sweep then needed 3
// days of catch-up and the surviving caches had 7 hours left. The margin is
// 4 days, not a guarantee, so assessRotationHeadroom below measures it each
// run rather than restating it.
//
...
// The cron cadence the rotation arithmetic above assumes. Named so the
// headroom check below measures against the same number the comment claims.
const RUN_INTERVAL_MS = 12 * 60 * 60 * 1000;

/**
 * Say whether the rotation can reach every stale country before its cache
 * expires, and name the ones it cannot.
 *
 * Why this exists (measured 2026-08-19): the sweep-fits-inside-the-expiry
 * claim above holds only while the cron actually fires. It stopped for ~5.7
 * days. When it resumed, 113 countries sat at 6.72 days against the 7-day
 * MAX_CACHE_AGE_MS while the queue could clear only 30 per 12h run, so the
 * backlog needed 2 more days to reach them and they had 7 hours of cache left.
 * Every one of them was about to be dropped from the published list at once.
 *
 * Nothing said so. Coverage only reports a country AFTER it has expired and
 * been dropped, which is the point at which the damage is already done and the
 * health row is already red. This measures the gap while it is still a
 * forecast, so an operator can force the sweep instead of reading about it
 * afterwards.
 *
 * A country whose cache carries no readable `cacheWrittenAt` counts as at
 * risk. Its remaining life is unknown, not full, and treating an unreadable
 * clock as a fresh one is how the silent version of this failure starts.
 */
export function assessRotationHeadroom({
  staleServed = [],
  backlogCount = 0,
  maxPerRun = MAX_COLD_FETCH_PER_RUN,
  runIntervalMs = RUN_INTERVAL_MS,
  maxCacheAgeMs = MAX_CACHE_AGE_MS,
  now = Date.now(),
} = {}) {
  const perRun = Math.max(1, maxPerRun);
  // Runs needed before the queue reaches the last country in the backlog. The
  // country served stale this run sits somewhere in that queue, so this is the
  // worst case for any one of them, which is the case worth reporting.
  const runsToDrain = Math.ceil(Math.max(0, backlogCount) / perRun);
  const horizonMs = runsToDrain * runIntervalMs;
  const atRisk = [];
  for (const entry of staleServed) {
    const writtenAt = entry?.cacheWrittenAt;
    if (!Number.isFinite(writtenAt)) {
      atRisk.push({ iso2: entry?.iso2, expiresInMs: null });
      continue;
    }
    const expiresInMs = writtenAt + maxCacheAgeMs - now;
    if (expiresInMs < horizonMs) atRisk.push({ iso2: entry?.iso2, expiresInMs });
  }
  atRisk.sort((a, b) => {
    // Unmeasurable first: an unknown clock outranks a known-short one.
...
export async function fetchAll(progress, { signal, expectedCountries = [] } = {}) {
  ...
  // Every country published this run off prior cache rather than a fresh
  // fetch, with the clock that decides when it stops being publishable.
  const staleServed = [];
  const retainPriorState = (iso2, payload, evaluatedAt) => {
    ...
    if (deferredState.status === 'stale') {
      ...
      staleServed.push({ iso2, cacheWrittenAt: deferredState.payload?.cacheWrittenAt });
      servedStaleCount++;
      return;
    }
  ...
  if (needsFetch.length > MAX_COLD_FETCH_PER_RUN) {
    ...
    // Ahead of that rotation sit the countries that would publish nothing at
    // all if deferred again, bounded by a consecutive-failure ceiling so a
    // permanently unavailable country cannot hold a slot forever.
    const ordered = orderColdFetchQueue(needsFetch, { now, maxCacheAgeMs: MAX_CACHE_AGE_MS });
  ...
  // Measured before the coverage report because it answers a different
  // question: coverage says who was dropped, this says who is about to be.
  const headroom = assessRotationHeadroom({
    staleServed,
    backlogCount: servedStaleCount + droppedTooOldCount + droppedNoCacheCount,
  });
  if (headroom.atRiskCount > 0) {
    const named = headroom.atRisk.slice(0, 10).map(({ iso2, expiresInMs }) => (
      expiresInMs === null ? `${iso2}:unknown` : `${iso2}:${(expiresInMs / 3_600_000).toFixed(1)}h`
    )).join(', ');
    console.error(
      `  [port-activity] ROTATION BEHIND: ${headroom.atRiskCount} stale-served ` +
      `countr${headroom.atRiskCount === 1 ? 'y expires' : 'ies expire'} before the queue reaches ` +
      `${headroom.runsToDrain} run(s) from now (${(headroom.horizonMs / 3_600_000).toFixed(0)}h). ` +
      `Soonest: ${named}${headroom.atRiskCount > 10 ? ', ...' : ''}. ` +
      'Run this seed repeatedly to drain the backlog before they drop out of coverage.',
    );
  }
