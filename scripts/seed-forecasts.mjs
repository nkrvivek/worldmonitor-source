...
// The seeds run in GitHub Actions and in the seed container it builds, so
// GITHUB_SHA is the only revision this fork is ever handed. Empty when a seed
// runs from a laptop, which is what an unstamped snapshot should read as.
...
async function warmPingChokepoints() {
  // WM_API_BASE_URL predates the fork-wide API_BASE_URL that seed-fallback.yml
  // exports; accept either so this warm-ping stops silently standing down.
  const baseUrl = process.env.WM_API_BASE_URL || process.env.API_BASE_URL;
  if (!baseUrl) { console.log('  [Chokepoints] Warm-ping skipped (no WM_API_BASE_URL/API_BASE_URL)'); return; }
...
// The groq entry is the free-tier/outage fallback. Per-stage
...
const FORECAST_LLM_PROVIDERS = [
  ...
  { name: 'groq', envKey: 'GROQ_API_KEY', apiUrl: 'https://api.groq.com/openai/v1/chat/completions', model: GROQ_FALLBACK_MODEL, timeout: 20_000, extraBody: GROQ_EXTRA_BODY },
];

// market_implications runs on openrouter alone.
//
// The 2026-08-11 groq stopgap is reverted here: it named its own exit condition
// ("adding OpenRouter credits and putting 'openrouter' back here") and the user
// added credits 2026-08-12. Verified against the live key that same day --
// /api/v1/key returns is_free_tier false with credit remaining, and a
// deepseek/deepseek-v4-flash completion answers 200 at $1.74e-06 for 12 tokens,
// so this stage's ~4,749 prompt tokens x 24 hourly runs costs on the order of a
// dollar a month. The free-tier 404 ("This model is unavailable for free") and
// the 402 on a paid call are both gone.
//
// Groq is deliberately NOT kept as a fallback. Its free tier binds per MINUTE,
// not only per day -- measured 2026-08-12, x-ratelimit-limit-tokens 12000 with
// x-ratelimit-reset-tokens 205ms -- on top of the 100k/day cap this stage alone
// would exceed (~114k). Listing it would add its full 20s to the chain
// reservation that getMarketImplicationsMinRunBudgetMs demands up front, and
// this is the LAST stage on a shared 200s run budget, so a bigger reservation
// buys more skips, not more coverage. Reserving for a provider that returns 429
// in 86ms raises the admission bar for nothing.
//
// A one-provider chain has no fallback to strand, so it takes one retry rather
// than the #5003 zero -- see getMarketImplicationsMaxRetries. Note the stage env
// override cannot change this in production: worker/seeds/env.ts lists
// FORECAST_LLM_MARKET_IMPLICATIONS_PROVIDER_ORDER under SEED_ENV_NOT_FORWARDED,
// so it reaches a local run and never the container.
...
function getForecastLlmCallOptions(stage = 'default') {
  ...
  if (stage === 'critical_signals' && !criticalProviderOrder) {
    return {
      // Groq is out of this stage, not re-pinned. It held
      // `llama-3.1-8b-instant`, and on 2026-08-24 a probe of
      // GET https://api.groq.com/openai/v1/models found Groq had dropped
      // every Llama id it used to serve, so that pin had been answering 404
      // and falling straight through to openrouter for weeks. Naming a
      // current Groq model here would move a probability-coupled stage onto
      // a model the #4930 baseline never scored, which is the one thing the
      // note above forbids. Dropping the dead entry keeps the behaviour this
      // stage already had and stops paying for the failing call.
...
// failureDetail carries the LAST attempt's reason (`openrouter http_402`,
// `groq timeout`, …). The failureReason alone collapses every provider fault to
// `provider_failed`, and the market_implications seed-meta then reads
// `llm_no_response` — which describes an unresponsive model. An exhausted
// OpenRouter balance writes exactly the same words, so the one fault an operator
// can fix in a minute is indistinguishable from one they cannot fix at all.
function createForecastLlmFailureResult(options, failureReason, failureDetail = '') {
  return options.returnFailureReason
    ? { text: '', model: '', provider: '', failureReason, failureDetail }
    : null;
}
...
async function callForecastLLM(systemPrompt, userPrompt, options = {}) {
  ...
  // stages (market_implications) pass 0 on a MULTI-provider chain so a slow
  // primary cannot burn the shared run budget across 4 attempts and strand the
  // fallback provider (#5003 review) — and 1 when the chain has a single
  // provider, where there is no fallback to strand and zero retries let one
  // self-clearing 429 kill the stage. See getMarketImplicationsMaxRetries.
  ...
  // Last attempt's provider + reason, carried out on the failure result so the
  // caller's seed-meta can name the fault instead of guessing at it.
    ...
    for (const provider of providers) {
      ...
      try {
        ...
        const resp = await withRetry(async () => {
          ...
          try {
            ...
            if (!response.ok) {
              ...
              lastFailureDetail = `${provider.name} http_${response.status}`;
            ...
            if (!err?.__llmAttemptRecorded && !isForecastLlmBudgetError(err)) {
              ...
              lastFailureDetail = `${provider.name} ${timedOut ? 'timeout' : 'fetch_error'}`;
        ...
        try {
          ...
          sawProviderFailure = true;
          lastFailureDetail = `${provider.name} invalid_json`;
        ...
        if (!text || text.length < 20) {
          sawProviderFailure = true;
          lastFailureDetail = `${provider.name} empty`;
...
// overridden) order that actually has an API key, ONE attempt each (the reservation
// stays at one attempt per provider even on the single-provider chain that now takes
// one retry — see getMarketImplicationsMaxRetries), summed with the 5s stage guard that getUsableForecastLlmBudgetMs
...
function getMarketImplicationsRunnableProviders(llmOptions = {}) {
  return resolveForecastLlmProviders(llmOptions).filter((provider) => process.env[provider.envKey]);
}

function getMarketImplicationsMinRunBudgetMs(llmOptions = {}) {
  const runnable = getMarketImplicationsRunnableProviders(llmOptions);
  const chainMs = runnable.reduce((sum, provider) => sum + (provider.timeout || 0), 0);
  return chainMs + FORECAST_LLM_STAGE_BUDGET_GUARD_MS;
}

// Retries for the tail stage, decided by how many providers the chain actually
// has (#5003 assumed two).
//
// With a MULTI-provider chain the original reasoning stands: a slow primary that
// retries four times burns the shared run budget and strands the fallback, so
// zero retries and fall straight through.
//
// With a SINGLE-provider chain there is no fallback to strand, and zero retries
// means one HTTP call decides the stage. That turned a self-clearing condition
// into a dead seed: MARKET_IMPLICATIONS_DEFAULT_PROVIDER_ORDER became ['groq']
// on 2026-08-11 when OpenRouter's free tier went away, and groq's free tier is
// capped per MINUTE, not only per day — measured 2026-08-12, x-ratelimit-limit-
// tokens 12000 with x-ratelimit-reset-tokens 205ms. So a 429 that clears in a
// fifth of a second wrote SEED_ERROR for the whole hour.
//
// One replay, not four. The reservation above is unchanged on purpose: a 429
// returns in milliseconds, so the replay is affordable out of what the first
// attempt did not spend, and the retry path re-reads the budget before every
// attempt (getUsableForecastLlmBudgetMs) and caps that attempt's timeout to
// what is left. A retry can therefore never overrun the budget, and a retry
// that gets squeezed is attributed to the budget rather than to the provider.
// Growing the reservation to cover a worst-case replay would over-skip real
// runs to protect against a case the mid-call guard already handles.
const MARKET_IMPLICATIONS_SINGLE_PROVIDER_RETRIES = 1;

function getMarketImplicationsMaxRetries(llmOptions = {}) {
  return getMarketImplicationsRunnableProviders(llmOptions).length <= 1
    ? MARKET_IMPLICATIONS_SINGLE_PROVIDER_RETRIES
    : 0;
}
...
async function buildAndSeedMarketImplications(inputs) {
  ...
  const result = await callForecastLLM(MARKET_IMPLICATIONS_SYSTEM_PROMPT, userPrompt, {
    ...
    maxRetries: getMarketImplicationsMaxRetries(llmOptions),
  });
  ...
  if (!result?.text) {
    ...
    // Name the last provider fault in the seed-meta. `llm_no_response` alone
    // reads as an unresponsive model; an exhausted OpenRouter balance writes the
    // same words, and that one is a one-minute fix an operator will never make
    // if the meta does not say `openrouter http_402`.
    const reason = result?.failureDetail
      ? `llm_no_response: ${result.failureDetail}`
      : 'llm_no_response';
    console.warn(`  [MarketImplications] LLM returned no response (${reason}) — writing error seed-meta`);
    await writeMarketImplicationsFailureMeta(reason);
    return;
  }
