...
import { loadEnvFile, CHROME_UA, getRedisCredentials, acquireLockOrWait, releaseLock, withRetry, writeFreshnessMetadata, logSeedResult, verifySeedKey, extendExistingTtl } from './_seed-utils.mjs';
...
// Written on EVERY run outcome, not just success: container stdout is unreachable,
// and the fetch-failed / 0-flights paths otherwise discard their own diagnostics.
const MILITARY_FLIGHTS_RUN_KEY = 'military:flights-run:v1';
const MILITARY_FLIGHTS_RUN_TTL = 21600;
...
// Undici reports every network failure as a bare "fetch failed"; the syscall code
// (ENOTFOUND vs ETIMEDOUT vs ECONNRESET) lives down the cause chain, and that code is
// what separates a DNS fault from an egress block when reading statuses out of Redis.
function describeNetworkError(error) {
  const parts = [error?.message || String(error)];
  let cause = error?.cause;
  while (cause && parts.length < 4) {
    const aggregated = Array.isArray(cause.errors) ? cause.errors[0] : null;
    parts.push(cause.code || aggregated?.code || cause.message || String(cause));
    cause = cause.cause || aggregated;
  }
  return parts.join('/');
}
...
// Best-effort by design: a diagnostics write must never fail the seed itself.
async function writeRunStatus(url, token, outcome, detail = {}) {
  try {
    await redisSet(url, token, MILITARY_FLIGHTS_RUN_KEY, { at: Date.now(), outcome, ...detail }, MILITARY_FLIGHTS_RUN_TTL);
    console.log(`  ${MILITARY_FLIGHTS_RUN_KEY}: ${outcome}`);
  } catch (err) {
    console.warn(`  run-status write failed: ${err.message || err}`);
  }
}
...
async function main() {
  ...
  const lockResult = await acquireLockOrWait('military:flights', runId, 120_000, { label: 'military:flights' });
  if (lockResult.skipped) {
    process.exit(0);
  }
  if (!lockResult.locked) {
    // 2026-08-25: two consecutive hourly green skips here left theaterPosture
    // unwritten for 165 minutes. The lock TTL is 120s and we already waited it
    // out once, so a lock still held is a stuck holder, not normal overlap.
    // Exit red so the gap shows in Actions instead of surfacing hours later
    // as a freshness-monitor breach.
    console.error('  FAILED: military:flights lock still held after waiting out its TTL — stuck holder, seed skipped');
    process.exit(1);
  }
  ...
  try {
    ...
    await writeRunStatus(url, token, 'fetch_failed', { error: String(err.message || err), fetchSources: fetchSources ?? null });
  ...
  if (flights.length === 0) {
    ...
    await writeRunStatus(url, token, 'no_flights', { rawStates: allStates.length, source, fetchSources });
    await extendExistingTtl([LIVE_KEY, 'seed-meta:military:flights'], LIVE_TTL);
    await extendExistingTtl([STALE_KEY, THEATER_POSTURE_STALE_KEY, MILITARY_SURGES_STALE_KEY, MILITARY_FORECAST_INPUTS_STALE_KEY, MILITARY_CLASSIFICATION_AUDIT_STALE_KEY], STALE_TTL);
    await extendExistingTtl([THEATER_POSTURE_LIVE_KEY, MILITARY_FORECAST_INPUTS_LIVE_KEY, MILITARY_CLASSIFICATION_AUDIT_LIVE_KEY], THEATER_POSTURE_LIVE_TTL);
    await extendExistingTtl([THEATER_POSTURE_BACKUP_KEY], THEATER_POSTURE_BACKUP_TTL);
  ...
  try {
    ...
    const theaterPostureLockResult = await acquireLockOrWait('theater-posture', runId, 120_000, { label: 'theater-posture' });
    if (theaterPostureLockResult.skipped || !theaterPostureLockResult.locked) {
      ...
      if (!theaterPostureLockResult.skipped) {
        // Same 2026-08-25 incident: an unpublished posture with a green run is
        // a silent gap. The rest of the run continues, but the run goes red.
        console.error('  FAILED: theater-posture lock still held after waiting out its TTL — posture not published this run');
        process.exitCode = 1;
      }
    } else {
      try {
    ...
    await writeRunStatus(url, token, 'published', { flights: flights.length, rawStates: allStates.length, source, fetchSources });
