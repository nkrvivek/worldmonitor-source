...
// A missing key is a failure, not a skip. This seeder is on the 45 */12 cron
// and `webcam:cameras:active` has a 30h TTL, so a silent exit 0 spends a day
// and a half looking like a clean run and then surfaces as a freshness
// blocker with no cause attached.
const WINDY_API_KEY = process.env.WINDY_API_KEY;
if (!WINDY_API_KEY) {
  console.error('WINDY_API_KEY not set — cannot seed webcams');
  process.exit(1);
}
...
const GEO_TTL = 86400;
// The v3 API rejects offsets past ~1050 with a 400, so a region is at most
// ~21 pages. Sorting by popularity makes that ceiling a curated top slice
// per region instead of an arbitrary one.
...
async function fetchRegion(bounds, regionName) {
  ...
  while (offset < MAX_OFFSET) {
    ...
    // v3 bbox order is N,E,S,W. The old `cameraBoundingBox` name (S,W,N,E)
    // is silently ignored — measured 2026-08-10: disjoint boxes returned the
    // identical global list, so 8 regions deduped to one region's worth.
    url.searchParams.set('bbox', `${N},${E},${S},${W}`);
    ...
    if (!resp.ok) {
      if (resp.status === 400 && offset > 0) {
        console.log(`  [${regionName}] API offset limit at ${offset}, keeping ${cameras.length} of ${regionTotal ?? '?'} cameras`);
        break;
      }
    ...
    if (regionTotal === null) regionTotal = data.total ?? null;
