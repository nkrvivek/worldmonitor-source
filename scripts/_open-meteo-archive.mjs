...
export async function fetchOpenMeteoArchiveBatch(zones, opts) {
  const {
    ...
    // Wall-clock instant the caller owns, shared across every batch of a walk.
    // Open-Meteo's Retry-After runs to 60s and we honour it four times, so one
    // 429'd batch can spend four minutes alone. seed-climate-zone-normals walks
    // 13 batches inside a 240s fetch deadline and died at 240336ms with nothing
    // written (measured 2026-08-05). Past this instant the helper stops
    // sleeping and goes to the proxy legs, so a walk loses zones instead of
    // losing everything. Unset means no budget — the old behaviour.
    deadlineAt = null,
  ...
  // A sleep that would land past the caller's budget buys nothing — the walk
  // is over by then. Stop the direct loop and let the proxy legs answer.
  const budgetAllows = (sleepMs) => !Number.isFinite(deadlineAt) || Date.now() + sleepMs < deadlineAt;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (attempt < maxRetries && budgetAllows(retryMs)) {
    ...
    if (RETRYABLE_STATUSES.has(resp.status) && attempt < maxRetries) {
      ...
      if (budgetAllows(retryMs)) {
        console.log(`  [OPEN_METEO] ${resp.status} for ${label}; retrying batch in ${Math.round(retryMs / 1000)}s`);
        await sleep(retryMs);
        continue;
      }
      console.log(`  [OPEN_METEO] ${resp.status} for ${label}; retry budget spent, going to proxy`);
    }
