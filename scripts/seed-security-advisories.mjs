...
// Direct fetch by default (2026-08-09). The old default was the upstream
// author's proxy.worldmonitor.app — infrastructure this fork does not run and
// cannot see fail. The proxy existed for the Railway relay's egress, and a
// worker-side /rss port would fetch from the same Cloudflare egress the
// container already uses, so proxying buys nothing here except an open-proxy
// route to secure. RELAY_URL stays as an escape hatch: set it and every feed
// routes through `${RELAY_URL}/rss?url=` again; a feed that blocks Cloudflare
// egress shows up as its own HTTP-status warn line in the seed log.
