...
// Analytics. Was a hosted Umami script from abacus.worldmonitor.app, a host
// that no longer answers; this posts to our own collector instead
// (worker/routes/analytics-collect.ts). /research/ is served from the same
// origin, so the relative path is all it needs. The nonce matches the one the
// corpus shell's CSP already allows.
const UMAMI_SCRIPT_TAG = `<script nonce="wm-static-bootstrap">
(function(){
  function id(){
  ...
  fetch('/api/send', {
    ...
    body: JSON.stringify({ type: 'event', payload: {
      id: id(), visitId: id(), hostname: location.hostname, url: location.pathname,
      referrer: document.referrer || '', language: navigator.language || '',
      screen: screen.width + 'x' + screen.height, name: 'pageview'
    } })
  }).catch(function(){});
})();
</script>`;
...
${justification}
  ...
  const jsonLd = {
    ...
    publisher: { '@type': 'Organization', name: 'World Monitor', url: `${SITE_ORIGIN}/` },
