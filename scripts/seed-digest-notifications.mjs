...
// No Request here — the cron mints these links from nothing — so the
// default is written in. It is this fork's host, not upstream's, which is
// what the value used to say. Keep it in step with FORK_HOST in
// api/_first-party-origin.ts; the seed container does not ship api/, so
// the two cannot share one constant.
const WORLDMONITOR_PUBLIC_BASE_URL =
  process.env.WORLDMONITOR_PUBLIC_BASE_URL ?? 'https://worldmonitor.sibt.ai';
