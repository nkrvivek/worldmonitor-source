...
/**
 * Where to relay SZSE metadata, or null to fetch it directly.
 *
 * The endpoint stays pinned and is never read from the environment. A relay
 * whose destination an env var can move is a relay that anyone who can set an
 * env var can point at themselves, secret and all.
 *
 * Upstream opened this gate on RAILWAY_ENVIRONMENT. This fork runs on
 * Cloudflare, which never sets it, so the gate could not open — and the pinned
 * endpoint lives on upstream's host, which this fork neither runs nor routes.
 * So the path stays off here, and CHINA_EXCHANGE_EDGE_EGRESS=1 is what turns it
 * back on for a deployment that does serve that endpoint.
 */
export function resolveChinaExchangeEdgeEgress(env = process.env) {
  const enabled = String(env.CHINA_EXCHANGE_EDGE_EGRESS || '') === '1';
