...
// Upstream ran seven hosts: an apex that served only /mcp, a www that served
// everything else, and five variant subdomains. This fork serves all of it from
// one host, so anything else in a sitemap of ours is a URL we do not answer.
const EXPECTED_PAGE_HOSTS = new Set([new URL(SITE_ORIGIN).hostname]);
...
const sitemapOwner = (value) => {
  ...
  if (pathname === '/docs/sitemap-index.xml' || pathname.startsWith('/docs/')) return 'docs';
  return 'root';
};
...
const expectedRootSitemaps = (origin) => [
  ...
  { url: `${origin}/docs/sitemap-index.xml`, owner: 'docs' },
];
