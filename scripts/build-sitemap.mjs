...
// Fork-owned. Re-exported because callers import SITE_ORIGIN from here.
 ...
 *
 * Blog URLs belong to the blog build's /blog/sitemap-index.xml and
 * documentation URLs to the docs build's /docs/sitemap-index.xml. Keep those
 * page families out of this manifest so their publishers remain the only
 * inventory owners.
 *
...
export const STATIC_ROUTE_MANIFEST = Object.freeze([
  ...
  // Upstream ran the five dashboard variants on their own subdomains and listed
  // each as its own URL. This fork serves them as paths off one host
  // (src/config/variant-meta.ts), and each path returns the same document as
  // /dashboard with `<link rel="canonical" href="…/dashboard">` in it. Measured
  // 2026-08-06 against production: /tech, /finance, /commodity, /happy and
  // /energy all answer 200 and all name /dashboard as their canonical. A URL
  // that points its canonical elsewhere does not belong in a sitemap, so the
  // dashboard-variant family is gone and /dashboard alone carries them.
]);
