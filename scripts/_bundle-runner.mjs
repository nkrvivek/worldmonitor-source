...
// How long a run record outlives the run that wrote it. Longer than the
// longest section interval in any bundle (30 days, FATF), so the record that
// explains why a monthly section never published is still there a month later.
export const RUN_RECORD_TTL_SECONDS = 35 * 86_400;

// Its own namespace, not seed-meta:*. Those keys carry a source's freshness
// and are held to a bare {fetchedAt, recordCount} shape by
// api/seed-contract-probe.ts; a run record is neither a source nor that shape.
export const RUN_RECORD_KEY_PREFIX = 'seed-run:';

/**
 * Persist one run record, best effort.
 *
 * Every section outcome above goes to console and nowhere else. On Railway
 * that was enough — the platform kept the logs. On a Cloudflare container it
 * is not: `wrangler tail` shows the cron invocation and the container's start
 * RPC with `logs: []`, and `wrangler containers` has no logs subcommand, so a
 * section that fails only in the container leaves no trace anyone can read.
 * That is how the FATF section stayed broken while the other 14 in its bundle
 * ran clean. This writes the same lines to a key instead.
 *
 * Never throws and never blocks the exit code: a bundle whose sections all
 * succeeded must still exit 0 when Redis is unreachable.
 */
async function writeRunRecord(key, value, writeKey = defaultWriteRedisKey) {
  try {
    await writeKey(key, value);
    return true;
  } catch (err) {
    // Say which way it failed rather than claiming a write that did not
    // happen. On a container nobody reads this line, which is exactly why the
    // caller must not treat a lost record as a lost run.
    console.error(`[Bundle] run record write failed for ${key}: ${err?.message || err}`);
    return false;
  }
}

async function defaultWriteRedisKey(key, value) {
  // Throw rather than return: a missing credential and a successful write must
  // not both log "Run record written".
...
// How much of a failing child's stderr the run record carries. The runner's own
// reason for a plain non-zero exit is the exit code and nothing else, which
// names the failure without explaining it. The child's last words usually do.
export const STDERR_TAIL_LINES = 12;
const STDERR_TAIL_LINE_CHARS = 300;
...
function spawnSeed(scriptPath, { timeoutMs, label, bundleStartedAtMs }) {
  return new Promise((resolve) => {
    ...
    // Keep the tail, not the head: a stack trace's cause is at the bottom, and
    // a chatty seeder would otherwise fill the buffer with warmup noise.
    const stderrTail = [];
    streamLines(child.stderr, (line) => {
      console.warn(`  [${label}] ${line}`);
      stderrTail.push(line.slice(0, STDERR_TAIL_LINE_CHARS));
    ...
    const settle = (value) => {
      ...
      // A section that succeeded explains itself. Only a failure needs the
      // child's last words carried out of the container.
      resolve(value.ok ? value : { ...value, stderrTail: [...stderrTail] });
    };
...
export async function runBundle(label, sections, opts = {}) {
  ...
  // One entry per section, in the order the loop reaches them, mirroring the
  // console lines below. Written to Redis at the end by writeRunRecord.
  const outcomes = [];
  ...
  for (const section of sections) {
    ...
    if (missingEnv) {
      ...
      outcomes.push({ label: section.label, status: 'CONFIG_ERROR', reason });
      failed++;
      continue;
    }
    ...
    if (freshness?.fetchedAt) {
      ...
      if (elapsed < section.intervalMs * 0.8) {
        ...
        outcomes.push({
          label: section.label,
          status: 'SKIPPED',
          reason: `last seeded ${agoMin}min ago (interval ${intervalMin}min)`,
        });
        skipped++;
        continue;
      }
    ...
    if (elapsedBundle + worstCase > maxBundleMs) {
      ...
      outcomes.push({
        label: section.label,
        status: 'DEFERRED',
        reason: `needs ${needSec}s but ${remainingSec}s left in budget`,
      });
      deferred++;
      continue;
    }
    ...
    if (result.ok) {
      ...
      outcomes.push({
        ...
        records: sc && typeof sc === 'object' ? sc.recordCount ?? null : null,
        state: sc && typeof sc === 'object' ? sc.state || 'OK' : null,
      });
      ...
      outcomes.push({
        ...
        status,
        elapsedSec: Number(result.elapsed),
        // The reason text is the whole point of the record: on a container it
        // is the only surviving description of why the section failed.
        reason: (result.reason || 'unknown').replace(/\s+/g, ' ').slice(0, 500),
        // `exit 1` names a failure without explaining it. These lines are the
        // explanation, and on a container they exist nowhere else.
        stderrTail: result.stderrTail?.length ? result.stderrTail : undefined,
      });
  ...
  const totalSec = ((finishedAt - t0) / 1000).toFixed(1);
  ...
  const runRecordKey = `${RUN_RECORD_KEY_PREFIX}${label}`;
  const recorded = await writeRunRecord(runRecordKey, {
    ...
    deferred,
    failed,
    graceful: gracefulFailed,
    sections: outcomes,
  });
  if (recorded) console.log(`[Bundle:${label}] Run record written to ${runRecordKey}`);
