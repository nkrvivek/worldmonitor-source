...
// How long the whole zone walk may spend fetching, and how long the run holds
// its lock. The walk stops first, so a slow run publishes the zones it has
// instead of dying with nothing.
//
// Measured 2026-08-05: 13 batches share one fetch deadline, Open-Meteo answers
// 429 on several of them, and each 429 honours a Retry-After of up to 60s four
// times. One bad batch could spend the whole default 240s budget alone
// (lockTtlMs 120s + FETCH_PHASE_DEADLINE_MARGIN_MS 120s in _seed-utils.mjs)
// and the phase died at 240336 ms.
//
// _seed-utils.mjs derives the fetch deadline from lockTtlMs on purpose — a
// seeder must never outlive its own lock — so widen the lock rather than pin a
// fetch timeout. 360s lock gives a 480s fetch deadline, inside the 600_000 ms
// seed-bundle-climate allows this section.
const NORMALS_WALK_BUDGET_MS = 300_000;
const NORMALS_LOCK_TTL_MS = 360_000;

// One budget for the fetch phase, not one per attempt. runSeed wraps the fetch
// in withRetry and races every attempt against a single deadline, so a retry
// that started its own clock would walk straight past it.
let sharedWalkDeadlineAt = null;

function walkDeadline() {
  if (sharedWalkDeadlineAt == null) sharedWalkDeadlineAt = Date.now() + NORMALS_WALK_BUDGET_MS;
  return sharedWalkDeadlineAt;
}
...
export async function fetchClimateZoneNormals({ deadlineAt = walkDeadline() } = {}) {
  const normals = [];
  let failures = 0;
  let skipped = 0;
  ...
  for (const batch of chunkItems(CLIMATE_ZONES, NORMALS_BATCH_SIZE)) {
    ...
    if (Date.now() < deadlineAt) await sleep(NORMALS_BATCH_DELAY_MS);
  }

  if (skipped > 0) {
    console.log(`  [CLIMATE_NORMALS] budget spent with ${skipped} zone(s) unreached; publishing ${normals.length}`);
  }
