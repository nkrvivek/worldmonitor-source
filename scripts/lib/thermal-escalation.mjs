...
// 7 days, equal to BASELINE_WINDOW_MS below. Nothing reads an entry older than the
// baseline window: this module is the history key's only reader, and it only
// averages the last 7 days. At 30 days the key reached 9.47MB (2,854 cells x ~27
// entries, measured 2026-09-02) and Upstash refused the next write with HTTP 413,
// which failed every thermal seed from 2026-09-02 12:15Z on. Keep this <= the
// request cap; a longer window needs a different store, not a bigger constant.
