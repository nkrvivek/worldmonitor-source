...
function usniGetRegionCoords(regionText) {
  ...
  for (const [key, coords] of Object.entries(USNI_REGION_COORDS)) {
    ...
    // Both directions. USNI writes "In the Caribbean" where the table says
    // "Caribbean Sea", and a one-way containment check misses that and hands
    // back 0,0. src/config/military.ts has always matched both ways, so a
    // one-way check here also made the two halves of the pipeline disagree.
    if (!lower.includes(normalizedKey) && !normalizedKey.includes(lower)) continue;
    // Longest key wins, so "Eastern Mediterranean" does not settle for
    // whichever shorter key the table happens to list first.
    if (!bestMatch || key.length > bestMatch.key.length) bestMatch = { key, coords };
  }
