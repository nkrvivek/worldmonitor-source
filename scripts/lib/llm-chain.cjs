...
const LLM_PROVIDERS = [
  ...
  // brief cache-version bumps. Do not swap it in isolation.
  {
    ...
    // Was `llama-3.1-8b-instant` until 2026-08-24, when a probe of
    // GET https://api.groq.com/openai/v1/models found Groq had dropped every
    // Llama id it used to serve. This is a repair of a dead pin, not the U4
    // brief-voice cutover the note above defers: the voice question is about
    // the openrouter primary, and a fallback pinned to a 404 answers nothing.
    // Literal, not the shared constant, because this file is CommonJS and
    // shared/groq-model.js is ESM; tests/groq-model-single-source.test.mjs
    // pins the two equal.
