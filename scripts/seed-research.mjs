...
// 540 = 1.5× the 6h registry cron (35 */6) — must match api/health.js researchArxivHnTrending.
export const RESEARCH_MAX_STALE_MIN = 540;
// 10h. MUST outlive BOTH the health staleness gate (540min → 32400s) and the 6h cron cadence,
// so a merely-late tick degrades to STALE_SEED (last-good still served) instead of an EMPTY crit
// (issue #5409). History: was 3600 (≈1× the then-hourly cron, BELOW the gate), then 10800 (3h) —
// which fell BELOW the 6h cadence when the cron moved to 35 */6, expiring the data key 3h into
// every window while the meta stayed fresh: status=EMPTY records=0 with a live seeder.
export const ARXIV_TTL = 36000;
const HN_TTL = 600;
...
async function fetchAll() {
  ...
  // Two keys, one payload. The RPC reads research:tech-events:v1; the bootstrap
  // bundle and api/health.js read the -bootstrap mirror, which only the retired
  // Railway relay ever wrote. Writing one and not the other is why the
  // techEvents health row read EMPTY while the panel had data. Both share the
  // one seed-meta key, so the mirror does not get a meta row of its own.
  if (allData.techEvents?.events?.length > 0) {
    const eventCount = allData.techEvents.events.length;
    await writeExtraKeyWithMeta('research:tech-events:v1', allData.techEvents, TECH_EVENTS_TTL, eventCount);
