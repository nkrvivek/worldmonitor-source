...
const productEntries = Object.entries(PRODUCT_CATALOG)
  .filter(([, e]) => e.providerPriceId)
  .map(([key, e]) => {
    const exportKey = KEY_MAP[key] || key.toUpperCase();
    return `  ${exportKey}: '${e.providerPriceId}',`;
  })
  .join('\n');
...
${Object.values(PRODUCT_CATALOG)
  ...
  .map((entry) => `  '${entry.providerPriceId}',`)
  .join('\n')}
]);
`;
...
for (const [tierGroup, entries] of tierGroups) {
  ...
  if (primary.selfServe && primary.providerPriceId) {
    tier.monthlyProductId = primary.providerPriceId;
    if (annual?.providerPriceId) {
      tier.annualProductId = annual.providerPriceId;
    }
  } else if (!primary.selfServe && primary.priceCents === 0) {
    tier.cta = 'Get Started';
    tier.href = 'https://worldmonitor.sibt.ai/dashboard';
  } else if (!primary.selfServe && primary.priceCents === null) {
    tier.cta = 'Contact Sales';
    tier.href = `mailto:${SUPPORT_EMAIL}`;
  }
