...
# `timeout` is GNU coreutils. macOS ships neither it nor `gtimeout`, so both run
# modes below died with "timeout: command not found" on every Mac here — exit
# 127, no tests run, and the two suites that cover this script red on a clean
# checkout. Find a real timeout binary when the machine has one, and otherwise
# hold the same limit with a watchdog rather than dropping it.
timeout_bin=""
...
# Exits with the command's own status, or 124 when the limit killed it — the
# status GNU timeout returns, so a caller reads the same number either way.
run_with_timeout() {
  local secs="$1"
  shift
  ...
  # The watchdog polls in one-second steps and retires itself the moment the
  # runner is gone. A single `sleep "$secs"` cannot be cancelled: a
  # non-interactive bash defers a signal until its foreground child returns, so
  # killing that watchdog would have blocked this function for the full limit
  ...
  # 143 is SIGTERM, and the watchdog is the only thing that sends it here.
...
case "$mode" in
  ...
  run-node)
    ...
    run_with_timeout "$TEST_TIMEOUT" npx tsx --test "${selected[@]}" || exit 1
    ;;
  ...
  run-dom)
    ...
    run_with_timeout "$TEST_TIMEOUT" npm run test:dom || exit 1
    ;;
esac
