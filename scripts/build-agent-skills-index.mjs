...
const INSTRUCTIONS = [
  ...
  `- MCP server (recommended): ${SITE_ORIGIN}/mcp — Streamable HTTP; issue \`tools/list\` for the live inventory.`,
  `- REST API: base ${SITE_ORIGIN} — OpenAPI spec at ${SITE_ORIGIN}/openapi.yaml.`,
  ...
  `- Auth: OAuth2 (\`scope=mcp\`) or an API-key header \`X-WorldMonitor-Key: wm_<40-hex>\`. Issue a key at ${SITE_ORIGIN}/pro.`,
].join('\n');
