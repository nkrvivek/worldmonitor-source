...
const HOSTS = (process.env.MCP_SMOKE_HOSTS ?? 'https://worldmonitor.sibt.ai')
  .split(',').map((h) => h.trim()).filter(Boolean);
const TIMEOUT_MS = 15_000;
const USER_AGENT = 'WorldMonitor-MCP-Smoke/1.0 (+https://worldmonitor.sibt.ai; github-actions)';
...
async function probeDiscovery(host) {
  ...
  try {
    ...
    if (res.status !== 200) {
      ...
      // The canonical URL is request-derived (api/_first-party-origin.ts), so
      // each host names its own /mcp. Hardcoding upstream's apex here made the
      // check pass only against upstream.
    } else if (!linkNamesCanonical(res.headers.get('link'), `${stripTrailingSlash(host)}/mcp`)) {
      fail(host, 'HEAD /mcp (crawler)', `guide lacks the canonical Link for ${stripTrailingSlash(host)}/mcp (got "${res.headers.get('link')}")`);
    } else {
...
//
// Off by default. The five product subdomains this was written for
// (tech, finance, commodity, happy, energy) live under a domain this
// deployment does not serve, so probing them measured someone else's edge
// config and reported it as ours. Set MCP_SMOKE_VARIANT_HOSTS to a
// comma-separated list of full origins to turn the checks back on, and
// MCP_SMOKE_CANONICAL_ORIGIN to the origin they should redirect to.
const VARIANT_HOSTS = (process.env.MCP_SMOKE_VARIANT_HOSTS ?? '')
  .split(',').map((v) => v.trim()).filter(Boolean);
const CANONICAL_ORIGIN = stripTrailingSlash(process.env.MCP_SMOKE_CANONICAL_ORIGIN ?? HOSTS[0] ?? '');
const CANONICAL_MCP_URL = `${CANONICAL_ORIGIN}/mcp`;

function stripTrailingSlash(value) {
  return value.endsWith('/') ? value.slice(0, -1) : value;
}

/** Does this Link header name `url` as the canonical URL? */
function linkNamesCanonical(link, url) {
  return (link ?? '')
    .split(',')
    .some((part) => part.includes(`<${url}>`) && /rel\s*=\s*"?canonical"?/i.test(part));
}
...
async function probeVariantCanonical(host) {
  ...
  try {
    ...
    if (res.status !== 308 || location !== CANONICAL_MCP_URL) {
      fail(host, 'GET /mcp → apex canonical', `expected 308 → , got ${res.status} → ${location}`);
    } else if (!/\bAccept\b(?!-)/i.test(res.headers.get('vary') ?? '')) {
  ...
  try {
    ...
    const location = res.headers.get('location');
    if (res.status !== 308 || location !== CANONICAL_MCP_URL) {
      fail(host, 'HEAD /mcp → apex canonical', `expected 308 → , got ${res.status} → ${location}`);
    } else if (!/\bAccept\b(?!-)/i.test(res.headers.get('vary') ?? '')) {
