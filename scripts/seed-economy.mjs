...
import { GSCPI_META_KEY, GSCPI_REDIS_KEY, GSCPI_TTL, buildGscpiPayload, fetchGscpiFromNyFed } from './_gscpi.mjs';
...
// This seeder runs once a day — `0 9 * * *` in worker/seeds/registry.ts. Every
// TTL below must outlive the gap between runs, or the key dies partway through
// the day and the endpoint serves nothing until tomorrow. The four sized here
// were written when the seeder ran every 6 hours; the Worker port moved it to
// daily and left them, so macro signals, the stress index and energy prices
// were gone for most of every day while their seed-meta (7-day TTL) still read
// healthy. Measured 2026-08-06: seed at 2:00 AM PT, all three null at 1:58 PM.
// 26 hours = the daily interval plus two hours of scheduling drift, the same
// bound FRED_TTL already carried. Raising any of these means moving its
// health-check maxStaleMin in api/health.js in lockstep — see
// tests/seed-economy-cron-ttl.test.mjs.
// 36h, not the old 26h. health.js gates these keys at 1560min (26h), so a 26h TTL
// let the data expire at the same minute health became willing to call it stale: a
// merely-late daily run reported EMPTY (crit) instead of STALE_SEED (warn). The TTL
// has to outlive the gate for the escalation to be ordered.
const STRESS_INDEX_TTL = 129600; // 36h — survive daily cron scheduling drift
const FRED_TTL = 129600; // 36h — survive daily cron scheduling drift
const ENERGY_TTL = 129600; // 36h — survive daily cron scheduling drift
const CAPACITY_TTL = 129600; // 36h — survive daily cron scheduling drift
const MACRO_TTL = 129600; // 36h — survive daily cron drift and extended Yahoo outages
...
// A series that bounces once used to cost a FULL DAY of staleness, so the batch
// gets ONE serial replay pass over whatever failed.
//
// Measured 2026-08-12 against production Redis: eight of the nine FRED series
// the forecast health check watches carried fetchedAt 2026-08-12T09:00:46Z (the
// daily cron), while M2SL alone carried 2026-08-11T23:20:48Z — /api/health read
// forecastFredM2sl STALE_SEED at seedAgeMin 1527 against maxStaleMin 1500.
// Nothing is wrong with M2SL; all 24 series fetch in under a second directly. It
// bounced once through the container's proxy, and a rejected series is a warn
// plus an absent key, so the next write was the next day's cron. registry.ts
// runs this at '0 9 * * *' = 1440 min apart against a 1500 min budget, which is
// 60 minutes of headroom — one miss is guaranteed to breach it.
//
// Two bounds, because the pass runs inside runSeed's 240s fetch phase and the
// concurrent batch above can already use ~164s of it:
//   - budget: the pass stops when its own wall clock is spent, so it can never
//     be the thing that breaches the deadline (the failure mode of #5037).
//   - max failures: above this the proxy is down rather than one series flaking,
//     and replaying the whole list serially at ~82s each buys nothing.
export const FRED_RETRY_BUDGET_MS = 50_000;
export const FRED_RETRY_MAX_FAILURES = 6;
...
/**
 * Read the GSCPI key already in Redis. A fallback now, not the source: this
 * seeder writes the key itself and only reads it back when the NY Fed fetch
 * fails, so a bad day upstream serves the last good index instead of nothing.
...
/**
 * Fetch GSCPI from the NY Fed and write it under the FRED-compatible key.
 *
 * Written every run, not only when the key is missing: api/health.js gates
 * seed-meta:economic:gscpi at 48h, so writing on a miss alone would let the
 * health row go stale for a day before the 72h data TTL expired and forced a
 * refetch. Returns whatever the stress index should use.
 *
 * @returns {Promise<{ observations: { date: string; value: number }[] } | null>}
 */
async function seedGscpi() {
  const observations = await fetchGscpiFromNyFed(_curlProxyAuth);
  if (!observations) {
    const cached = await fetchGscpiFromRedis();
    if (cached) console.log('  [GSCPI] NY Fed unavailable — serving the key already in Redis');
    return cached;
  }
  const latest = observations[observations.length - 1];
  await writeExtraKeyWithMeta(
    GSCPI_REDIS_KEY,
    buildGscpiPayload(observations),
    GSCPI_TTL,
    observations.length,
    GSCPI_META_KEY,
  );
...
// One serial replay of whatever the concurrent batch dropped. Mutates `results`
// with anything that comes back; a series that fails twice stays absent, because
// an omitted key is honest and a fabricated one is not.
//
// Serial on purpose: the batch already put 24 series through the proxy at once,
// and a burst is the likeliest reason one of them bounced. Adding a second burst
// on top would be the same mistake twice.
async function replayFailedFredSeries({ failed, results, apiKey, fredFetchFn, retryBudgetMs }) {
  if (failed.length === 0) return;

  if (failed.length > FRED_RETRY_MAX_FAILURES) {
    console.warn(
      `  [WARN] FRED: ${failed.length} series failed (> ${FRED_RETRY_MAX_FAILURES}) — not replaying. `
      + 'That many at once is the proxy or the API being down, not one series flaking; '
      + `their keys stay at their previous fetchedAt: ${failed.join(', ')}`,
    );
    return;
  }
  ...
  const unreplayed = [];
  for (const seriesId of failed) {
    if (Date.now() >= deadline) {
      unreplayed.push(seriesId);
      continue;
    }
    try {
      results[seriesId] = await fetchOneFredSeries(seriesId, apiKey, fredFetchFn);
      console.log(`  FRED ${seriesId}: recovered on replay`);
    } catch (e) {
      console.warn(`  FRED ${seriesId}: replay also failed — ${e?.message || e}`);
    }
  }

  // Never silent: a series the budget ran out on reads exactly like one that was
  // never retried, and that is the confusion this whole pass exists to end.
  if (unreplayed.length > 0) {
    console.warn(
      `  [WARN] FRED: replay budget (${retryBudgetMs}ms) spent before reaching `
      + `${unreplayed.length} series: ${unreplayed.join(', ')}`,
    );
  }
}
...
export async function fetchFredSeries({
  ...
  retryBudgetMs = FRED_RETRY_BUDGET_MS,
} = {}) {
  ...
  settled.forEach((s, i) => {
    ...
    if (s.status === 'fulfilled') results[seriesId] = s.value;
      ...
      console.warn(`  FRED ${seriesId}: ${s.reason?.message || s.reason}`);
    }
  });

  await replayFailedFredSeries({ failed, results, apiKey, fredFetchFn, retryBudgetMs });
...
async function fetchAll() {
  ...
  // GSCPI comes from the NY Fed CSV, not from FRED, so it is seeded outside the
  // frHasData branch: its own health row must be written even on a run where
  // every FRED series failed.
  const gscpi = await seedGscpi();

  if (frHasData) {
    if (gscpi) {
      fr['GSCPI'] = gscpi;
    } else {
      console.warn('  [StressIndex] GSCPI unavailable — excluding from the stress index');
    }
...
if (process.argv[1]?.endsWith('seed-economy.mjs')) {
  runSeed('economic', 'energy-prices', KEYS.energyPrices, fetchAll, {
    ...
    declareRecords,
    schemaVersion: 1,
    maxStaleMin: 1560,  // health.js has judged this seeder's keys at 1560 (26h) since the cron moved to `0 9 * * *`; 150 was left over from the */15 relay
  }).catch((err) => {
