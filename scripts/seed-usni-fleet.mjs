...
// the container starts direct-only — a blocked run fails loudly in the seed
// log (HTTP 403 and recordCount 0), and the proxy leg gets added only if
// that is measured, not preemptively.

import { createRequire } from 'node:module';
import { loadEnvFile, CHROME_UA, runSeed, atomicPublish } from './_seed-utils.mjs';
...
async function fetchUsniFleet() {
  const resp = await fetch(USNI_URL, {
