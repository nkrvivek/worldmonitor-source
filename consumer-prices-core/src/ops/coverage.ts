 ...
 * scrape→aggregate→publish window. Health reads this marker to tell a market
 * that has never published apart from one that published and then broke, so the
 * marker is a ONE-WAY, DURABLE (no TTL) claim: "market <code> has published real
 * coverage under schema v<N> at least once." (A bounded `ROLLOUT_PENDING`
 * softening also bridged that gap on first deploy; its windows closed
 * 2026-08-03 and that machinery has been pruned. This half is the durable one.)
 *
