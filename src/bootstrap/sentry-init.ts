...
const THIRD_PARTY_FETCH_HOST_ALLOWLIST = new Set([
  ...
  'protomaps.github.io',
  // Supabase auth (GoTrue). supabase-js fetches it for session/token refresh
  // and retries transient failures itself; a
  // `Failed to fetch (twyzyvgqzzuvlvsftcjj.supabase.co)` that leaks to
  // onunhandledrejection is an SDK-internal network blip, not our code. NOT
  // our own API origin, which stays off the list so genuine API regressions
  // still surface (WORLDMONITOR-SA/SB, kept from the Clerk-era entry this
  // replaced).
  'twyzyvgqzzuvlvsftcjj.supabase.co',
  ...
  // user and unactionable, same disposition as the Supabase-SDK-internal fetch
      ...
      if (dynamicImportAssetUrlMatch) {
        ...
        if (assetUrl.startsWith('/')) {
          ...
          try {
            ...
            isOwnedDynamicImportAssetUrl = host === 'worldmonitor.sibt.ai'
