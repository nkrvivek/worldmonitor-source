 ...
 *      closes and we re-enter via subscribeAuth().
 ...
 *      `<functions host>/oauth/authorize-pro?...` — this page never
 *      controls the host; the server picks it from its own allowlist).
 */

import { initAuth, getAuthToken, getCurrentUser, openSignIn, subscribeAuth } from '@/services/auth';
import {
  classifyGrantDenial,
  describeGrantDelay,
...
/**
 * `subscribeAuth` can re-enter `reactToAuth` -> `loadContext` at any moment.
...
async function loadContextAttempt(
  ...
  try {
    ...
    // Staleness FIRST. subscribeAuth can re-enter loadContext at any moment,
...
async function onAuthorizeClick(nonce: string): Promise<void> {
  ...
  // Defense-in-depth: this page MUST navigate only to the host that runs the
  // functions. Upstream splits that across worldmonitor.app and
  // api.worldmonitor.app; this fork runs both on one host, so its own origin is
  // the right target. The server builds the URL from the same allowlist, but
  // check anyway so a future server bug (or an XSS that swaps the response)
  // cannot bounce to an attacker-controlled host.
  let target: URL;
  try {
  ...
  if (target.origin !== 'https://api.worldmonitor.app' && target.origin !== window.location.origin) {
...
async function bootstrap(): Promise<void> {
  ...
  const reactToAuth = async (): Promise<void> => {
    if (!getCurrentUser()) {
      // Open sign-in modal; on success subscribeAuth() fires reactToAuth again.
      openSignIn();
      return;
    }
  ...
  subscribeAuth(() => { void reactToAuth(); });
  await reactToAuth();
}
