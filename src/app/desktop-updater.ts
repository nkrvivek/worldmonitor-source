...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      // Our own host. We serve api/version.js and api/download.js, so a desktop
      // build of this fork asks this deployment what the latest release is
      // rather than upstream's api.worldmonitor.app. Same answer today — both
      // handlers read the same GitHub repo (see api/_github-release.js) — but a
      // host we run is a host we can fix.
      const res = await fetch(`${SITE_ORIGIN}/api/version`, {
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `${SITE_ORIGIN}/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {
