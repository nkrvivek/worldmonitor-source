...
import { SITE_ORIGIN, PRO_PRICING_URL, BLOG_URL, DOCS_URL } from '@/config/site';
...
import { showCheckoutSuccess, clearCheckoutAttempt, loadCheckoutAttempt } from '@/services/checkout';
import {
...
export class PanelLayoutManager implements AppModule {
  ...
  constructor(ctx: AppContext, callbacks: PanelLayoutManagerCallbacks) {
    ...
    // Payments: entitlement subscription + billing watch for ALL users.
    ...
    //
    // One return path seeds the transition detector as post-checkout:
    // handleCheckoutReturn() reads the subscription_id/status URL params the
    // hosted page sends the buyer back with, and cleans them off the URL.
    const returnResult = handleCheckoutReturn();
    const returnedFromCheckout = returnResult.kind === 'success';
    ...
    if (returnedFromCheckout) {
      ...
      trackCheckoutSuccess('url-return');
      ...
      // branch structurally cannot reach here). A return that carries no
      // attempt record → the marker omits productId and the boot hook falls
      // back to the live entitlement snapshot for plan identity (never a
      // write-time frozen fallback — see decideActivationMount).
      const activationProductId = loadCheckoutAttempt()?.productId ?? null;
      markProActivationPending(activationProductId);
      // handleCheckoutReturn cleared the URL params; clear the attempt
      // record here so a stale one cannot outlive the purchase.
    ...
    // #4934 round-5: /pro checkout-start events that died with the
    // checkout redirect are mirrored in sessionStorage; the buyer lands back here
    ...
    // are followed by a navigation (the checkout redirect) that outlives any
    ...
    // Defer Convex subscriptions until a real signed-in identity exists.
    //
    ...
    // with `setAuth(getAuthToken)` returning null, which the Convex SDK
    ...
    //
    // When we just returned from a full-page redirect checkout, seed
  ...
  destroy(): void {
    ...
    const destroyOnce = (target: { destroy?: () => void } | null | undefined): void => {
      ...
      // single panel throwing must not abort the remaining panel and
      // subscription cleanup below (nor the rest of App.destroy(), which iterates
  ...
  async renderLayout(): Promise<void> {
    ...
    const referenceLinksHtml = DASHBOARD_REFERENCE_LINKS.map(({ label, path }) => {
      const href = this.ctx.isDesktopApp ? `${SITE_ORIGIN}${path}` : path;
    ...
    setTrustedHtml(this.ctx.container, trustedHtml(`
      ...
      <div class="header">
        ...
        return `
            <a href="${vHref('full', `${SITE_ORIGIN}/dashboard`)}"
            ...
            <a href="${vHref('tech', `${SITE_ORIGIN}/tech`)}"
            ...
            <span class="variant-divider"></span>
            <a href="${vHref('finance', `${SITE_ORIGIN}/finance`)}"
            ...
            <span class="variant-divider"></span>
            <a href="${vHref('commodity', `${SITE_ORIGIN}/commodity`)}"
            ...
            <span class="variant-divider"></span>
            <a href="${vHref('energy', `${SITE_ORIGIN}/energy`)}"
            ...
            <span class="variant-divider"></span>
            <a href="${vHref('happy', `${SITE_ORIGIN}/happy`)}"
      ...
      <nav class="mobile-menu" id="mobileMenu">
        ...
        <div class="mobile-menu-footer-links">
          ${referenceLinksHtml}
          <a href="${this.ctx.isDesktopApp ? PRO_PRICING_URL : '/pro#pricing'}" target="_blank" rel="noopener">Pricing</a>
          <a href="${this.ctx.isDesktopApp ? BLOG_URL : '/blog/'}" target="_blank" rel="noopener">Blog</a>
          <a href="${this.ctx.isDesktopApp ? DOCS_URL : '/docs'}" target="_blank" rel="noopener">Docs</a>
        </div>
  ...
  private async createPanels(): Promise<void> {
    ...
    //
    // hasPremiumAccess() folds in isEntitled() (Convex entitlement) per
