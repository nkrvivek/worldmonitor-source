 ...
 * Analytics facade.
 *
 * Dashboard analytics start after first paint; calls made before the tracker
 * is installed are kept in a small bounded queue and replayed once it is.
 *
 * The tracker itself is ours (src/services/analytics-tracker.ts) and posts to
 * this site's own collector. It keeps the `window.umami` shape the queue,
 * retry and identity machinery below were written against, so the swap away
 * from the hosted Umami script touched only where events go, not what the rest
 * of this file does with them.
 *
 * `window.umami` and the `data-umami-event` attributes keep that name on
 * purpose: they are the wire between this file, the tracker, the /pro click
 * delegation in pro-test/index.html and the inline script in the brief page.
 * Every name that is only ours reads as analytics, because no part of this
 * code talks to Umami any more.
 */
...
import { getUserCreatedAtMs } from './auth';
...
const CRITICAL_TRACK_EVENTS = new Set<AnalyticsEvent>([
  'checkout-start',
  'checkout-success',
  'checkout-failed',
]);
...
type QueuedAnalyticsCall =
  | { kind: 'track'; event: AnalyticsEvent; data?: Record<string, unknown>; retryAttempt?: number }
  | {
      kind: 'identify';
...
type IdentifyCall = Extract<QueuedAnalyticsCall, { kind: 'identify' }>;

const pendingAnalyticsCalls: QueuedAnalyticsCall[] = [];
let trackerLoadScheduled = false;
let trackerLoadStarted = false;
...
configureCollectorTransport({
  endpoint: ANALYTICS_COLLECTOR_PATH,
  healthEndpoint: ANALYTICS_HEALTH_PATH,
  isCriticalEvent: (name) => CRITICAL_TRACK_EVENTS.has(name as AnalyticsEvent),
...
function queueAnalyticsCall(call: QueuedAnalyticsCall): void {
  ...
  if (call.kind === 'identify') {
    for (let index = pendingAnalyticsCalls.length - 1; index >= 0; index -= 1) {
      if (pendingAnalyticsCalls[index]?.kind === 'identify') {
        pendingAnalyticsCalls.splice(index, 1);
      }
    }
  }
  if (pendingAnalyticsCalls.length >= ANALYTICS_QUEUE_LIMIT) {
...
function createIdentifyCall(data: Record<string, unknown>): QueuedAnalyticsCall {
...
function scheduleIdentityRetry(call: IdentifyCall): void {
  ...
  identifyRetryTimer = setTimeout(() => {
    ...
    if (!sendAnalyticsCall(retryCall)) {
      queueAnalyticsCall(retryCall);
    }
  }, delay);
}
...
function finishIdentityDelivery(call: IdentifyCall, generation: number, error?: unknown): void {
  ...
  if (nextCall) {
    if (!sendAnalyticsCall(nextCall)) {
      queueAnalyticsCall(nextCall);
    }
    return;
  }
...
function scheduleTrackRetry(call: Extract<QueuedAnalyticsCall, { kind: 'track' }>, error: unknown): void {
  ...
  setTimeout(() => {
    ...
    if (!sendAnalyticsCall(retryCall)) queueAnalyticsCall(retryCall);
  }, delay);
}
 ...
 */
function clearUnobservableCriticalMarker(call: Extract<QueuedAnalyticsCall, { kind: 'track' }>): void {
...
function sendAnalyticsCall(call: QueuedAnalyticsCall): boolean {
...
function flushPendingAnalyticsCalls(): void {
  if (pendingAnalyticsCalls.length === 0) return;
  if (typeof window === 'undefined' || !window.umami) return;
  installCollectorFetchGate();
  const calls = pendingAnalyticsCalls.splice(0, pendingAnalyticsCalls.length);
  for (const call of calls) sendAnalyticsCall(call);
}

/**
 * Install the tracker and drain whatever the queue collected while waiting.
 *
 * There is no load event to wait for and no failure to retry: the tracker ships
 * in this bundle, so by the time this runs it is already here. The only way to
 * come back empty-handed is a host with no window or no fetch, and neither of
 * those recovers on a second attempt.
 */
function startTracker(): void {
  if (trackerLoadStarted) return;
  installCollectorFetchGate();
  if (!installLocalTracker()) return;
  trackerLoadStarted = true;
  flushPendingAnalyticsCalls();
}

/** Type-safe tracker wrapper. Safe to call before the tracker installs. */
export function track(event: AnalyticsEvent, data?: Record<string, unknown>): void {
  ...
  if (!sendAnalyticsCall({ kind: 'track', event, data: enrichedData })) {
    queueAnalyticsCall({ kind: 'track', event, data: enrichedData });
  }
}
...
export function initAnalytics(): void {
  if (trackerLoadScheduled || typeof window === 'undefined' || typeof document === 'undefined') return;
  trackerLoadScheduled = true;
  scheduleAfterFirstPaint(startTracker, 3000);
}

// ---------------------------------------------------------------------------
// User identity — call after auth state resolves so the collector can segment
// events by user/plan. Safe to call before the tracker installs.
// ---------------------------------------------------------------------------
...
export function identifyUser(
  ...
  if (!sendAnalyticsCall(call)) {
    queueAnalyticsCall(call);
  }
}
...
/**
 * Call once after initAuthState() to keep the analytics identity in sync with
  ...
  _unsubAuth = subscribeAuthState((state) => {
    ...
    if (prevUserId !== nextUserId) {
      ...
      if (
        ...
        isLikelyFreshSignup(prevUserId, nextUserId, getUserCreatedAtMs(), Date.now())
      ) {
...
export function resetAnalyticsForTesting(): void {
  ...
  trackerLoadScheduled = false;
  trackerLoadStarted = false;
 ...
 * cardinality into the collector. Unknown ids collapse to 'unknown'; the checkout
 ...
 * page the moment Pro lands — often before the deferred analytics queue flushes
 * (#4934 round-2 F2). A sessionStorage marker written at track time and
 * cleared only on actual delivery (see sendAnalyticsCall) lets the next boot
 ...
 * `source` has one value: redirect mode is the only way back from the
 * hosted checkout page (see panel-layout.ts checkout-return wiring). It
 * stays a named parameter so the event keeps its shape in the collector.
 */
export function trackCheckoutSuccess(source: 'url-return'): void {
  try {
 ...
 * marker: it stays until sendAnalyticsCall confirms delivery, so repeated
 ...
 * junk can't loop forever) and removed in sendAnalyticsCall once a replayed
 ...
 * joins key on the userId the collector already receives via identifyUser(). Mirrors
 ...
 * only planKey / step / the aggregate exit counts ever reach the collector.
 */
...
// ---------------------------------------------------------------------------
// Generic (kept as no-ops — too noisy / not useful in the collector)
// ---------------------------------------------------------------------------
...
/**
 * Fire-and-forget: `track` short-circuits before the tracker installs.
