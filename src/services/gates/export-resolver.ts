...
export interface ExportGateInputs {
  /** The served product catalog exposes a purchasable tier that sells export. */
  gateActive: boolean;
 ...
 * takeaway can only bite once the tier that sells export is actually
 * purchasable, so the two flip together regardless of PR/deploy timing.
 */
 ...
 * the same affirmative-denial rule as `resolveExportGate`: before the paid
 * catalog is live, and while identity or entitlement data is unknown, preserve
 * the pre-gate menu instead of turning a transient read failure into a paid-user
 * lockout.
 *
 ...
 * because it distinguishes a free row from a paid one.
 */
...
// The tier that sells data export. It was Pro Business until the two Pro
// plans merged on 2026-08-05; Pro carries `dataExport` now, so Pro is what the
// probe looks for. Normalized identities are matched by prefix, so this also
// matches `pro_monthly`, `pro_annual` and any older `pro_business*` payload
// still being served by a mirror that has not redeployed.
const EXPORT_TIER_MARKER = 'pro';

/**
 * Identity fields only. Marketing copy (`features`, `description`) is
 * deliberately NOT scanned: a Free card that upsells "upgrade to Pro" must not
 * activate the gate.
 */
...
/**
 * Does a served `/api/product-catalog` payload expose the tier that sells
 * export?
 *
 * The endpoint's tier objects carry no single stable group key across the
 * mirrors, so every plausible identity field is normalized and matched — `pro`,
 * `Pro`, `pro_monthly` and the older `pro_business` names all resolve to the
 * same marker.
 */
export function catalogExposesExportTier(payload: unknown): boolean {
  ...
  return tiers.some((tier) => {
    ...
    return TIER_IDENTITY_FIELDS.some((field) =>
      normalizeIdentity(record[field]).startsWith(EXPORT_TIER_MARKER),
    );
  });
}
...
export function primeExportGateActivation(fetchImpl?: CatalogFetch): Promise<boolean> {
  ...
  const probe = (async (): Promise<boolean | null> => {
    try {
      ...
      return catalogExposesExportTier(await res.json());
    } catch {
      return null;
    }
  })().then((verdict) => {
