...
import { safeBillingPortalUrl } from './provider-hosted-urls';
...
export async function initSubscriptionWatch(
  ...
  const isExpectedAccount = (): boolean => (
    isCurrent() && (_userId === undefined || getCurrentUser()?.id === _userId)
  );
  ...
  try {
    ...
    enqueueSentryCall((s) => s.captureException(
      initErr,
      { tags: { component: 'billing', action: 'initSubscriptionWatch' } },
    ));
  }
}
...
/**
 * What we tell a user when no portal page exists to send them to — either the
 * provider has no customer row for them, or no session came back. One string
 * for both, because the buyer's next step is the same either way and they do
 ...
 * session URL. Returns the URL that was opened (useful for agent/programmatic
 * callers).
 *
 * There is no fallback URL any more. Under Dodo, every failure branch here
 * navigated to `customer.dodopayments.com`, a generic portal that at least
 * recognised some buyers. Stripe has no such page — a portal session is the
 * only way in, and `billing.stripe.com` on its own is a dead end. So a failure
 * now closes the reserved tab and returns `unavailable`, and callers say so.
 * Opening a blank tab onto nothing is worse than a sentence of explanation.
 */
/**
...
export async function openBillingPortal(
  ...
  // sub, or a sub state where the provider already purged the customer row)
  // but no customer record exists to open a portal session for. Close the
  ...
  // pre-reserved tab. WORLDMONITOR-R5.
  ...
  try {
    ...
    assertAccountStillCurrent(userId, 'opening the billing portal');
    // Guard the host for the same reason redirect-mode checkout guards its
    // own: this string comes off the wire and goes straight into a tab.
    ...
    // have no customer row, so it shouldn't drown real config/SDK bugs
    // in error-level alerts. Anything else (PROVIDER_API_KEY_MISSING, a
    // provider SDK throw, network failure, unknown shape) stays at the
    // default `error` level. WORLDMONITOR-R5.
