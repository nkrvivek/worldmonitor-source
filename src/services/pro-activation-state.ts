 ...
 *
 * Plan identity is an allowlist against the Pro-family price lookup keys. The
 * literals are mirrored from config/products.generated.ts (DODO_PRODUCTS
 * PRO_MONTHLY / PRO_ANNUAL) rather than imported — keeping the leaf
 * import-free — and tests/pro-activation-state.test.mts asserts them against
 * the generated catalog so any drift goes red.
 */
 ...
 * `PRO_ANNUAL` (kept in sync by the drift-guard test). Anything else —
 * api_starter, api_starter_annual, api_business, enterprise, or an unknown id —
 * is non-Pro and must never trigger onboarding.
 */
export const PRO_PRODUCT_IDS: readonly string[] = [
  'wm_pro_monthly', // PRO_MONTHLY
  'wm_pro_annual', // PRO_ANNUAL
];
