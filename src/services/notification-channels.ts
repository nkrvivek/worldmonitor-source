...
function assertExpectedAccount(expectedUserId?: string): void {
  if (expectedUserId && clientDeps.getCurrentUser()?.id !== expectedUserId) {
...
async function authFetch(
  ...
  const pinnedUserId = expectedUserId ?? clientDeps.getCurrentUser()?.id;
  const first = await sendOnce(path, init, pinnedUserId);
  if (first.status !== 503) return first;

  // No identity could be pinned — `getCurrentUser()` is a synchronous
  // `clerkInstance?.user` read that never triggers a load, while
  // `getAuthToken()` above can itself initialise Clerk and succeed. So a call
