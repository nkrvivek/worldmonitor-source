 ...
 *      (gateway.ts: `if (isTierGated) resolveSession(...)`); for
...
export async function premiumFetch(
  ...
  if (isPremiumRpcTarget(input, forcePremium)) {
    try {
      ...
      // clearAuthTokenCache() bumps the token generation (so a rotating
      // user's stale JWT is never painted), which makes any in-flight
      // getAuthToken() abandon to null. A signed-in user's premium fetch
