...
/**
 * Where /api/story and /api/og-story live for the page the reader is on.
 *
 * Deliberately not getCanonicalApiOrigin(): that one answers with upstream's
 * API host on every host it does not recognise, which is right for
 * /api/download (this fork does not serve the installers) and wrong for a
 * share card (this fork does serve those, at worker/routes/social-preview.ts).
 * Pointing a share link at api.worldmonitor.app from a fork hands the reader
 * a card for a site the sharer is not on.
 *
 * On an upstream host, or with no window to read, it falls back to
 * getCanonicalApiOrigin() so upstream's behaviour is untouched.
 */
export function getSocialPreviewOrigin(): string {
  if (typeof window === 'undefined' || isDesktopRuntime()) {
    return getCanonicalApiOrigin();
  }

  const hostname = window.location?.hostname ?? '';
  if (!hostname || isWorldMonitorWebHost(hostname)) {
    return getCanonicalApiOrigin();
  }

  return window.location?.origin ?? getCanonicalApiOrigin();
}
...
export function installWebApiRedirect(): void {
   ...
   *   4. Signed-in session → Authorization: Bearer <token>
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    // Signed-in session: inject Bearer token (fallback for users without a tester key)
