...
import { OFFERED_NOTIFICATION_CHANNELS } from '@/config/notification-channels-offered';
...
export function renderNotificationsSettings(host: NotificationsSettingsHost): NotificationsSettingsResult {
  ...
  if (isPro) {
    ...
    html += `<div class="wm-pref-group-content wm-notif-tab-content">`;
    html += `<div class="ai-flow-toggle-desc">Get real-time intelligence alerts delivered to Telegram, Slack and Email with configurable sensitivity, quiet hours, and digest scheduling.</div>`;
  ...
  return {
    ...
    attach(container: HTMLElement): () => void {
      ...
      function renderNotifContent(
        ...
        // Fork-owned list — see src/config/notification-channels-offered.ts.
        const channelTypes = OFFERED_NOTIFICATION_CHANNELS;
