...
import { getSocialPreviewOrigin } from '@/services/runtime';
...
export function updateMetaTagsForStory(meta: StoryMeta): void {
  ...
  // Resolved per call, not at module scope: this module is imported before
  // the first render, and the origin depends on the host the page is served
  // from.
  const apiOrigin = getSocialPreviewOrigin();
  const storyUrl = `${apiOrigin}/api/story?c=${countryCode}&t=${type}`;
  let imageUrl = `${apiOrigin}/api/og-story?c=${countryCode}&t=${type}`;
