 ...
 *
 *   PENDING_CHECKOUT_KEY      — "should we resume a deferred checkout on
 *                                next mount?" Cleared once resumed, to
 *                                avoid silent auto-retries.
 *
 *   LAST_CHECKOUT_ATTEMPT_KEY — "what product should the failure-retry
 *                                banner re-open?" MUST survive the trip
 *                                out to the hosted checkout page and
 *                                back to ?status=failed, so the retry
 *                                button has context.
 *
 * Living in its own file so unit tests can exercise the helpers without
 * pulling in the browser-only checkout service graph.
 */
