 ...
 *
 * Served from our own static assets, beside the base file above. This used to
 * fetch https://maps.worldmonitor.app/country-boundary-overrides.geojson — the
 * upstream project's R2 bucket, which we do not control. That copy holds one
 * feature (Pakistan); the copy committed here holds two (Pakistan and India,
 * added upstream in #1796 but never uploaded to their bucket). So the fetch
 * crossed an origin we do not own to get strictly less data than the file we
 * already ship.
 */
