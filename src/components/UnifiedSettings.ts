...
import { PRO_URL } from '@/config/site';
...
import { BILLING_PORTAL_UNAVAILABLE_MESSAGE, getSubscription, onSubscriptionChange, openBillingPortal, prereserveBillingPortalTab } from '@/services/billing';
...
export class UnifiedSettings {
  ...
  constructor(config: UnifiedSettingsConfig) {
    ...
    this.overlay.addEventListener('click', (e) => {
      ...
      if (target.closest('.upgrade-to-business-btn')) {
        // Self-serve Starter→Business upgrade (#4634): open the billing
        // portal, which surfaces the prorated plan change. Same hosted open
        // path as Manage Billing — the provider owns payment, 3DS and
        // proration; a failed charge leaves the customer on Starter.
        const reservedWin = prereserveBillingPortalTab();
        void openBillingPortal(reservedWin).then((result) => {
          if (result.outcome === 'no-customer' || result.outcome === 'unavailable') {
            showToast(BILLING_PORTAL_UNAVAILABLE_MESSAGE);
          }
        });
        return;
      }
      ...
      if (target.closest('.manage-billing-btn')) {
        ...
        const reservedWin = prereserveBillingPortalTab();
        void openBillingPortal(reservedWin).then((result) => {
          // NO_CUSTOMER: user is entitled but has no customer row (comp
          // grant, restore race, or post-purge cancellation). UNAVAILABLE:
          // no portal session came back at all. Either way there is no page
          // to land on, so say so instead of leaving a blank tab open.
          if (result.outcome === 'no-customer' || result.outcome === 'unavailable') {
            showToast(BILLING_PORTAL_UNAVAILABLE_MESSAGE);
          }
        });
  ...
  private renderUpgradeSection(): string {
    // Non-subscription premium (API key / tester key / pro role without a
    ...
    if (isEntitled()) {
      ...
      // the Manage Billing CTA that would 404 against the provider.
  ...
  private handlePlanLimitNoticeCta(noticeId: string): void {
    ...
    if (notice.ctaKind === 'checkout') {
      ...
      // support path for a subscription managed outside the billing provider.
      if (isEntitled()) {
        const reservedWin = prereserveBillingPortalTab();
        void openBillingPortal(reservedWin).then((result) => {
          if (result.outcome === 'no-customer' || result.outcome === 'unavailable') {
    ...
    if (notice.ctaKind === 'contact_support') {
      window.location.href = `mailto:hello@sibt.ai?subject=${encodeURIComponent('WorldMonitor API plan limit upgrade')}`;
    }
  }
    ...
    if (gateBtn) {
      gateBtn.addEventListener('click', () => {
        if (!getAuthState().user) {
          this.close();
          import('@/services/auth').then(m => m.openSignIn()).catch(() => {});
        } else {
          this.close();
  ...
  private renderMcpClientsList(): void {
    ...
    if (active.length === 0 && revoked.length === 0) {
      // Same origin as the page: vercel.json rewrites /mcp to /api/mcp on
      // whichever host serves the dashboard. Hardcoding a host hands the
      // user a URL that points at someone else's deployment.
