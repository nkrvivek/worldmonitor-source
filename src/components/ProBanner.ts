...
import { getCurrentUser, isAuthEnabled } from '@/services/auth';
...
export function showProBanner(container: HTMLElement): void {
  ...
  const decision = decideProBannerMount({
    ...
    premiumHint: readPremiumHint(),
    authPending: auth.isPending,
    clerkConfigured: isAuthEnabled(),
    signedIn: getCurrentUser() !== null || auth.user !== null,
...
function syncProBanner(): void {
  ...
  const auth = getAuthState();
  const signedIn = getCurrentUser() !== null || auth.user !== null;
