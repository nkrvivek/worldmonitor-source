...
// This module is imported both by the Vite frontend (where `import.meta.env`
// is typed via vite/client) and by server/worldmonitor/supply-chain/v1's
// route-explorer handler (typechecked under tsconfig.worker.json, which uses
// @cloudflare/workers-types and has no vite/client). Cast at this one
// boundary instead of widening the worker tsconfig's global types: `env` is
// simply absent from `import.meta` in a Worker, so `isDevBuild` is `undefined`
// there and the (dev-only, logging-only) branches below stay off, matching
// existing runtime behavior — Workers builds never define `import.meta.env`.
...
export function resolveTradeRouteSegments(): TradeRouteSegment[] {
  ...
  for (const route of TRADE_ROUTES) {
    ...
    if (!fromCoord || !toCoord) {
      if (isDevBuild) console.error(`[trade-routes] Missing port: ${!fromCoord ? route.from : route.to}`);
      continue;
    }
    ...
    for (const wpId of route.waypoints) {
      ...
      if (!coord) {
        if (isDevBuild) console.error(`[trade-routes] Missing waterway: ${wpId}`);
        valid = false;
        break;
      }
