...
test('isAllowedOrigin accepts the worldmonitor.sibt.ai host (mirrors api/_cors.js)', () => {
  // The app is served from a Cloudflare Worker at worldmonitor.sibt.ai. The
  // Worker MUST mirror api/_cors.js exactly — if it stays narrower, preflights
  // from that host echo the canonical worldmonitor.app fallback and the browser
  // blocks them before the request reaches the origin.
  assert.equal(isAllowedOrigin('https://worldmonitor.sibt.ai'), true);
  assert.equal(isAllowedOrigin('https://api.worldmonitor.sibt.ai'), true);
  // Tight allowlist: never a bare *.sibt.ai, and never a suffix lookalike.
  assert.equal(isAllowedOrigin('https://sibt.ai'), false);
  assert.equal(isAllowedOrigin('https://evilworldmonitor.sibt.ai'), false);
  assert.equal(isAllowedOrigin('https://worldmonitor.sibt.ai.evil.com'), false);
});

test('isAllowedOrigin rejects every Vercel origin', () => {
  // This fork does not deploy to Vercel. An allowlist entry for a host nobody
  // here controls is a standing hole, so none may pass.
