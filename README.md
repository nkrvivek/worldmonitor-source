...
- **MCP server** — `https://worldmonitor.sibt.ai/mcp` (Streamable HTTP). Public `tools/list`; `tools/call` authenticates with a `X-WorldMonitor-Key` header or OAuth.
- **REST API** — base `https://worldmonitor.sibt.ai`, described by the [OpenAPI spec](https://worldmonitor.sibt.ai/openapi.yaml).
