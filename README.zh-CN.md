...
- **MCP Server** — `https://worldmonitor.sibt.ai/mcp`（Streamable HTTP）。公开提供 `tools/list`；`tools/call` 通过 `X-WorldMonitor-Key` 请求头或 OAuth 进行身份验证。
- **REST API** — 基础地址为 `https://worldmonitor.sibt.ai`，详见 [OpenAPI 规范](https://worldmonitor.sibt.ai/openapi.yaml)。
