...
Both map files ship in the Worker's static assets and load from the same origin as the app:

| URL | Size | Purpose |
|-----|------|---------|
| `/data/countries.geojson` | ~210 KB | Base country polygons (ISO 3166-1 Alpha-2 coded) |
| `/data/country-boundary-overrides.geojson` | ~45 KB | Higher-resolution Natural Earth boundary overrides |

The upstream project serves these from a Cloudflare R2 bucket (`worldmonitor-maps`, fronted by `maps.worldmonitor.app`). This instance has no such bucket, and at this size it does not need one: 255 KB costs the bundle little, and a same-origin fetch cannot fail on someone else's DNS.

Vector tiles are the case that forces a bucket — `*.pmtiles` runs to tens of gigabytes and never belongs in a bundle. Point `VITE_PMTILES_URL` at one when you stand it up. Unset, the basemap falls back to the hosted tile providers in `src/config/basemap.ts`.

### If you do stand up a bucket

Serve it through a CF-proxied custom domain. Never put a raw `pub-*.r2.dev` URL in code: it gets no CF caching and sends no CORS headers.
...
1. `countries.geojson` — base polygons with ISO codes and names, served from `/data/` (the Worker's static assets)
2. `country-boundary-overrides.geojson` — optional higher-resolution polygons from [Natural Earth](https://www.naturalearthdata.com/), served from `/data/` alongside the base file. Features matched by `ISO3166-1-Alpha-2` (or `ISO_A2`) code; matching features replace the base geometry
...
PMTiles need a bucket of their own — see [Static map files](#static-map-files) above. `VITE_PMTILES_URL` is unset on this instance, so the basemap uses the hosted providers.

## Common Mistakes

| Mistake | Fix |
|---------|-----|
| Using `pub-*.r2.dev` URLs in code | Serve any bucket through a CF-proxied custom domain |
| Fetching a map file from another project's CDN | Ship it in `public/data/` — a host you do not control can serve stale data, or none |
| Fetching overrides without a timeout | Always use `AbortSignal.timeout` — the file is optional and the map must paint without it |
