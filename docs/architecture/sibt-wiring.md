...
Status: the two blockers below are gone. Twelve signals answer with real data
and trade-refresh pulls all twelve as of 2026-08-08. Step 5 closed when the
three economic seeds — macro signals, economic stress, energy prices — joined
the pull; `shipping-stress` stays out with no writer behind its key (step 4,
...
4. Decide how transit summaries get written now that the Railway relay is gone.
   ...
   **The refusal cost more than it should have, and that is now fixed
   (2026-08-07, `52f12f822`).** `fetchRelayTransits()` throws when the feed is
   not delivering, which is right. But it sat inside the `Promise.all` in
   `fetchAll()`, so the throw aborted the whole run and nothing was written at
   all. `supply_chain:transit-summaries:v1` went unwritten for 2h40m against a
   ten-minute cron, and health read `STALE_SEED`, which reads as a broken cron.
   The five risk fields went with it — they come from
   `supply_chain:corridorrisk:v1` over plain HTTP and never touch AIS. A dead
   vessel feed cost us the war-risk data it has nothing to do with.

   `fetchRelayTransitsOrNull()` catches the refusal and returns null. The four
   count fields fall to zero and nothing else moves. Zero counts render as
   nothing: the panel gates its vessel span on `todayTotal > 0`, and the raw
   counts key carries `skipWhenEmpty: true`, so an empty run extends its TTL
   instead of erasing the last good counts. The counts gate themselves. No flag
   stands in for them.

   Two measurements killed a larger version of this fix before it shipped, and
   both are worth keeping. Widening `dataAvailable` to cover the relay would
   drive `declareRecords` to zero, and `_seed-utils.mjs:2052` turns a zero count
   into `contractState = 'RETRY'`, which skips the publish entirely — the fix
   would have defeated itself. Flipping it server-side is worse:
   `get-chokepoint-status.ts:392` counts covered chokepoints and raises
   `upstreamUnavailable` on the whole response, which the trade-refresh puller
   reads. So `dataAvailable` means PortWatch and only PortWatch, everywhere. The
   proto comment on `data_available` said otherwise and was wrong; it now says
   what every consumer already did.

   Verified live the same day. Deployed as version
   `7934611f-81a5-4616-a823-97b71d8251f8`; the next `*/10` run took
   `transitSummaries` from `STALE_SEED` at 160 minutes to `OK` with 13 records
   at age 0. All 13 carry `dataAvailable: true`, five carry risk fields, and
   none carries a vessel count — Hormuz publishes `riskLevel: critical` and
   `incidentCount7d: 455` beside `todayTotal: 0`, which is the shape this was
   for. `chokepointTransits` stays `STALE_SEED` with its 15 rows intact: the
   skip preserved them and did not advance the meta, so the raw counts report
   honestly as old rather than quietly as zero.
5. Add the recovered signals to the same pull as they come back. Each one needs
   ...
   Done for the three economic seeds on 2026-08-08 — same cost, three paths,
   three extractors, three fixtures, seven tests. What their shapes forced:
   `get-macro-signals` computes its verdict from BTC and QQQ series, so the
   consumer's row names the flavor or "CASH" masquerades as an equity regime
   call; `get-economic-stress` averages components and a component marked
   `missing` must be barred from the "worst" slot, because an absent reading
   and a measured stress are different claims; `get-energy-prices` returns
   `price` as a string and its only date as `priceAt` epoch milliseconds —
   date the price, not the fetch. Step 5 is closed: every signal with a
   writer behind it is in the pull. `shipping-stress` joins when step 4
   gives its key a writer.
6. ~~Give autopilot a narrower read: one daily risk number, from the same vault
   ...
   **Done 2026-08-07.** `worldmonitor_context.py` in autopilot reads the doc
   through the existing `vault_github` rail and hands `morning_briefing` one
   line. Twenty tests.

   Only two of the nine signals could carry the number. A disrupted share needs
   a count against a watched total, and only `energy-disruptions` ("33 ongoing
   of 52 events") and `chokepoint-status` ("4 disrupted of 13") report one.
   Four of the rest are levels — gas storage and collected duties do not belong
   in a share. The two WTO feeds looked like they qualified and do not: they
   read "26 of 50 returned" and "50 of 50 returned" because 50 is where the
   sample stops, so the denominator measures our request, not the world.

   It takes the max of the two rather than blending them. One blocked strait
   and one refinery outage are not the same event, and any weight that made
   them add would be a number nobody measured.

   It refuses rather than degrading: a missing row, one that reported `empty`
   or `error`, one past the six-hour bound, one whose detail names no
   denominator. The refusal still reaches the briefing carrying its reason,
   because a silent drop reads as "no supply risk today" and a max over the
   fresh half understates without saying so.

   Context, never a trigger. Nothing there may gate or size an order.
7. ~~The dossier reads the vault doc too, for the same reason.~~ **Dropped
   2026-08-07, on measurement.** This step assumed the dossier was a data
   consumer. It is not.

   What is there: `docs/dossier/sibt-dossier-site/public/index.html` in
   autopilot, 984 lines of hand-written HTML behind an assets-only Worker.
   Five commits, every one a human editing prose. Every figure on the page is
   typed into the markup. The single `<script>` block draws SVG tooltips over
   two hardcoded arrays and fetches nothing. There is no build step and no
   cron — the site ships when someone runs the deploy wrapper.

   So a supply-risk figure on that page would be stale from the moment it
   deployed, and would keep asserting itself for however many weeks passed
   before the next hand deploy. That is the failure this whole architecture
   exists to prevent, arriving through the front door: a number with no
   freshness story, on a page that gives readers no way to tell.

   The dossier describes how the engine works. It is not a panel. If a live
   supply-risk figure is wanted in public, it belongs on something with a
   refresh behind it, and that is a new thing to build rather than a wiring
   step to finish.

Step 6 is deliberately not an integration. One puller, one document, many
