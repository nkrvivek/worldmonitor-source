...
- **Server-to-server**: call `https://worldmonitor.sibt.ai/api/bootstrap` directly with `X-WorldMonitor-Key: wm_...`. There is no separate gateway host, token exchange, activation step, or IP allow-list requirement for this endpoint.
...
Returns the latest **GitHub Release** of `koala73/worldmonitor`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed commit of this site.
...
Per-user dashboard preferences (layout, toggles, filters). Supabase bearer required. Backed by Convex.
...
Invalidates a user's entitlement cache after a subscription change (Stripe webhook → Convex → this endpoint). Internal — requires `RELAY_SHARED_SECRET`.
