...
1. **Feature flag**: `RESILIENCE_PILLAR_COMBINE_ENABLED`, read dynamically from `process.env` per call. The published pillar-combined methodology is the runtime default: the production and Railway validation environments set this to `true`, and an unset value also resolves to `true`. Set the flag explicitly to `false` only for the legacy six-domain rollback. Formula-tag checks (`pc` versus `d6`) reject cached scores from the other methodology, so a default or operator change cannot silently reuse cross-formula cache entries.
