...
```
CelesTrak (free) ──2h──▶ Railway relay ──Redis──▶ Worker edge ──CDN 1h──▶ Browser
...
| Worker edge | ~$0 | CDN caches 1h (`s-maxage=3600`), stale-while-revalidate 30min |
...
| `api/satellites.js` | Worker edge handler (Redis read, CDN cache) |
