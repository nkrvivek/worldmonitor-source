...
```bash
curl -s 'https://worldmonitor.sibt.ai/api/conflict/v1/get-humanitarian-summary?country_code=SD' \
...
const res = await fetch(
  'https://worldmonitor.sibt.ai/api/market/v1/list-market-quotes?symbols=AAPL&symbols=TSLA',
...
r = httpx.get(
    "https://worldmonitor.sibt.ai/api/economic/v1/get-fred-series",
...
```bash
curl -s 'https://worldmonitor.sibt.ai/api/bootstrap' \
  -H 'X-WorldMonitor-Key: wm_0123456789abcdef0123456789abcdef01234567'
```
...
```
https://worldmonitor.sibt.ai/api/mcp
```
