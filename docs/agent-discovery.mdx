...
description: "Point any AI agent, codegen tool, or MCP client at worldmonitor.sibt.ai and discover every API, OpenAPI spec, MCP server, and OAuth endpoint from a single root URL."
---
...
No prior knowledge required. Just `GET https://worldmonitor.sibt.ai/`.
...
```bash
curl -sI https://worldmonitor.sibt.ai/ | grep -i '^link:'
```
...
|---|---|---|
| [`/.well-known/api-catalog`](https://worldmonitor.sibt.ai/.well-known/api-catalog) | [RFC 9727](https://datatracker.ietf.org/doc/html/rfc9727) | JSON linkset bundling every other discovery URL — start here if you want one fetch and a map |
| [`/openapi.yaml`](https://worldmonitor.sibt.ai/openapi.yaml) | OpenAPI 3.1 | Single bundled spec covering **every** REST service (Conflict, Resilience, Market, Economic, Maritime, Aviation, Climate, …) — feed it to any code-generator |
| [`/openapi.json`](https://worldmonitor.sibt.ai/openapi.json) | OpenAPI 3.1 | Byte-identical bundle as minified JSON — same spec as `/openapi.yaml`, for tooling and scanners that only parse JSON |
| [`/docs/api-versioning`](https://worldmonitor.sibt.ai/docs/api-versioning) | Policy | REST compatibility, deprecation notice, and sunset-header contract |
| [`/.well-known/mcp/server-card.json`](https://worldmonitor.sibt.ai/.well-known/mcp/server-card.json) | MCP | Transport (`streamableHttp`), endpoint, OAuth resource, scopes, streaming, capability flags |
| [`/.well-known/oauth-authorization-server`](https://worldmonitor.sibt.ai/.well-known/oauth-authorization-server) | [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) | OAuth 2.1 authorization-server metadata (PKCE, DCR, token endpoints) |
| [`/.well-known/oauth-protected-resource`](https://worldmonitor.sibt.ai/.well-known/oauth-protected-resource) | [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) | Resource metadata for `https://worldmonitor.sibt.ai/mcp` |
| [`/.well-known/agent-skills/index.json`](https://worldmonitor.sibt.ai/.well-known/agent-skills/index.json) | (custom) | Index of pre-packaged agent skills (`fetch-country-brief`, `fetch-resilience-score`, …) — each skill is a self-contained recipe |
| [`/llms.txt`](https://worldmonitor.sibt.ai/llms.txt) | [llmstxt.org](https://llmstxt.org) | LLM-friendly markdown briefing — overview, capabilities, links |
| [`/llms-full.txt`](https://worldmonitor.sibt.ai/llms-full.txt) | (extension) | Long-form variant covering all data layers, panels, and data sources |
| [`/api/health`](https://worldmonitor.sibt.ai/api/health) | (custom) | Per-key seed status, freshness, record counts — agents can gate fetches on this |
...
curl -s https://worldmonitor.sibt.ai/.well-known/api-catalog \
  | jq -r '.linkset[] | select(.anchor == "https://worldmonitor.sibt.ai/")."service-desc"[0].href'
# → https://worldmonitor.sibt.ai/openapi.yaml

# 2. Generate clients
curl -s https://worldmonitor.sibt.ai/openapi.yaml -o worldmonitor.openapi.yaml
...
curl -s https://worldmonitor.sibt.ai/.well-known/mcp/server-card.json
# → endpoint (https://worldmonitor.sibt.ai/mcp), transport, OAuth scopes,
#   streaming support, and authorization_servers: ["https://worldmonitor.sibt.ai"]

# 2. Fetch authorization-server metadata from that host
curl -s https://worldmonitor.sibt.ai/.well-known/oauth-authorization-server
...
```bash
curl -s https://worldmonitor.sibt.ai/api/resilience/v1/get-resilience-ranking \
  -H "X-WorldMonitor-Key: $WM_KEY"
```

PRO subscribers get a key from [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro). See [Authentication](/usage-auth).
