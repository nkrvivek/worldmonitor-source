...
Data calls (`tools/call`) need a user API key — issue one at [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro). See [Authentication](/usage-auth) for how keys, OAuth, and browser sessions differ, and [Rate Limits](/usage-rate-limits) for per-plan allowances.
