...
| Capability | Desktop behavior vs web | Classification | Evidence |
| --- | --- | --- | --- |
| Sign-in | No desktop gating in code. Supabase replaced Clerk, and the desktop build passes `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`, so sign-in works in shipped builds | parity | `src/services/auth.ts:485-493`; `.github/workflows/build-desktop.yml:252-253` |
| Convex entitlements | The desktop build now passes `VITE_CONVEX_URL` alongside the build-time `CONVEX_URL`, so the client reads it | parity | `src/services/entitlements.ts:96-99`; `.github/workflows/build-desktop.yml:254` |
...
| Billing portal / checkout | **GAP**: `openBillingPortal()` and `startCheckout()` still have no desktop branch — they navigate or pop the WebView instead of calling `open_url` to the OS browser, and the checkout return URL is built from `window.location.origin`, which on desktop is a `tauri://` or localhost origin. Several upgrade entry points are unguarded the same way. Stripe replaced Dodo without changing any of this | blocked → #5911 | `src/services/billing.ts:363`; `src/services/checkout.ts:332,429` |
| #5901 unified user menu | No desktop-specific handling; hosts the billing surface above. It needs a signed-in session, which shipped builds now have | inherits the billing row above | `src/components/UserMenu.ts:104` |
