...
- **服务器到服务器**:直接用 `X-WorldMonitor-Key: wm_...` 调用 `https://worldmonitor.sibt.ai/api/bootstrap`。此端点没有单独的网关主机、令牌交换、激活步骤或 IP 白名单要求。
...
返回 `koala73/worldmonitor` 的最新 **GitHub Release**。桌面应用使用此端点检测是否有更新的发布版本,并提示用户升级。它**不是**本站当前部署的 commit。
...
每用户仪表盘偏好设置(布局、开关、过滤器)。需要 Supabase bearer 凭据。由 Convex 提供支持。
...
在订阅变更后(Stripe webhook → Convex → 此端点)使用户的权益(entitlement)缓存失效。内部端点 —— 需要 `RELAY_SHARED_SECRET`。
