...
- **鉴权**：需要 PRO 权限。可通过以下任一方式授予：(a) 有效的 `X-WorldMonitor-Key`（来自 `WORLDMONITOR_VALID_KEYS` 的环境密钥，或所有者拥有 `apiAccess` 权限的用户专属 `wm_` 前缀密钥），**或** (b) 用户角色为 `pro` 或权限层级 ≥ 1 的 Supabase bearer token。仅凭受信任的浏览器 Origin **不足以**通过鉴权 — `server/_shared/premium-check.ts` 中的 `isCallerPremium()` 仅认可显式凭证。浏览器调用可用，是因为 `premiumFetch()`（`src/services/premium-fetch.ts`）代表调用方注入了上述两种凭证形式之一。
- **速率限制**：
