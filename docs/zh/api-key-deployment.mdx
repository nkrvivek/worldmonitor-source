...
### Cloudflare Worker 密钥

用 `wrangler secret put <NAME>` 逐个设置到服务本站的 Worker 上。
...
把它们合成一个逗号分隔的 `WORLDMONITOR_VALID_KEYS` 密钥，中间不留空格：
...
# 5. 将部署 URL 设为 Worker 的 CONVEX_URL 密钥
...
- Web 来源（`worldmonitor.sibt.ai`）无需密钥即可通过
- 非桌面端来源带密钥头：密钥仍会验证
...
请勿将环境导出保留在仓库根目录。`.env.vercel-backup`
和 `.env.vercel-export`（`scripts/check-local-secret-dumps.mjs` 至今仍在盯的两个文件名）被 Git 忽略，但它们仍然是明文
...
- [ ] 设置 `WORLDMONITOR_VALID_KEYS` Worker 密钥
- [ ] 设置 `CONVEX_URL` Worker 密钥
