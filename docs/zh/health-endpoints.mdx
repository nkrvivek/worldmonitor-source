---
title: "健康检查端点"
description: "World Monitor 提供两个健康检查端点用于持续监控数据管道完整性与实时性，服务在边缘运行时上以低延迟运行并查询 Redis (Upstash) 评估各数据源状态，帮助 SRE 与运维团队快速定位新闻、市场、海事等采集任务的中断与滞后。"
---
...
Edge 健康注册表随 PR 合并即刻部署，但它读取的键只能在 consumer-prices-core 的下一次每日抓取（02:00 UTC）→ 聚合（02:15）→ 发布（02:30）窗口之后才会出现。[#6059](https://github.com/koala73/worldmonitor/issues/6059) 用两道关卡关闭这个窗口：一道有界的 `ROLLOUT_PENDING` 软化，其窗口已于 2026-08-03 到期，相关机制随后按计划移除；另一道是持久的激活标记，保留至今。

每日发布器（`consumer-prices-core/src/jobs/publish.ts`，且仅限它）为某市场写入「确实尝试过页面」的覆盖率快照后，才会以**无 TTL** 的方式 `SET` `seed-activated:consumer-prices:coverage:v1:<market>`。真实但为空的快照（无启用零售商，或零售商从未运行）**不会**激活。键名中的 `v1` 是覆盖率 schema 版本，因此未来的 schema 变更会按其自身形态重新获得激活，而不会继承旧形态的结果。

八个覆盖率检查在**所有**状态下都会附带 `activated` 字段（持久标记存在时为 true），因此无需手动探测 Redis 即可区分「已激活后损坏」与「从未运行」。

**若某市场读数为 `EMPTY`**，需要排查的是生产者而非健康读取端。查看该市场的 `seed-consumer-prices-publish` 服务日志：`coverage-activation:<market> failed` 表示快照已发布但标记写入失败（下次运行会重试），`coverage:<market> published without attempted pages — activation marker withheld` 则表示该市场根本没有尝试过任何零售商页面，属于抓取问题。作为最后手段，`scripts/seed-consumer-prices.mjs --force` 可以重新发布单个市场的数据，但它刻意**不会**激活：它的覆盖率键只有 30 分钟 TTL，用一个 30 分钟的产物换取永久激活，会在数据过期后把该市场永久卡在无法恢复的 `EMPTY`（严重）状态。只有写入 26 小时键的每日发布器才会激活一个市场。

覆盖率计数绝不合成，成功状态也绝不从无关的规范消费价格键推断，因此 `EMPTY_DATA`、`COVERAGE_DEGRADED`、`COVERAGE_PARTIAL` 与 `STALE_SEED` 都如实反映生产者实际写入的内容。
...
将关键字监控器指向 `https://worldmonitor.sibt.ai/api/health?compact=1`，并在紧凑令牌 `"status":"HEALTHY"`（冒号后无空格）从响应体中**缺失**时告警。紧凑模式序列化时不带缩进，因此无论格式如何，此确切令牌都是稳定的。
