---
title: "中继参数（中继 + 边缘）"
description: "AIS 与 OpenSky 中继路径使用的每一个环境变量清单 —— 包含 Railway 后端与边缘前端部署环境、请求超时与重试设置、上游 API 凭据与密钥接线方式，以及降级策略配置，帮助自托管用户与运维团队在部署船舶与航空追踪管线时快速定位配置问题与安全边界。"
---
本文档涵盖 AIS/OpenSky 中继路径使用的所有环境变量：

- Railway 中继进程：`scripts/ais-relay.cjs`
- 边缘中继代理端点（旧版）：`api/opensky.js`、`api/ais-snapshot.js`、`api/polymarket.js`、`api/rss-proxy.js`
...
| `RELAY_SHARED_SECRET` | 是 | `wm_relay_prod_...` | 必须与边缘侧的值完全匹配。 |
| `RELAY_AUTH_HEADER` | 推荐 | `x-relay-key` | 如果从默认值更改，必须与边缘侧匹配。 |
...
| `WS_RELAY_URL` | 边缘 + 服务器处理器 | 无 | 是（针对中继支持的功能） | 边缘与服务器处理器用于访问中继的基础 URL。 |
| `VITE_WS_RELAY_URL` | 浏览器（本地开发） | 无 | 否 | 仅在开发环境中用于直接浏览器调用的 localhost 回退路径。 |
| `RELAY_SHARED_SECRET` | Railway + 边缘 + 自托管 | 空 | **是（始终）** | 非公开中继路由的共享密钥。没有此密钥中继将拒绝启动，除非 `I_UNDERSTAND_THIS_DISABLES_AUTH=true`。 |
| `RELAY_AUTH_HEADER` | Railway + 边缘 | `x-relay-key` | 否（但建议显式设置） | 携带中继密钥的标头名称。 |
...
| `WINGBITS_API_KEY` | 边缘/服务器 | 无 | 否 | 服务器处理器使用的军用数据增强/回退源；中继核心不需要。 |
