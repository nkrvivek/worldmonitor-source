...
| **随处运行**                    | 同一代码库从单一 Cloudflare Worker 产出六个专用变体（地缘政治、科技、金融、大宗商品、正能量、能源），并部署到 Cloudflare Workers（Web 与中继）、Tauri（桌面）和 PWA（可安装）。桌面 sidecar 在本地镜像所有云端 API 处理器。Service worker 缓存地图瓦片以供离线使用，同时保持情报数据始终新鲜（NetworkOnly）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
**影响**：冷启动时间下降约 85% — 行情报价请求不再加载网络威胁情报解析器、OREF 告警处理器或气候异常检测器。在 Worker 运行时上，这转化为大多数端点低于 100ms 的冷启动，而单体处理器需 500ms+。

### 单一部署变体合并

所有六个仪表盘变体（World Monitor、Tech Monitor、Finance Monitor、Commodity Monitor、Happy Monitor、Energy Monitor）在同一主机上从**单一 Cloudflare Worker** 提供服务。变体在运行时由路径读取：
...
上游项目改为给每个变体一个独立子域名，`src/config/variant.ts` 见到子域名时仍会读取它，因为变体子域名指明的是整个部署。以路径提供服务可让分享出去的链接留在本站，也避免搜索引擎把这里的页面当作上游页面的副本。

在桌面应用上，变体存储在 `localStorage['worldmonitor-variant']` 中，可在不重新构建的情况下切换。标题栏中的变体选择器在 Web 上在各路径间导航，或在桌面端切换 localStorage 值。

此架构取代了原来的多部署方式（每个变体一个独立部署），并提供以下优势：

- **即时切换** — 用户在标题栏切换变体，无需 DNS 查询
- **共享 CDN 缓存** — 静态 SPA 资源在各变体间相同；只有运行时配置不同。CDN 缓存命中率比独立部署高 4 倍
- **单一 CI 管道** — 一次构建、一次 Worker 部署、一套路由。无跨部署配置漂移
- **社交机器人路由** — OG 图片端点根据请求路径生成变体专属预览卡片，因此分享 Tech Monitor 链接会生成科技品牌社交预览
...
每个 API 边缘函数包含 `Cache-Control` 头，使 Cloudflare 的 CDN 能在不命中源站的情况下提供缓存响应：
...
Sentry 在生产环境中捕获未处理异常和 Promise 拒绝，具备环境感知路由（生产环境在 `worldmonitor.sibt.ai`，localhost 和 Tauri 桌面端禁用）。`src/bootstrap/sentry-init.ts` 中的 `*.vercel.app` 预览分支属于上游，在本站永远不会触发。
...
- 部署管道（GitHub Actions、Cloudflare 中断）
