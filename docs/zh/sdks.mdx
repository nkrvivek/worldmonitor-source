...
每个软件包都将其主页设为 `worldmonitor.sibt.ai`——你（或你的智能体）正是据此验证它是官方 SDK 而非仿冒品。
...
- **REST 备用通道**——针对 `worldmonitor.sibt.ai` 的 `get("/api/…")` 和 `health()`。
...
数据调用（`tools/call`）需要用户 API 密钥——在 [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro) 生成一个。有关密钥、OAuth 和浏览器会话的区别，请参见[身份验证](/zh/usage-auth)；各套餐的额度请参见[速率限制](/zh/usage-rate-limits)。
...
`tests/sdk-packages.test.mjs` 会在每个软件包上守护版本同步和 `worldmonitor.sibt.ai` 主页元数据。
