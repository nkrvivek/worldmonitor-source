...
数据命令映射到 MCP `tools/call`，需要用户 API 密钥。在 [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro) 获取一个，然后可以每次调用时传入，或一次性导出它：
...
| `--mcp-url <url>` | MCP 端点（默认 `https://worldmonitor.sibt.ai/mcp`） |
| `--base-url <url>` | REST 基础地址（默认 `https://worldmonitor.sibt.ai`） |
