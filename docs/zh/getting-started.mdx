...
description: "在本地环境安装、配置并运行 WorldMonitor 的完整分步入门指南，全面涵盖原生 TypeScript 与 Vite 技术栈概览、项目目录结构、Redis 与 Upstash 等必需的第三方 API 依赖、环境变量配置、开发调试技巧、生产部署要点以及首次启动运行的完整流程与常见问题排查。"
---
...
| **部署** | Cloudflare Workers | 边缘路由、API 处理器与 Durable Objects |
...
├── styles/
└── types/
api/                          # Edge API handlers (two kinds, see Architecture)
...
某些发布商封锁来自云提供商（Cloudflare、AWS 等）的请求：
