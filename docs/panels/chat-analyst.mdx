...
The panel POSTs to `/api/chat-analyst` (edge function backed by `api/chat-analyst.ts`) via `premiumFetch()`, which injects the caller's credentials (`WORLDMONITOR_API_KEY` on desktop, Supabase bearer + PRO in the browser). The edge function composes context from the intelligence, conflict, markets, forecasts, and risk caches (scoped to the selected domain), streams tokens back, and the panel renders incrementally.
