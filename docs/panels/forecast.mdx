...
Forecast probabilities come from deterministic, rule-based signal detectors in `scripts/seed-forecasts.mjs`, with an optional prediction-market blend when a matching market anchor exists. LLM calls do not set the numeric probability. They generate the narrative scenario, case-file, branches, and perspective text around the already-scored forecast. The default source-backed LLM routes in the seeder are OpenRouter `deepseek/deepseek-v4-flash` (primary, reasoning disabled) and Groq `openai/gpt-oss-120b` (fallback). One tracked exception: the critical-signals extraction stage contributes signal strength/confidence into the deterministic scoring of state-derived (market/supply-chain) forecasts, so that stage stays pinned to OpenRouter `google/gemini-2.5-flash` until the calibration question is settled (see issue #4963). It ran with a Groq fallback until 2026-08-24, when a probe found Groq had dropped every Llama id it served, so that entry had been answering 404 for weeks. The stage now names OpenRouter alone: putting a current Groq model there would move a probability-coupled stage onto a model the baseline never scored.
