...
Upgrade or generate a key at [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro). The rest of this guide assumes Claude Desktop + OAuth (the simplest path); if you'd rather paste a `wm_…` key into a `curl` script, jump to [Server-side curl](#server-side-curl) at the bottom.
...
```json
{
  "mcpServers": {
    "worldmonitor": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "https://worldmonitor.sibt.ai/mcp", "--header", "X-WorldMonitor-Key:wm_YOUR_KEY"]
    }
  }
}
```
...
- **Prefer OAuth?** Omit `--header`: your first tool call returns the 401 challenge that triggers `mcp-remote`'s browser consent flow (dynamic client registration works on both `worldmonitor.sibt.ai` and `worldmonitor.sibt.ai`). Older `mcp-remote` versions handle this mid-session flow less reliably than the connect-time one, so if the consent window never appears, the explicit key header above is the deterministic path.
...
```bash
export WM_KEY="wm_0123456789abcdef0123456789abcdef01234567"  # API Starter+ key from worldmonitor.sibt.ai/settings
...
Every `tools/call` emits a structured telemetry line tagged `mcp.toolcall` with latency, payload bytes (pre and post JMESPath), `jmespath_used`, and `budget_exceeded`. `initialize` emits `mcp.tools_list_emitted` with tool-count and tools-list byte metrics. Point a log-drain consumer at these lines if you want real P95 and payload-size tracking. Caller-facing behaviour is unaffected.
