...
1. Open the [World Monitor dashboard](https://worldmonitor.sibt.ai/dashboard) and sign in with the account that owns the API subscription.
...
If you already bought an API plan, confirm that you signed in with the account that owns the subscription. If the plan is still not recognized, email [hello@sibt.ai](mailto:hello@sibt.ai) from the purchase email address. Include the plan name and, if available, only the short key prefix—**never send a complete key**.
