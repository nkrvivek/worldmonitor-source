...
| Endpoint | Purpose |
|----------|---------|
| `https://worldmonitor.sibt.ai/mcp` | JSON-RPC server (Streamable HTTP transport; JSON responses by default, SSE responses when clients advertise `text/event-stream`; `initialize` defaults to protocol `2025-03-26`) |
| `https://worldmonitor.sibt.ai/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://worldmonitor.sibt.ai/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://worldmonitor.sibt.ai/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://worldmonitor.sibt.ai/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://worldmonitor.sibt.ai/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |
...
1. **Add the server URL** in your MCP client. The canonical entrypoint is:
   ...
   (`https://worldmonitor.sibt.ai/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with WorldMonitor Pro"** on the consent page. This is the default CTA. You'll bounce through `worldmonitor.sibt.ai/mcp-grant` (sign-in protected — sign in if needed), then back to `worldmonitor.sibt.ai/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://worldmonitor.sibt.ai/settings)**.
...
|------|------------|------------|-----------------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns `403 INSUFFICIENT_TIER`. Upgrade at [worldmonitor.sibt.ai/pro](https://worldmonitor.sibt.ai/pro). |
| **Pro** | ✅ Yes | OAuth only | **50 quota-consuming calls / UTC day** | Bounce-via-apex Supabase sign-in. No `wm_…` key needed or stored. |
...
```json
{
  "mcpServers": {
    "worldmonitor": {
      "url": "https://worldmonitor.sibt.ai/mcp"
    }
  }
}
```
...
```bash
npx @modelcontextprotocol/inspector https://worldmonitor.sibt.ai/mcp
```
...
# 1. List tools
curl -s https://worldmonitor.sibt.ai/mcp \
...
Seed-level health per key: `GET /api/health?compact=1` (public, no key).
...
- [Pro features](https://worldmonitor.sibt.ai/pro)
