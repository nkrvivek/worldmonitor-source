...
| `502` | Upstream / Convex / Stripe failure | Yes — exponential backoff |
...
| `error` value | Meaning |
|---------------|---------|
| `UNAUTHENTICATED` | No valid Supabase JWT or API key. |
...
- Every edge response carries a `cf-ray` header naming the request — include it when reporting issues.
- Seed-level health is public at `GET /api/health?compact=1`.
