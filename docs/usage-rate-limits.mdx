...
Rate limits are enforced in the Cloudflare Worker. A `CounterDO` Durable Object holds one sliding-window count per client, so every request for a given key reaches the same counter. All limits are **sliding 60-second windows** unless noted.
