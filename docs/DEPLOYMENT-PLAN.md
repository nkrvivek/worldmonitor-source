...
- The Convex production deployment `benevolent-impala-683` has no
  `SUPABASE_JWT_ISSUER` set, so `npx convex deploy` fails against it. The app
  points at the dev deployment until that value exists.
