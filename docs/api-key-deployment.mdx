...
```
Desktop App                          Cloud (Cloudflare Worker)
┌──────────────────┐                ┌──────────────────────┐
...
Set each one with `wrangler secret put <NAME>` against the Worker that serves this site.
...
Set them as one comma-separated `WORLDMONITOR_VALID_KEYS` secret, no spaces:

```bash
wrangler secret put WORLDMONITOR_VALID_KEYS
```
...
# 5. Set the deployment URL as the Worker's CONVEX_URL secret
...
- Web origins (`worldmonitor.sibt.ai`) pass through without a key
...
Do not keep environment exports in the repository root. `.env.vercel-backup`
and `.env.vercel-export` — the two filenames `scripts/check-local-secret-dumps.mjs`
still watches for — are ignored by Git, but they are still plaintext
...
- [ ] Set the `WORLDMONITOR_VALID_KEYS` Worker secret
- [ ] Set the `CONVEX_URL` Worker secret
