 ...
 *
 * Flow: Supabase sign-in → same-origin edge endpoint → top-level redirect to
 * Stripe Checkout (`window.location.assign`). The buyer comes back to the
 * dashboard through the guarded `?wm_checkout=return` contract.
 *
 * This mirrors `src/services/checkout.ts` in the main app, which the /pro build
 * cannot import: pro-test is a separate Vite app whose `@` alias points at its
 * own root. Where the behaviour has to match, the comment says so.
 *
 * The Dodo overlay SDK that used to sit here is gone. It had no callers from
 * the day redirect mode shipped, and Stripe Checkout is a top-level navigation,
 * so nothing takes its place.
 *
 * `API_BASE` is relative on purpose. /pro is served from the same origin as the
 * dashboard, and the absolute upstream host it used to carry sent our buyers'
 * checkout, entitlement and portal calls to someone else's API.
 *
 * No Convex client needed — the edge endpoint handles relay.
 */

import * as Sentry from '@sentry/react';
import { ensureAuth, getAuthToken, getCurrentUser, openSignIn } from './auth';
...
import { safeBillingPortalUrl, safeHostedCheckoutUrl } from './provider-hosted-urls';
import {
 ...
 * straight into doCheckout's top-level redirect to Stripe, unloading the
 * page before the flush poll ever runs. Queued checkout-start events are
 * mirrored into sessionStorage (per-tab, survives the Stripe round-trip)
 ...
 * the sign-in modal is open the pricing section is covered by the modal
...
// ensureAuth() — rapid double-clicks both pass the entry check and
...
async function startCheckoutInner(
  ...
  try {
    ...
    console.error('[checkout] Failed to load auth:', err);
    Sentry.captureException(err, { tags: { surface: 'pro-marketing', action: 'load-auth' } });
    return false;
  }
  ...
  // marks intent clicks that detour through the sign-in modal first.
  ...
  if (!user) {
    // Intent travels in the return URL, bound to THIS openSignIn call. On a
    // password sign-in the modal navigates there itself; on Google it is the
    // OAuth redirect target. Either way the URL carries the checkout intent in
    // its query string and tryResumeCheckoutFromUrl picks it up on page load.
    // Dismissing the modal navigates nowhere, so no resume. Other /pro sign-in
    // paths pass no return URL, so they cannot trigger a surprise purchase.
...
async function doCheckout(
  ...
  // edge call, then redirect) is the only time the
  ...
  // Best-effort visual bridge between the sign-in modal closing and
  // Stripe Checkout painting. Covers two sources of blank-screen feel:
  //   1. Auto-resume after sign-in fires doCheckout synchronously; the
  //      modal's close animation leaves a visual void until the browser
  //      leaves for checkout.stripe.com, which needs an
  //      /api/create-checkout round-trip first.
  //   2. Direct click from an already-signed-in user still waits on that
  //      same round-trip before anything moves.
  // A 10s safety fallback shows a toast instead of leaving the
  // interstitial wedged if the network hangs.
  try {
    ...
    const resp = await postCreateCheckout(createDefaultCheckoutTransportDeps(), {
      ...
      payload: {
        ...
        // flow, Stripe sends the buyer to this URL for EVERY outcome (success,
        ...
        // success against authoritative provider evidence. See checkout-return.ts.
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 429) {
        ...
        // from this closure: an access token can expire while the dialog
        // sits open, and getAuthToken() refreshes near expiry.
    ...
    // #4449: navigate the top window to the provider's hosted checkout rather
    // than an overlay iframe. An overlay cannot host a nested 3DS/fraud stack —
    // the device sensors it needs are blocked two frames deep, so card payments
    // requiring 3DS hung forever at "Processing…" (HAR-confirmed; see
    // #4449/#4450). Stripe Checkout is a top-level page by design, so redirect
    // is the only flow here.
...
function unmountCheckoutInterstitial(): void {
  ...
  // the browser has left for checkout or the request has errored.
...
/** Red toast, one per id. Used for rate limits and for portal failures. */
function showCheckoutErrorToast(id: string, message: string, ttlMs: number): void {
...
function showCheckoutRateLimitToast(retryAfterSeconds: number): void {
  showCheckoutErrorToast(
    'wm-checkout-rate-limit-toast',
    `Checkout is temporarily rate limited. Try again in ${retryAfterSeconds} ${
...
/**
 * Message shown when the portal cannot be reached. Same wording as
 * BILLING_PORTAL_UNAVAILABLE_MESSAGE in src/services/billing.ts.
 */
const BILLING_PORTAL_UNAVAILABLE_MESSAGE =
  "We couldn't open your billing portal. Email hello@sibt.ai and we'll sort it out.";

/** Long enough to read an email address off the screen. */
const BILLING_PORTAL_TOAST_MS = 12_000;

/**
 * There is no fallback URL. Under Dodo every failure branch here navigated to
 * `customer.dodopayments.com`, a generic portal that at least recognised some
 * buyers. Stripe has no such page — a portal session is the only way in, and
 * `billing.stripe.com` on its own is a dead end. A failure now closes the
 * reserved tab and says so, which beats opening a blank tab onto nothing.
 */
async function openBillingPortal(token: string, preopened?: Window | null): Promise<void> {
  ...
  const giveUp = (): void => {
    if (reservedWin && !reservedWin.closed) reservedWin.close();
    showCheckoutErrorToast(
      'wm-billing-portal-error-toast',
      BILLING_PORTAL_UNAVAILABLE_MESSAGE,
      BILLING_PORTAL_TOAST_MS,
    );
  };
  ...
  try {
    ...
    const url = safeBillingPortalUrl(result?.portal_url);
    if (!url) {
      console.error('[checkout] Customer portal returned no usable portal_url');
      Sentry.captureMessage('Billing portal returned 200 without a usable portal_url', {
        level: 'error',
        tags: { surface: 'pro-marketing', code: 'missing_portal_url' },
      });
      giveUp();
      return;
    }
...
const PRO_PLAN_DISPLAY_NAMES: Readonly<Record<string, string>> = {
  ...
  api_starter: 'API Monthly',
  api_starter_annual: 'API Annual',
  api_business: 'API Business',
  api_business_annual: 'API Business Annual',
};
