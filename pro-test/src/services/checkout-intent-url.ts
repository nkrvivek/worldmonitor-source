 ...
 *
 * Intent is bound to the specific sign-in attempt via the sign-in flow's
 * afterSignInUrl / afterSignUpUrl. On successful sign-in from our
 * openSignIn call, the visitor returns to a URL carrying these
 * params. On page load, we parse, consume, and strip them.
 *
 * Extracted as a pure module (no DOM, no auth SDK, no payment SDK
 ...
 * time to construct the sign-in return URL.
 *
