...
// worldmonitor.sibt.ai is deliberately absent. The RUM token above belongs to the upstream
// project's DebugBear account, and its monthly quota is already tight (see the sampling note).
// Adding this fork's host would spend someone else's paid allowance, so RUM is off here. To turn
// it on, get a token for an account we own and add the host in the same change.
