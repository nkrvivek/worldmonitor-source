...
export default function WelcomeApp() {
  useEffect(() => {
    ...
    // decide this from the stored session alone so the auth SDK never loads on
    // the welcome critical path (issue #4428). Idle signed-in users (expired
    // access token) stay here and use the Launch CTA; /dashboard validates auth
    // either way, so it never bounces a signed-out visitor back to /, and no
    // redirect loop is possible.
    maybeRedirectWelcomeVisitor(readStoredSession(), window.location);
  }, []);
