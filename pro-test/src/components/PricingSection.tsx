...
// Same-origin. /pro is served by our own worker, and its /api/product-catalog
// prices come from our Stripe account — the upstream host would price the page
// off someone else's catalog.
...
function isInProductHref(href: string): boolean {
  ...
  try {
    const base = typeof window !== 'undefined' ? window.location.href : 'https://worldmonitor.sibt.ai/';
    const url = new URL(href, looksRelative ? base : undefined);
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    // This fork serves the product from its own host. Without it here, every
    // free-tier CTA on our pages counted as off-platform and opened a new tab.
    return url.hostname === 'worldmonitor.sibt.ai' ||
...
export function PricingSection({ refCode }: { refCode?: string }) {
  ...
  // the clicked CTA. During the sign-in modal window, phase stays idle —
  ...
  return (
    <section id="pricing" className="py-24 px-6 border-t border-wm-border bg-[#060606]">
      <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            >
              {/* Lock billing toggle ONLY during creating_checkout (narrow
                  post-auth window). Through the sign-in modal the toggle
        ...
        {/* Tier cards grid */}
        {/* 4 self-serve tiers since Pro Business merged into Pro (2026-08-05):
            2-up on tablet and laptop, all four across on desktop.
            Enterprise renders below as a band, not as a fifth column. */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {/* Personal / Commercial axis header — xl only. It is a pair of grid
              ...
              (Personal = Free, Commercial = the three licensed tiers — Pro
              carries a commercial license since it merged with Pro Business).
              Below xl the grid wraps to 2-up, where a spanning header
          ...
          <div className="hidden xl:block xl:col-span-1 border-t border-wm-border pt-3">
