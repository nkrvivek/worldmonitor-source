...
import {
  ...
  Send, MessageCircle, Mail, ChevronDown,
...
import { t } from './i18n';
import { tryResumeCheckoutFromUrl } from './services/checkout';
import {
  getAuthToken,
  openSignIn,
  scheduleAuthLoad,
...
import { startAuthUserStateSync, type AuthUserState } from './services/auth-user-state';
import { hasLiveClientSession } from './services/auth-session';
import { AuthModal } from './components/AuthModal';
import { PricingSection } from './components/PricingSection';
import { SoonBadge } from './components/SoonBadge';
import { Logo } from './components/Logo';
import { SITE_BYLINE } from './config/brand';
...
// Same-origin. This app is served from worldmonitor.sibt.ai, and /api there is
// our own worker — pointing it at api.worldmonitor.app would call upstream's.
const API_BASE = '/api';
...
/** Click handler — drops the event so it can't be read as a return URL. */
function handleSignInClick(): void {
  openSignIn();
}

/**
 * Lightweight /pro auth state. The stored session gives us an immediate
 * signed-in signal without loading the auth SDK; the real user is filled in
 * only after the SDK loads, from an auth action or an idle signed-in load.
 *
 * Used by the Navbar to swap the SIGN IN button for the account menu once the
 * visitor is authenticated, and by the Hero to hide its redundant SIGN IN CTA.
 * Single source of truth for "is the /pro visitor signed in".
 */
function useAuthUser(): AuthUserState {
  ...
  useEffect(() => {
    return startAuthUserStateSync(setState, {
      hasLiveClientSession,
      subscribeAuthLoaded,
      scheduleAuthLoad,
      onLoadError(err) {
        console.error('[auth] Failed to load auth for nav state:', err);
        Sentry.captureException(err, { tags: { surface: 'pro-marketing', action: 'load-auth-for-nav' } });
 ...
 * visitor has an active Convex subscription (tier >= 1). The provider below performs
...
function ProEntitlementProvider({ children }: { children: ReactNode }): ReactElement {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // getAuthToken() loads the SDK, reads the session and refreshes a
        // token that is near expiry, so one call is enough here — the
        // Clerk version needed a retry because its token endpoint could
        // answer null while the user object already existed.
...
/** First letter of the email, or a dot while the user is still loading. */
function avatarInitial(user: User | null): string {
  const email = user?.email ?? '';
  return email ? email[0].toUpperCase() : '·';
}

/**
 * Avatar plus a small menu holding the email and a sign-out item.
 *
 * Clerk shipped a hosted UserButton widget and this replaces it. Supabase has
 * no such widget, and a menu with two rows is not worth pulling one in.
 *
 * `user` is null for the first frames of a signed-in visit: the navbar knows
 * from storage that someone is signed in before the SDK has said who. The
 * avatar renders either way, so the nav does not jump when the user arrives.
 */
function AccountMenu({ user }: { user: User | null }): ReactElement {
  ...
  const onSignOut = (): void => {
    signOut()
      .catch((err) => {
        ...
        Sentry.captureException(err, { tags: { surface: 'pro-marketing', action: 'sign-out' } });
      })
      // Reload either way. A failed sign-out still cleared local state often
      // enough that leaving a half-signed-in navbar on screen is worse.
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="flex h-8 w-8 items-center justify-center rounded-full border border-wm-border bg-wm-card font-mono text-xs text-wm-text hover:border-wm-text transition-colors"
      >
        {avatarInitial(user)}
      </button>
      {open && (
        <div
          role="menu"
          className="absolute right-0 mt-2 min-w-[13rem] rounded-sm border border-wm-border bg-wm-card p-1 font-mono text-xs shadow-lg"
        >
          {user?.email && (
            <p className="truncate px-3 py-2 text-wm-muted" title={user.email}>
              {user.email}
            </p>
          )}
          <a
            href={DASHBOARD_URL}
            role="menuitem"
            className="block px-3 py-2 text-wm-text hover:bg-wm-bg"
          >
            {t('nav.goToDashboard')}
          </a>
          <button
            ...
            role="menuitem"
            onClick={onSignOut}
            className="block w-full px-3 py-2 text-left text-wm-text hover:bg-wm-bg"
          >
            Sign out
          </button>
        </div>
...
/* ─── 0. Navbar ─── */
const Navbar = () => {
  const { user, isLoaded, signedIn } = useAuthUser();
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            ...
            {showSignIn && (
              <button type="button" onClick={handleSignInClick} className="border border-wm-border text-wm-text px-6 py-3 rounded-sm font-mono text-sm uppercase tracking-wider font-bold hover:border-wm-text transition-colors">
                {t('hero.signIn')}
              </button>
            )}
          </div>
      ...
      <div className="order-2 lg:order-1">
        <div className="bg-black border border-wm-border rounded-lg overflow-hidden font-mono text-sm">
          <div className="bg-wm-card px-4 py-2 border-b border-wm-border flex items-center gap-2">
            ...
            <span className="text-wm-muted text-xs">worldmonitor.sibt.ai</span>
          </div>
          <div className="p-6 text-gray-300 overflow-x-auto">
            <pre><code>
<span className="text-wm-blue">curl</span> \<br/>
  <span className="text-wm-green">"https://worldmonitor.sibt.ai/api/conflict/v1/list-acled-events?country=LB"</span> \<br/>
  -H <span className="text-wm-green">"Authorization: Bearer wm_live_xxx"</span><br/><br/>
<span className="text-wm-muted">{"{"}</span><br/>
  <span className="text-wm-blue">"events"</span>: <span className="text-wm-muted">{"["}</span><br/>
    <span className="text-wm-muted">{"{"}</span><br/>
      <span className="text-wm-blue">"eventType"</span>: <span className="text-wm-green">"Explosions/Remote violence"</span>,<br/>
      <span className="text-wm-blue">"actors"</span>: <span className="text-wm-muted">["Military Forces of Israel", "Hezbollah"]</span>,<br/>
      <span className="text-wm-blue">"fatalities"</span>: <span className="text-orange-400">3</span>,<br/>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between max-w-7xl mx-auto text-xs text-wm-muted font-mono">
        <div className="flex items-center gap-3 mb-4 md:mb-0">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] uppercase tracking-[2px] opacity-60 mt-0.5">{SITE_BYLINE}</span>
          </div>
        </div>
        <div className="flex items-center gap-6">
          ...
          <a href="/docs" className="hover:text-wm-text transition-colors">Docs</a>
          <a href="https://github.com/koala73/worldmonitor" target="_blank" rel="noreferrer" className="hover:text-wm-text transition-colors">Original project</a>
        </div>
...
export default function App() {
  ...
  // #4449: there is no checkout overlay on this page. Checkout redirects the
  // top window to the provider's hosted page (see startCheckout), and the
  // buyer lands on the dashboard afterwards, not back here — the return URL
  // carries `?wm_checkout=…` and handleCheckoutReturn in
  // src/services/checkout-return.ts owns what the buyer sees. The overlay this
  // replaced pulled a heavy SDK in on every /pro mount to register a success
  // banner that could never fire.
  useEffect(() => {
  ...
  // AuthModal sits outside the page switch: openSignIn() can be called from
  // either page, and from services that never see this tree.
  if (page === 'enterprise') {
    return (
      <>
        <EnterprisePage />
