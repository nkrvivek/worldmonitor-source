...
import { writeStartFailureRecord } from './seeds/run-record';
  ...
  scheduled: (event: { cron: string }, env: SeedEnv) =>
    handleScheduled(event, async (script) => {
      await getContainer(env.SEED_CONTAINER, script).start({
        // Through the recorder, never straight at the script. A seed that
        // fails only inside a container leaves no trace: handleScheduled
        // catches the start error so one dead seed cannot stop its siblings,
        // Cloudflare still reports the invocation a success, and
        // `wrangler containers` has no logs subcommand. The wrapper writes
        // `seed-run:script:<path>` holding the exit code and the tail of the
        // output, which is how anyone finds out. Measured 2026-08-19 on
        // seed-fuel-prices.mjs, silent for two Sunday ticks.
        entrypoint: ['node', 'scripts/run-seed-recorded.mjs', script],
        enableInternet: true,
        envVars: seedEnvVars(env),
      });
    },
    // A start that never happened gets a record too, under its own status
    // word. Without it a refused container is indistinguishable from a cron
    // that never fired, in the one key anyone reads.
    async (script, err) => {
      await writeStartFailureRecord(env, script, err);
    }),
};
