 ...
 *   seed-consumer-prices — CORRECTED 2026-08-30: this one is NOT merely
 *   credential-blocked, it must NEVER be scheduled here. Its own header says
 *   "Do NOT configure as a Railway cron" — it is a manual fallback that writes
 *   10-60min TTLs and stomps the authoritative 26h TTLs from
 *   consumer-prices-core's own publish.ts (scrape -> aggregate -> publish),
 *   which now runs daily on Railway (project consumer-prices-core, service
 *   cpc-jobs, cron 0 9 * * *). The Worker holds CONSUMER_PRICES_CORE_API_KEY
 *   and CONSUMER_PRICES_CORE_BASE_URL as of 2026-08-30 so the fallback can be
 *   run by hand with --force if publish.ts ever breaks, but it stays off cron.
 *   seed-aviation (AVIATIONSTACK_API
 ...
 *   ICAO_API_KEY same posture: trial quota), (seed-grocery-basket left this group
 *   2026-08-30: a Firecrawl account was created and FIRECRAWL_API_KEY added to
 *   the Worker, so it is scheduled on the Sunday price tick below alongside
 *   seed-bigmac and seed-fuel-prices)
...
export const SEED_JOBS: readonly SeedJob[] = [
  ...
  // 3h TTL each, 6x this cron. They carried 30m (= exactly one interval) until
  // 2026-08-19: the key expired at the moment the next run was due, so one late
  // tick reported EMPTY on a healthy seeder.
  // seed-sector-summary joined 2026-08-30, the last of the dead-Railway-relay
  // market ports (market:sectors:v2 had been EMPTY since the relay died). It
  // belongs on this tick rather than one of its own: same Finnhub + Yahoo
  // sources as its two neighbours, so the three share one rate-limit budget
  // instead of competing at unrelated minutes. Its maxStaleMin moved 30 -> 60
  // in api/health.js and api/mcp/constants.ts to stay 2x THIS cron; the old 30
  // was 2x the relay's 15-minute cadence and would be exactly 1x here.
  {
    cron: '11,41 * * * *',
    scripts: [
      'scripts/seed-market-quotes.mjs',
      'scripts/seed-commodity-quotes.mjs',
      'scripts/seed-sector-summary.mjs',
    ],
  },
  ...
  // seed-pizzint joined 2026-08-30, ported off the dead Railway relay. It takes
  // a live 30-minute expression rather than a new one so wrangler.jsonc's cron
  // list stays put (the registry test pins the two lists equal), same reasoning
  // seed-bigmac and seed-grocery-basket used. The relay ran a 10-minute loop;
  // 30 minutes is ample for Google popular-times data, and the health row moves
  // 30 -> 60 to stay 2x this cron.
  {
    cron: '7,37 * * * *',
    scripts: ['scripts/seed-insights.mjs', 'scripts/seed-pizzint.mjs'],
  },
  ...
  // ---- every 2 hours ------------------------------------------------------
  // 6h TTL on a regulator RSS sweep. Satellites shares the slot: CelesTrak
  // republishes elements every couple of hours and the health gate is 240 min.
  {
    cron: '50 */2 * * *',
    scripts: [
      'scripts/seed-regulatory-actions.mjs',
      'scripts/seed-satellites.mjs',
    ],
  },
  ...
  // seed-fuel-prices also runs from a GitHub runner, .github/workflows/
  // seed-fuel-prices.yml at 23 10 * * SUN. That is not redundancy. Measured
  // 2026-08-19: the container's egress IP is blocked by Mexico, Brazil and New
  // Zealand, and MX is not in the seed's TOLERATED_FAILURES, so validateFuel
  // rejects every publish and the key expires to EMPTY 10 days later. A GitHub
  // runner reaches 7/7 sources. Keep both: the container tick is harmless and
  // the GH run is what actually publishes. Same shape as seed-military-flights.
  // seed-grocery-basket joined this tick 2026-08-30, same reasoning as
  // seed-bigmac: identical CACHE_TTL (864000) and maxStaleMin 10080, it is a
  // price series, and reusing a live expression keeps wrangler.jsonc's cron
  // list unchanged. It left the credential-blocked group when FIRECRAWL_API_KEY
  // was added to the Worker on 2026-08-30 (it needs Firecrawl for extraction on
  // top of the Exa key it already had).
  {
    cron: '0 10 * * SUN',
    scripts: [
