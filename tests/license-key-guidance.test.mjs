...
// The docs guide lives on the same host the app ships under, so derive the
// expected link from SITE_ORIGIN rather than repeating the host here.
const DOCS_LINK = new RegExp(`${new URL(SITE_ORIGIN).host.replaceAll('.', '\\.')}/docs/api-keys`);
...
test('desktop settings give blocked users an exact help path', () => {
  ...
  assert.match(fullCopy.register.description, DOCS_LINK);
...
test('every locale describes the launched API-key flow instead of a waitlist', () => {
  ...
  for (const file of localeFiles) {
    ...
    assert.match(copy.register.description, DOCS_LINK, `${file}: help copy must link the guide`);
