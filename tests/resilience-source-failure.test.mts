...
const TRACKED_STANDALONE_META_KEYS_NOT_IN_HEALTH = new Set([
  ...
  // The two trade keys that sat here until 2026-08-06 are gone on purpose:
  // being tracked by the resilience scorer and by nothing an operator reads is
  // exactly how their seeder went quiet for seven days unnoticed. Both are
  // /api/health probes now (tradeBarriers, tradeRestrictions).
]);
