...
function installTransport(responses: Response[]) {
  ...
  __setNotificationChannelsClientDepsForTests({
    ...
    getCurrentUser: () => ({ id: 'user_1' }) as never,
...
describe('authFetch honors a retryable billing-verification 503', () => {
  ...
  it('does not retry under a different account than the one the caller pinned', async () => {
    ...
    __setNotificationChannelsClientDepsForTests({
      getAuthToken: async () => 'token-abc',
      getCurrentUser: () => ({ id: current }) as never,
      fetch: (async (path: string, init?: RequestInit) => {
        calls.push({ path, init });
  ...
  it('re-derives the auth token for the retry rather than reusing the first', async () => {
    ...
    assert.equal(tokenIssues, 2, 'the retry must call getAuthToken again');
  ...
  it('declines the retry when no signed-in identity can be pinned', async () => {
    // getCurrentUser() is a synchronous read of the cached Supabase session
    // that never triggers a load, while getAuthToken() can itself refresh the
    // session and succeed — so a call in that window reaches the retry with
