...
describe('research report corpus (#5668)', () => {
  ...
  it('separates evidence layers and states failure modes in plain language', () => {
    ...
    // Analytics ship inline and post to this site's own collector. The script
    // tag that used to sit here loaded a host that no longer exists.
    assert.ok(html.includes("fetch('/api/send'"), 'research pages must post to the first-party collector');
    assert.ok(
