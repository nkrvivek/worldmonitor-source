...
describe('agent-mode view (/?mode=agent)', () => {
  ...
  it('advertises the sandbox and quickstart endpoints', () => {
    assert.equal(view.endpoints.sandbox.url, `${SITE_ORIGIN}/sandbox/index.json`);
    assert.doesNotThrow(
      () => readFileSync(join(ROOT, 'public/sandbox/index.json')),
      'sandbox index advertised but public/sandbox/index.json is missing',
    );
    // Upstream also advertised a docs MCP endpoint at /docs/mcp. This fork
    // never ported that route — it returns 530 — so the view must stay quiet
    // about it rather than send agents at a dead endpoint.
    assert.ok(!('docsMcp' in view.endpoints), 'agent-view must not advertise the unported /docs/mcp');
  ...
  it('the marketing homepage points at the agent view via link rel=alternate', () => {
    ...
    const linkTag =
      '<link rel="alternate" type="application/json" href="https://worldmonitor.sibt.ai/?mode=agent"';
  ...
  it('advertises the schemamap and every section llms.txt', () => {
    assert.equal(view.discovery.schemamap, `${SITE_ORIGIN}/schemamap.xml`);
    ...
    // /docs/llms.txt comes out of the docs build; pin the URL so a docs-host
    // move shows up here.
    assert.equal(sections.docs, `${SITE_ORIGIN}/docs/llms.txt`);
  });
  ...
  it('stays in parity with the MCP server card and A2A agent card', () => {
    ...
    assert.equal(view.endpoints.nlweb.url, `${SITE_ORIGIN}/ask`);
  });
  ...
  it('every discovery URL it advertises resolves to a tracked file or a live rewrite', () => {
    ...
    const trackedPaths = {
      [`${SITE_ORIGIN}/.well-known/agent-skills/index.json`]:
        'public/.well-known/agent-skills/index.json',
      [`${SITE_ORIGIN}/.well-known/api-catalog`]: 'public/.well-known/api-catalog',
      [`${SITE_ORIGIN}/.well-known/ai-catalog.json`]: 'public/.well-known/ai-catalog.json',
      [`${SITE_ORIGIN}/llms.txt`]: 'public/llms.txt',
    };
