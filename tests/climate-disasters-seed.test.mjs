...
// The seed has two sources and publishes whenever either one returns rows. That
// is the right behaviour — half a disaster map beats none — but for as long as
// RELIEFWEB_APPNAME is unset it means the seed reports a healthy run on the
// natural-events half alone, and health has no way to know. These pin the
// reading that closes that gap.
describe('summarizeDisasterCoverage', () => {
  const relief = { status: 'fulfilled', value: [{ id: 'relief-1' }] };
  const natural = { status: 'fulfilled', value: [{ id: 'nat-1' }] };
  const labels = ['ReliefWeb', 'NaturalEvents'];

  it('reports partial coverage when a source was never configured', () => {
    const coverage = summarizeDisasterCoverage(
      [{ status: 'fulfilled', value: [] }, natural],
      labels,
      [false, true],
    );

    // 'partial' is not a new word: api/health.js already maps a coverage block
    // with this status to COVERAGE_PARTIAL, which is a warn.
    assert.equal(coverage.status, 'partial');
    assert.equal(coverage.completedPages, 1);
    assert.equal(coverage.failedPages, 1);
    assert.equal(coverage.completionRatio, 0.5);
    assert.deepEqual(coverage.sources[0], { name: 'ReliefWeb', state: 'not-configured', records: 0 });
  });

  it('reports partial coverage when a configured source failed', () => {
    const coverage = summarizeDisasterCoverage(
      [{ status: 'rejected', reason: new Error('HTTP 503') }, natural],
      labels,
      [true, true],
    );

    assert.equal(coverage.status, 'partial');
    assert.deepEqual(coverage.sources[0], { name: 'ReliefWeb', state: 'failed', records: 0 });
  });

  it('treats a quiet week from a source that answered as covered', () => {
    // The distinction this whole block exists for. A configured source that
    // returned nothing has measured a calm week; an unconfigured one has
    // measured nothing at all. A record-count floor reports both, which is why
    // coverage reads configuration and settlement rather than row counts.
    const coverage = summarizeDisasterCoverage(
      [{ status: 'fulfilled', value: [] }, natural],
      labels,
      [true, true],
    ...
    assert.equal(coverage.failedPages, 0);
    assert.deepEqual(coverage.sources[0], { name: 'ReliefWeb', state: 'ok', records: 0 });
  });

  it('reports full coverage when both sources answered', () => {
    const coverage = summarizeDisasterCoverage([relief, natural], labels, [true, true]);

    assert.equal(coverage.status, 'ok');
    assert.equal(coverage.completedPages, 2);
    assert.equal(coverage.completionRatio, 1);
  });

  it('names every source it was given, so a dark half is never anonymous', () => {
    const coverage = summarizeDisasterCoverage([relief, natural], labels, [true, true]);

    assert.deepEqual(coverage.sources.map((source) => source.name), labels);
  });
});
