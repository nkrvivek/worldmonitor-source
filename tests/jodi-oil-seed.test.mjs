...
describe('extractCountryData schema', () => {
  ...
  it('keeps a month JODI has not assessed, and says so', () => {
    // Code 3 is JODI's white background: "data has not been assessed". That is
    // an unchecked measurement, not a missing one, so it is kept and the level
    // travels with the record.
    const rows = makeFullRows('DE', '2025-11', '3');
    const result = extractCountryData(rows, 'DE');
    assert.ok(result !== null, 'an unassessed month is still a month');
    assert.equal(result.dataMonth, '2025-11');
    assert.deepEqual(result.assessment, { code: '3', label: 'unassessed' });
  });

  it('returns null when no row carries a code JODI publishes', () => {
    const rows = makeFullRows('DE', '2025-11', '9');
    assert.equal(extractCountryData(rows, 'DE'), null, 'code 9 is not a level we can read');
    assert.equal(
      extractCountryData(makeFullRows('DE', '2025-11', ''), 'DE'),
      null,
      'a blank code is not a level either',
    );
  });
  ...
  it('assessment_code=3 keeps its value and sets the record level', () => {
    const rows = [
    ...
    assert.equal(result.diesel.demandKbd, 894, 'code=3 TOTDEMO is unassessed, not unobserved');
    assert.equal(result.diesel.refOutputKbd, 500, 'code=1 REFGROUT should have value');
    assert.deepEqual(
      result.assessment,
      { code: '3', label: 'unassessed' },
      'the least-confident level present is what the record can claim',
    );
  });

  it('an unreadable assessment code is dropped while readable ones stay', () => {
    const rows = [
      makeRow({ REF_AREA: 'DE', TIME_PERIOD: '2025-11', ENERGY_PRODUCT: 'GASDIES', FLOW_BREAKDOWN: 'TOTDEMO', OBS_VALUE: '894', ASSESSMENT_CODE: '' }),
      makeRow({ REF_AREA: 'DE', TIME_PERIOD: '2025-11', ENERGY_PRODUCT: 'GASDIES', FLOW_BREAKDOWN: 'REFGROUT', OBS_VALUE: '500', ASSESSMENT_CODE: '1' }),
    ];
    const result = extractCountryData(rows, 'DE');
    assert.ok(result !== null);
    assert.equal(result.diesel.demandKbd, null, 'a blank code is not a level we can read');
    assert.equal(result.diesel.refOutputKbd, 500);
    assert.deepEqual(result.assessment, { code: '1', label: 'comparable' });
  });
  ...
  it('keeps the newest month even when only it is unassessed', () => {
    const rows = [
    ...
    const result = extractCountryData(rows, 'DE');
    assert.ok(result !== null);
    assert.equal(result.dataMonth, '2025-11', 'falling back a month would age the seed for nothing');
    assert.equal(result.diesel.demandKbd, 900);
    assert.deepEqual(result.assessment, { code: '3', label: 'unassessed' });
  });

  it('skips a month whose codes are all unreadable and uses the earlier one', () => {
    const rows = [
      makeRow({ REF_AREA: 'DE', TIME_PERIOD: '2025-11', ENERGY_PRODUCT: 'GASDIES', FLOW_BREAKDOWN: 'TOTDEMO', OBS_VALUE: '900', ASSESSMENT_CODE: '7' }),
      makeRow({ REF_AREA: 'DE', TIME_PERIOD: '2025-10', ENERGY_PRODUCT: 'GASDIES', FLOW_BREAKDOWN: 'TOTDEMO', OBS_VALUE: '800', ASSESSMENT_CODE: '1' }),
    ];
    const result = extractCountryData(rows, 'DE');
    assert.ok(result !== null);
  ...
  it('assessment_code=2 (consult the metadata) is accepted', () => {
    const rows = [
...
describe('computeOilDemandChange', () => {
  ...
  it('refuses a change built on unusable observations', () => {
    assert.equal(
      computeOilDemandChange([
        ...demandRows('CN', '2026-04', { GASDIES: 600, GASOLINE: 600, JETKERO: 600 }),
        ...demandRows('CN', '2025-04', { GASDIES: 600, GASOLINE: 600, JETKERO: 600 }, '8'),
      ], 'CN', '2026-04'),
      null,
      'a code JODI does not publish is not an observation',
    );
    assert.equal(
      computeOilDemandChange([
        ...demandRows('CN', '2026-04', { GASDIES: 600, GASOLINE: 600, JETKERO: 600 }),
...
describe('jodiYearUrls', () => {
  it('offers both names JODI may serve one year under, freshest first', () => {
    assert.deepEqual(jodiYearUrls('secondary', 2026), [
  ...
  it('uses the same two forms for the primary file', () => {
    const urls = jodiYearUrls('primary', 2025);
    assert.equal(urls.length, 2);
    assert.ok(urls[0].endsWith('/primary/primaryyear2025.csv'));
    assert.ok(urls[1].endsWith('/primary/2025.csv'));
  });

  it('asks for the year-to-date file before the closed-year one', () => {
    // The year in progress is published only under the `<kind>year<year>` name.
    // Asking for the closed-year name first would take the older file whenever
    // both resolve, which is how the seed silently aged eight months.
    const [first, second] = jodiYearUrls('secondary', 2026);
    assert.ok(first.includes('secondaryyear2026'));
    assert.ok(!second.includes('secondaryyear2026'));
  });
});
