...
describe('extractBillingErrorKind — structured-data path', () => {
  ...
  it('reads PROVIDER_API_KEY_MISSING from err.data.kind', () => {
    const err = Object.assign(new Error('any message'), {
      data: { kind: 'PROVIDER_API_KEY_MISSING' },
    });
    assert.equal(extractBillingErrorKind(err), 'PROVIDER_API_KEY_MISSING');
  });
  ...
  it('reads PORTAL_ERROR from err.data.kind', () => {
    ...
    // The action now re-throws `new ConvexError({ kind: 'PORTAL_ERROR' })`
    // so the event buckets under a `billing_error_kind` Sentry tag.
    const err = Object.assign(new Error('[Request ID: x] Server Error'), {
      data: { kind: 'PORTAL_ERROR' },
    });
    assert.equal(extractBillingErrorKind(err), 'PORTAL_ERROR');
  });
