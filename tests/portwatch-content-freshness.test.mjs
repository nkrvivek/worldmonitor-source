...
// Production evidence (2026-08-11): seed-meta read recordCount 163 against a
// target of 174 with CR CU CV DE DJ DM FJ FO GA GD GH missing. Every one of
// the eleven still held a cached payload with ports, and IMF's ArcGIS held
// 25-475 rows for each inside the last thirty days, so nothing was absent
// upstream. What happened is the rotation: one run attempted all eleven and
// they failed together in a single batch window (3 fetch_error, 8
// empty_activity), which stamped refreshAttemptedAt. Oldest-attempt-first then
// sorted them behind every country whose attempt was older -- US at 126h, BR
// at 114h -- so the next run refreshed neither, their caches crossed the
// seven-day hard expiry five hours later, and they stopped publishing.
//
// Deferring a country that still publishes costs one cycle of freshness.
// Deferring one that does not costs a record. The cold-fetch slots belong to
// the second group first.
describe('orderColdFetchQueue — countries that publish nothing go first', () => {
  const DAY = 86_400_000;
  const NOW_MS = 100 * DAY;

  function country(iso2, { cacheAgeDays, attemptAgeDays, consecutiveFailures } = {}) {
    const prevPayload = { iso2 };
    if (Number.isFinite(cacheAgeDays)) prevPayload.cacheWrittenAt = NOW_MS - cacheAgeDays * DAY;
    if (Number.isFinite(attemptAgeDays)) prevPayload.refreshAttemptedAt = NOW_MS - attemptAgeDays * DAY;
    if (Number.isFinite(consecutiveFailures)) {
      prevPayload.refreshFailure = { code: 'empty_activity', consecutiveFailures };
    }
    return { iso2, iso3: `${iso2}X`, prevPayload };
  }

  it('refreshes an expired cache before a live one with an older attempt', () => {
    const ordered = orderColdFetchQueue([
      // Publishes today. Its attempt is the oldest in the queue, so
      // oldest-attempt-first alone puts it first.
      country('US', { cacheAgeDays: 5, attemptAgeDays: 5 }),
      // The live shape: failed 18 hours ago, cache now past the seven-day
      // expiry, contributes nothing to recordCount until it succeeds.
      country('GH', { cacheAgeDays: 7.3, attemptAgeDays: 0.75, consecutiveFailures: 1 }),
    ], { now: NOW_MS }).map((item) => item.iso2);

    assert.deepEqual(ordered, ['GH', 'US']);
  });

  it('treats a never-cached country as a hole too', () => {
    const ordered = orderColdFetchQueue([
      country('US', { cacheAgeDays: 1, attemptAgeDays: 6 }),
      { iso2: 'ZW', iso3: 'ZWE', prevPayload: null },
    ], { now: NOW_MS }).map((item) => item.iso2);

    assert.deepEqual(ordered, ['ZW', 'US']);
  });

  it('still puts decision-critical countries ahead of a hole', () => {
    const ordered = orderColdFetchQueue([
      country('GH', { cacheAgeDays: 7.3, attemptAgeDays: 0.75, consecutiveFailures: 1 }),
      country('CN', { cacheAgeDays: 0.2, attemptAgeDays: 0.2 }),
    ], { now: NOW_MS }).map((item) => item.iso2);

    assert.deepEqual(ordered, ['CN', 'GH']);
  });

  it('orders holes among themselves by oldest attempt', () => {
    const ordered = orderColdFetchQueue([
      country('GH', { cacheAgeDays: 8, attemptAgeDays: 1 }),
      country('CR', { cacheAgeDays: 8, attemptAgeDays: 3 }),
      country('CU', { cacheAgeDays: 8, attemptAgeDays: 2 }),
    ], { now: NOW_MS }).map((item) => item.iso2);

    assert.deepEqual(ordered, ['CR', 'CU', 'GH']);
  });

  it('drops a country back into ordinary rotation once it keeps failing', () => {
    // Anti-starvation. A country upstream genuinely cannot serve would
    // otherwise hold a priority slot on every run forever, which is the
    // property tests/portwatch-port-activity-recovery.test.mjs guards.
    const ordered = orderColdFetchQueue([
      country('US', { cacheAgeDays: 5, attemptAgeDays: 5 }),
      country('GH', {
        cacheAgeDays: 9,
        attemptAgeDays: 0.75,
        consecutiveFailures: PORTWATCH_COVERAGE_RETRY_MAX_FAILURES,
      }),
    ], { now: NOW_MS }).map((item) => item.iso2);

    assert.deepEqual(ordered, ['US', 'GH']);
  });

  it('leaves room in the per-run cap for the ordinary rotation to advance', () => {
    assert.ok(
      PORTWATCH_COVERAGE_RETRY_MAX_FAILURES >= 1 && PORTWATCH_COVERAGE_RETRY_MAX_FAILURES <= 5,
      'the backoff ceiling must bound how long a hole can hold its slot',
    );
  });
});
