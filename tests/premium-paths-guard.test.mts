...
const PREMIUM_FETCH_BYPASS_ALLOWLIST: Record<string, string> = {
  ...
  '/api/me/entitlement':
    'No browser code calls this. It was polled by entitlement-watchdog.ts, ' +
    'which covered a Dodo overlay that could finish a payment without ' +
    'telling the parent page; Stripe Checkout redirects instead, so the ' +
    'watchdog was deleted. The endpoint stays for API callers, which attach ' +
    'their own Bearer rather than going through premiumFetch.',
};
