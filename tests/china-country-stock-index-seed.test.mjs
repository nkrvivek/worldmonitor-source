...
test('the market seed maintains the China cache alongside its public stock bootstrap contract', () => {
  ...
  assert.match(handlerSource, /const SEEDED_COUNTRY_INDEX_KEY = 'market:stock-index:v1:CN';/);
  assert.match(handlerSource, /getCachedJson\(SEEDED_COUNTRY_INDEX_KEY, true\)/);
