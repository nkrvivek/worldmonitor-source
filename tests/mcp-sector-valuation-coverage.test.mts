...
describe('get_market_data sector valuation coverage contract', () => {
  ...
  it('checks the sector seed metadata as part of aggregate market freshness', () => {
    ...
    // 60, not 30: seed-sector-summary.mjs runs on '11,41 * * * *' from the
    // container rail (worker/seeds/registry.ts) since 2026-08-30. The old 30
    // was 2x the retired Railway relay's 15-minute loop; against a 30-minute
    // cron it is exactly 1x, so ordinary tick jitter would report stale sector
    // valuations to agents on a seeder that had just succeeded.
  ...
  it('marks valuation coverage stale once the published snapshot ages past the sector budget', () => {
    ...
    // 61 minutes is past the 60-minute budget; evaluating at the snapshot's own
    // timestamp is not.
    applySectorValuationFreshness(data, now);
    assert.equal(data.sectors.valuationCoverage.stale, true);

    applySectorValuationFreshness(data, now - 61 * 60_000);
