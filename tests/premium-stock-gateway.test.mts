...
// `plan: 'pro'` claim from publicMetadata that every gate read as an early
// allow; Supabase carries no per-user store we may trust for authorization,
// so these tests source the plan the way production now does. Only the users
// listed here hold a paid row — everyone else is tier 0.
const PAID_USER_IDS = new Set(['user_pro']);
...
function installRateLimitRedisFake(): void {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // afterEach reinstalls this fake, so the entitlement leg has to live here
    // rather than in a describe's before() — a stub installed once is gone
    // after the suite's first test. Tests that need a different row install
    // their own stub over this one and restore it themselves.
    if (process.env.CONVEX_SITE_URL && url.endsWith('/api/internal-entitlements')) {
...
describe('premium gateway bearer token auth', () => {
  ...
  before(async () => {
    ...
    process.env.SUPABASE_JWT_ISSUER = `http://127.0.0.1:${jwksPort}`;
    process.env.WORLDMONITOR_VALID_KEYS = 'real-key-123';
    process.env.CONVEX_SITE_URL = 'https://test.convex.site';
    process.env.CONVEX_SERVER_SHARED_SECRET = 'test-secret';
    // Pick up the entitlement leg now that the Convex vars are set.
    installRateLimitRedisFake();
  ...
  it('a tier-1 entitlement row unlocks tier-1 gated endpoints', async () => {
    const token = await signToken({ sub: 'user_pro' });
  ...
  it('a token that claims Pro for itself unlocks nothing', async () => {
    // Under Clerk the plan rode in the token, and Clerk alone could write it.
    // Supabase's only per-user token store is `user_metadata`, which its own
    // docs say is "editable by the user without any checks" — so a claim in the
    // token is now the account holder's word about their own plan, and the row
    // is the only answer. Every shape a user could plausibly reach for.
    for (const claims of [
      { sub: 'user_claims_pro', plan: 'pro' },
      { sub: 'user_claims_pro', role: 'pro' },
      { sub: 'user_claims_pro', user_metadata: { plan: 'pro' } },
      { sub: 'user_claims_pro', app_metadata: { plan: 'pro' } },
    ]) {
      ...
      assert.equal(res.status, 403, `self-claimed Pro was accepted: ${JSON.stringify(claims)}`);
    }
  });
