...
vi.mock('@/services/auth', () => ({
  getCurrentUser: () => ({ id: 'user_push', email: 'push@worldmonitor.test' }),
}));
...
describe('notification Settings browser push', () => {
  it('renders denied permission as browser site-settings guidance without Enable', async () => {
    ...
    // This fork offers no Discord app, so the picker must not draw a connect
    // button that cannot finish. See src/config/notification-channels-offered.ts.
    expect(container.querySelector('#usConnectDiscord')).toBeNull();
  });
