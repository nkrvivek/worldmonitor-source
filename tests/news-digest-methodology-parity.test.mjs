...
import { GROQ_FALLBACK_MODEL } from '../shared/groq-model.js';
...
describe('news digest methodology parity', () => {
  ...
  it('documents regional weekly brief provider chain separately from digest prose', () => {
    const providerNames = [...weeklyBriefSrc.matchAll(/name:\s*'([^']+)'/g)]
      .map((m) => m[1]);
    // The groq model is a shared constant, not a literal, so a model swap is
    // one edit instead of the six-file sweep the last one needed. Resolve the
    // identifier here rather than loosening the assertion to a substring.
    const providerModels = [
      ...weeklyBriefSrc.matchAll(/model:\s*(?:'([^']+)'|(GROQ_FALLBACK_MODEL))/g),
    ].map((m) => (m[2] ? GROQ_FALLBACK_MODEL : m[1]));
    ...
    assert.deepEqual(providerModels, ['deepseek/deepseek-v4-flash', GROQ_FALLBACK_MODEL]);
    assert.equal(weeklyTemperature, 0.3);

    assertDocMatches(
      new RegExp(
        `Regional weekly briefs[\\s\\S]*tr(?:y|ies) OpenRouter first[\\s\\S]*\`deepseek/deepseek-v4-flash\`[\\s\\S]*Groq \`${GROQ_FALLBACK_MODEL}\`[\\s\\S]*temperature\\s+\`0\\.3\``,
      ),
