...
describe('api/mcp.ts — /.well-known/mcp dual-role alias', () => {
  ...
  it('plain GET serves the server card (manifest role), even with a foreign Origin', async () => {
    ...
    assert.equal(card.url, 'https://worldmonitor.sibt.ai/mcp');
    assert.equal(card.kind, 'product');
    assert.equal(card.serverUrl, 'https://worldmonitor.sibt.ai/mcp');
    assert.equal(card.remotes?.[0]?.url, 'https://worldmonitor.sibt.ai/mcp');
  ...
  it('plain GET /mcp serves the human-readable server guide (crawler-accessible discovery)', async () => {
    // Straight to the handler on a fork-host request. The canonical link is
    // derived from the host that answered, and the localhost harness cannot
    // carry one: undici rewrites a Host header to the URL it dialled.
    ...
    assert.match(body, /https:\/\/worldmonitor\.sibt\.ai\/mcp/, 'guide must advertise the apex transport URL');
    assert.match(res.headers.get('link') ?? '', /<https:\/\/worldmonitor\.sibt\.ai\/mcp>;\s*rel="canonical"/,
