...
//   4. the Commerce OpenAPI example price lookup keys still exist in the catalog.
//
...
// Pro Business merged into Pro on 2026-08-05, so its two keys are gone.
const PLAN_KEYS = ['pro_monthly', 'pro_annual', 'api_starter', 'api_starter_annual', 'api_business', 'api_business_annual'];
...
const PRICE_EXPECT = [
  ...
  ['API Monthly', 'api_starter'],
  ['API Annual', 'api_starter_annual'],
...
// The price lookup keys are surfaced by GET /api/product-catalog as
// monthlyProductId / annualProductId, and
// docs/openapi/CommerceService.openapi.yaml embeds several as examples. A
// renamed key in the catalog must not leave the published OpenAPI example
// pointing at a price that no longer exists.
//
// The keys are read out of the catalog rather than matched by the `wm_`
// prefix: that prefix is also the user API-key prefix and the checkout
// metadata prefix, so a bare-prefix regex would sweep up unrelated strings.
test('CommerceService.openapi.yaml example price lookup keys exist in productCatalog.ts', () => {
  const catalogLookupKeys = new Set(
    [...catalogSrc.matchAll(/providerPriceId:\s*"([^"]+)"/g)].map((m) => m[1])
  );
  assert.ok(catalogLookupKeys.size > 0, 'productCatalog.ts must declare providerPriceId lookup keys');

  const spec = read('docs/openapi/CommerceService.openapi.yaml');
  // `[ \t]` not `\s`: the schema section has a bare `monthlyProductId:` whose
  // value sits on the next line, and matching across the newline would read
  // its `type:` as a lookup key.
  const exampleKeys = [...spec.matchAll(/(?:monthly|annual)ProductId:[ \t]+(\S+)/g)].map((m) => m[1]);
  assert.ok(exampleKeys.length > 0, 'spec example must include at least one price lookup key');
  for (const key of exampleKeys) {
    assert.ok(
      catalogLookupKeys.has(key),
      `spec example lookup key ${key} is not present in productCatalog.ts`
    );
  }
});
