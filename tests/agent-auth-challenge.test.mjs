...
describe('agent-auth WWW-Authenticate challenge (/agent/auth)', () => {
  ...
  it('names this fork’s own host, not the upstream apex', async () => {
    const res = await call('worldmonitor.sibt.ai', { method: 'GET' });
    assert.equal(
      res.headers.get('www-authenticate'),
      'Bearer realm="worldmonitor", resource_metadata="https://worldmonitor.sibt.ai/.well-known/oauth-protected-resource"',
    );
  });
