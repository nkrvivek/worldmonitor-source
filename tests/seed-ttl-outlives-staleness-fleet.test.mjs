...
//   seed-economy            cron */15   ttl 60min  = 4x interval  (was safe; see below)
...
//
// seed-economy retired 2026-08-06, and it shows why "4x interval (safe)" above has a
// shelf life. The Worker port moved that seeder from */15 to `0 9 * * *` and left its
// TTLs at the old cadence, so a 1h energy TTL now ran against a 24h gap: the key was
// gone for 23 of every 24 hours while seed-meta still read healthy. The margin is
// measured against the CRON, and this test only sees maxStaleMin. When a cron changes,
// nothing here notices — tests/seed-economy-cron-ttl.test.mjs pins that seeder's cron
// so its next move fails a test instead of going quiet.
