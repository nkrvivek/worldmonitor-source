...
describe('locale staleness classification', () => {
  ...
  it('marks only the affected CLDR plural categories stale', () => {
    ...
    // Sorted, not in CLDR order: ICU returns these alphabetically on some Node
    // builds and in CLDR order on others. The code iterates them as a set, so
    // only membership means anything.
    assert.deepEqual([...categories].sort(), ['few', 'many', 'one', 'other']);
