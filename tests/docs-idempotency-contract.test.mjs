...
describe('docs Idempotency-Key prose contract', () => {
  ...
  it('notify docs match the standalone handler auth contract', () => {
    ...
    assert.match(notifySource, /validateBearerToken/, 'notify handler validates Supabase bearer tokens');
    assert.match(
      notifySource,
    ...
    assert.match(
      ...
      /Supabase bearer auth/i,
      'notify route registry must document Supabase bearer auth',
    );
    assert.match(
      notifyException?.reason ?? '',
      /PRO/i,
    ...
    assert.match(
      notifyDocs,
      /Supabase bearer[\s\S]*PRO/i,
      'notify docs must document Supabase bearer auth and PRO entitlement',
    );
    assert.doesNotMatch(
      `${notifyDocs}\n${notifyException?.reason ?? ''}`,
