...
describe('publisher orchestration seams (#4927 review)', () => {
  ...
  it('a missing digest is warmed and read back rather than skipped', async () => {
    // The digest is an on-demand cache with a 900s TTL, warmed on a `*/15` cron
    // that never refreshes a live key. This job reads it once a day at a moment
    // nobody chose, so losing that race is normal operation, not a fault.
    const nowMs = Date.parse('2026-07-30T15:00:00.000Z');
    const articles = Array.from({ length: 20 }, (_, index) => ({
      title: `Reference headline number ${index} with enough words`,
      url: `https://example.com/reference-${index}`,
    }));
    const digest = { categories: { world: { items: [{ title: articles[0].title }] } } };
    const writes = [];
    const requests = [];
    let digestReads = 0;
    process.env.WORLDMONITOR_RELAY_KEY = 'test-relay-key';
    ...
    try {
      await runRecallBenchmark({
        ...
        _redisCommand: async (_creds, command) => {
          if (command[0] === 'GET' && command[1] === 'news:digest:v1:full:en') {
            digestReads += 1;
            // Absent on the first read, present once the warm has landed.
            return digestReads === 1 ? { result: null } : { result: JSON.stringify(digest) };
          }
          if (command[0] === 'GET' && command[1] === GDELT_BULK_ARTICLES_KEY) {
            return { result: JSON.stringify({ articles, fetchedAt: nowMs }) };
          }
          writes.push(command);
          return { result: 'OK' };
        },
    ...
    assert.equal(requests.length, 1, 'exactly one warm self-call');
    assert.ok(requests[0].url.endsWith('/api/news/v1/list-feed-digest?variant=full&lang=en'));
    assert.equal(requests[0].headers['X-WorldMonitor-Key'], 'test-relay-key');
    assert.ok(
      writes.some((command) => command[0] === 'SET' && command[1] === 'news:recall-benchmark:v1'),
      'the benchmark publishes after the warm lands',
    );
  });

  it('a digest that never lands is announced, not skipped quietly', async () => {
    // `WARN … skipping` plus exit 0 is what let four green runs publish nothing
    // while the freshness monitor reported newsRecallBenchmark EMPTY every three
    // hours. The annotation puts it on the workflow summary.
    const warnings = [];
    const writes = [];

    await runRecallBenchmark({
      _getCreds: () => ({ url: 'https://redis.invalid', token: 'test' }),
      _now: () => Date.parse('2026-07-30T15:00:00.000Z'),
      _fetch: async () => ({ ok: false, status: 503 }),
      _sleep: async () => {},
      _redisCommand: async (_creds, command) => {
      ...
      _logger: { log() {}, warn: (message) => warnings.push(message) },
    });

    assert.ok(
      warnings.some((message) => message.startsWith('::warning title=recall-benchmark skipped::')),
      `expected a GitHub annotation, got: ${JSON.stringify(warnings)}`,
    );
    assert.deepEqual(writes, [], 'nothing is published when the digest never appears');
  });
