...
describe('China official policy adapters (#5576)', () => {
  ...
  it('parses bounded hostile markup without superlinear rescans', () => {
    ...
    // Port-owned: this measures wall clock, and the suite runs with
    // --test-concurrency=16, so the number says as much about how
    // oversubscribed the box is as about the parser. Upstream's public repo
    // gets 4-core runners and lands at ~490ms against a flat 1_000ms budget;
    // this fork is private, gets 2-core runners, and lands at ~1_300ms —
    // same parser, half the cores. Scale the budget by the oversubscription
    // instead of pinning a number that only holds on the larger runner. On 4
    // cores this stays upstream's 1_000ms exactly. A superlinear rescan of
    // 16_000 tags overruns any of these by orders of magnitude, so the guard
    // this test exists for still bites.
    const budgetMs = (1_000 * 4) / Math.max(1, availableParallelism());

    assert.ok(
      elapsedMs < budgetMs,
      `hostile markup parsing took ${elapsedMs.toFixed(1)}ms (budget ${budgetMs.toFixed(0)}ms on ${availableParallelism()} cores)`,
    );
  });
