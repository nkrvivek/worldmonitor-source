...
async function loadChatAnalystPanel() {
  ...
  const stubModules = new Map([
    ...
    ['auth-state-stub', `
      ...
      // AuthModal reaches this stub because services/auth lazily imports the
      // modal, so esbuild pulls it into the bundle. The real subscribeAuthState
      // emits the current state synchronously on subscribe and returns an
      // unsubscribe; AuthModal leans on that first call to tell "was already
      // signed in" from "just signed in", so the stub keeps both halves.
