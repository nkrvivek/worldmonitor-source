...
test('entitlement cache key prefix is derived identically by every writer and reader', () => {
  ...
  for (const [file, src] of sources) {
    const derivation = src.match(
      /const ([A-Za-z_][A-Za-z0-9_]*) = process\.env\.PAYMENTS_ENVIRONMENT === ['"]live_mode['"]\s*\?\s*['"]live['"]\s*:\s*['"]test['"]/,
    );
    assert.ok(
      derivation,
    ...
    const derivations = [
      ...src.matchAll(/process\.env\.PAYMENTS_ENVIRONMENT === ['"]live_mode['"]/g),
    ];
    assert.equal(
      derivations.length,
      1,
...
test('exactly three files derive the entitlements cache key', async () => {
  ...
  // `:!convex/_generated` drops the compiled mirror of convex/payments/
  // cacheActions.ts that upstream now tracks. It is build output, not a fourth
  // derivation — counting it would mean every regeneration reads as drift,
  // while a genuine new derivation still shows up because nothing else lives
  // under that path.
  const out = execFileSync(
    'git',
    ['grep', '-l', '-F', 'entitlements:${', '--', 'api', 'server', 'convex', 'src', 'scripts', ':!convex/_generated'],
