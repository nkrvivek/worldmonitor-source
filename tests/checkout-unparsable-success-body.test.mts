...
import { PRO_URL } from '../src/config/site.ts';
...
const stubSources: Record<string, string> = {
  ...
  `,
  './auth': `
    export const getCurrentUser = () => globalThis.__xvHarness.currentUser;
    export const getAuthToken = async () => 'tok_test';
...
async function loadCheckoutModule(): Promise<{ startCheckout: (productId: string) => Promise<boolean> }> {
  const result = await build({
    ...
    target: 'es2022',
    write: false,
    define: { 'import.meta.env.VITE_PAYMENTS_ENVIRONMENT': '"test_mode"' },
...
describe('create-checkout 200 with an unparsable body (WORLDMONITOR-XV)', () => {
  it('reports the contract violation instead of letting the parse error escape', async () => {
    ...
    assert.deepEqual(globalThis.__xvHarness.assignedUrls, [PRO_URL]);
  ...
  it('a parsed 200 reports key names only — no payload values reach Sentry', async () => {
    ...
    // from the payment provider, which we do not control, so nothing
    // value-level may ship — not the claim token, not the session id.
    resetHarness(
      JSON.stringify({
  ...
  it('a valid success payload still navigates and reports nothing', async () => {
    ...
    resetHarness('{"checkout_url":"https://checkout.stripe.com/c/pay/cs_test_ok0000000000000000000"}');
    const checkout = await loadCheckoutModule();

    assert.equal(await checkout.startCheckout('prod_monthly'), true);
    assert.deepEqual(globalThis.__xvHarness.assignedUrls, [
      'https://checkout.stripe.com/c/pay/cs_test_ok0000000000000000000',
    ]);
  ...
  it('still persists the anonymous claim token on a successful checkout', async () => {
    ...
    resetHarness(
      '{"checkout_url":"https://checkout.stripe.com/c/pay/cs_test_ok0000000000000000000","anonymous_claim_token":"tok_live_KEEP"}',
    );
    const checkout = await loadCheckoutModule();
