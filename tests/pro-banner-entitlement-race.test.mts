...
describe('pre-paint reservation honors entitlement hint', () => {
  function runPrepaint(storageSeed: Record<string, string>): Set<string> {
    ...
    runInNewContext(script[1], {
      ...
      location: { hostname: 'www.worldmonitor.app', pathname: '/' },
      window: windowObj,
      Date,
    });
    return classes;
...
describe('wiring contracts (#5728)', () => {
  it('ProBanner uses account-backed resolution and auth/entitlement retries', () => {
    ...
    assert.match(src, /isAuthEnabled\(/);
  ...
  it('auth load failure settles pending auth subscribers without dropping the queue', () => {
    const src = readFileSync(resolve(root, 'src/services/auth.ts'), 'utf-8');
  ...
  // The overlay success path used to write the hint from checkout.ts too.
  // Redirect mode leaves one writer: the return handler.
  it('the checkout return path writes the pro banner entitlement hint', () => {
