...
describe('plan classification helpers (allowlist)', () => {
  it('isProProductId: only the Pro price keys are Pro', () => {
    ...
    assert.equal(isProProductId('wm_unknown'), false);
  });

  it('isProProductId: the retired Pro Business keys are not Pro', () => {
    // They were, until the tiers merged on 2026-08-05. A build that still
    // answers true for them is reading a catalog that grew the tier back.
    assert.equal(isProProductId('wm_pro_business_monthly'), false);
    assert.equal(isProProductId('wm_pro_business_annual'), false);
  });

  it('isProPlanKey: only the pro plan keys are Pro', () => {
    assert.equal(isProPlanKey('pro_monthly'), true);
    assert.equal(isProPlanKey('pro_annual'), true);
    assert.equal(isProPlanKey('pro_business_monthly'), false);
    assert.equal(isProPlanKey('pro_business_annual'), false);
  ...
  it('PRO_PRODUCT_IDS stays in sync with the generated catalog (drift guard)', () => {
    assert.deepEqual(
      ...
      [DODO_PRODUCTS.PRO_MONTHLY, DODO_PRODUCTS.PRO_ANNUAL].sort(),
    );
  });
