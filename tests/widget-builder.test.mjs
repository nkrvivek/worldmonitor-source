...
describe('PRO widget — store and sanitizer', () => {
  ...
  it('widget sandbox allows both product hosts and rejects lookalike origins', () => {
    assert.ok(
    ...
    assert.ok(
      sandbox.includes("url.hostname === 'worldmonitor.sibt.ai'")
        && sandbox.includes("url.hostname.endsWith('.worldmonitor.sibt.ai')"),
      'sandbox must allow the worldmonitor.sibt.ai host the app is served from',
    );
    assert.ok(
    ...
    assert.ok(
      !sandbox.includes("endsWith('worldmonitor.sibt.ai')") && !sandbox.includes('endsWith("worldmonitor.sibt.ai")'),
      'sandbox must not use raw suffix checks that allow evilworldmonitor.sibt.ai',
    );
    // This fork does not deploy to Vercel. An allowlist entry for a host
    // nobody here controls is a standing hole, so the sandbox must carry none.
    assert.ok(
      !sandbox.includes('vercel'),
      'sandbox must not allow any vercel.app origin (security allowlist)',
    );
  });

  it('widget sandbox behavior accepts both product hosts and blocks spoofed parents', () => {
    ...
    const allowed = runSandbox('https://worldmonitor.sibt.ai/dashboard');
    assert.equal(allowed.readyMessages.length, 1);
    assert.equal(allowed.readyMessages[0].targetOrigin, 'https://worldmonitor.sibt.ai');
    ...
    allowed.message({
      ...
      origin: 'https://worldmonitor.sibt.ai',
    ...
    const spoofed = runSandbox('https://worldmonitor.sibt.ai.evil.com/');
    assert.deepEqual(spoofed.readyMessages, []);
    spoofed.message({
      data: { type: 'wm-html', id: 'wm-1', token: 'test-token', html: '<p>bad</p>' },
      origin: 'https://worldmonitor.sibt.ai.evil.com',
      source: spoofed.parent,
    });
    assert.deepEqual(spoofed.writes, []);

    const deadPreview = runSandbox('https://worldmonitor-git-feature-eliewm.vercel.app/dashboard');
    assert.deepEqual(deadPreview.readyMessages, [], 'no Vercel origin may reach the sandbox');
  });
...
describe('widget-agent relay — error classifier', () => {
  ...
  it('classifier scrubs API keys in the 400 branch (defence-in-depth on every rawMsg interpolation)', () => {
    ...
    const out = fn(
      { status: 400, error: { type: 'invalid_request_error' }, message: 'bad header sk-ant-api03-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA was rejected' }, // fixture, not a key: mirror-scan-allow
      'claude-sonnet-4-6',
    );
    assert.ok(
      /\[REDACTED\]/.test(out),
