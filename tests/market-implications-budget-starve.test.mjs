...
test('pre-call guard skips when the run budget cannot cover the full provider chain (#1/#4)', async () => {
  ...
  __setRedisStoreForTests(store);
  seedLastGood(store);
  // Default chain, keys present so a broken/too-low guard would actually invoke
  ...
  // Read the reservation rather than restating it. Hardcoding the number is how
  // this test went stale: it named a 40s two-provider chain the stage had not
  // used since the default became single-provider, and it only failed once the
  // provider changed and the arithmetic moved under it.
  const reservation = getMarketImplicationsMinRunBudgetMs(getForecastLlmCallOptions('market_implications'));
  // A budget just under the reservation must SKIP cleanly and preserve last-good
  // instead of attempting an ambiguous, doomed call.
  __setForecastLlmRunDeadlineForTests(Date.now() + reservation - 1_000);
