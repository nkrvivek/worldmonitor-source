...
test('classifyKey: present-but-stale seed → STALE_SEED (warn), data still present', () => {
  const entry = classifyKey('earthquakes', BOOTSTRAP_KEYS.earthquakes, { allowOnDemand: false },
    makeCtx({
      strens: { [BOOTSTRAP_KEYS.earthquakes]: 1234 },
      // earthquakes maxStaleMin=90; 200 min exceeds it
...
// The live shape on 2026-08-19: the seeder held fetchedAt at the served LKG
// through a rejection, so the generic 75-minute gate reported STALE_SEED while
// the synthesisFailure contract was still inside both its bounds. The code
// naming which rejection it was used to be published only on SEED_ERROR, which
// is the state this one has not reached yet.
test('classifyKey: a stale insights seed still names the outstanding rejection', () => {
  const entry = classifyNewsInsights({
    fetchedAt: NOW - 90 * ONE_MIN_MS,
    recordCount: 8,
    lastAttemptAt: NOW - 2 * ONE_MIN_MS,
    lastSuccessAt: NOW - 90 * ONE_MIN_MS,
    servedGeneratedAt: '2026-08-19T05:38:31.328Z',
    consecutiveFailures: 1,
    lastSynthesisFailureCode: 'INSIGHTS_SYNTHESIS_PROVIDER',
  });

  assert.equal(entry.status, 'STALE_SEED');
  assert.equal(entry.lastSynthesisFailureCode, 'INSIGHTS_SYNTHESIS_PROVIDER');
});

// No outstanding rejection means there is nothing to name, and a code left
// over from an older run would read as a live one.
test('classifyKey: a healthy insights seed publishes no rejection code', () => {
  const entry = classifyNewsInsights({
    fetchedAt: NOW - ONE_MIN_MS,
    recordCount: 8,
    lastAttemptAt: NOW - ONE_MIN_MS,
    lastSuccessAt: NOW - ONE_MIN_MS,
    servedGeneratedAt: '2026-08-19T07:38:31.328Z',
    consecutiveFailures: 0,
    lastSynthesisFailureCode: 'INSIGHTS_SYNTHESIS_PROVIDER',
  });

  assert.equal(entry.status, 'OK');
  assert.equal(entry.lastSynthesisFailureCode, undefined);
});
...
const ISSUE_5055_HEALTH_REGISTRATIONS = [
  ['energyPrices', 'economic:energy:v1:all', 'seed-meta:economic:energy-prices', 1560], // seed-economy is a daily cron; see tests/seed-economy-cron-ttl.test.mjs
  ['researchArxivHnTrending', 'research:arxiv:v1:cs.AI::50', 'seed-meta:research:arxiv-hn-trending', 540],
...
test('classifyKey: issue #5055 strict seeds surface missing metadata instead of reporting OK', () => {
  ...
  assert.equal(entry.status, 'STALE_SEED');
  assert.equal(entry.records, 1);
  assert.equal(entry.maxStaleMin, 1560); // seed-economy is a daily cron; see tests/seed-economy-cron-ttl.test.mjs
  assert.equal(STATUS_COUNTS[entry.status], 'warn');
});
...
test('classifyKey: expired transitional producers fail closed when missing or empty', () => {
  const graduatedNames = [
    'fxYoy',
    'hyperliquidFlow',
    // The two relay:heartbeat:* producers left this list when the Node relay
    // that wrote them was retired. Nothing writes those keys now, so holding
    // them here would assert a permanent EMPTY.
    'eiaPetroleum',
    'digestNotifications',
  ];
...
// test inside the gate did. GATE alone covers eight rules, and the rule name
// used to reach container logs only, so naming it meant tailing a worker until
// the next tick.
test('classifyKey: a gate rejection names the editorial rule that decided it', () => {
  const entry = classifyNewsInsights({
    fetchedAt: NOW - 5 * ONE_MIN_MS,
    recordCount: 8,
    lastAttemptAt: NOW - 2 * ONE_MIN_MS,
    ...
    consecutiveFailures: 2,
    lastSynthesisFailureCode: 'INSIGHTS_SYNTHESIS_GATE',
    lastSynthesisRejectRule: 'lead-anchor-grounding',
  });

  assert.equal(entry.lastSynthesisFailureCode, 'INSIGHTS_SYNTHESIS_GATE');
  assert.equal(entry.lastSynthesisRejectRule, 'lead-anchor-grounding');
});

// The compact payload is public. Anything outside the fixed vocabulary is model
// output or drift, and either way health does not republish it.
test('classifyKey: a rule outside the vocabulary is dropped, not passed through', () => {
  for (const bogus of ['lead-proper-noun ', 'the model said Belgium', '', 42, {}]) {
    const entry = classifyNewsInsights({
      fetchedAt: NOW - 5 * ONE_MIN_MS,
      recordCount: 8,
      lastAttemptAt: NOW - 2 * ONE_MIN_MS,
      ...
      consecutiveFailures: 2,
      lastSynthesisFailureCode: 'INSIGHTS_SYNTHESIS_GATE',
      lastSynthesisRejectRule: bogus,
    });
    assert.equal(entry.lastSynthesisRejectRule, undefined, `expected ${JSON.stringify(bogus)} to be dropped`);
  }
});

// Same condition as the code it refines: no outstanding rejection, nothing to
// name. A rule left over from an older run would read as a live one.
test('classifyKey: a healthy insights seed publishes no rejection rule', () => {
  const entry = classifyNewsInsights({
    fetchedAt: NOW - ONE_MIN_MS,
    recordCount: 8,
    lastAttemptAt: NOW - ONE_MIN_MS,
    ...
    consecutiveFailures: 0,
    lastSynthesisFailureCode: 'INSIGHTS_SYNTHESIS_GATE',
    lastSynthesisRejectRule: 'lead-fact',
  });

  assert.equal(entry.status, 'OK');
  assert.equal(entry.lastSynthesisRejectRule, undefined);
});
