...
describe('root sitemap generator', () => {
  /**
   * Every URL a sitemap names is a URL we are telling crawlers we serve.
   * Upstream's manifest named worldmonitor.app and five product subdomains,
   * and this fork inherited it — so the sitemap our host published pointed
   * search engines at someone else's site for all 254 entries, and the five
   * subdomains do not resolve at all here.
   */
  it('names only hosts this deployment serves', () => {
    const committed = readFileSync(join(repoRoot, 'public/sitemap.xml'), 'utf8');
    const published = [...committed.matchAll(/<loc>\s*([^<]+?)\s*<\/loc>/g)].map((m) => m[1]);
    assert.ok(published.length > 0, 'the committed sitemap must contain URLs');

    const hosts = new Set([
      ...STATIC_ROUTE_MANIFEST.map((route) => new URL(route.loc).origin),
      ...published.map((loc) => new URL(loc).origin),
    ]);
    assert.deepEqual([...hosts], [SITE_ORIGIN]);
  });
  ...
  it('declares owned material-content sources without copying blog or docs inventories', () => {
    ...
    assert.ok(locations.includes(`${SITE_ORIGIN}/mcp`));
    assert.ok(locations.includes(`${SITE_ORIGIN}/pricing.md`));
    // The five dashboard variants are paths here, and each names /dashboard as
    // its canonical, so none of them is a separate sitemap entry.
    assert.ok(!STATIC_ROUTE_MANIFEST.some((route) => route.family === 'dashboard-variant'));
  ...
  it('generates byte-identical canonical entries from static owners and corpus page metadata', () => {
    ...
    try {
      ...
      assert.match(first, /<loc>https:\/\/worldmonitor\.sibt\.ai\/countries\/norway\/<\/loc>/);
      assert.match(first, /<loc>https:\/\/worldmonitor\.sibt\.ai\/tools\/natural-hazard-pulse\/<\/loc>/);
      ...
      assert.doesNotMatch(first, /worldmonitor\.sibt\.ai\/blog/);
      assert.doesNotMatch(first, /worldmonitor\.sibt\.ai\/docs/);
  ...
  it('keeps the committed artifact generated and robots ownership exact', () => {
    ...
    assert.doesNotMatch(sitemap, /worldmonitor\.sibt\.ai\/blog/);
    assert.match(sitemap, /<loc>https:\/\/worldmonitor\.sibt\.ai\/mcp<\/loc>/);

    for (const url of [
      `${SITE_ORIGIN}/sitemap.xml`,
      `${SITE_ORIGIN}/blog/sitemap-index.xml`,
      `${SITE_ORIGIN}/docs/sitemap-index.xml`,
    ]) {
