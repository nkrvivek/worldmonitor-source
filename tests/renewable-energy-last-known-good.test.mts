...
/**
 * The circuit breaker writes to persistent storage fire-and-forget, behind a
 * dynamic `import('../services/persistent-cache')` — see
 * src/utils/circuit-breaker.ts. The first write in a process also pays for
 * loading that module, so how many ticks pass before the value lands is not
 * fixed. Waiting exactly one tick raced it and failed about one run in three.
 *
 * Presence is polled with a deadline. Absence gets a plain settle: polling for
 * something that must never appear would return on the first read and prove
 * nothing, so that test waits a few ticks and then looks.
 */
async function settle(ticks = 1): Promise<void> {
  for (let i = 0; i < ticks; i++) {
    await new Promise(resolve => setTimeout(resolve, 0));
  }
}

/** Waits for a persisted key, then returns — the caller does the asserting. */
async function waitForStoredKey(key: string, deadlineMs = 2_000): Promise<void> {
  const startedAt = performance.now();
  while (storage.getItem(key) === null && performance.now() - startedAt < deadlineMs) {
    await settle();
  }
}
...
describe('renewable energy last-known-good recovery (#5497)', () => {
  it('stores a validated canonical response and reports it as live', async () => {
    ...
    const persistedKey = `worldmonitor-persistent-cache:breaker:${breakerName}:${RENEWABLE_DATA_CACHE_KEY}`;
    await waitForStoredKey(persistedKey);
  ...
  it('rejects last-known-good data outside the seven-day ceiling', async () => {
    ...
    await waitForStoredKey(
      `worldmonitor-persistent-cache:breaker:${breakerName}:${RENEWABLE_DATA_CACHE_KEY}`,
    );
