 ...
 *
 * The full flow (openSignIn + return redirect + doCheckout) requires a
 * browser plus the auth and payment SDKs, and is covered by manual smoke tests
...
describe('reviewer scenario coverage (regression guards)', () => {
  it('scenario 1: click paid tier signed-out → sign in → checkout auto-resumes', () => {
    ...
    // 2. The sign-in flow returns the user to returnUrl after success
  ...
  it('scenario 2: click paid → dismiss sign-in → later generic sign-in does NOT resume', () => {
    // 1. User click creates returnUrl — but the user never reaches the
    ...
    assert.equal(intent, null, 'no intent in URL because sign-in never redirected');
    // 2. Later, user clicks generic Sign In on /pro. That path calls
    //    openSignIn() with no return URL, so the modal closes in place
    //    and nothing navigates (NOT our intent URL).
