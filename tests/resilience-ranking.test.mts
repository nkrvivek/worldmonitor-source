...
describe('resilience ranking contracts', () => {
  ...
  // CounterDO path (task 7, #cf-ratelimit-do). Same dormant-seam shape as
  // api/mcp/quota.ts::reserveQuota and server/_shared/api-key-rate-limit.ts's
  // reserveDailyMeter — an optional trailing counter stub, Upstash EVAL kept
  // as the fallback. Unlike the daily-meter writers, this primitive has no
  // per-user namespace to shard on: the caller already selected the lock key,
  // so the fake stub below is a single-instance store, not a
  // idFromName()-keyed binding.
  describe('resilience ranking refresh lock — CounterDO compare-delete path (task 7, #cf-ratelimit-do)', () => {
    function makeCounterStub(initial: Record<string, string>) {
      const store = new Map(Object.entries(initial));
      const requestBodies: Array<{ op: string; key: string; expected: string }> = [];
      const stub = {
        async fetch(request: Request) {
          const body = JSON.parse(await request.text()) as { op: string; key: string; expected: string };
          requestBodies.push(body);
          const current = store.get(body.key);
          const deleted = current !== undefined && current === body.expected;
          if (deleted) store.delete(body.key);
          return new Response(JSON.stringify({ op: 'compare-delete', deleted }), { status: 200 });
        },
      };
      return { stub, requestBodies, store };
    }

    it('posts {op:"compare-delete", key, expected} with the raw=false key prefixed exactly like the Redis path', async () => {
      process.env.VERCEL_ENV = 'preview';
      process.env.VERCEL_GIT_COMMIT_SHA = 'abcdef12ffff';
      __resetKeyPrefixCacheForTests();
      const { stub, requestBodies } = makeCounterStub({ 'preview:abcdef12:lock-key': 'owner-a' });

      const result = await compareAndDeleteRedisKey('lock-key', 'owner-a', false, stub);

      assert.deepEqual(requestBodies, [{ op: 'compare-delete', key: 'preview:abcdef12:lock-key', expected: 'owner-a' }]);
      assert.equal(result, true);
    });

    it('raw=true sends the bare (un-prefixed) key', async () => {
      process.env.VERCEL_ENV = 'preview';
      process.env.VERCEL_GIT_COMMIT_SHA = 'abcdef12ffff';
      __resetKeyPrefixCacheForTests();
      const { stub, requestBodies } = makeCounterStub({ 'lock-key': 'owner-a' });

      const result = await compareAndDeleteRedisKey('lock-key', 'owner-a', true, stub);

      assert.deepEqual(requestBodies, [{ op: 'compare-delete', key: 'lock-key', expected: 'owner-a' }]);
      assert.equal(result, true);
    });

    it('a matching expected value deletes and returns true', async () => {
      const { stub, store } = makeCounterStub({ 'lock-key': 'owner-a' });
      assert.equal(await compareAndDeleteRedisKey('lock-key', 'owner-a', true, stub), true);
      assert.equal(store.has('lock-key'), false, 'matching token must delete the lock');
    });

    it('a non-matching expected value returns false and does not delete — a lock is a lock', async () => {
      const { stub, store } = makeCounterStub({ 'lock-key': 'owner-a' });
      assert.equal(await compareAndDeleteRedisKey('lock-key', 'owner-b', true, stub), false);
      assert.equal(store.get('lock-key'), 'owner-a', 'wrong token must not delete another owner lock');
    });

    it('a stub that throws returns false, matching every existing failure branch in this function', async () => {
      const throwingStub = {
        fetch: async () => {
          throw new Error('counter DO unreachable');
        },
      };
      assert.equal(await compareAndDeleteRedisKey('lock-key', 'owner-a', true, throwingStub), false);
    });

    it('with no stub passed, the existing Redis path still runs unchanged', async () => {
      const { redis } = installRedis({});
      redis.set('lock-key', 'owner-a');
      assert.equal(await compareAndDeleteRedisKey('lock-key', 'owner-b', true), false);
      assert.equal(await compareAndDeleteRedisKey('lock-key', 'owner-a', true), true);
      assert.equal(redis.has('lock-key'), false);
    });
  });
