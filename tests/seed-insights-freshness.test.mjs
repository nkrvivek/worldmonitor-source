...
import { BRIEF_REJECT_RULES } from '../scripts/_insights-brief.mjs';
...
// GATE covered eight distinct editorial rules with one word, and the rule name
// was written to container logs only. An operator reading the alarm could not
// tell a model that invented a proper noun from a gate that had become
// impossible to satisfy, and those two need opposite fixes.
test('a gate rejection records which editorial rule decided it', () => {
  const patch = buildInsightsFreshnessMetaPatch({
    previousMeta: { consecutiveFailures: 0 },
    outcome: 'lkg_preserved',
    failureCode: INSIGHTS_SYNTHESIS_FAILURE_CODES.GATE,
    rejectRule: 'lead-proper-noun',
    nowMs: 1_754_000_120_000,
  });

  assert.equal(patch.lastSynthesisFailureCode, INSIGHTS_SYNTHESIS_FAILURE_CODES.GATE);
  assert.equal(patch.lastSynthesisRejectRule, 'lead-proper-noun');
});

test('an unrecognized rule name is dropped, never defaulted', () => {
  // Same contract as the failure code. A wrong rule name is worse than no rule
  // name: it sends the reader at a test that did not fire. Model output must
  // never reach meta, so anything outside the vocabulary becomes null.
  for (const bogus of ['lead-proper-noun ', 'GATE', 'the model said Belgium', '', null, 42, {}]) {
    const patch = buildInsightsFreshnessMetaPatch({
      previousMeta: {},
      outcome: 'lkg_preserved',
      failureCode: INSIGHTS_SYNTHESIS_FAILURE_CODES.GATE,
    ...
    assert.equal(patch.lastSynthesisRejectRule, null, `expected ${JSON.stringify(bogus)} to be dropped`);
  }
});

test('every rule in the published vocabulary survives the round trip', () => {
  for (const rule of BRIEF_REJECT_RULES) {
    const patch = buildInsightsFreshnessMetaPatch({
      previousMeta: {},
      outcome: 'lkg_preserved',
      failureCode: INSIGHTS_SYNTHESIS_FAILURE_CODES.GATE,
    ...
    assert.equal(patch.lastSynthesisRejectRule, rule);
  }
});

test('publication clears the rule along with the code', () => {
  const patch = buildInsightsFreshnessMetaPatch({
    previousMeta: {
      consecutiveFailures: 4,
      lastSynthesisFailureCode: INSIGHTS_SYNTHESIS_FAILURE_CODES.GATE,
  ...
  assert.equal(patch.lastSynthesisFailureCode, null);
  assert.equal(patch.lastSynthesisRejectRule, null);
});
