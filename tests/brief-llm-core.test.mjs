...
describe('validateNoHallucinatedProperNouns — May 19 regression + class', () => {
  ...
  // Captured 2026-08-09: enforce mode rejected every L1 synthesis for two days
  // (seed-meta consecutiveFailures=90, both providers validate_reject). The
  // synthesis prompt asks the model to MERGE stories, and merged phrasing
  // compounds nouns the sources carry separately. Both fixtures below are the
  // actual rejected model output against the actual digest titles.
  it('COMPOUND (captured): hyphenated "Russia-Ukraine strikes" grounds against "Russia and Ukraine exchange strikes"', () => {
    const headline = "Russia and Ukraine exchange overnight strikes that leave 7 dead, many injured";
    const summary = "Russia-Ukraine strikes kill 7.";
    const r = validateNoHallucinatedProperNouns(summary, headline);
    assert.equal(r.ok, true, `unexpectedly flagged: ${JSON.stringify(r)}`);
  });

  it('COMPOUND (captured): possessive chain "Iran\'s Hormuz deal" grounds against a title carrying both nouns', () => {
    const headline = "Iran says Oman deal is in 'final stages' but US must act to open Hormuz - Reuters";
    const summary = "Iran's Hormuz deal is in 'final stages'.";
    const r = validateNoHallucinatedProperNouns(summary, headline);
    assert.equal(r.ok, true, `unexpectedly flagged: ${JSON.stringify(r)}`);
  });

  it('COMPOUND boundary: a compound with an ungrounded constituent stays flagged', () => {
    const headline = "Iran says Oman deal is in 'final stages' but US must act to open Hormuz - Reuters";
    const summary = "Iran-Israel talks are in 'final stages'.";
    const r = validateNoHallucinatedProperNouns(summary, headline);
    assert.equal(r.ok, false, 'summary introduced "Israel" not in headline — must flag');
  });

  it('COMPOUND boundary: an invented name pair stays flagged even near grounded nouns', () => {
    const headline = "Russia and Ukraine exchange overnight strikes that leave 7 dead, many injured";
    const summary = "Vladimir Putin's Ukraine strikes kill 7.";
    const r = validateNoHallucinatedProperNouns(summary, headline);
    assert.equal(r.ok, false, 'summary introduced "Vladimir Putin" not in headline — must flag');
  });
