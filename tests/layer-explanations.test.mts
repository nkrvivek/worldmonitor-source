...
function warmPingWorkflowMinutes(): number {
  const source = readSource('.github/workflows/warm-ping.yml');
  const match = source.match(/-\s*cron:\s*"([^"]+)"/);
  assert.ok(match, '.github/workflows/warm-ping.yml must declare a schedule cron');
  const [minuteField] = match[1].split(' ');
  const minutes = minuteField.split(',').map(Number);
  assert.ok(minutes.length > 1 && minutes.every(Number.isFinite), `warm-ping cron minute field must list fixed minutes: ${match[1]}`);
  const gaps = new Set(minutes.slice(1).map((m, i) => m - minutes[i]));
  assert.equal(gaps.size, 1, `warm-ping cron minutes must be evenly spaced: ${match[1]}`);
  const interval = 60 / minutes.length;
  assert.equal([...gaps][0], interval, `warm-ping cron spacing must cover the hour evenly: ${match[1]}`);
  return interval;
}
...
describe('layer explanation metadata', () => {
  ...
  test('curated freshness text is a data contract against seeder and runtime cadence sources', () => {
    ...
    // The Railway relay's 8-minute CII warm loop is dead; warm-ping.yml is
    // the live cadence source, and the 45-minute bound is 3x that cadence
    // (a303674e7: GitHub crons skip ticks, 2x guaranteed EMPTY windows).
    assertDuration(renderedFreshnessText('ciiChoropleth'), /every\s+([0-9]+)\s+(minute)s?/i, warmPingWorkflowMinutes(), 'CII warm-ping cadence');
    ...
    assert.equal(healthMaxStale('riskScores'), warmPingWorkflowMinutes() * 3, 'CII health budget should stay 3x the warm-ping workflow cadence');
