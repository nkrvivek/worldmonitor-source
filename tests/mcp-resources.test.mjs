...
import { UI_CONNECT_DOMAINS } from '../api/mcp/ui/shell.ts';
...
describe('api/mcp.ts — resources capability + stability + auth-symmetry', () => {
  ...
  it('resources/read ui:// response carries spec _meta.ui.csp with secure empty allowlists', async () => {
    ...
    // connectDomains mirrors the HTML meta CSP's connect-src (the hosts a widget
    // may fetch back to). Both sides read UI_CONNECT_DOMAINS, so this asserts they
    // agree rather than re-pinning the list a third time;
    // the other allowlists stay empty (secure default — app loads nothing external).
    assert.deepEqual(meta.ui.csp.connectDomains, [...UI_CONNECT_DOMAINS],
