...
describe('blog SEO and GEO corpus contract', () => {
  ...
  it('keeps blog JSON-LD attribution accurate', () => {
    const blogIndex = readFileSync(resolve(root, 'blog-site/src/pages/index.astro'), 'utf8');

    assert.ok(
      blogIndex.includes("import { DEFAULT_AUTHOR, DEFAULT_AUTHOR_URL, DEFAULT_AUTHOR_ID } from '../config/brand';"),
      'the default author must come from the fork-owned brand config, not a literal',
    );
    assert.ok(
    ...
    assert.ok(
      ...
      'blog JSON-LD must honor a custom authorUrl without assigning the default URL to custom authors',
    );
    assert.ok(
      blogIndex.includes('...(authorName === DEFAULT_AUTHOR ? { "@id": DEFAULT_AUTHOR_ID } : {})'),
      'blog JSON-LD must assign the stable publication ID only to the default author',
    );
  });
});
