...
describe('runner dispatch', () => {
  ...
  // `timeout` is GNU coreutils and macOS ships neither it nor `gtimeout`, so
  // both run modes exited 127 "timeout: command not found" here — the gate ran
  // no tests at all while reading as a plain failure. A stub runner that never
  // returns proves the limit is enforced on whatever platform this runs on.
  test('a runner that hangs is killed at the limit, on a machine with no GNU timeout', () => {
    const dir = fixtureDir('wm-prepush-timeout-');
    const stub = join(dir, 'npx');
    // `exec`, so the process the limit kills is the one that hangs. GNU timeout
    // signals its direct child too and leaves grandchildren alone; this asserts
    // the limit fires, not that it reaps a process tree.
    writeFileSync(stub, '#!/bin/sh\nexec sleep 60\n');
    chmodSync(stub, 0o755);
    ...
    let status = 0;
    try {
      execFileSync('bash', [SCRIPT, 'run-node'], {
        cwd: REPO_ROOT,
        input: nulList(['tests/handlers.test.mts']),
        encoding: 'utf8',
        env: { ...process.env, PATH: `${dir}:${process.env.PATH}`, WM_PREPUSH_TEST_TIMEOUT: '2' },
      });
    } catch (err) {
      status = err.status;
    }
    ...
    assert.equal(status, 1, 'a runner killed by the limit must fail the push');
    assert.ok(elapsedMs < 30_000, `the limit did not fire: waited ${elapsedMs}ms for a 2s limit`);
  });
