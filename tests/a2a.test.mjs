...
describe('a2a: agent card contract', () => {
  it('carries the required A2A v0.3.0 fields', () => {
    ...
    assert.equal(card.url, 'https://worldmonitor.sibt.ai/a2a');
...
describe('a2a: JSON-RPC endpoint', () => {
  ...
  // The bug this guards: an agent asks this fork which endpoint to call next
  // and is handed worldmonitor.app, a site we do not run.
  it('every link it hands back names the host that answered', async () => {
    const res = await handler(
      new Request('https://worldmonitor.sibt.ai/a2a', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          jsonrpc: '2.0',
          ...
          method: 'message/send',
          params: {
            message: { role: 'user', messageId: 'm-fork', parts: [{ kind: 'text', text: 'chokepoint status' }] },
          },
        }),
      }),
    );
    const body = await res.json();
    assert.ok(body.result, `expected result, got ${JSON.stringify(body.error)}`);
    assert.doesNotMatch(JSON.stringify(body), /worldmonitor\.app/);
    const data = body.result.parts.find((p) => p.kind === 'data').data;
    assert.equal(data.howToCall.mcp.endpoint, 'https://worldmonitor.sibt.ai/mcp');
    assert.equal(data.howToCall.rest.base, 'https://worldmonitor.sibt.ai');
  });

  // Off the fork, upstream's own two hosts come back unchanged.
  it('keeps the upstream apex/api split on an upstream request', async () => {
    const res = await rpc('message/send', {
      message: { role: 'user', messageId: 'm-up', parts: [{ kind: 'text', text: 'chokepoint status' }] },
    });
    const body = await res.json();
    const data = body.result.parts.find((p) => p.kind === 'data').data;
    assert.equal(data.howToCall.mcp.endpoint, 'https://worldmonitor.app/mcp');
    assert.equal(data.howToCall.rest.base, 'https://api.worldmonitor.app');
  });
