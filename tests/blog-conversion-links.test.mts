...
describe('blog conversion attribution', () => {
  ...
  it('keeps the shared templates on the measurable, post-launch CTA contract', () => {
    ...
    // No tracker script and no hostname allowlist in this layout. The blog is
    // deployed separately from the collector, which sends no CORS headers, and
    // tests/funnel-analytics-policy.test.mjs bars naming a host at all.
    assert.doesNotMatch(base, /data-domains|<script[^>]+src="https?:/);
