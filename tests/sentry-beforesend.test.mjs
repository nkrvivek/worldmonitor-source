...
describe('existing beforeSend filters', () => {
  ...
  it('suppresses a Supabase auth "Failed to fetch" even with a first-party frame', () => {
    // WORLDMONITOR-SA/SB (Clerk-era test, re-anchored on the auth SDK that
    // replaced it): supabase-js fetches its auth host for token refresh and
    // retries transient failures itself. A leaked
    // `Failed to fetch (twyzyvgqzzuvlvsftcjj.supabase.co)` is an
    // SDK-internal network blip, not our code. The auth chunk reads as
    // first-party (not in the vendor list), so the host allowlist — not
    // hasFirstParty — must decide.
    const event = makeEvent('Failed to fetch (twyzyvgqzzuvlvsftcjj.supabase.co)', 'TypeError', [
      { filename: '/assets/index-D1PHjyuu.js', lineno: 848, function: 'i' },
    ...
    assert.equal(beforeSend(event), null, 'Supabase auth fetch failure should be suppressed');
  });
