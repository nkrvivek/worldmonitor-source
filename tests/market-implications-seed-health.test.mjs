...
test('all three LLM-failure guards write an error seed-meta before returning', () => {
  ...
  // Guard 1: LLM returned no response. The reason is built from the last
  // attempt's provider fault rather than being the bare literal, because an
  // exhausted OpenRouter balance and an unresponsive model both land here and
  // `llm_no_response` alone describes only the second. Assert both halves: the
  // detail is carried when there is one, and `llm_no_response` still leads so
  // the existing errorReason prefix does not move.
  assert.match(
    buildRegion,
    /if \(!result\?\.text\) \{[\s\S]*?await writeMarketImplicationsFailureMeta\(reason\);\s*return;/,
  );
  assert.match(
    buildRegion,
    /const reason = result\?\.failureDetail\s*\?\s*`llm_no_response: \$\{result\.failureDetail\}`\s*:\s*'llm_no_response';/,
  );
