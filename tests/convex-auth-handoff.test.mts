...
/**
 * Install a Supabase client and session for one identity.
 *
 * The tests care about two things per account: which user id is selected, and
 * what its token fetch does (resolve late, change value, fail). Supabase keeps
 * both behind its client, so this builds the smallest client that routes
 * `getSession()` through the supplied fetcher, and pairs it with a session
 * carrying the user id. Tokens here have no `exp` claim, so `getAuthToken()`
 * treats them as usable and calls the fetcher exactly once per request — the
 * one-fetch-per-handoff bound these tests assert.
 */
/** A JWT whose `exp` sits in the past, so it is never served from storage. */
const STALE_TOKEN = `header.${Buffer.from(JSON.stringify({
  sub: 'user', exp: 1,
...
function setAuthIdentity(
  ...
  const fetchToken = identity.session?.getToken ?? (async () => null);
  const userId = identity.user?.id ?? 'user';
  const sessionFor = (accessToken: string) => ({
  ...
  // The stored token is stale on purpose. These tests count token fetches, so
  // every getAuthToken() must reach the fetcher — a token that still looked
  // fresh would be served from storage and the count would never move.
  const stored = sessionFor(STALE_TOKEN);
  const client = {
    auth: {
      ...
      refreshSession: async () => {
        ...
        return {
          data: { session: token === null ? null : sessionFor(token) },
          error: null,
        };
      },
  ...
  __setAuthForTests(client as never, stored as never);
}
...
describe('generation-scoped Convex auth handoff', () => {
  ...
  it('invalidates the old token before, but not during, the new token fetch', async () => {
    ...
    const cloudPrefsToken = getAuthToken();
    await flushMicrotasks();
  ...
  it('invalidates cached, in-flight, and Convex credentials on sign-out', async () => {
    ...
    const inFlightToken = getAuthToken();
    invalidateConvexAuthForSignOut();
    assert.equal(fake.authConfigs.length, 2);
