...
// This fork serves both the API and the web app off one Worker on one host.
// The defaults used to name upstream's two hosts, so every scheduled run
// measured someone else's deployment and reported it green.
const API_BASE = stripTrailingSlash(process.env.WM_LIVE_API_BASE_URL || 'https://worldmonitor.sibt.ai');
const WEB_BASE = stripTrailingSlash(process.env.WM_LIVE_WEB_BASE_URL || 'https://worldmonitor.sibt.ai');
...
/**
 * The shared cache on this host is the Worker itself (worker/edge-cache.ts).
 * A Worker-generated response never enters Cloudflare's own cache, so
 * `cf-cache-status` is absent on every one of these URLs and reading only
 * that header made the sweep report "never cached" for a cache that was
 * working. Upstream's two CDN headers stay in the check so the same suite
 * still measures a Vercel or CDN-fronted host.
 */
function edgeCacheStatus(resp) {
  return resp.headers.get('x-wm-edge-cache') || '';
}

function isSharedCacheHit(resp) {
  return cfCacheStatus(resp).toUpperCase() === 'HIT'
    || vercelCacheStatus(resp).toUpperCase() === 'HIT'
    || edgeCacheStatus(resp).toUpperCase() === 'HIT';
}
...
/**
 * Fetch a fake-auth probe, re-issuing while the origin answers 503.
 *
 * A `wm_` key is validated against Convex, and api/bootstrap.js deliberately
 * surfaces a Convex outage as a retryable 503 + Retry-After rather than a
 * misleading "Invalid API key" 401. So a 503 here says the key store was
 * unreachable for those few seconds — it says nothing about the cache/auth
 * posture this suite exists to measure, and asserting 401 straight through
 * turns someone else's outage into a red that no commit can fix. Run
 * 31113704707 failed exactly that way at 08:00 PT while the run six hours
 * earlier and a probe of the same URL afterwards both passed.
 *
 * Each attempt busts the cache key, so a retry cannot be answered from
 * anything the previous attempt warmed. A 503 that survives every attempt
 * still fails, and names why.
 */
async function fetchFakeAuthRejection(url, name) {
  let result;
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    result = await fetchText(bust(url), {
      headers: { 'X-WorldMonitor-Key': FAKE_WM_KEY },
    });
    if (result.resp.status !== 503) return result;
    await new Promise((resolve) => setTimeout(resolve, 1_000 * attempt));
  }
  assert.fail(
    `${name}: origin answered 503 on all 3 attempts — the key store stayed unreachable, `
      + 'so this run could not measure auth posture at all',
  );
  return result;
}
...
async function waitForSharedCacheHit(url, name) {
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    ...
    attempts.push(
      `cf=${cfCacheStatus(result.resp) || '-'},vercel=${vercelCacheStatus(result.resp) || '-'},edge=${edgeCacheStatus(result.resp) || '-'}`,
    );
...
describe(`live API cache/auth regression sweep (${LIVE ? 'ENABLED' : 'SKIPPED - set LIVE_API_CACHE_TESTS=1'})`, { skip: !LIVE }, () => {
  ...
  it('bootstrap rejects fake auth as dynamic no-store while public weather stays cacheable', async () => {
    const fake = await fetchFakeAuthRejection(`${API_BASE}/api/bootstrap?keys=weatherAlerts`, 'bootstrap fake auth');
  ...
  it('generated RPCs reject fake auth as dynamic no-store while public no-auth RPCs stay cacheable', async () => {
    const fake = await fetchFakeAuthRejection(`${API_BASE}/api/market/v1/list-market-quotes?symbols=AAPL`, 'generated RPC fake auth');
    assert.equal(fake.resp.status, 401);
  ...
  it('premium RPC fake auth fails closed without shared cache headers', async () => {
    const fake = await fetchFakeAuthRejection(`${API_BASE}/api/market/v1/analyze-stock?symbol=AAPL`, 'premium RPC fake auth');
    assert.equal(fake.resp.status, 401);
  ...
  it('MCP OPTIONS, public discovery, and gated data method are protocol-valid no-store responses', async () => {
    ...
    assert.equal(isSharedCacheHit(discover.resp), false, 'anonymous discovery 200 must not be a shared-cache HIT');
