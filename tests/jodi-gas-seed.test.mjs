...
describe('parseCsvRows', () => {
  ...
  it('keeps assessment_code 3, which JODI defines as not yet assessed', () => {
    const csv = [
      SAMPLE_CSV_HEADER,
      makeRow('DE', '2025-10', 'IMPLNG', 'TJ', '95000', '1'),
    ...
    const rows = parseCsvRows(csv);
    assert.equal(rows.length, 2, 'an unassessed flow is still a reported flow');
    assert.deepEqual(
      rows.map(r => r.assessmentCode),
      ['1', '3'],
      'the level stays on the row instead of being discarded',
    );
  });

  it('excludes a code JODI does not publish', () => {
    const csv = [
      SAMPLE_CSV_HEADER,
      makeRow('DE', '2025-10', 'IMPLNG', 'TJ', '95000', '1'),
      makeRow('DE', '2025-10', 'IMPPIP', 'TJ', '380000', '6'),
      makeRow('DE', '2025-10', 'PRODUCTION', 'TJ', '900000', ''),
    ].join('\n');
    const rows = parseCsvRows(csv);
    assert.equal(rows.length, 1, 'unknown and blank levels are excluded, never assumed');
    assert.equal(rows[0].flow, 'IMPLNG');
  });
...
describe('resolveLatestGasEdition', () => {
  const upTo = highest => async url => Number(url.match(/gas\/(\d+)\//)[1]) <= highest;

  it('takes the highest contiguous edition at or above the floor', async () => {
    const edition = await resolveLatestGasEdition(upTo(ZIP_EDITION_FLOOR + 3));
    assert.equal(edition, ZIP_EDITION_FLOOR + 3);
  });

  it('accepts the floor itself when nothing newer is published yet', async () => {
    assert.equal(await resolveLatestGasEdition(upTo(ZIP_EDITION_FLOOR)), ZIP_EDITION_FLOOR);
  });

  it('never falls back to an older edition when the floor is gone', async () => {
    // Stale editions answer 200 forever, so a silent fallback is how the seeder
    // downloaded four-month-old data every month without a single error.
    await assert.rejects(
      () => resolveLatestGasEdition(upTo(ZIP_EDITION_FLOOR - 1)),
      /publisher numbering changed/,
    );
  });

  it('stops at the first gap rather than probing past it', async () => {
    const asked = [];
    const exists = async url => {
      const edition = Number(url.match(/gas\/(\d+)\//)[1]);
      asked.push(edition);
      return edition < ZIP_EDITION_FLOOR + 2;
    };
    assert.equal(await resolveLatestGasEdition(exists), ZIP_EDITION_FLOOR + 1);
    assert.deepEqual(asked, [ZIP_EDITION_FLOOR, ZIP_EDITION_FLOOR + 1, ZIP_EDITION_FLOOR + 2]);
  });

  it('builds the published ZIP path', () => {
    assert.equal(
      gasZipUrl(24),
