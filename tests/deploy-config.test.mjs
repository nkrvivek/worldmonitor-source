...
// Every .mdx under docs/ becomes a published page, English and translated
// alike. Walk the tree rather than list filenames so a new page is covered the
// day it lands.
function collectMdxPages(dir) {
  const found = [];
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const full = resolve(dir, entry.name);
    if (entry.isDirectory()) found.push(...collectMdxPages(full));
...
// Upstream's app roots, plus this fork's own host — every variant is a path
// here, so one alternation covers the whole deployment.
const APP_ROOT_HOST_PATTERN =
  '^(?:(?:www|tech|finance|commodity|happy|energy)\\.)?worldmonitor\\.app$|^worldmonitor\\.sibt\\.ai$';
...
// Read the values, not the file text. The old regexes only matched single-quoted
// literals, so building a URL from a constant made every variant read as absent.
const getVariantHosts = () => (
  [...new Set(Object.values(VARIANT_META).map((meta) => new URL(meta.url).host))].sort()
);

const getVariantUrls = () => (
  Object.fromEntries(Object.entries(VARIANT_META).map(([variant, meta]) => [variant, meta.url]))
);
...
describe('crawlable content corpus deployment contracts', () => {
  ...
  it('keeps robots.txt advertising the root, blog, and docs sitemaps', () => {
    assert.ok(robotsSource.includes(`\nSitemap: ${SITE_ORIGIN}/sitemap.xml\n`));
    assert.ok(robotsSource.includes(`\nSitemap: ${SITE_ORIGIN}/blog/sitemap-index.xml\n`));
    // Both the blog and the docs are Astro builds, and @astrojs/sitemap names
    // its index sitemap-index.xml.
    assert.ok(robotsSource.includes(`\nSitemap: ${SITE_ORIGIN}/docs/sitemap-index.xml\n`));
  });
  ...
  it('keeps the root sitemap generated while delegating blog and docs inventories', () => {
    ...
    assert.ok(!sitemapSource.includes(`${SITE_ORIGIN}/blog/`));
    assert.ok(!sitemapSource.includes(`${SITE_ORIGIN}/docs/`));
  ...
  it('discovers canonical generated corpus pages and validates changelog pagination links', () => {
    ...
    try {
      writeFixturePage(
        ...
        `<link rel="canonical" href="${SITE_ORIGIN}/countries/ukraine/" /><meta name="lastmod" content="2026-07-08" />`
      );
      writeFixturePage(
        publicDir,
        'chokepoints/suez-canal/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/chokepoints/suez-canal/" />`
      );
      writeFixturePage(
        publicDir,
        'crises/ukraine-war/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/crises/ukraine-war/" />`
      );
      writeFixturePage(
        publicDir,
        'tools/natural-hazard-pulse/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/tools/natural-hazard-pulse/" />`
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/1/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/reference/changelog/page/1/" /><link rel="next" href="${SITE_ORIGIN}/reference/changelog/page/2/" />`
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/2/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/reference/changelog/page/2/" /><link rel="prev" href="${SITE_ORIGIN}/reference/changelog/page/1/" />`
      );

      const pages = discoverContentCorpusPages({ publicDir });
      const locations = pages.map((page) => page.loc).sort();
      assert.deepEqual(locations, [
        `${SITE_ORIGIN}/reference/changelog/page/1/`,
        `${SITE_ORIGIN}/reference/changelog/page/2/`,
        `${SITE_ORIGIN}/chokepoints/suez-canal/`,
        `${SITE_ORIGIN}/crises/ukraine-war/`,
        `${SITE_ORIGIN}/countries/ukraine/`,
        `${SITE_ORIGIN}/tools/natural-hazard-pulse/`,
      ].sort());

      writeFixturePage(
        publicDir,
        'reference/changelog/page/3/index.html',
        `<link rel="canonical" href="${SITE_ORIGIN}/reference/changelog/page/3/" />`
      );
      assert.throws(
...
// docs-site shipped for months with nothing installing its dependencies:
// `npm run build` calls build:docs, build:docs runs `cd docs-site && npm run
// build`, and postinstall named only blog-site. On any machine that had not
// installed docs-site by hand the build died with "sh: astro: command not
// found". Anything the build chain cd's into needs someone to install it, and
// nothing was checking which sub-packages that set contains.
describe('sub-package install coverage', () => {
  const repoRoot = resolve(__dirname, '..');

  // Follows `npm run X` from a root script and returns every command string
  // the chain actually runs, so a sub-package added one level down is still
  // seen.
  const expandScript = (name, seen = new Set()) => {
    ...
    if (!script) return [];
    const nested = [...script.matchAll(/npm run ([\w:-]+)/g)]
      .flatMap((match) => expandScript(match[1], seen));
    return [script, ...nested];
  };

  it('installs every sub-package the build chain enters', () => {
    const postinstall = packageJson.scripts.postinstall ?? '';

    for (const rootScript of ['build', 'build:full']) {
      for (const command of expandScript(rootScript)) {
        for (const match of command.matchAll(/cd\s+([\w.-]+)\s*&&/g)) {
          const directory = match[1];
          if (!existsSync(join(repoRoot, directory, 'package.json'))) continue;
          assert.ok(
            postinstall.includes(directory) || /npm (ci|install)/.test(command),
            `${rootScript} enters ${directory}/ but nothing installs it — `
              + 'add it to postinstall or give its own script an npm ci',
          );
        }
      }
    }
...
describe('deploy/cache configuration guardrails', () => {
  ...
  it('serves /pro hashed assets immutable — broader /pro rules must not override', () => {
    ...
    // AFTER the /pro catch-alls or every hashed chunk (including the auth
    // SDK) is re-downloaded on each repeat visit.
    assert.equal(
      effectiveCacheControl('/pro/assets/supabase-abc123.js'),
      'public, max-age=31536000, immutable'
    );
    assert.equal(
  ...
  it('keeps the lazy auth SDK out of the PWA precache', () => {
    assert.match(viteConfigSource, /globIgnores:\s*\[[^\]]*'\*\*\/supabase-\*\.js'[^\]]*\]/s);
    assert.match(
      viteConfigSource,
      /if\s*\(\s*id\.includes\('\/@supabase\/'\)\s*\)\s*\{[^{}]*\breturn 'supabase';\s*\}/s
    );
  });
...
describe('welcome landing page routing', () => {
  ...
  // This fork serves every variant as a path on one host instead of on its
  // own subdomain, so each canonical is /<variant> rather than /dashboard.
  it('keeps variant canonicals aligned with the one-host path routing', () => {
    const variantUrls = getVariantUrls();
    assert.equal(variantUrls.full, `${SITE_ORIGIN}/dashboard`);
    ...
    for (const [variant, url] of nonFullUrls) {
      ...
      assert.equal(parsed.origin, SITE_ORIGIN, `${variant} canonical must stay on this deployment`);
      assert.equal(
        parsed.pathname,
        `/${variant}`,
        `${variant} canonical must point at /${variant} while the root serves welcome`
      );
    }
  });
  ...
  it('sitemap lists dashboard routes and does not list legacy /welcome', () => {
    ...
    assert.ok(
      sitemap.includes(`<loc>${SITE_ORIGIN}/dashboard</loc>`),
      `public/sitemap.xml must list ${SITE_ORIGIN}/dashboard`
    );
    // Upstream gave each dashboard variant its own subdomain and its own
    // sitemap entry. Here they are paths off one host, and each answers with
    // /dashboard as its canonical, so /dashboard is the only entry.
    for (const variant of ['tech', 'finance', 'commodity', 'happy', 'energy']) {
      assert.ok(
        !sitemap.includes(`<loc>${SITE_ORIGIN}/${variant}</loc>`),
        `public/sitemap.xml must not list /${variant}, which canonicalizes to /dashboard`
      );
    }
    assert.ok(
      !sitemap.includes(`<loc>${SITE_ORIGIN}/welcome</loc>`),
      `public/sitemap.xml must not list legacy ${SITE_ORIGIN}/welcome`
    );
  });
  ...
  it('pins welcome and dashboard SEO canonicals to their new routes', () => {
    ...
    assert.ok(
      welcomeHtml.includes(`<link rel="canonical" href="${SITE_ORIGIN}/" />`),
      'welcome source must canonicalize to root'
    );
    assert.ok(
      !welcomeHtml.includes(`${SITE_ORIGIN}/welcome`),
      'welcome source must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes(`<link rel="canonical" href="${SITE_ORIGIN}/" />`),
      'generated welcome HTML must canonicalize to root'
    );
    assert.ok(
      !generatedWelcomeHtml.includes(`${SITE_ORIGIN}/welcome`),
      'generated welcome HTML must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes(`${SITE_ORIGIN}/dashboard`),
      'generated welcome HTML must launch the dashboard at /dashboard'
    );
    assert.ok(
      dashboardHtml.includes(`<link rel="canonical" href="${SITE_ORIGIN}/dashboard" />`),
  ...
  it('redirects signed-in welcome visitors to /dashboard client-side without loading the auth SDK', () => {
    const welcomeApp = readFileSync(resolve(__dirname, '../pro-test/src/WelcomeApp.tsx'), 'utf-8');
    // The auth SDK must NOT be on the welcome critical path (issue #4428):
    // the redirect is decided from the stored session alone. auth-session.ts
    // reads that storage key without pulling supabase-js in.
    assert.ok(!welcomeApp.includes("import('./services/auth')"));
    assert.ok(!welcomeApp.includes("from './services/auth'"));
    assert.ok(!welcomeApp.includes("import('./services/checkout')"));
    assert.ok(welcomeApp.includes('maybeRedirectWelcomeVisitor(readStoredSession(), window.location)'));
  });
});
...
describe('security header guardrails', () => {
  ...
  it('Permissions-Policy disables all expected browser APIs', () => {
    ...
    const expectedDisabled = [
      ...
      'magnetometer=()',
      'midi=()',
      'payment=(self "https://pay.google.com" "https://hooks.stripe.com" "https://js.stripe.com")',
      'screen-wake-lock=()',
      'serial=()',
      'usb=()',
  ...
  it('grants no auth-provider frame origin — sign-in renders in our own DOM', () => {
    // The sign-in modal is first-party markup and Google OAuth is a full-page
    // redirect, so nothing auth-related is framed. The previous provider needed
    // two frame origins; leaving them behind would widen frame-src for nothing.
    const csp = getHeaderValue('Content-Security-Policy');
    const frameSrc = csp.match(/frame-src\s+([^;]+)/)?.[1] ?? '';
    assert.ok(frameSrc, 'CSP must declare frame-src');
    assert.doesNotMatch(frameSrc, /clerk|supabase\.co/);

    const nginxCsp = getNginxHeaderValue('Content-Security-Policy');
    assert.ok(nginxCsp, 'nginx-security-headers.conf must have a Content-Security-Policy header');
    const nginxFrameSrc = nginxCsp.match(/frame-src\s+([^;]+)/)?.[1] ?? '';
    assert.ok(nginxFrameSrc, 'nginx CSP must declare frame-src');
    assert.doesNotMatch(nginxFrameSrc, /clerk|supabase\.co/);
  });
...
describe('agent readiness: api-catalog + openapi build', () => {
  ...
  // Upstream split the site across worldmonitor.app, www. and api. — this fork
  // serves all three roles from one origin, so the API context object is
  // anchored at the site root.
  const apiEntry = apiCatalog.linkset.find((entry) => entry.anchor === `${SITE_ORIGIN}/`);

  it('linkset[0] is the catalog anchor and enumerates each API via RFC 9727 item links', () => {
    assert.equal(catalogEntry.anchor, `${SITE_ORIGIN}/.well-known/api-catalog`);
    ...
    assert.ok(itemHrefs.includes(`${SITE_ORIGIN}/`), 'item list must enumerate the REST API host root');
    assert.ok(itemHrefs.includes(`${SITE_ORIGIN}/mcp`), 'item list must enumerate the MCP server');
  });

  // Upstream advertised a docs MCP server at /docs/mcp (#4958). This fork does
  // not serve it: api/docs-mcp.ts proxies worldmonitor.mintlify.dev, which is
  // upstream's docs instance, and the route was never ported — a live request
  // to /docs/mcp returns 530 (Origin DNS error, vercel-origin.worldmonitor.app).
  // Advertising a dead endpoint is the same drift #4958 was written to catch,
  // pointed the other way, so the corpus must stay silent about it until the
  // route answers.
  it('does not advertise the docs MCP server, which this fork does not serve', () => {
    for (const surface of ['llms.txt', 'agents.md', 'api/llms.txt', 'agent.txt', 'developers/llms.txt', 'mcp-server.md']) {
      const content = readFileSync(resolve(__dirname, `../public/${surface}`), 'utf-8');
      // Anchored at the end of the path: /docs/mcp-jmespath and friends are
      // real doc pages and stay advertised.
      assert.doesNotMatch(content, /\/docs\/mcp(?![\w-])/, `public/${surface} must not advertise /docs/mcp`);
    }
    assert.ok(
      !existsSync(resolve(__dirname, '../public/.well-known/mcp/docs-server-card.json')),
      'the docs server-card must not ship while /docs/mcp returns 530'
    );
    const catalogAnchors = apiCatalog.linkset.map((e) => e.anchor);
    assert.ok(
      !catalogAnchors.some((a) => a.endsWith('/docs/mcp')),
      'api-catalog must not anchor a context object at /docs/mcp'
    );
    // The published docs are a surface too, and the six public/* files above
    // did not cover them: docs/sandbox.mdx told readers the endpoint was
    // "entirely public" for as long as it returned 530, in both languages.
    // Every page here ships to /docs, so it advertises as loudly as llms.txt.
    for (const page of collectMdxPages(resolve(__dirname, '../docs'))) {
      const content = readFileSync(page, 'utf-8');
      assert.doesNotMatch(
        content,
        /\/docs\/mcp(?![\w-])/,
        `${relative(resolve(__dirname, '..'), page)} must not advertise /docs/mcp`
      );
    }
  });

  // This fork signs in against Supabase; Clerk was deleted with
  // src/services/clerk.ts. The agent corpus kept telling callers that
  // `Authorization: Bearer` carries "Clerk JWTs" in 25 skill files, and that
  // Pro accounts come from "Clerk sign-up" in llms.txt — an agent that read it
  // and went looking for a Clerk tenant would find nothing.
  it('the public corpus names the auth provider this fork actually runs', () => {
    const publicDir = resolve(__dirname, '../public');
    const collect = (dir) => {
      const found = [];
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        // public/docs and public/pro are build output, not source: their
        // contents are rewritten by `npm run build:docs` and the pro-test
        // build, so fixing a string here would not stick.
        if (entry.name === 'docs' || entry.name === 'pro') continue;
        const full = resolve(dir, entry.name);
        if (entry.isDirectory()) found.push(...collect(full));
        else if (/\.(md|txt)$/.test(entry.name)) found.push(full);
      }
      return found;
    };
    for (const page of collect(publicDir)) {
      assert.doesNotMatch(
        readFileSync(page, 'utf-8'),
        /clerk/i,
        `${relative(resolve(__dirname, '..'), page)} names Clerk; this fork runs Supabase auth`
      );
    }
  });

  it('the api host root has its own context object', () => {
    assert.ok(apiEntry, `linkset must contain a context object anchored at ${SITE_ORIGIN}/`);
  });
  ...
  it('status href points at the KEYLESS compact form of /api/health', () => {
    ...
    assert.ok(
      statusHref.startsWith(SITE_ORIGIN),
      `status href must be on ${SITE_HOST}, got: ${statusHref}`
    );
    assert.equal(
      statusHref,
      `${SITE_ORIGIN}/api/health?compact=1`,
  ...
  it('service-meta advertises the machine-readable pricing + support surfaces', () => {
    ...
    assert.ok(hrefs.includes(`${SITE_ORIGIN}/pricing.md`), 'service-meta must advertise pricing.md');
    assert.ok(
      hrefs.includes(`${SITE_ORIGIN}/api/product-catalog`),
      'service-meta must advertise the live product-catalog JSON endpoint'
    );
    assert.ok(hrefs.includes(`${SITE_ORIGIN}/support.md`), 'service-meta must advertise support.md');
    assert.ok(hrefs.includes(`${SITE_ORIGIN}/agents.md`), 'service-meta must advertise agents.md (#4952)');
    ...
    const commerceSpec = meta.find(
      (entry) => entry.href === `${SITE_ORIGIN}/docs/openapi/CommerceService.openapi.yaml`
    );
  ...
  it('has a second anchor for the MCP server-card', () => {
    const mcpEntry = apiCatalog.linkset.find((entry) => entry.anchor === `${SITE_ORIGIN}/mcp`);
    assert.ok(mcpEntry, `linkset must contain an anchor for ${SITE_ORIGIN}/mcp`);
...
describe('agent readiness: MCP/OAuth origin alignment', () => {
  ...
  it('MCP server card authentication.resource is a valid https URL on a known host', () => {
    ...
    // Our own committed card, so it names the one host this fork serves.
    assert.equal(u.host, SITE_HOST, `unexpected host: ${u.host}`);
  });
...
describe('agent readiness: auth.md walkthrough', () => {
  ...
  it('advertises a register endpoint that resolves (matches the agent_auth register_uri path)', () => {
    assert.ok(
      authMd.includes(`${SITE_ORIGIN}/oauth/register`),
  ...
  it('serves /agent.txt as plain text and keeps it off the SPA catch-all', () => {
    ...
    assert.ok(agentTxt.includes(`${SITE_ORIGIN}/mcp`), 'agent.txt must point at the MCP server');
  });
});
...
describe('agent readiness: homepage Link headers', () => {
  ...
  for (const source of ['/', '/dashboard', '/dashboard.html']) {
    it(`${source} emits a Link header`, () => {
      ...
      // Upstream advertised a second MCP surface here (#4958/#4964), anchored
      // at /docs/mcp. This fork does not serve that route — it returns 530 —
      // so the header must not name it or the card that describes it.
      assert.ok(
        !linkHeader.value.includes('docs-server-card'),
        'Link header must not advertise the docs server-card while /docs/mcp returns 530'
      );
      ...
      // Upstream also double-advertised mcp-server-card with the docs card;
      // that one is gone here because /docs/mcp is not served.
...
describe('api test coverage', () => {
  // Every api test file must be named by some npm script, and every api test
  // file a script names must exist. Both halves failed at once: test:sidecar
  // still listed api/cyber-threats.test.mjs and api/usni-fleet.test.mjs, which
  // #106 deleted, while api/analytics-health, api/bootstrap-cors,
  // api/health-cors and api/og-story were run by nothing at all. `node --test`
  // prints "Could not find <path>" for a missing argument and still exits 0, so
  // the script reported green while running four fewer files than it claimed
  // and skipping four more it never knew about.
  //
  // This replaces a .vercelignore guard. Upstream deploys api/ on Vercel, where
  // every non-underscore file becomes a live serverless function and a deployed
  // *.test.mjs answers requests by running its whole suite against production
  // env (WORLDMONITOR-VD). This fork routes api/ handlers by explicit import in
  // worker/index.ts and worker/routes/, so no file ships by being present, and
  // .vercelignore was deleted with the guard that read it.
  const repoRoot = resolve(__dirname, '..');
  ...
  const apiTestFiles = collectApiTestFiles(resolve(repoRoot, 'api'))
    .map((file) => relative(repoRoot, file));

  const allScripts = Object.values(packageJson.scripts).join(' ');
  const apiPathsNamedByScripts = [
    ...new Set([...allScripts.matchAll(/\bapi\/[\w./-]+\.test\.[cm]?[jt]sx?\b/g)].map((m) => m[0])),
  ];

  it('runs every api test file', () => {
    assert.ok(apiTestFiles.length > 0, 'expected api test files to exist (walker broke?)');
    for (const file of apiTestFiles) {
      assert.ok(
        apiPathsNamedByScripts.includes(file),
        `${file}: no npm script runs this — add it to test:sidecar (node:test) or test:data (tsx)`,
      );
    }
  });

  it('names no api test file that has been deleted', () => {
    for (const named of apiPathsNamedByScripts) {
      assert.ok(
        existsSync(join(repoRoot, named)),
        `${named}: named by an npm script but missing from disk — node --test exits 0 on a `
          + 'path it cannot find, so the script would keep reporting green',
      );
    }
  });
});
...
describe('agent readiness: registry branding + ARD catalog', () => {
  ...
  it('server-card carries the full branding trio and the icon asset exists', () => {
    ...
    assert.ok(
      (serverCard.icon ?? '').startsWith(`${SITE_ORIGIN}/`),
      `server-card icon must be an absolute ${SITE_HOST} URL`
    );
  ...
  it('ai-catalog.json declares the World Monitor host identity', () => {
    ...
    assert.strictEqual(aiCatalog.host?.identifier, `did:web:${SITE_HOST}`);
  ...
  it('every ai-catalog entry is domain-anchored and complete', () => {
    for (const entry of aiCatalog.entries) {
      ...
      assert.match(
        entry.identifier ?? '',
        new RegExp(`^urn:air:${SITE_HOST.replace(/\./g, '\\.')}:[a-z-]+:[a-z0-9-]+$`),
      ...
      assert.ok(
        (entry.url ?? '').startsWith(`${SITE_ORIGIN}/`),
...
describe('docs host scoping — the docs are www-only (#5345)', () => {
  // The docs used to be a host-blind proxy to Mintlify, so every variant
  // subdomain served the full docs site and Googlebot crawled the duplicates.
  // They are our own build now, served from the assets, and the same problem
  // would apply. Redirects run before both rewrites and asset lookups, so this
  // host-scoped redirect is what keeps subdomain /docs requests off the docs.
  // The host list is derived from the /dashboard variant rewrites so a new
  // variant subdomain cannot ship outside the docs redirect.
  ...
  it('the host condition covers every variant subdomain plus api., and never www.', () => {
    ...
    assert.ok(!hostRe.test('www.worldmonitor.app'), 'www must keep serving the docs');
  });

  it('sends prefix-less /api-reference/* to the docs home on www', () => {
    // Mintlify's RSC flight payload embedded its internal routes without the
    // /docs base path, and Googlebot still fetches those strings against
    // whatever host served the page. Without this redirect they 404 (the SPA
    // catch-all excludes ^api), flooding GSC. Our own build has no page family
    // at those paths, so they land on the docs home rather than a deep link.
    ...
    assert.equal(redirect.destination, 'https://www.worldmonitor.app/docs/documentation/');
    assert.equal(redirect.permanent, true);
    assert.equal(redirect.has, undefined, 'must apply on every host, www included');
  });

  it('no rewrite sends /docs off-origin any more', () => {
    const offOrigin = vercelConfig.rewrites.filter(
      (r) => String(r.source).startsWith('/docs') && /^https?:/.test(String(r.destination))
    );
    assert.deepEqual(offOrigin, [], 'docs pages are served from our own build');
  });
});
...
describe('markdown canonical Link headers (#4999)', () => {
  ...
  it('every sitemap-listed .md URL has the canonical Link header rule', () => {
    const sitemap = readFileSync(resolve(__dirname, '../public/sitemap.xml'), 'utf-8');
    const mdPattern = new RegExp(`<loc>${SITE_ORIGIN}(/[^<]+\\.md)</loc>`, 'g');
    const mdUrls = [...sitemap.matchAll(mdPattern)].map((m) => m[1]);
...
describe('agent readiness: named developer-resource pages (#4953)', () => {
  ...
  it('advertises the developer portal + resource pages across the discovery chain', () => {
    ...
    for (const page of DEV_PAGES) {
      ...
      assert.ok(
        sitemap.includes(`${SITE_ORIGIN}${page.path}`),
        `sitemap.xml must register ${page.path}`
      );
...
describe('section-scoped llms.txt files', () => {
  ...
  it('the site-wide llms.txt cross-links every section file and the sandbox', () => {
    ...
    for (const url of [
      ...
      `${SITE_ORIGIN}/docs/llms.txt`,
      `${SITE_ORIGIN}/developers/llms.txt`,
      `${SITE_ORIGIN}/blog/llms.txt`,
      `${SITE_ORIGIN}/sandbox/index.json`,
      `${SITE_ORIGIN}/schemamap.xml`,
    ]) {
