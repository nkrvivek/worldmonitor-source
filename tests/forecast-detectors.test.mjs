...
describe('forecast llm overrides', () => {
  ...
  it('pins critical_signals to the pre-#4944 chain (probability-coupled stage)', () => {
    ...
    // forecast probabilities, so the DeepSeek migration must not reach it
    // before the #4930 resolver baseline exists.
    //
    // The groq slot is gone rather than re-pinned. It held
    // `llama-3.1-8b-instant`, and on 2026-08-24 a probe of
    // GET https://api.groq.com/openai/v1/models found Groq had dropped every
    // Llama id it used to serve, so that entry had been answering 404 and
    // falling through to openrouter for weeks. Naming a current Groq model
    // here would move a probability-coupled stage onto a model the #4930
    // baseline never scored. Dropping it keeps the behaviour the stage
    // already had and stops paying for a call that always fails.
    ...
    assert.deepEqual(
      ...
      'pinned OpenRouter model keeps the mandatory provider policy without adding a reasoning override',
    );
    assert.equal(
      providers.find((p) => p.name === 'groq'),
      undefined,
      'no groq entry: every Llama id it served was decommissioned, and a current one was never baselined here',
    );
  });
  ...
  it('supports a stronger combined-model override without changing scenario defaults', () => {
    ...
    assert.equal(scenarioProviders[1]?.model, 'openai/gpt-oss-120b');
  });
