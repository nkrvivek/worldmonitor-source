...
// This fork does not deploy to Vercel, so no *.vercel.app origin may pass the
// allowlist. Browsers send Origin on the POST to /api/wm-session, and a stale
// entry here is a standing hole in a security allowlist for a host nobody here
// controls.
describe('isAllowedOrigin — no Vercel origin passes the allowlist', () => {
  ...
  const REJECTED = [
    ...
    ['suffix-spoofed origin', 'https://worldmonitor-git-feature-eliewm.vercel.app.evil.com'],
    ['dead personal-scope preview', 'https://worldmonitor-feature-elie-abc123.vercel.app'],
  ];
...
describe('isAllowedOrigin — worldmonitor.sibt.ai (Cloudflare Worker host, Plan 4d Task 6)', () => {
  // Origin for the JS twin (api/_cors.js exports isDisallowedOrigin, not the
  // bare predicate) — same allow/deny outcome proves both files stay in sync.
  const allowedByJsTwin = (origin: string) =>
    !isDisallowedOriginJs(new Request('https://worldmonitor.app/x', { headers: { Origin: origin } }));
  ...
  const ALLOWED = [
    ['apex sibt.ai host', 'https://worldmonitor.sibt.ai'],
    ['subdomain of the sibt.ai host', 'https://tech.worldmonitor.sibt.ai'],
  ];

  const REJECTED = [
    ['bare sibt.ai (no worldmonitor segment)', 'https://sibt.ai'],
    ['unrelated app under sibt.ai', 'https://other-app.sibt.ai'],
    ['suffix-spoofed sibt.ai origin', 'https://worldmonitor.sibt.ai.evil.com'],
  ];
...
describe('CORS triplet parity — all three twins carry the same host list', () => {
  // Twins drifting is what caused the original 403 outage. THREE surfaces gate
  // CORS and must move together; guard each for:
  // (1) the worldmonitor.sibt.ai host this fork is served from, and
  // (2) no vercel.app origin at all — we do not deploy there, so an entry for
  //     it is a hole in a security allowlist pointing at somebody else's host.
  ...
  for (const rel of TWINS) {
    it(`${rel} allows worldmonitor.sibt.ai and no Vercel host`, async () => {
      const source = await readFile(new URL(rel, import.meta.url), 'utf8');
      assert.ok(
        source.includes('worldmonitor\\.sibt\\.ai'),
        `${rel} must allow the worldmonitor.sibt.ai host the app is served from`,
      );
      assert.ok(
        !/\/\^https[^\n]*vercel\\.app/.test(source),
        `${rel} must not allow any vercel.app origin (security allowlist)`,
      );
    });
  }
});
...
describe('CORS Worker superset invariant — edge allowlist ⊇ function allowlist', () => {
  ...
  // function↔Worker drift (the bug that left the previews dark).
  //
