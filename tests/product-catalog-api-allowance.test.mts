...
describe('dashboard-AI allowance is separate from MCP', () => {
  // Pro carries what Pro Business carried — the two merged on 2026-08-05, so
  // 2,500 AI calls and 250 MCP calls are now what $29 buys. These are the
  // numbers the /pro page and the docs quote; nothing else pins them.
  it('sets the agreed daily dashboard-AI limits by plan family', () => {
    ...
    assert.equal(getEntitlementFeatures('pro_monthly').planLimits?.dashboardAiCallsPerDay, 2500);
    assert.equal(getEntitlementFeatures('pro_annual').planLimits?.dashboardAiCallsPerDay, 2500);
  ...
  it('keeps the MCP allowances the merged tiers agreed on', () => {
    assert.equal(getEntitlementFeatures('pro_monthly').planLimits?.mcpCallsPerDay, 250);
    assert.equal(getEntitlementFeatures('pro_annual').planLimits?.mcpCallsPerDay, 250);
    assert.equal(getEntitlementFeatures('free').planLimits?.mcpCallsPerDay, 0);
  });
