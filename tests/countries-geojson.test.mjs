...
// The file the app actually loads: country-geometry.ts fetches
// /data/countries.geojson, which the Worker serves from public/data/. This
// test used to fetch https://maps.worldmonitor.app/countries.geojson instead —
// the upstream project's bucket, which we neither serve nor control, so a
// broken file here passed and a broken file there failed. Reading from disk
// also drops a network call from the test run.
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const features = JSON.parse(
  readFileSync(resolve(repoRoot, 'public/data/countries.geojson'), 'utf8'),
).features;
