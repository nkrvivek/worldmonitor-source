...
describe('consentPage — preserved invariants (regression guard for U6+)', () => {
  ...
  // Upstream wrote www.worldmonitor.app into the chrome links and the bare apex
  // into the grant CTA. Both are now one derived origin, so the assertion is on
  // the link's purpose -- a /pro CTA on whichever host served the page.
  it('legacy "Get an API key" footer link is preserved', async () => {
    const html = await renderHtml(BASE_PARAMS, NONCE);
    assert.match(html, /href="https:\/\/worldmonitor\.app\/pro"[^>]*>Get an API key/);
  });

  it('every link points at the host that served the page', async () => {
    const forkOrigins = { app: 'https://worldmonitor.sibt.ai', api: 'https://worldmonitor.sibt.ai' };
    const res = consentPage(BASE_PARAMS, NONCE, '', forkOrigins);
    const html = await res.text();
    // The bug this guards: a consent page served by this fork that hands the
    // user off to worldmonitor.app, a site we do not run.
    assert.doesNotMatch(html, /worldmonitor\.app/);
    assert.match(html, /<a id="pc"[^>]*href="https:\/\/worldmonitor\.sibt\.ai\/mcp-grant\?nonce=/);
    assert.match(html, /action="https:\/\/worldmonitor\.sibt\.ai\/oauth\/authorize"/);
    assert.match(html, /href="https:\/\/worldmonitor\.sibt\.ai\/pro"/);
  });
