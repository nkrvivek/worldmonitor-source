...
describe('renderBriefMagazine — publicMode', () => {
  ...
  it('prepends a Subscribe strip on public views', () => {
    ...
    assert.ok(html.includes('worldmonitor.sibt.ai/pro'), 'strip links to /pro');
  ...
  it('attaches ?ref= to public CTAs when refCode is provided', () => {
    ...
    assert.ok(html.includes('worldmonitor.sibt.ai/pro?ref=ABC123'), 'refCode appended to /pro URL');
  });
