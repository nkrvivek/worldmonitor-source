...
test('seed-health CII risk score freshness mirrors api/health riskScores', () => {
  ...
  assert.equal(
    Number(seedHealthMatch[2].replaceAll('_', '')) * 3,
    healthRiskScores.minutes,
    'seed-health intervalMin*3 must match api/health.js riskScores maxStaleMin (3x absorbs GitHub cron skips, a303674e7)',
  );
  assert.match(
    health,
    /riskScores:\s*\{\s*key:\s*'seed-meta:intelligence:risk-scores',\s*maxStaleMin:\s*45,\s*minRecordCount:\s*3\s*\}/,
