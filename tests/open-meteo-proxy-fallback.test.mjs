...
// ---------------------------------------------------------------------------
// Shared retry budget (`deadlineAt`)
//
// Measured 2026-08-05: seed-climate-zone-normals walks 13 sequential batches
// and Open-Meteo 429s several of them. Each 429 honours a Retry-After of up to
// 60s, four times, so one bad batch can spend four minutes on its own. The
// seed's fetch phase died at 240336ms with nothing written.
//
// `deadlineAt` is a wall-clock instant the caller owns. Past it the helper
// stops sleeping and goes straight to the proxy legs, so a walk degrades to
// "fewer zones" instead of "no data at all".
// ---------------------------------------------------------------------------

test('stops retrying once the caller deadline has passed', async () => {
  const { fetchOpenMeteoArchiveBatch } = await import(`../scripts/_open-meteo-archive.mjs?t=${Date.now()}`);

  // Arrange — every direct attempt 429s, and the deadline is already spent.
  let directCalls = 0;
  globalThis.fetch = async () => {
    directCalls += 1;
  ...
  const data = await fetchOpenMeteoArchiveBatch(ZONES, {
    ...ARCHIVE_OPTS,
    maxRetries: 4,
    retryBaseMs: 5_000,
    ...
    _proxyCurlFetcher: () => { throw new Error('curl should not run'); },
  });

  // Assert — one direct try, no 60s sleep, answer came from the proxy leg.
  assert.equal(directCalls, 1);
  assert.ok(Date.now() - started < 5_000, 'must not sleep through Retry-After past the deadline');
  assert.deepEqual(data, VALID_PAYLOAD);
});

test('still retries normally when the deadline leaves room', async () => {
  const { fetchOpenMeteoArchiveBatch } = await import(`../scripts/_open-meteo-archive.mjs?t=${Date.now()}`);

  // Arrange — first attempt 429s, second succeeds, deadline is far away.
  let directCalls = 0;
  globalThis.fetch = async () => {
    directCalls += 1;
    if (directCalls === 1) {
    ...
    return { ok: true, status: 200, headers: { get: () => null }, json: async () => VALID_PAYLOAD };
  };
