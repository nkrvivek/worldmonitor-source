...
import { stallUntilAborted } from './helpers/stall-until-aborted.mjs';
...
describe('readBootstrapTierObject', () => {
  ...
  it('returns timeout promptly when the caller-supplied signal expires', async () => {
    ...
    const result = await readBootstrapTierObject('fast', readerOptions(null, {
      timeoutMs,
      awsClientFactory: () => ({
        fetch: async (_url, { signal }) => await stallUntilAborted(signal),
      }),
    }));
    ...
    const result = await readBootstrapTierObject('fast', readerOptions(null, {
      ...
      awsClientFactory: () => ({
        fetch: async (_url, { signal }) => ({
          status: 200,
          ok: true,
          json: async () => await stallUntilAborted(signal),
        }),
      }),
    }));
