...
describe('scheduled seed freshness monitor', () => {
  ...
  describe('acceptance report', () => {
    ...
    // Issue #87. A source that stops appearing for one run has not been fixed,
    // it has flapped. The old text read "remove it from
    // scripts/seed-freshness-baseline.json (#N)", an operator did exactly that
    // for crossStraitActivityJapanMod on 2026-08-19 07:06Z, and the same
    // failure blocked the gate twenty hours later as a brand new problem. The
    // recovery is still worth printing; the instruction to act on it in the
    // same breath is what has to go. expiresAt is the review that has the
    // whole file in front of it.
    it('notes a recovery without telling the operator to prune the row', () => {
      const report = formatAcceptanceReport(
        baselineResult({
          cleared: [{ name: 'crossStraitActivityJapanMod', status: 'SEED_ERROR', issue: 88 }],
          expiresAt: '2026-09-18',
        }),
        '2026-08-20T21:00:00Z',
      );

      const recovered = report.info.find((line) => line.startsWith('- recovered:'));
      assert.ok(recovered, 'the recovery is still reported');
      assert.match(recovered, /crossStraitActivityJapanMod:SEED_ERROR/);
      assert.doesNotMatch(
        recovered,
        /remove it/i,
        'one clean run is not proof of a fix, so the report must not ask for a prune',
      );
      assert.match(recovered, /2026-09-18/, 'it points at the dated re-review instead');
    });

    it('never advises a prune anywhere in a passing report', () => {
      const report = formatAcceptanceReport(
        baselineResult({
          acknowledged: [{ name: 'gdeltIntel', status: 'SEED_ERROR', records: 1, issue: 5766 }],
          cleared: [{ name: 'shippingRates', status: 'STALE_SEED', issue: 5769 }],
      ...
      for (const line of report.info) {
        assert.doesNotMatch(line, /remove it from scripts\/seed-freshness-baseline\.json/);
      }
    });
  });

  // The exit status is the only part of this run most people ever read, so it
  // has to mean one thing: something changed for the worse. These four cases
  // are that contract end to end -- baseline split THEN report -- because the
  // process exit code is driven by `report.failed`, not by any one split.
  describe('exit status', () => {
    const baseline = {
      expiresAt: '2026-09-18',
      acknowledged: [
        // Unbounded: the old shape, and still the common one.
        { name: 'cyberThreats', status: 'EMPTY', issue: 32 },
        // Bounded: acknowledged AT a level, not in perpetuity.
        { name: 'portwatchPortActivity', status: 'COVERAGE_PARTIAL', issue: 91, minRecords: 128 },
        { name: 'crossStraitActivityJapanMod', status: 'SEED_ERROR', issue: 88, maxSeedAgeMin: 14000 },
      ],
    };
    const now = Date.parse('2026-08-29T05:25:00Z');
    const run = (problems) =>
      formatAcceptanceReport(
        applyAcceptanceBaseline(problems, baseline, now),
        '2026-08-29T05:25:36.737Z',
      );

    it('exits 0 when every live problem is acknowledged', () => {
      // The whole point of the registry. 33234495599 printed 34 of these and
      // still exited 1 -- because of five genuinely unacknowledged rows, not
      // because of the acknowledged ones. Pin that they are never the cause.
      const report = run([
        { name: 'crossStraitActivityJapanMod', status: 'SEED_ERROR', records: 2, seedAgeMin: 13330, maxStaleMin: 720 },
        { name: 'cyberThreats', status: 'EMPTY', records: 0 },
        { name: 'portwatchPortActivity', status: 'COVERAGE_PARTIAL', records: 128, seedAgeMin: 324 },
      ]);

      assert.equal(report.failed, false);
      assert.deepEqual(report.errors, []);
      assert.equal(
        report.info.filter((line) => line.startsWith('- acknowledged')).length,
        3,
        'a passing run must still print every acknowledged row in full',
      );
    });

    it('exits 1 on a brand-new failure that is not in the registry', () => {
      const report = run([
        { name: 'cyberThreats', status: 'EMPTY', records: 0 },
        { name: 'bisDsr', status: 'STALE_SEED', records: 32, seedAgeMin: 2724, maxStaleMin: 2160 },
      ]);

      assert.equal(report.failed, true);
      assert.match(report.errors[0], /1 unacknowledged problem\(s\)/);
      assert.deepEqual(report.errors.slice(1), [
        '- bisDsr: status=STALE_SEED records=32 age=2724m max=2160m',
      ]);
      // The acknowledged row is still reported, just not as a cause.
      assert.match(report.info.join('\n'), /acknowledged \(#32\): cyberThreats/);
    });

    it('exits 1 when an acknowledged source degrades past the level accepted', () => {
      // portwatchPortActivity was acknowledged at 159 of 174 and reported
      // `acknowledged` all the way down to 128 without ever failing a run. A
      // floor is what makes the acknowledgement a statement about a level.
      const report = run([
        { name: 'portwatchPortActivity', status: 'COVERAGE_PARTIAL', records: 96, seedAgeMin: 324 },
      ]);

      assert.equal(report.failed, true);
      assert.match(
        report.errors.join('\n'),
        /regressed beyond acknowledgement \(#91\).*records fell to 96, below the acknowledged floor of 128/s,
      );
      assert.equal(
        report.info.filter((line) => line.startsWith('- acknowledged')).length,
        0,
        'a regressed row must not also be reported as still-acknowledged',
      );
      assert.doesNotMatch(
        report.info.join('\n'),
        /recovered: portwatchPortActivity/,
        'a regression is the source getting worse, not the acknowledgement disappearing',
      );
    });

    it('exits 1 when an acknowledged source ages past the ceiling accepted', () => {
      const report = run([
        { name: 'crossStraitActivityJapanMod', status: 'SEED_ERROR', records: 2, seedAgeMin: 20000, maxStaleMin: 720 },
      ]);

      assert.equal(report.failed, true);
      assert.match(
        report.errors.join('\n'),
        /age rose to 20000m, past the acknowledged ceiling of 14000m/,
      );
    });

    it('exits 0 when the only news is a recovery', () => {
      // Recovery must never be a failure: it is the run that proves things
      // improved, and failing there is how the monitor taught people to ignore
      // it. Every registry row is absent here, so all three read `recovered`.
      const report = run([]);

      assert.equal(report.failed, false);
      assert.deepEqual(report.errors, []);
      assert.equal(
        report.info.filter((line) => line.startsWith('- recovered:')).length,
        3,
      );
    });

    it('leaves an unbounded acknowledgement exactly as strict as it was', () => {
      // Bounds are opt-in. Adding the mechanism must not turn a single existing
      // row red on its own, or the fix would have re-created the problem.
      assert.equal(findAcknowledgementBreach({ records: 0, seedAgeMin: 999_999 }, { issue: 32 }), null);
      // A bound only grades the dimension it declares, and only when health
      // actually reported that dimension.
      assert.equal(findAcknowledgementBreach({ records: undefined }, { minRecords: 5 }), null);
      assert.equal(findAcknowledgementBreach({ records: 5 }, { minRecords: 5 }), null);
      assert.match(findAcknowledgementBreach({ records: 4 }, { minRecords: 5 }), /below the acknowledged floor/);
      assert.equal(findAcknowledgementBreach({ seedAgeMin: 60 }, { maxSeedAgeMin: 60 }), null);
      assert.match(findAcknowledgementBreach({ seedAgeMin: 61 }, { maxSeedAgeMin: 60 }), /past the acknowledged ceiling/);
    });
    ...
    it('rejects a bound that is not a usable number', () => {
      const withBound = (bound) => () =>
        validateAcceptanceBaseline({
          expiresAt: '2026-09-18',
          acknowledged: [{ name: 'x', status: 'EMPTY', issue: 1, ...bound }],
        });

      assert.throws(withBound({ minRecords: 'many' }), /non-numeric minRecords/);
      assert.throws(withBound({ maxSeedAgeMin: -1 }), /non-numeric maxSeedAgeMin/);
      assert.doesNotThrow(withBound({ minRecords: 0 }));
      assert.doesNotThrow(withBound({}));
    });

    it('grades the committed baseline against the live shape it was measured on', () => {
      // The bounds committed on 2026-08-29 are ratchets set at that day's
      // measured level. If any of them were set BELOW what production was
      // already serving, the very next scheduled run would go red on a row that
      // had not moved -- which is the failure this whole change is undoing.
      const measured = [
        { name: 'crossStraitActivityJapanMod', status: 'SEED_ERROR', records: 2, seedAgeMin: 13330, maxStaleMin: 720 },
        { name: 'portwatchPortActivity', status: 'COVERAGE_PARTIAL', records: 128, seedAgeMin: 324, maxStaleMin: 2160 },
        { name: 'riskScores', status: 'COVERAGE_PARTIAL', records: 2, seedAgeMin: 10, maxStaleMin: 45 },
      ];
      const result = applyAcceptanceBaseline(measured, readCommittedBaseline(), now);

      assert.deepEqual(result.regressed, [], 'committed bounds must not block the level they were measured at');
      assert.deepEqual(result.blocking, []);
      assert.equal(result.acknowledged.length, 3);
    });
  });
  ...
  it('runs on a schedule without grading pre-deployment ingestion pushes', () => {
    ...
    // Every three hours since 2026-08-12. At */15 the monitor re-probed
    // production 96 times a day to report the same stale seed, and a check that
    // repeats itself that often is one nobody reads. The seeds it grades run on
    // schedules measured in hours, so a quarter-hour cadence was never
    // measuring anything new.
    assert.equal(on.schedule[0].cron, '0 */3 * * *');
