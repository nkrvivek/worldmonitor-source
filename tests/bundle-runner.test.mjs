...
// Container stdout is not readable: `wrangler tail` shows the cron invocation
// and the container's start RPC with `logs: []`, and `wrangler containers` has
// no logs subcommand. Every line the runner prints above therefore reaches
// nobody in production, which is how one macro section stayed broken for weeks
// while the other fourteen in its bundle ran clean. The run record is the only
// copy of that information anyone can read afterwards.

// Stands in for the Upstash REST endpoint. Returns the port and the request
// bodies it received.
...
test('writes one run record naming every section outcome', async () => {
  const cleanupOk = writeFixture('_bundle-fixture-record-ok.mjs', `console.log('ok');\n`);
  const cleanupFail = writeFixture(
    '_bundle-fixture-record-fail.mjs',
    `console.error('boom: upstream said no'); process.exit(1);\n`,
  );
  try {
    const { code, stdout, record } = await withFakeRedis(async (url, received) => {
      const result = await runBundleWith(
        [
          { label: 'Good', script: '_bundle-fixture-record-ok.mjs', intervalMs: 1, timeoutMs: 5000 },
          { label: 'Bad', script: '_bundle-fixture-record-fail.mjs', intervalMs: 1, timeoutMs: 5000 },
        ],
        {},
        { UPSTASH_REDIS_REST_URL: url, UPSTASH_REDIS_REST_TOKEN: 'test-token' },
      );
      // The freshness reads use GET paths this server also answers, so filter
      // to the SET the run record makes.
      const sets = received
        .map((r) => { try { return JSON.parse(r.body); } catch { return null; } })
        .filter((cmd) => Array.isArray(cmd) && cmd[0] === 'SET');
      return { ...result, record: sets.at(-1) };
    });

    // A failed section still fails the bundle — recording is not forgiving.
    assert.notEqual(code, 0);
    assert.match(stdout, new RegExp(`Run record written to ${RUN_RECORD_KEY_PREFIX}test`));

    assert.ok(record, 'expected a SET command carrying the run record');
    const [, key, payload, ex, ttl] = record;
    assert.equal(key, `${RUN_RECORD_KEY_PREFIX}test`);
    assert.equal(ex, 'EX');
    assert.equal(ttl, RUN_RECORD_TTL_SECONDS);

    const parsed = JSON.parse(payload);
    assert.equal(parsed.bundle, 'test');
    assert.equal(parsed.ran, 1);
    assert.equal(parsed.failed, 1);
    assert.deepEqual(parsed.sections.map((s) => [s.label, s.status]), [
      ['Good', 'OK'],
      ['Bad', 'FAILED'],
    ]);
    // The reason text is what makes the record worth writing.
    assert.match(parsed.sections[1].reason, /exit(ed)? (with )?(code )?1|boom/i);
    // …and `exit 1` alone names the failure without explaining it. The child's
    // own stderr is the explanation, and in a container it survives nowhere else.
    assert.deepEqual(parsed.sections[1].stderrTail, ['boom: upstream said no']);
    assert.equal(
      parsed.sections[0].stderrTail,
      undefined,
      'a section that succeeded explains itself; no tail is carried',
    );
  } finally {
    cleanupOk();
    cleanupFail();
...
test('keeps the last stderr lines of a failing section, not the first', async () => {
  const cleanup = writeFixture(
    '_bundle-fixture-record-chatty.mjs',
    `for (let i = 1; i <= 40; i++) console.error('line ' + i);\nprocess.exit(1);\n`,
  );
  try {
    const { record } = await withFakeRedis(async (url, received) => {
      await runBundleWith(
        [{ label: 'Chatty', script: '_bundle-fixture-record-chatty.mjs', intervalMs: 1, timeoutMs: 5000 }],
        {},
        { UPSTASH_REDIS_REST_URL: url, UPSTASH_REDIS_REST_TOKEN: 'test-token' },
      );
      const sets = received
        .map((r) => { try { return JSON.parse(r.body); } catch { return null; } })
        .filter((cmd) => Array.isArray(cmd) && cmd[0] === 'SET');
      return { record: sets.at(-1) };
    });
    const tail = JSON.parse(record[2]).sections[0].stderrTail;
    assert.equal(tail.length, STDERR_TAIL_LINES);
    // The bottom of a stack trace holds the cause; the top holds warmup noise.
    assert.equal(tail.at(-1), 'line 40');
    assert.equal(tail[0], `line ${40 - STDERR_TAIL_LINES + 1}`);
  } finally {
    cleanup();
  }
});

test('a Redis that refuses the write does not change the exit code', async () => {
  const cleanup = writeFixture('_bundle-fixture-record-nowrite.mjs', `console.log('ok');\n`);
  try {
    // Port 1 with nothing on it: the write throws rather than returning.
    const { code, stdout, stderr } = await runBundleWith(
      [{ label: 'Good', script: '_bundle-fixture-record-nowrite.mjs', intervalMs: 1, timeoutMs: 5000 }],
      {},
      { UPSTASH_REDIS_REST_URL: 'http://127.0.0.1:1', UPSTASH_REDIS_REST_TOKEN: 'test-token' },
    );
    assert.equal(code, 0, `a lost run record is not a lost run; stderr:\n${stderr}`);
    assert.doesNotMatch(stdout, /Run record written/);
    assert.match(stderr, /run record write failed/);
  } finally {
    cleanup();
  }
});
