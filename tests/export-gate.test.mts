...
describe('catalogExposesExportTier — served-catalog detection', () => {
  // The marker moved from Pro Business to Pro on 2026-08-05 when the two Pro
  // plans merged: Pro is the tier that carries `dataExport` now.
  it('detects the tier by group id', () => {
    assert.equal(catalogExposesExportTier({ tiers: [{ name: 'Free' }, { id: 'pro' }] }), true);
  });

  it('detects the tier by locale key', () => {
    assert.equal(catalogExposesExportTier({ tiers: [{ localeKey: 'pro' }] }), true);
  });

  it('detects the tier by display name', () => {
    assert.equal(catalogExposesExportTier({ tiers: [{ name: 'Pro' }] }), true);
  });

  it('detects the tier by plan key', () => {
    assert.equal(catalogExposesExportTier({ tiers: [{ planKey: 'pro_annual' }] }), true);
  });

  it('still detects a mirror still serving the old Pro Business names', () => {
    // Identities match by prefix, so a mirror that has not redeployed since the
    // merge keeps activating the gate instead of silently opening it.
    assert.equal(catalogExposesExportTier({ tiers: [{ planKey: 'pro_business_monthly' }] }), true);
    assert.equal(catalogExposesExportTier({ tiers: [{ localeKey: 'proBusiness' }] }), true);
  });
  ...
  it('does not false-positive on marketing copy that merely mentions the tier', () => {
    assert.equal(
      catalogExposesExportTier({
        tiers: [{ name: 'Free', localeKey: 'free', features: ['Upgrade to Pro for data export'], description: 'Pro coming soon' }],
      }),
      false,
    );
  });
  ...
  it('returns false for malformed payloads', () => {
    assert.equal(catalogExposesExportTier(null), false);
    assert.equal(catalogExposesExportTier({}), false);
    assert.equal(catalogExposesExportTier({ tiers: 'pro' }), false);
    assert.equal(catalogExposesExportTier({ tiers: [null, 42, 'pro'] }), false);
  });
});
...
describe('primeExportGateActivation — single-flight probe', () => {
  ...
  it('stays inactive on a non-ok response', async () => {
    await primeExportGateActivation(async () => ({ ok: false, json: async () => ({ tiers: [{ id: 'pro' }] }) }) as never);
    assert.equal(isExportGateActive(), false);
  });
  ...
  it('a non-ok response is NOT cached either', async () => {
    ...
    const probe = async () => {
      ...
      return (calls === 1
        ? { ok: false, json: async () => ({}) }
        : { ok: true, json: async () => ({ tiers: [{ id: 'pro' }] }) }) as never;
    };
  ...
  it('a definitive "tier not served" verdict IS cached — one fetch per session', async () => {
    ...
    const probe = async () => {
      calls += 1;
      return { ok: true, json: async () => ({ tiers: [{ id: 'free' }] }) } as never;
    };
    await primeExportGateActivation(probe);
    await primeExportGateActivation(probe);
    assert.equal(calls, 1);
