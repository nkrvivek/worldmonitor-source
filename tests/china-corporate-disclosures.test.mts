...
describe('official China corporate disclosures (#5577)', () => {
  ...
  it('pins edge egress to the canonical endpoint, and stays off unless asked', () => {
    assert.equal(resolveChinaExchangeEdgeEgress({}), null);
    assert.equal(resolveChinaExchangeEdgeEgress({
      CHINA_EXCHANGE_EDGE_EGRESS: '1',
    }), null);
    // The platform env var upstream gated on never opens the gate here. This
    // fork does not run on that platform, so a stray copy of it must not turn
    // the relay on.
    assert.equal(resolveChinaExchangeEdgeEgress({
    ...
    // An env-supplied endpoint is ignored even once the gate is open: the
    // shared secret goes to the pinned host or nowhere.
    assert.deepEqual(resolveChinaExchangeEdgeEgress({
      CHINA_EXCHANGE_EDGE_EGRESS: '1',
