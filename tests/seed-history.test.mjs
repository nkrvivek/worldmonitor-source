...
describe('appendSeedHistory', () => {
  ...
  it('names the relay host in the error, so a 401 says which deployment refused', async () => {
    // A 401 with no host reads as "the secret is wrong". It is just as often
    // the right secret sent to the wrong Convex deployment, and the two need
    // opposite fixes. Measured 2026-08-19: energy:intelligence had 47
    // consecutive 401s while the same secret authenticated against both known
    // deployments by hand, and nothing in the health record said where the
    // seeds container had actually posted.
    const { fetchImpl } = stubFetch({ status: 401, text: '{"error":"UNAUTHORIZED"}' });
    const { embed } = stubEmbed();
    ...
    await assert.rejects(
      ...
      (err) => {
        assert.match(err.message, /example\.convex\.site/);
        return true;
      },
    );
  });

  it('keeps the secret out of the error even when the URL carries one', async () => {
    const { fetchImpl } = stubFetch({ status: 401, text: 'nope' });
    const { embed } = stubEmbed();
      ...
      () =>
        appendSeedHistory(
          { domain: 'conflict', resource: 'acled', runId: 'run-1', records: [record(1)] },
          {
            fetchImpl,
            embed,
            env: { ...ENV, CONVEX_SITE_URL: 'https://example.convex.site?token=hunter2' },
          },
        ),
      (err) => {
