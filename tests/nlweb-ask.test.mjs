...
describe('nlweb: /ask endpoint', () => {
  ...
  // The bug this guards: an asking agent is answered by this fork and pointed
  // at worldmonitor.app, a site we do not run.
  it('every result names the host that answered', async () => {
    const res = await handler(
      new Request('https://worldmonitor.sibt.ai/ask?query=sanctions', { method: 'GET' }),
    );
    const body = await res.json();
    assert.doesNotMatch(JSON.stringify(body), /worldmonitor\.app/);
    assert.equal(body.results[0].site, 'worldmonitor.sibt.ai');
    assert.equal(body.results[0].url, 'https://worldmonitor.sibt.ai/docs/mcp-tools-reference');
    assert.equal(body.results[0].schema_object.url, 'https://worldmonitor.sibt.ai/mcp');
  });

  it('the no-match fallback points at this host too', async () => {
    const res = await handler(
      new Request('https://worldmonitor.sibt.ai/ask?query=zzzqqq', { method: 'GET' }),
    );
    const body = await res.json();
    assert.equal(body.results[0].url, 'https://worldmonitor.sibt.ai/llms.txt');
  });
