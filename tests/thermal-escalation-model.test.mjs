...
describe('thermal escalation history retention', () => {
  it('drops history entries older than the 7-day baseline window so the key stays under the Upstash request cap', () => {
    const nowMs = Date.parse('2026-09-02T09:15:00.000Z');
    const day = 24 * 60 * 60 * 1000;
    const entry = (ageDays) => ({
      observedAt: new Date(nowMs - ageDays * day).toISOString(),
      observationCount: 3,
      totalFrp: 30,
      status: 'THERMAL_STATUS_ELEVATED',
    });
    const previousHistory = {
      updatedAt: new Date(nowMs - 3 * 60 * 60 * 1000).toISOString(),
      cells: {
        '50.5:30.5': { entries: [entry(20), entry(8), entry(6), entry(1)] },
        '10.5:10.5': { entries: [entry(25), entry(9)] },
      },
    };
    const detections = [
      makeDetection('a', 50.45, 30.52, nowMs - 60 * 60 * 1000),
      makeDetection('b', 50.46, 30.53, nowMs - 50 * 60 * 1000),
    ];

    const { history } = computeThermalEscalationWatch(detections, previousHistory, { nowMs });

    const kept = history.cells['50.5:30.5'].entries.map((e) => e.observedAt);
    assert.deepEqual(kept, [entry(6).observedAt, entry(1).observedAt, new Date(nowMs).toISOString()]);
    assert.equal(history.cells['10.5:10.5'], undefined, 'a cell with only stale entries is dropped');
  });
});
