...
describe('climate zone normals', () => {
  ...
  // Measured 2026-08-05: the walk covers 13 batches, Open-Meteo 429s several of
  // them, and each 429 honours a Retry-After of up to 60s four times. The fetch
  // phase died at 240336 ms with nothing written. A spent budget must end the
  // walk rather than sleep through the batches it can no longer reach.
  it('stops walking zones once the fetch budget is spent', async () => {
    // Arrange — the budget is already gone before the first batch.
    const originalFetch = globalThis.fetch;
    let calls = 0;
    globalThis.fetch = async () => {
      calls += 1;
      throw new Error('no batch should be fetched past the deadline');
    };
    ...
    try {
      await assert.rejects(
        () => fetchClimateZoneNormals({ deadlineAt: Date.now() - 1 }),
        /0\/\d+ zones returned normals/,
      );
    } finally {
    ...
    assert.ok(Date.now() - startedAt < 5_000, 'must not sleep between batches it cannot reach');
  });
});
