...
describe('gateway telemetry payload — bearer identity propagation', () => {
  ...
  it('records customer_id from a successful legacy premium bearer call', async () => {
    ...
    // The row is what makes this caller Pro. It used to be the token's own
    // `plan: 'pro'` claim, which Clerk minted from publicMetadata and Supabase
    // has no trustworthy equivalent for — so the entitlement row is now the
    // only Pro signal, on this path and every other.
    const spy = installAxiomFetchSpy(ORIGINAL_FETCH, {
    ...
    // A wire value, not a code identifier: Axiom dashboards and saved queries
    // filter on this exact string, so the provider swap deliberately leaves it
    // alone. Renaming it would silently empty every existing query.
  ...
  it("records tier=2 for an entitlement-gated success (the path the round-1 P2 fix targets)", async () => {
    ...
    const token = await signToken({ sub: 'user_api' });
    const recorder = makeRecordingCtx();
    const res = await handler(
