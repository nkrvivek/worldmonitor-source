...
describe('api/brief followed-countries telemetry fetch', () => {
  ...
  it('drains the in-flight telemetry lookup when envelope read returns 503', () => {
    assert.match(
      src,
      /catch \(err\) \{[\s\S]*?step: 'envelope-read'[\s\S]*?ctx\?\.waitUntil\(followedCountriesPromise\)[\s\S]*?return htmlResponse\(req, 503, unavailablePage\(req\)\)/,
  ...
  it('drains the in-flight telemetry lookup when the envelope is missing', () => {
    assert.match(
      src,
      /if \(!envelope\) \{\s*ctx\?\.waitUntil\(followedCountriesPromise\);\s*return htmlResponse\(req, 404, expiredPage\(req\)\);\s*\}/,
