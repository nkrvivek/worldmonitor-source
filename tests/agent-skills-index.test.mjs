...
describe('agent readiness: agent-skills index', () => {
  ...
  it('keeps the human docs catalog in sync with the advertised skills', () => {
    ...
    assert.ok(
      page.includes('https://worldmonitor.sibt.ai/.well-known/agent-skills/index.json'),
  ...
  it('carries top-level when-to-use guidance for agent discovery', () => {
    ...
    assert.match(index.instructions, /worldmonitor\.sibt\.ai\/mcp/, 'instructions must say how to call the MCP server');
  ...
  it('every entry points at a real SKILL.md whose bytes match the declared digest', () => {
    for (const skill of index.skills) {
      ...
      assert.match(
        skill.url,
        /^https:\/\/worldmonitor\.sibt\.ai\/\.well-known\/agent-skills\/[^/]+\/SKILL\.md$/,
