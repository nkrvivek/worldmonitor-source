...
describe('brief-render — U11 source-link stamping', () => {
  ...
  it('emits the inline brief-thread-open tracker script in the magazine HTML', () => {
    ...
    // The tracker that defines window.umami ships inline and posts to this
    // site's own collector. A third-party script tag would be a request to a
    // host we do not control, and the last one pointed at a host that is gone.
    assert.ok(
      html.includes("fetch('/api/send'"),
      'the inline tracker posts to the first-party collector',
    );
    assert.ok(
      html.includes('window.umami ='),
      'the loader defines the window.umami surface the click handler calls',
    );
    assert.ok(
      !html.includes('abacus.worldmonitor.app'),
      'no script may be loaded from the retired analytics host',
    );
  });
});
...
function extractTrackerScript(html) {
  // The page carries several inline scripts. Take each one whole and pick the
  // one naming the event — a scan that starts at the first `<script>` and runs
  // to the first `brief-thread-open` swallows every block in between.
  const bodies = [...html.matchAll(/<script>([\s\S]*?)<\/script>/g)].map((match) => match[1]);
  const tracker = bodies.find((body) => body.includes('brief-thread-open'));
  assert.ok(tracker, 'tracker script must be present in rendered HTML');
  return tracker;
}
